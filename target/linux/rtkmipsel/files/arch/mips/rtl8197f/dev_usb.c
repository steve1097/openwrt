...
static struct resource rtl819x_ohci_resources[] = {
	[0] = {
	       ...
	       .end =
	       PADDR(OHCI_RTL819x_USB_BASE) + OHCI_RTL819x_USB_REG_SIZE - 1,
...
static struct resource rtl819x_ehci_resources[] = {
	[0] = {
	       ...
	       .end =
	       PADDR(EHCI_RTL819x_USB_BASE) + EHCI_RTL819x_USB_REG_SIZE - 1,
	       .flags = IORESOURCE_MEM,
	       },
	[1] = {
	       .start = RTL819x_USB_IRQ,
...
//      tmp = tmp & ~((0xff<<11)|(0xff<<0));
	...
	REG32(0xb80210A4) = (0x00003000 | (reg_l << 8));
	USB2_PHY_DELAY;
	//REG32(0xb80210A4) = (0x00200000 | (reg_l<<16)); USB2_PHY_DELAY;       
	REG32(0xb80210A4) = (0x00002000 | (reg_l << 8));
	USB2_PHY_DELAY;
	...
	REG32(0xb80210A4) = (0x00003000 | (reg_h << 8));
	USB2_PHY_DELAY;

	//REG32(0xb80210A4) = (0x00200000 | (reg_h<<16)); USB2_PHY_DELAY;       
	REG32(0xb80210A4) = (0x00002000 | (reg_h << 8));
	USB2_PHY_DELAY;
...
#define	USB2_PHY_DELAY	{mdelay(10);}
	...
	if (portnum == 0) {
		...
		REG32(0xb80210A4) = (0x00005000 | (reg_l << 8));
		USB2_PHY_DELAY;
		//printk("(0x003000 | (reg_l<<8))=%x\n", (0x003000 | (reg_l<<8)) );
		REG32(0xb80210A4) = (0x00004000 | (reg_l << 8));
		USB2_PHY_DELAY;
		//printk("(0x002000 | (reg_l<<8))=%x\n", (0x002000 | (reg_l<<8)) );
		REG32(0xb80210A4) = (0x00005000 | (reg_l << 8));
		USB2_PHY_DELAY;
		//printk("(0x003000 | (reg_l<<8))=%x\n", (0x003000 | (reg_l<<8)) );
		REG32(0xb80210A4) = (0x00005000 | (reg_h << 8));
		USB2_PHY_DELAY;
		//printk("(0x003000 | (reg_h<<8))=%x\n", (0x003000 | (reg_h<<8)) );
		REG32(0xb80210A4) = (0x00004000 | (reg_h << 8));
		USB2_PHY_DELAY;
		//printk("(0x003000 | (reg_h<<8))=%x\n", (0x002000 | (reg_h<<8)) );
		REG32(0xb80210A4) = (0x00005000 | (reg_h << 8));
		USB2_PHY_DELAY;
...
//#define       USB2_PHY_DELAY  {int i=100; while(i>0) {i--;}}
...
#if 0
	/* For Port-0 */
	writel(0x0000000E, 0xb8003314);
	USB2_PHY_DELAY;
	writel(0x00340000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x00240000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x00340000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x003E0000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x002E0000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x003E0000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x000000D8, 0xb8003314);
	USB2_PHY_DELAY;
	writel(0x00360000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x00260000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x00360000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x003E0000, 0xb80210A4);
	...
	/* For Port-1 */
	writel(0x000E0000, 0xb8003314);
	USB2_PHY_DELAY;
	writel(0x00540000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x00440000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x00540000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x005E0000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x004E0000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x005E0000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x00D80000, 0xb8003314);
	USB2_PHY_DELAY;
	writel(0x00560000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x00460000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x00560000, 0xb80210A4);
	USB2_PHY_DELAY;
	writel(0x005E0000, 0xb80210A4);
	...
	REG32(0xb814020c) = 0x0000000E;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x00340000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x00240000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x00340000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x003E0000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x002E0000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x003E0000;
	USB2_PHY_DELAY;
	//REG32(0xb8003314) = 0x000000D8;       USB2_PHY_DELAY;
	REG32(0xb814020c) = 0x000000D8;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x00360000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x00260000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x00360000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x003E0000;
	...
	REG32(0xb814020c) = 0x000E0000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x00540000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x00440000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x00540000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x005E0000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x004E0000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x005E0000;
	USB2_PHY_DELAY;
	//REG32(0xb8003314) = 0x00D80000;       USB2_PHY_DELAY;
	REG32(0xb814020c) = 0x00D80000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x00560000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x00460000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x00560000;
	USB2_PHY_DELAY;
	REG32(0xb80210A4) = 0x005E0000;
	...
	//8196C demo board: 0xE0:99, 0xE1:A8, 0xE2:98, 0xE3:C1,  0xE5:91, 
	SetUSBPhy(0xe0, 0x99);
	if (GetUSBPhy(0xe0) != 0x99)
		printk("reg 0xe0 not correct\n");
	SetUSBPhy(0xe1, 0xa8);
	if (GetUSBPhy(0xe1) != 0xa8)
		printk("reg 0xe1 not correct\n");
	SetUSBPhy(0xe2, 0x98);
	if (GetUSBPhy(0xe2) != 0x98)
		printk("reg 0xe2 not correct\n");
	SetUSBPhy(0xe3, 0xc1);
	if (GetUSBPhy(0xe3) != 0xc1)
		printk("reg 0xe3 not correct\n");
	SetUSBPhy(0xe5, 0x91);
	if (GetUSBPhy(0xe5) != 0x91)
		printk("reg 0xe5 not correct\n");
...
//config port 1 u28phy
	...
	if (uphy_port1 == 2) {
		...
		if (GetUSBPhy_1(0xf4) != 0x9b)
			printk("reg 0xf4 not correct!!!!!!!!!\n");
		if (GetUSBPhy_1(0xe0) != 0xe1)
			printk("reg 0xe0 not correct!!!!!!!!!!!\n");
		if (GetUSBPhy_1(0xe1) != 0x2e)
			printk("reg 0xe1 not correct!!!!!!!!!!\n");
		if (GetUSBPhy_1(0xe2) != 0xb5)
			printk("reg 0xe2 not correct!!!!!!!!!!\n");
		if (GetUSBPhy_1(0xe4) != 0x9b)
			printk("reg 0xe4 not correct!!!!!!!!!!\n");
		if (GetUSBPhy_1(0xe6) != 0xca)
			printk("reg 0xca not correct!!!!!!!!\n");
		...
		if (GetUSBPhy_1(0xf4) != 0xbb)
			printk
			    ("port 1 reg 0xf4  not correct!!!!!!!!!!!!!!!!!!!\n");
		if (GetUSBPhy_1(0xe0) != 0x25)
			printk("p1 reg 0xe0 not correct!!!!!!!!!!!!!!!!\n");
		if (GetUSBPhy_1(0xe1) != 0xef)
			printk("p1 reg 0xe1 not correct!!!!!!!!!!!!!!!!!!\n");
		if (GetUSBPhy_1(0xe5) != 0x0f)
			printk("p1 reg 0xe5 not correct!!!!!!!!!!!!!!!!!\n");
		if (GetUSBPhy_1(0xe6) != 0x18)
			printk("p1 reg 0xe6 not correct!!!!!!!!!!!!!!!!!!!!\n");
		if (GetUSBPhy_1(0xe7) != 0xe3)
			printk
			    ("p1 reg 0xe7 not correct!!!!!!!!!!!!!!!!!!!!!!\n");
...
//u28 phy port 1
	...
	if (GetUSBPhy_1(0xf4) != 0xbb)
		printk("port 1 reg 0xf4  not correct!!!!!!!!!!!!!!!!!!!\n");
	if (GetUSBPhy_1(0xe0) != 0x25)
		printk("p1 reg 0xe0 not correct!!!!!!!!!!!!!!!!\n");
	if (GetUSBPhy_1(0xe1) != 0xef)
		printk("p1 reg 0xe1 not correct!!!!!!!!!!!!!!!!!!\n");
	...
	if (GetUSBPhy_1(0xe6) != 0x18)
		printk("p1 reg 0xe6 not correct!!!!!!!!!!!!!!!!!!!!\n");
	if (GetUSBPhy_1(0xe7) != 0xe3)
		printk("p1 reg 0xe7 not correct!!!!!!!!!!!!!!!!!!!!!!\n");
	//}
	//else
	//      printk("U55 PHY \n");
#endif
