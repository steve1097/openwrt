...
static int rtl819x_gpio_direction_input(struct gpio_chip *chip, unsigned pin)
{
	unsigned long flags;
	...
	case BSP_GPIO_PIN_H5:
		printk(KERN_ERR
		       "rtl819x_gpio: request rejection, GPIO%d is only for output\n",
		       pin);
		return -EINVAL;
	}
...
void rtl819x_gpio_pin_enable(u32 pin)
{
	...
	if (mask != 0)
		__raw_writel(((mux & (~mask)) | (val)),
			     (void __iomem *)mux_reg);

	/* 0 as BSP_GPIO pin */
	if (pin < BSP_GPIO_PIN_MAX) {
		__raw_writel(__raw_readl((void __iomem *)BSP_GPIO_CNR_REG(pin))
			     & ~(1 << BSP_GPIO_BIT(pin)),
