...
static void bsp_gpio_irq_demux(unsigned int irq)
{
	...
	start =
	    (irq == BSP_GPIO_PABCD_IRQ) ? BSP_GPIO_PIN_A0 : BSP_GPIO_2ND_REG;
	end = (irq == BSP_GPIO_PABCD_IRQ) ? BSP_GPIO_2ND_REG : BSP_GPIO_PIN_MAX;
	status =
	    __raw_readl((void __iomem *)((irq == BSP_GPIO_PABCD_IRQ) ?
					 BSP_PABCD_ISR : BSP_PEFGH_ISR));
	mask1 =
	    __raw_readl((void __iomem *)((irq == BSP_GPIO_PABCD_IRQ) ?
					 BSP_PAB_IMR : BSP_PEF_IMR));
	mask2 =
	    __raw_readl((void __iomem *)((irq == BSP_GPIO_PABCD_IRQ) ?
					 BSP_PCD_IMR : BSP_PGH_IMR));

	/* get the mask for interrupt status register by ourself */
	for (i = start; i < end; i++) {
		unsigned int m = (BSP_GPIO_BIT(i) < 16) ? mask1 : mask2;
		mask3 |=
		    ((0x3 << BSP_GPIO_2BITS(i)) & m ? 1 : 0) << BSP_GPIO_BIT(i);
	}

	/* mask the pins which don't have interrupt */
	status &= mask3;
	pr_debug
	    ("[%s][%d]: GPIO interrupt demux, mask1=0x%8x mask2=0x%x mask3=0x%x status=0x%x\n",
...
static void bsp_gpio_irq_mask(struct irq_data *data)
{
	...
	unsigned int mask =
	    __raw_readl((void __iomem
			 *)(BSP_GPIO_IMR_REG(BSP_IRQ_TO_GPIO(irq))));

	gpio_pin_int_masks[BSP_IRQ_TO_GPIO(irq)] =
	    (mask >> BSP_GPIO_2BITS(BSP_IRQ_TO_GPIO(irq))) & 0x3;
...
static void bsp_gpio_irq_unmask(struct irq_data *data)
{
	...
	mask |=
	    (gpio_pin_int_masks[BSP_IRQ_TO_GPIO(irq)] << BSP_IRQ_TO_GPIO(irq));
	__raw_writel(mask,
		     (void __iomem *)BSP_GPIO_IMR_REG(BSP_IRQ_TO_GPIO(irq)));
...
static int bsp_gpio_irq_set_type(struct irq_data *data, unsigned int type)
{
	...
	unsigned int mask =
	    __raw_readl((void __iomem
			 *)(BSP_GPIO_IMR_REG(BSP_IRQ_TO_GPIO(data->irq))));
	...
	__raw_writel(mask, (void __iomem *)BSP_GPIO_IMR_REG(pin));
	return 0;
}
...
void bsp_gpio_irq_init(int irq_base)
{
	...
	for (irq = irq_base; irq < irq_base + BSP_GPIO_PIN_MAX; irq++)
