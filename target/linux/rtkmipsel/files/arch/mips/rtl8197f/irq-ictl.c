...
irqreturn_t bsp_ictl_irq_dispatch(int cpl, void *dev_id)
{
	unsigned int pending =
	    REG32(BSP_GISR) & REG32(BSP_GIMR) & (~(BSP_GIMR_TOCPU_MASK));
	int irq;
	...
	if (irq >= 0) {
		...
		unsigned int pending2 =
		    REG32(BSP_GISR2) & REG32(BSP_GIMR2) &
		    (~(BSP_GIMR_TOCPU_MASK2));
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0)
		irq_set_chip_and_handler(irq_base + i, &bsp_ictl_irq,
					 handle_level_irq);
#else
		set_irq_chip_and_handler(irq_base + i, &bsp_ictl_irq,
					 handle_level_irq);
#endif
