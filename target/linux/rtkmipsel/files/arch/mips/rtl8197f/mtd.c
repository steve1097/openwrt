...
#ifdef CONFIG_MTD_CONCAT
offset: (CONFIG_RTL_SPI_FLASH1_SIZE +
	  CONFIG_RTL_SPI_FLASH2_SIZE - RTL_HOMEKIT_SIZE),
#else
offset: (CONFIG_RTL_SPI_FLASH1_SIZE - RTL_HOMEKIT_SIZE),
#endif
#else
...
offset: (CONFIG_RTL_FLATFS_IMAGE_OFFSET -
	  RTL_WAPI_SIZE - RTL_HOMEKIT_SIZE),
#else
offset: (CONFIG_RTL_FLASH_SIZE - RTL_WAPI_SIZE - RTL_HOMEKIT_SIZE),
#endif
#endif
	 }
	,
...
#else
#ifdef CONFIG_MTD_CHAR
offset: (CONFIG_RTL_FLATFS_IMAGE_OFFSET - RTL_HOMEKIT_SIZE),
#else
offset: (CONFIG_RTL_FLASH_SIZE - RTL_HOMEKIT_SIZE),
#endif
#endif
	 }
...
{name: "linux", offset: 0x30000, size:0x130000,},
	//0x130000+ rootfs
...
static inline void rtk_read_bootinfo_from_flash(unsigned int bootinfo_addr,
...
static void rtk_init_bootinfo(BOOTINFO_P boot)
{
	...
	printk
	    ("Error!!Please update ur bootcode to support dualimage function !!\n");
...
void detect_flash_map(struct mtd_partition *rtl819x_parts)
{
	...
	while ((offset) < mtd_size) {
		...
		if (*((unsigned int *)buf) == ntohl(SQUASHFS_MAGIC_LE)) {
			...
			rtl819x_parts[1].offset =
			    active_bank_offset + RTK_LINUX_PART_OFFSET;
			rtl819x_parts[1].size =
			    (mtd_size + active_bank_offset) -
			    rtl819x_parts[1].offset;
			break;
		}
