...
static int __init rtl819x_spi_peripheral_init(void)
{
	...
	if (err) {
		printk(KERN_ERR
		       "rtl819x_spi: cannot add rtl819x SPI platform device, error=%d\n",
		       err);
		return err;
	}
#ifdef CONFIG_RTL819X_DW_SPI0
	/*Enable SPIC0 */
	__raw_writel((__raw_readl((void __iomem *)BSP_PIN_MUX_SEL0) &
		      (~0x0fff0000)) | 0x02220000,
		     (void __iomem *)BSP_PIN_MUX_SEL0);
	__raw_writel((__raw_readl((void __iomem *)BSP_PIN_MUX_SEL2) &
		      (~0x000f0000)) | 0x00030000,
