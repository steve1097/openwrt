...
int32 HMACMD5(uint8 * data, uint32 dataLen, uint8 * key, uint32 keyLen,
	      uint8 * digest)
{
	MD5_CTX context;
	uint8 k_ipad[HMAC_MAX_MD_CBLOCK + 1], k_opad[HMAC_MAX_MD_CBLOCK + 1],
	    tk[MD5_DIGEST_LENGTH];
	uint32 i;
...
int32 HMACSHA1(uint8 * data, uint32 dataLen, uint8 * key, uint32 keyLen,
	       uint8 * digest)
{
	SHA1_CTX context;
	uint8 k_ipad[HMAC_MAX_MD_CBLOCK + 1], k_opad[HMAC_MAX_MD_CBLOCK + 1],
	    tk[SHA_DIGEST_LENGTH];
	uint32 i;
