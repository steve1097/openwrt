...
static uint8 g_authPadding[HMAC_MAX_MD_CBLOCK * 2 + 32]
...
int32 __rtl_ipsecEngine_init(uint32 descNum, int8 mode32Bytes)
{
	...
	WRITE_MEM32(CLK_MANAGE,
		    READ_MEM32(CLK_MANAGE) | _ACTIVE_IPSEC | _ACTIVE_1X1 |
		    _ACTIVE_1X1_ARB);
#endif
	 ...
	 */
	WRITE_MEM32(OTG_CONTROL,
		    READ_MEM32(OTG_CONTROL) &
		    (~(_ACTIVE_OTGCTRL | _OTGCTRL_STRAT | _PJ_RESET_ENABLE)));
...
#ifdef CONFIG_RTL_8197F
	WRITE_MEM32(IPSCTR,
		    IPS_SWRXFF | IPS_SWHASH | IPS_SWTXFF | IPS_LXLITTLE |
		    IPS_CKE | burstSize | IPS_DDLEN_8);
#else
	WRITE_MEM32(IPSCTR, 0 /*IPS_LBKM */  | IPS_SAWB | IPS_CKE | burstSize);
#endif
	/* write 1 to clear */
	WRITE_MEM32(IPSCSR,
		    READ_MEM32(IPSCSR) | IPS_SDUEIP | IPS_SDLEIP | IPS_DDUEIP |
		    IPS_DDOKIP);
...
int32 rtl_ipsecEngine(uint32 modeCrypto, uint32 modeAuth,
		      uint32 cntScatter, rtl_ipsecScatter_t * scatter,
		      void *pCryptResult, uint32 lenCryptoKey, void *pCryptoKey,
		      uint32 lenAuthKey, void *pAuthKey, void *pIv, void *pPad,
		      void *pDigest, uint32 a2eo, uint32 enl)
{
	...
	if (g_rtl_ipsec_dbg) {
		printk
		    ("%s: modeCrypto=0x%x, modeAuth=0x%x, lenCryptoKey=%d, pCryptoKey=%p, "
		     "lenAuthKey=%d, pAuthKey=%p, pIv=%p, a2eo=%d, enl=%d, cntScatter=%d\n",
		     __FUNCTION__, modeCrypto, modeAuth, lenCryptoKey,
		     pCryptoKey, lenAuthKey, pAuthKey, pIv, a2eo, enl,
		     cntScatter);
	}
	...
	if (unlikely(totalLen != a2eo + enl)) {
		printk
		    ("%s():%d Parameters error: totalLen(%d) != a2eo+enl(%d).\n",
	...
	*****************************************/
		...
		if (hmac) {
			if (unlikely(totalLen + apl >= 0x3FC0)) {	/* 2**14, MAX_PKTLEN */
				printk("%s():%d dmaLen=%d too long.\n",
				       __FUNCTION__, __LINE__, totalLen + apl);
	...
	if (modeCrypto != _MD_NOCRYPTO) {
		if (modeCrypto & _MASK_CRYPTOAES) {	/* AES */
			if (unlikely
			    (totalLen == 0 || totalLen > 16368 || enl & 0xf)) {
				printk
				    ("%s():%d Invalid input length (total=%d, enl=%d)\n",
			...
			default:
				printk
				    ("%s():%d Invalid AES key length: %d bytes.\n",
			...
			default:
				printk
				    ("%s():%d Unsupported crypto mode: %02x\n",
			...
			if (unlikely(srcDesc->sdbp & 0x3)) {
				printk
				    ("%s():%d Unable to process non-4-byte aligned KEY\n",
			...
			if (unlikely(srcDesc->sdbp & 0x3)) {
				printk
				    ("%s():%d Unable to process non-4-byte aligned IV\n",
				     __FUNCTION__, __LINE__);
				return FAILED;
			}
			...
			if (unlikely
			    (totalLen == 0 || totalLen > 16376 || enl & 0x7)) {
				printk
				    ("%s():%d Invalid input length (total=%d, enl=%d)\n",
				     __FUNCTION__, __LINE__, totalLen, enl);
				return FAILED;
			...
			if (modeCrypto & 1) {
				...
				if (unlikely(lenCryptoKey != 64 / 8 * 3)) {
					printk
					    ("%s():%d Invalid 3DES key length: %d bytes.\n",
					     __FUNCTION__, __LINE__,
					     lenCryptoKey);
					return FAILED;
				}
				...
				if (unlikely(lenCryptoKey != 64 / 8)) {
					printk
					    ("%s():%d Invalid DES key length: %d bytes.\n",
					     __FUNCTION__, __LINE__,
					     lenCryptoKey);
					return FAILED;
				}
	...
	if (modeAuth != _MD_NOAUTH) {
			...
			if (modeAuth & _MASK_AUTH_IOPAD_READY) {
				...
				if (lenAuthKey > HMAC_MAX_MD_CBLOCK) {
					if (modeAuth & _MASK_AUTHSHA1MD5) {
						...
						SHA1Update(&sha1Context,
							   pAuthKey,
							   lenAuthKey);
						SHA1Final(AuthTempKey,
							  &sha1Context);
						...
						/* MD5 */
						MD5Init(&md5Context);
						MD5Update(&md5Context, pAuthKey,
							  lenAuthKey);
						MD5Final(AuthTempKey,
							 &md5Context);
						pAuthKey = AuthTempKey;
				...
				g_rtl_authEngineOutputPad = (uint8 *)
				    UNCACHED_ADDRESS(&g_IOPAD
						     [HMAC_MAX_MD_CBLOCK]);

				memset(g_rtl_authEngineInputPad, 0x36,
				       HMAC_MAX_MD_CBLOCK);
				memset(g_rtl_authEngineOutputPad, 0x5c,
				       HMAC_MAX_MD_CBLOCK);

				for (i = 0; i < lenAuthKey; i++) {
					g_rtl_authEngineInputPad[i] ^=
					    ((uint8 *) pAuthKey)[i];
					g_rtl_authEngineOutputPad[i] ^=
					    ((uint8 *) pAuthKey)[i];
				}
		...
		if (unlikely(srcDesc->sdbp & 0x3)) {
			printk
			    ("%s():%d Unable to process non-4-byte aligned HMAC_PAD\n",
			     __FUNCTION__, __LINE__);
			return FAILED;
		}
	...
	if (modeAuth != _MD_NOAUTH) {
		...
		if (unlikely(srcDesc->sdbp & 0x3)) {
			printk
			    ("%s():%d Unable to process non-4-byte aligned AUTH_PAD\n",
			     __FUNCTION__, __LINE__);
			return FAILED;
		}
	}
	...
	if (unlikely(((modeCrypto != (uint32) - 1) && (modeCrypto & 0x08)) ||
		...
		/* blocking mode */
		g_idxAsicSrc = ((rtl_ipsec_source_t *)
				UNCACHED_ADDRESS(READ_MEM32(IPSSDAR))) -
		    g_ipssdar;
		assert(g_idxAsicSrc == g_idxCpuSrc);
		g_idxAsicDst =
		    ((rtl_ipsec_dest_t *) UNCACHED_ADDRESS(READ_MEM32(IPSDDAR)))
		    - g_ipsddar;
...
RE_POLL:
	...
	src_offset =
	    (unsigned long)(UNCACHED_ADDRESS(READ_MEM32(IPSSDAR))) -
	    (unsigned long)(&g_ipssdar[0]);
	g_idxCpuSrc =
	    (src_offset == 0 ? 0 : (src_offset / sizeof(rtl_ipsec_source_t)));

	dst_offset =
	    (unsigned long)(UNCACHED_ADDRESS(READ_MEM32(IPSDDAR))) -
	    (unsigned long)(&g_ipsddar[0]);
	g_idxCpuDst =
	    (dst_offset == 0 ? 0 : (dst_offset / sizeof(rtl_ipsec_dest_t)));
	...
	if (g_idxCpuSrc_pre != g_idxCpuSrc || g_idxCpuDst_pre != g_idxCpuDst) {
		...
		printk
		    ("\tsrc: pre[0x%x], last[0x%x], reg[0x%x]; idx[%d], re-cal[%d]\n",
		     pre_src_ptr, src_ptr,
		     (unsigned long)(UNCACHED_ADDRESS(READ_MEM32(IPSSDAR))),
		     g_idxCpuSrc_pre, g_idxCpuSrc);
		printk
		    ("\tdst: pre[0x%x], last[0x%x], reg[0x%x]; idx[%d], re-cal[%d]\n\n",
		     pre_dst_ptr, dst_ptr,
		     (unsigned long)(UNCACHED_ADDRESS(READ_MEM32(IPSDDAR))),
		     g_idxCpuDst_pre, g_idxCpuDst);
	}
	...
	*(uint32 *) & g_ipsddar[g_idxCpuDst] = *(uint32 *) & prepDst[0];
#endif
	memcpy((char *)&g_ipsddar[g_idxCpuDst], (char *)&prepDst[0],
	       sizeof(g_ipsddar[0]));
	...
	for (i = 0; i < numSrcDesc; i++) {
...
#endif
		{
			...
			prepSrc[i].eor =
			    (((g_idxCpuSrc + i) % g_numSrc) == (g_numSrc - 1));
#if 0
			/* skip first word. */
			memcpy(((char *)
				&g_ipssdar[(g_idxCpuSrc + i) % g_numSrc]) + 4,
			       ((char *)&prepSrc[i]) + 4,
			       sizeof(g_ipssdar[0]) - 4);
			/* copy the first word. */
			*(uint32 *) & g_ipssdar[(g_idxCpuSrc + i) % g_numSrc] =
			...
			printk("%s():%d[%d] g_numSrc=%d, prepNum=%d\n",
			       __FUNCTION__, __LINE__,
			       (g_idxCpuSrc + i) % g_numSrc, g_numSrc,
			       numSrcDesc);
			return FAILED;
		}
#endif /* !HW_REINIT */
...
#ifdef DBG_HW_REINIT
	/* check own bit */
	if (((rtl_ipsec_source_t
	      *) (((unsigned long)(&g_ipssdar[g_idxCpuSrc % g_numSrc])) |
		  0x20000000))->own == 0) {
		printk("==> Src, Owned by CPU.\n");
	}
	if (((rtl_ipsec_dest_t *) (((unsigned long)(&g_ipsddar[g_idxCpuDst])) |
				   0x20000000))->own == 0) {
	...
	if (g_rtl_ipsec_dbg) {
		printk
		    ("g_numSrc=%d prepNum=%d g_idxCpuSrc=%d(%d) g_idxCpuDst=%d(%d)\n",
		     g_numSrc, numSrcDesc, g_idxCpuSrc, g_idxAsicSrc,
		     g_idxCpuDst, g_idxAsicDst);
...
#define MOST_SCHED_COUNT 10000
		while (((READ_MEM32(IPSCSR) & ips_mask) == 0) && (atomic_read(&schedule_count) < MOST_SCHED_COUNT)) {	/*schedule for long enough */
			...
			if ((loopWait == 0)
			    ||
			    ((atomic_read(&schedule_count) >= MOST_SCHED_COUNT)
			     && ((READ_MEM32(IPSCSR) & ips_mask) == 0))) {
				printk
				    ("%s():%d Wait Timeout. READ_MEM32(IPSCSR)=0x%08x.\n",
				     __FUNCTION__, __LINE__,
				     READ_MEM32(IPSCSR));

#ifdef DBG_HW_REINIT
				fail_cnt++;
				SAVE_DESC_ENTRY(&saved_prepSrc, &prepSrc,
						sizeof(saved_prepSrc));
				SAVE_DESC_ENTRY(&saved_prepDst, &prepDst,
						sizeof(saved_prepDst));
#endif
				...
				if (++re_poll < 5) {
					...
					reinit_ipsecEngine_hw(__func__,
							      __LINE__);
					goto RE_POLL;
				} else {
					printk
					    ("==> %s:%d, reach max re_poll, fail\n",
					     __FUNCTION__, __LINE__);
		...
		assert(g_ipsddar[(g_idxCpuDst + g_numDst - 1) % g_numDst].own ==
		       0);

		ips_mask = IPS_SDUEIP | IPS_SDLEIP | IPS_DDUEIP | IPS_DABFIP;
		if (READ_MEM32(IPSCSR) & ips_mask) {
			printk
			    ("%s():%d IPS_SDLEIP or IPS_DABFI is ON. READ_MEM32(IPSCSR)=0x%08x.\n",
		...
		if (READ_MEM32(IPSCSR) & IPS_POLL) {
			...
			if (READ_MEM32(IPSCSR) & IPS_POLL) {
				printk
				    ("==> %s:%d, afte hw reinit, poll bit is still set??\n",
				     __func__, __LINE__);
			}
		}
#endif /*  HW_REINIT */
	...
	if (modeAuth != _MD_NOAUTH) {
		if (g_rtl_ipsec_dbg) {
			if (md5)
				memDump(g_ipsddar
					[(g_idxCpuDst + g_numDst -
					  1) % g_numDst].icv, MD5_DIGEST_LENGTH,
					"MD5 Digest: g_ipsddar[(g_idxCpuDst+g_numDst-1) % g_numDst].icv");
			else
				memDump(g_ipsddar
					[(g_idxCpuDst + g_numDst -
					  1) % g_numDst].icv, SHA_DIGEST_LENGTH,
					"SHA Digest: g_ipsddar[(g_idxCpuDst+g_numDst-1) % g_numDst].icv");
		}

		if (md5)
			memcpy(pDigest, g_ipsddar[(g_idxCpuDst + g_numDst - 1) % g_numDst].icv, MD5_DIGEST_LENGTH);	/* Avoid 4-byte alignment limitation */
		else
			memcpy(pDigest, g_ipsddar[(g_idxCpuDst + g_numDst - 1) % g_numDst].icv, SHA_DIGEST_LENGTH);	/* Avoid 4-byte alignment limitation */
	}
...
int32 rtl_ipsecSetOption(enum RTL_IPSEC_OPTION option, uint32 value)
{
	...
	case RTL_IPSOPT_DMBS:
		if (value == 16)
			WRITE_MEM32(IPSCTR,
				    (READ_MEM32(IPSCTR) & ~IPS_DMBS_MASK) |
				    IPS_DMBS_16);
		else if (value == 32)
			WRITE_MEM32(IPSCTR,
				    (READ_MEM32(IPSCTR) & ~IPS_DMBS_MASK) |
	...
	case RTL_IPSOPT_SMBS:
		if (value == 16)
			WRITE_MEM32(IPSCTR,
				    (READ_MEM32(IPSCTR) & ~IPS_SMBS_MASK) |
				    IPS_SMBS_16);
		else if (value == 32)
			WRITE_MEM32(IPSCTR,
				    (READ_MEM32(IPSCTR) & ~IPS_SMBS_MASK) |
...
void rtl_ipsec_info(void)
{
	...
	g_idxAsicSrc =
	    ((rtl_ipsec_source_t *) UNCACHED_ADDRESS(READ_MEM32(IPSSDAR))) -
	    g_ipssdar;
	for (i = 0; i < g_numSrc; i++) {
		sprintf(buffer, "ipssdar[%d]:%s%s%s%s (%d+%d+%d,%d)", i,
			...
			g_ipssdar[i].eor ? " [E]" : "",
			g_ipssdar[i].a2eo,
			g_ipssdar[i].enl, g_ipssdar[i].apl, g_ipssdar[i].sbl);
	...
	sprintf(buffer, "\nSaved address:\n"
		...
		"Saved_Dst: ", pre_src_ptr, src_ptr, pre_dst_ptr, dst_ptr);
