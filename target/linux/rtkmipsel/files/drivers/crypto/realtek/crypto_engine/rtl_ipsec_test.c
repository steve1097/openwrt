...
int32 rtl_ipsecTest_alloc(void)
{
	...
	if (!cryptoKey || !cryptoIv || !authKey || !pad || !asic_orig
	    || !asic_enc || !asic_digest) {
	       ...
	       uint32 lenA2eo, uint32 lenEnl)
{
	if ((modeAuth != (uint32) - 1) && (modeCrypto != (uint32) - 1)) {
		if (modeCrypto & 0x04)	// encrypt then auth
		{
			if (modeCrypto & 0x20)	// aes 
				aesSim_aes(modeCrypto, data + lenA2eo,
					   pCryptResult + lenA2eo, lenEnl,
					   lenCryptoKey, pCryptoKey, pIv);
			else	// des
				desSim_des(modeCrypto, data + lenA2eo,
					   pCryptResult + lenA2eo, lenEnl,
					   pCryptoKey, pIv);
	...
	} else if (modeCrypto != (uint32) - 1)	// crypto only
	{
		if (modeCrypto & 0x20)	// aes
			aesSim_aes(modeCrypto, data + lenA2eo,
				   pCryptResult + lenA2eo, lenEnl, lenCryptoKey,
				   pCryptoKey, pIv);
		else		// des
			desSim_des(modeCrypto, data + lenA2eo,
				   pCryptResult + lenA2eo, lenEnl, pCryptoKey,
				   pIv);
	} else {
		return FAILED;
	}
...
char *mixModeString(int32 cryptoIdx, int32 authIdx)
{
	...
	if (cryptoIdx == -1)	// auth only
	{
		if (authIdx >= 0 && authIdx <= 3)
			len +=
			    sprintf(&modestr[len], "%s",
				    authModeString[authIdx]);
	} else if (authIdx == -1)	// encrypt/decrypt only
	{
		if (cryptoIdx >= 0x0 && cryptoIdx <= 0x3)
			len +=
			    sprintf(&modestr[len], "DECRYPT_%s",
				    cryptoModeString[cryptoIdx]);
		else if (cryptoIdx >= 0x4 && cryptoIdx <= 0x7)
			len +=
			    sprintf(&modestr[len], "ENCRYPT_%s",
				    cryptoModeString[cryptoIdx - 0x4]);
		else if (cryptoIdx >= 0x20 && cryptoIdx <= 0x23)
			len +=
			    sprintf(&modestr[len], "DECRYPT_%s",
				    cryptoModeString[cryptoIdx - 0x20 + 4]);
		else if (cryptoIdx >= 0x24 && cryptoIdx <= 0x27)
			len +=
			    sprintf(&modestr[len], "ENCRYPT_%s",
				    cryptoModeString[cryptoIdx - 0x24 + 4]);
	...
	} else			// auth first, then decode
	{
		...
		if (cryptoIdx >= 0x0 && cryptoIdx <= 0x3)
			len +=
			    sprintf(&modestr[len], ", then DECRYPT_%s",
				    cryptoModeString[cryptoIdx]);
		else if (cryptoIdx >= 0x20 && cryptoIdx <= 0x23)
			len +=
			    sprintf(&modestr[len], ", then DECRYPT_%s",
...
int32 mix8651bAsicThroughput(int32 round, int32 cryptoStart, int32 cryptoEnd,
			     int32 authStart, int32 authEnd, int32 CryptoKeyLen,
			     int32 AuthKeyLen, int32 pktLen, int32 a2eoLen)
{
...
#endif
	rtlglue_printf
	    ("Evaluate 8651b throughput(round=%d,cryptoStart=%d,cryptoEnd=%d,authStart=%d,authEnd=%d,"
	     "CryptoKeyLen=%d,AuthKeyLen=%d,pktLen=%d,a2eoLen=%d)\n", round,
	     cryptoStart, cryptoEnd, authStart, authEnd, CryptoKeyLen,
	     AuthKeyLen, pktLen, a2eoLen);
	...
	for (cryptoIdx = cryptoStart; cryptoIdx <= cryptoEnd; cryptoIdx++) {
		...
		if (cryptoIdx != (uint32) - 1) {
			if (cryptoIdx & 0x20) {	/* AES, ENL is the times of 16 bytes. */
				if (lenEnl & 0xf) {
					SMP_UNLOCK_IPSEC;
		...
		if (cryptoIdx == 0x00 || cryptoIdx == 0x02 || cryptoIdx == 0x04
		    || cryptoIdx == 0x06)
			CryptoKeyLen = 8;	// DES
		else if (cryptoIdx == 0x01 || cryptoIdx == 0x03
			 || cryptoIdx == 0x05 || cryptoIdx == 0x07)
		...
		for (authIdx = authStart; authIdx <= authEnd; authIdx++) {
			...
			for (testRound = 0; testRound <= round; testRound++) {
				if (rtl_ipsecEngine
				    (cryptoIdx, authIdx, cntScatter, src,
				     pAsicCrypt, CryptoKeyLen, pCryptoKey,
				     AuthKeyLen, pAuthKey, pIv, pPad,
				     pAsicDigest, a2eoLen, lenEnl) != SUCCESS) {
					SMP_UNLOCK_IPSEC;
					rtlglue_printf
					    ("testRound=%d, rtl8651x_ipsecEngine(modeIdx=%d,authIdx=%d) failed... pktlen=%d\n",
					     testRound, cryptoIdx, authIdx,
					     pktLen);
					return FAILED;
				}
			}
			...
			if (eTime - sTime == 0) {
				SMP_UNLOCK_IPSEC;
				rtlglue_printf
				    ("round is too small to measure throughput, try larger round number!\n");
				return FAILED;
			} else {
				bps =
				    pktLen * 8 * 1000 /
				    ((uint32) (eTime - sTime)) * round;
				if (bps > 1000000)
					rtlglue_printf
					    ("%s round %u len %u time %u throughput %u.%02u mbps\n",
					...
					rtlglue_printf
					    ("%s round %u len %u time %u throughput %u.%02u kbps\n",
					     mixModeString(cryptoIdx, authIdx),
					     round, pktLen,
					     (uint32) (eTime - sTime),
					...
					rtlglue_printf
					    ("%s round %u len %u time %u throughput %u bps\n",
					     mixModeString(cryptoIdx, authIdx),
					     round, pktLen,
					     (uint32) (eTime - sTime), bps);
			}
		}
	}
	SMP_UNLOCK_IPSEC;
...
static int32 ipsec_GeneralApiTestItem(uint32 modeCrypto, uint32 modeAuth,
				      ...
				      void *pCryptResult, uint32 lenCryptoKey,
				      void *pCryptoKey, uint32 lenAuthKey,
				      void *pAuthKey, void *pIv, void *pPad,
				      void *pDigest, uint32 lenA2eo,
				      uint32 lenEnl)
{
	int i;
	int32 retval;
	...
	if (modeCrypto != (uint32) - 1) {
		if (sawb)	// use sawb, ASIC write back to scatter buffer
		{
			cur = sw_enc;
			for (i = 0; i < cntScatter; i++) {
				if (memcmp(cur, scatter[i].ptr, scatter[i].len)
				    != 0) {
					printk
					    ("ENC DIFF![%d]crypt:%x auth:%x CKey:%d AKey:%d lenA2eo:%d\n",
					     i, modeCrypto, modeAuth,
					     lenCryptoKey, lenAuthKey, lenA2eo);
					memDump(cur, scatter[i].len,
						"Software");
					memDump(scatter[i].ptr, scatter[i].len,
						"ASIC");
					return FAILED;
				}
				cur += scatter[i].len;
		...
		{
			if (memcmp
			    (sw_enc + lenA2eo, pCryptResult + lenA2eo,
			     lenEnl) != 0) {
				printk
				    ("ENC DIFF![%d]crypt:%x auth:%x CKey:%d AKey:%d lenA2eo:%d\n",
				     i, modeCrypto, modeAuth, lenCryptoKey,
				     lenAuthKey, lenA2eo);
	...
	if (modeAuth != (uint32) - 1) {
		if (memcmp
		    (sw_digest, pDigest,
		     modeAuth & 1 ? 20 /*SHA1 */ : 16 /*MD5 */ ) != 0) {
			printk
			    ("ENC DIGEST! crypt:%x auth:%x CKey:%d AKey:%d lenA2eo:%d\n",
			     modeCrypto, modeAuth, lenCryptoKey, lenAuthKey,
			     lenA2eo);
			memDump(sw_digest,
				modeAuth & 1 ? 20 /*SHA1 */ : 16 /*MD5 */ ,
				"Software");
			memDump(pDigest,
				modeAuth & 1 ? 20 /*SHA1 */ : 16 /*MD5 */ ,
				"ASIC");
	...
	if (modeCrypto == 0x20	/*AES_CBC */
	    || modeCrypto == 0x22 /*AES_ECB */ ) {
		AES_KEY aes_key;
		...
		switch (lenCryptoKey) {
		case 128 / 8:
			memcpy((void *)UNCACHED_ADDRESS(key_sch),
			       &aes_key.rd_key[4 * 10], 16);
			break;
		case 192 / 8:
			memcpy((void *)UNCACHED_ADDRESS(key_sch),
			       &aes_key.rd_key[4 * 12], 16);
			memcpy((void *)UNCACHED_ADDRESS(&key_sch[16]),
			       &aes_key.rd_key[4 * 11 + 2], 8);
			break;
		case 256 / 8:
			memcpy((void *)UNCACHED_ADDRESS(key_sch),
			       &aes_key.rd_key[4 * 14], 16);
			memcpy((void *)UNCACHED_ADDRESS(&key_sch[16]),
			       &aes_key.rd_key[4 * 13], 16);
			break;
		default:
	...
	retval = rtl_ipsecEngine(modeCrypto /*dec */ , modeAuth,
				 cntScatter, scatter, pCryptResult,
				 lenCryptoKey, pDecryptoKey, lenAuthKey,
				 pAuthKey, pIv, pPad, pDigest, lenA2eo, lenEnl);
	...
	if (modeAuth != (uint32) - 1) {
		if (memcmp
		    (sw_digest, pDigest,
		     modeAuth & 1 ? 20 /*SHA1 */ : 16 /*MD5 */ ) != 0) {
			printk
			    ("DEC DIGEST! crypt:%x auth:%x CKey:%d AKey:%d lenA2eo:%d\n",
			     modeCrypto, modeAuth, lenCryptoKey, lenAuthKey,
			     lenA2eo);
			memDump(sw_digest,
				modeAuth & 1 ? 20 /*SHA1 */ : 16 /*MD5 */ ,
	...
	if (modeCrypto != (uint32) - 1) {
		if (memcmp(sw_orig + lenA2eo, sw_dec + lenA2eo, lenEnl) != 0) {
			printk
			    ("DEC DIFF![%d]crypt:%x auth:%x CKey:%d AKey:%d lenA2eo:%d\n",
			     i, modeCrypto, modeAuth, lenCryptoKey, lenAuthKey,
			     lenA2eo);
		...
		{
			if (memcmp
			    (sw_dec + lenA2eo, pCryptResult + lenA2eo,
			     lenEnl) != 0) {
				printk
				    ("DEC DIFF![%d]crypt:%x auth:%x CKey:%d AKey:%d lenA2eo:%d\n",
				     i, modeCrypto, modeAuth, lenCryptoKey,
	...
	for (roundIdx = 0; roundIdx < round; roundIdx++) {
		for (pktLen = lenStart; pktLen <= lenEnd; pktLen += 8) {
			for (offset = offsetStart; offset <= offsetEnd;
				...
				for (modeCrypto = cryptoStart;
				     modeCrypto <= cryptoEnd; modeCrypto++) {
					for (modeAuth = authStart;
					     modeAuth <= authEnd; modeAuth++) {
						for (lenCryptoKey =
						     cryptoKeyStart;
						     lenCryptoKey <=
						     cryptoKeyEnd;
							...
							for (lenAuthKey =
								...
								for (lenA2eo =
									    ...
									    FAILED)
									{
										SMP_UNLOCK_IPSEC;
										printk
										    ("Len:%d Offser:%d Crypto:0x%x Auth:0x%x CKey:%d AKey:%d A2EO:%d [FAILED]\n",
										     pktLen,
										     offset,
										     modeCrypto,
										     modeAuth,
									     ...
									     1)
									{
										...
										printk
										    ("Len:%d Offset:%d Crypto:0x%x Auth:0x%x CKey:%d AKey:%d A2EO:%d [SKIP]\n",
										     pktLen,
										     offset,
										     modeCrypto,
										     modeAuth,
...
#if defined(CRYPTOTEST_DEBUG)
										printk
										    ("Len:%d Offser:%d Crypto:0x%x Auth:0x%x CKey:%d AKey:%d A2EO:%d [SUCCESS]\n",
										     pktLen,
										     offset,
										     modeCrypto,
										     modeAuth,
...
int32 ipsec_GeneralApiRandTest(uint32 seed, uint32 round)
{
	...
	int32 ret = SUCCESS;
	int32 modeCrypto_val[8] =
	    { -1, 0x00, 0x01, 0x02, 0x03, 0x20, 0x22, 0x23 };
	...
	for (roundIdx = 0; roundIdx < round; roundIdx++) {
		...
		} else		// mix mode
		{
			...
			if ((cntScatter == 1) || (!sawb)) {
				...
				max_a2eo =
				    152 +
				    dma_bytes[rtl_ipsecEngine_dma_mode()] - 16;
			} else {
				max_a2eo = 152 - 16;
			}
		...
		if (ret == FAILED) {
			...
			printk
			    ("Len:%d Offser:%d Crypto:0x%x Auth:0x%x CKey:%d AKey:%d A2EO:%d [FAILED]\n",
			     pktLen, offset, modeCrypto, modeAuth, lenCryptoKey,
			     lenAuthKey, lenA2eo);
			return FAILED;
		} else if (ret > 0) {
#if defined(CRYPTOTEST_DEBUG)
			printk
			    ("Len:%d Offser:%d Crypto:0x%x Auth:0x%x CKey:%d AKey:%d A2EO:%d [SKIP %d]\n",
			     pktLen, offset, modeCrypto, modeAuth, lenCryptoKey,
			     lenAuthKey, lenA2eo, ret);
#endif
		} else {
#if defined(CRYPTOTEST_DEBUG)
			printk
...
void rtl_ipsec_test(int test)
{
	...
	if (test == 10) {
		...
		case 11:	// DES & 3DES throughput
			mix8651bAsicThroughput(10000, 0, 7, 0, 3, 0, 8, 1400,
					       16);
			break;

		case 12:	// AES 128 throughput
			mix8651bAsicThroughput(10000, 0x20, 0x27, 2, 3, 16, 8,
					       1408, 16);
			break;

		case 13:	// AES 192 throughput
			mix8651bAsicThroughput(10000, 0x20, 0x27, 2, 3, 24, 8,
					       1408, 16);
			break;

		case 14:	// AES 256 throughput
			mix8651bAsicThroughput(10000, 0x20, 0x27, 2, 3, 32, 8,
					       1408, 16);
			break;

		case 15:	// DES test
			ipsec_GeneralApiTest(1, 1408, 1408, 0, 0, 0, 3, 0, 3, 8,
					     24, 16, 16, 16, 16);
			break;

		case 16:	// AES test
			ipsec_GeneralApiTest(1, 1408, 1408, 0, 0, 0x20, 0x23, 0,
					     3, 16, 32, 16, 16, 16, 16);
			break;
...
#if !defined(CRYPTOTEST_USE_UNCACHED_MALLOC)
				cryptoKey =
				    (void *)UNCACHED_ADDRESS(&_cryptoKey[32]);
				cryptoIv =
				    (void *)UNCACHED_ADDRESS(&_cryptoIv[32]);
				asic_orig =
				    (void *)UNCACHED_ADDRESS(&_asic_orig[32]);
				asic_enc =
				    (void *)UNCACHED_ADDRESS(&_asic_enc[32]);
#else
				rtl_ipsecTest_Init();
#endif
				AES_set_encrypt_key(key, klen * 8, &aes_key);
				memcpy((void *)cryptoKey,
				       &aes_key.rd_key[4 * 10], 16);
		...
		case 44:	//ecb(des)  and cbc(des)
			...
			if (ret == 0)
				printk
				    ("ecb(des)  and cbc(des) test success! ret=%d \n",
				     ret);
			else
				printk
				    ("ecb(des)  and cbc(des) test failed! ret=%d \n",
				     ret);
			break;
		...
		case 45:	//ecb(des3_ede) and cbc(des3_ede)
			...
			if (ret == 0)
				printk
				    ("ecb(des3_ede) and cbc(des3_ede) test success! ret=%d \n",
				     ret);
			else
				printk
				    ("ecb(des3_ede) and cbc(des3_ede) test failed! ret=%d \n",
				     ret);
			break;
		...
		case 46:	//ecb(aes) and cbc(aes) and ctr(aes)
			...
			if (ret == 0)
				printk
				    ("ecb(aes) and cbc(aes) and ctr(aes) test success! ret=%d \n",
				     ret);
			else
				printk
				    ("ecb(aes) and cbc(aes) and ctr(aes) test failed! ret=%d \n",
				     ret);
			break;
#endif
...
#else
static int32 rtk_ipsec_test_read(char *page, char **start, off_t off, int count,
				 int *eof, void *data)
{
	int len;
...
#endif
static int32 rtk_ipsec_test_write(struct file *filp, const char *buff,
				  unsigned long len, void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(tmpbuf, "all", strlen("all"))) {
			...
			if ((rtk_ipsec_test == 10) || (rtk_ipsec_test == 99)
			    || (rtk_ipsec_test == 19)
				...
				printk
				    ("current support: 10/99/11~16/19/21/31/all \n");
#if defined(CONFIG_CRYPTO_DEV_REALTEK_LINUX_SELFTEST)
				printk("                 40~46 \n");
#endif
				return len;
...
static ssize_t rtk_ipsec_test_single_write(struct file *file,
...
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("rtk_ipsec_test", 0, &proc_root,
			 &rtk_ipsec_test_proc_fops, NULL);
#else
