...
int32 aesSim_aes(uint32 mode, int8 * input, int8 * output, uint32 len,
		 uint32 lenKey, int8 * key, int8 * iv)
{
	AES_KEY aesKey;
	int done;
	...
	case SWDECRYPT_ECB_AES:
		...
		for (done = 0; done < len; done += 16)
			AES_ecb_encrypt(&input[done], &output[done], &aesKey,
					AES_DECRYPT);
		break;
	...
	case SWDECRYPT_CTR_AES:
		...
		num = 0;
		AES_ctr128_encrypt(input, output, len, &aesKey, counter,
				   ecount_buf, &num);
		break;
