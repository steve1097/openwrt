...
static uint8 authSimInputPad[HMAC_MAX_MD_CBLOCK],
    authSimOutputPad[HMAC_MAX_MD_CBLOCK];
static uint8 authSimMd5TempKey[MD5_DIGEST_LENGTH],
    authSimSha1TempKey[SHA_DIGEST_LENGTH];
...
int32 authSim_md5(uint8 * data, uint32 dataLen, uint8 * digest)
{
	...
	if (i > 8) {
		memcpy((void *)(data + dataLen), (void *)authSim_md5Paddat,
		       i - 8);
...
//Data length must be long enough to fill padding ((64+8)-byte at most)
int32 authSim_hmacMd5(uint8 * data, uint32 dataLen, uint8 * key, uint32 keyLen,
		      uint8 * digest)
{
	MD5_CTX context;
	uint32 i, dmaLen;
...
int32 authSim_hmacSha1(uint8 * data, uint32 dataLen, uint8 * key, uint32 keyLen,
		       uint8 * digest)
{
	SHA1_CTX context;
	uint32 i, dmaLen;
...
int32 authSim(uint32 mode, uint8 * data, uint32 dataLen, uint8 * key,
	      uint32 keyLen, uint8 * digest)
{
	MD5_CTX md5Context;
...
int32 authSimBatch(uint32 mode, uint8 ** data, uint32 * dataLen, uint8 ** key,
		   uint32 * keyLen, uint8 ** digest, uint32 dataNum)
{
	uint32 i;

	for (i = 0; i < dataNum; i++) {
		if (authSim
		    (mode, data[i], dataLen[i], key[i], keyLen[i],
		     digest[i]) == FAILED)
			return FAILED;
	}
	return SUCCESS;
