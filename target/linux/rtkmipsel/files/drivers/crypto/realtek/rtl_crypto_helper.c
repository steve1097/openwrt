...
int32 rtl_crypto_helper_alloc(void)
{
	...
	if (!cryptoKey || !aesDeKey || !cryptoIv || !asic_orig || !asic_enc
	    || !asic_digest) {
...
int rtl_hash_digest(struct rtl_hash_ctx *ctx, u8 * data, u32 length, u8 * out)
{
	 ...
	 */
#if defined(CRYPTOTEST_USE_UNCACHED_MALLOC)
	ret = rtl_ipsecEngine(-1, ctx->mode, 1, scatter, NULL, 0, NULL, 0, NULL,
			      //NULL, NULL, ctx->digest,      
			      NULL, NULL, out, 0, length);
	...
	ret = rtl_ipsecEngine(-1, ctx->mode, 1, scatter, NULL,
			      0, NULL, 0, NULL, NULL, NULL, out, 0, length);
	if (ctx->mode == 0x00) {
		_dma_cache_wback_inv((u32) out, MD5_DIGEST_LENGTH);
	} else {
		_dma_cache_wback_inv((u32) out, SHA_DIGEST_LENGTH);
...
int rtl_cipher_init_ctx(struct crypto_tfm *tfm, struct rtl_cipher_ctx *ctx)
{
	const char *algname = crypto_tfm_alg_name(tfm);
	...
	printk("%s: alg=%s, driver=%s, mode=%x\n", __FUNCTION__,
	       ...
	       crypto_tfm_alg_driver_name(tfm), ctx->mode);
#endif
#if defined(CRYPTOTEST_USE_UNCACHED_MALLOC)
...
int rtl_cipher_setkey(struct crypto_cipher *cipher,
		      struct rtl_cipher_ctx *ctx, const u8 * key,
		      unsigned int keylen)
{
#if defined(CRYPTOTEST_USE_UNCACHED_MALLOC)
	...
	if ((ctx->mode >= 0) && (ctx->mode & 0x20)) {
		...
		case 128 / 8:
			((u32 *) aes_dekey)[0] =
			    cpu_to_le32(round_key[4 * 10 + 0]);
			((u32 *) aes_dekey)[1] =
			    cpu_to_le32(round_key[4 * 10 + 1]);
			((u32 *) aes_dekey)[2] =
			    cpu_to_le32(round_key[4 * 10 + 2]);
			((u32 *) aes_dekey)[3] =
			    cpu_to_le32(round_key[4 * 10 + 3]);
			break;
		case 192 / 8:
			((u32 *) aes_dekey)[0] =
			    cpu_to_le32(round_key[4 * 12 + 0]);
			((u32 *) aes_dekey)[1] =
			    cpu_to_le32(round_key[4 * 12 + 1]);
			((u32 *) aes_dekey)[2] =
			    cpu_to_le32(round_key[4 * 12 + 2]);
			((u32 *) aes_dekey)[3] =
			    cpu_to_le32(round_key[4 * 12 + 3]);
			((u32 *) aes_dekey)[4] =
			    cpu_to_le32(round_key[4 * 11 + 2]);
			((u32 *) aes_dekey)[5] =
			    cpu_to_le32(round_key[4 * 11 + 3]);
			break;
		case 256 / 8:
			((u32 *) aes_dekey)[0] =
			    cpu_to_le32(round_key[4 * 14 + 0]);
			((u32 *) aes_dekey)[1] =
			    cpu_to_le32(round_key[4 * 14 + 1]);
			((u32 *) aes_dekey)[2] =
			    cpu_to_le32(round_key[4 * 14 + 2]);
			((u32 *) aes_dekey)[3] =
			    cpu_to_le32(round_key[4 * 14 + 3]);
			((u32 *) aes_dekey)[4] =
			    cpu_to_le32(round_key[4 * 13 + 0]);
			((u32 *) aes_dekey)[5] =
			    cpu_to_le32(round_key[4 * 13 + 1]);
			((u32 *) aes_dekey)[6] =
			    cpu_to_le32(round_key[4 * 13 + 2]);
			((u32 *) aes_dekey)[7] =
			    cpu_to_le32(round_key[4 * 13 + 3]);
			break;
		default:
...
int rtl_cipher_crypt(struct crypto_cipher *cipher, u8 bEncrypt,
		     struct rtl_cipher_ctx *ctx, u8 * src, unsigned int nbytes,
		     u8 * iv, u8 * dst)
{
	unsigned int bsize = crypto_cipher_blocksize(cipher);
	u8 *key =
	    bEncrypt ? ctx->key : (ctx->mode & 0x20) ? ctx->aes_dekey : ctx->
	    key;
	rtl_ipsecScatter_t scatter[1];
...
#ifdef CONFIG_RTK_VOIP_DBG
	printk
	    ("%s: src=%p, len=%d, blk=%d, key=%p, iv=%p, dst=%p ctx->mode=0x%x bEncrypt=%d\n",
	     __FUNCTION__, src, nbytes, bsize, key, iv, dst, ctx->mode,
	     bEncrypt);
	...
	err = rtl_ipsecEngine(ctx->mode | flag_encrypt,
			      ...
			      0, NULL,
			      (void *)cryptoIv, NULL, NULL, 0, scatter[0].len);
#endif
	if (unlikely(err))
