...
static void rtk_nand_read_id(struct mtd_info *mtd, unsigned char id[5],
			     int chip_sel);
static int rtk_read_oob(struct mtd_info *mtd, u16 chipnr, int page, int len,
			u_char * buf);
static int rtk_read_ecc_page(struct mtd_info *mtd, u16 chipnr,
			     unsigned int page, u_char * data,
			     u_char * oob_buf);
static int rtk_write_oob(struct mtd_info *mtd, u16 chipnr, int page, int len,
			 const u_char * buf);
static int rtk_write_ecc_page(struct mtd_info *mtd, u16 chipnr,
			      unsigned int page, const u_char * data,
...
{name: "linux_backup", offset: (0x400000 + 0x30000), size:0x130000,},
	//0x130000+ rootfs
#endif
};
...
static void rtk_init_flash_proc(void)
{
	...
	//init hwpart proc
	hwpart_proc_entry =
	    proc_create(HWPART_NAME, 0, flash_proc_dir, &hwpart_fops);
...
void detect_nand_flash_map(struct mtd_info *mtd,
			   struct mtd_partition *rtl819x_parts)
{
	...
	while ((offset + page_size) < mtd_size) {
		...
		if (*((__u32 *) data_buf) == SQUASHFS_MAGIC_LE) {
			...
			rtl819x_parts[3].size =
			    rtl819x_parts[3].size + rtl819x_parts[3].offset -
			    offset;
	...
	rtl819x_parts[3].size =
	    (mtd_size + back_bank_offset) - rtl819x_parts[3].offset;
...
static void write_oob_to_TableSram(__u8 * w_oobbuf, __u8 w_oob_1stB, int shift)
{
	...
	if (shift) {
		...
		reg_oob =
		    w_oob_1stB | (w_oobbuf[0] << 8) | (w_oobbuf[1] << 16) |
		    (w_oobbuf[2] << 24);
		...
		reg_oob =
		    w_oobbuf[4] | (w_oobbuf[5] << 8) | (w_oobbuf[6] << 16) |
		    (w_oobbuf[7] << 24);
		rtk_writel(reg_oob, reg_num);

		reg_num = REG_BASE_ADDR + 8 * 4;
		reg_oob =
		    w_oobbuf[8] | (w_oobbuf[9] << 8) | (w_oobbuf[10] << 16) |
		    (w_oobbuf[11] << 24);
		rtk_writel(reg_oob, reg_num);

		reg_num = REG_BASE_ADDR + 12 * 4;
		reg_oob =
		    w_oobbuf[12] | (w_oobbuf[13] << 8) | (w_oobbuf[14] << 16) |
		    (w_oobbuf[15] << 24);
		rtk_writel(reg_oob, reg_num);
	} else {
...
static int rtk_check_pageData(struct mtd_info *mtd, u16 chipnr,
			      unsigned int page, int isLastSector)

static int rtk_read_ecc_page(struct mtd_info *mtd, u16 chipnr, unsigned int page, u_char * data_buf, u_char * oob_buf)	//czyao move it
static int rtk_write_oob(struct mtd_info *mtd, u16 chipnr, int page, int len, const u_char * oob_buf)	//czyao moves it
int rtk_write_ecc_page(struct mtd_info *mtd, u16 chipnr, unsigned int page, const u_char * data_buf, const u_char * oob_buf, int isBBT)	//czyao moves it
#endif
...
static void read_oob_867x(struct mtd_info *mtd, __u8 * r_oobbuf, int section,
			  int page)
{
...
static void write_oob_867x(__u8 * w_oobbuf, __u8 w_oob_1stB, int section,
			   int page)
{
	unsigned int reg_oob;
	unsigned oobsize512 = 16;	//512 byte data + 16 bytes OOB
...
static int rtk_erase_block(struct mtd_info *mtd, u16 chipnr, int page)
{
	...
	if ((page < 128) || page > (this->chipsize / page_size - 1)) {
		printk
		    ("[%s:] %d,page num = %d fatal error, try to wirte boot page\n",
...
//      printk("E:%u\n",page/ppb);
	...
	memset((__u32 *) (this->erase_page_flag) + chip_section + section, 0xff,
	       ppb >> 3);
	spin_unlock_irqrestore(&lock_nand, flags);
...
static int rtk_check_pageData(struct mtd_info *mtd, u16 chipnr, int page,
			      int offset, int isLastSector)
{
	struct nand_chip *this = (struct nand_chip *)mtd->priv;
	unsigned int chip_section = (chipnr * this->page_num) >> 5;
	unsigned int section = page >> 5;
...
//printk("[%s:%d] status=%x page %x offset %x idx %x\n",__FUNCTION__,__LINE__,status, page,offset, index);
	...
	if ((status & NDRS) == NDRS) {
		...
		if (this->erase_page_flag[chip_section + section] &
		    (1 << index)) {
			...
			if (error_count <= 4 && error_count > 0) {
				printk
				    ("[%s] R: Correctable HW ECC Error at page=%u, status=0x%08X\n\r",
				...
				printk
				    ("[%s] R: Un-Correctable HW ECC Error at page=%u, status=0x%08X\n\r",
				     __FUNCTION__, page, status);
				status &= 0x0f;	//clear NECN
				rtk_writel(status, NASR);
				return -1;
		...
		if (this->erase_page_flag[chip_section + section] &
			...
			printk
			    ("[%s] A NAND Flash write failed at page=%u, status=0x%08X\n\r",
			     __FUNCTION__, page, status);
			rtk_writel(status, NASR);
			return -1;
		}
			      ...
			      unsigned int page, int isLastSector)
{
	...
	if (rtk_readl(NASR) & 0x0C) {
		if (this->erase_page_flag[chip_section + section] &
			...
			if (rtk_readl(REG_ECC_STATE) & 0x08) {
				if (chipnr == 0 && page >= 0
				    && page < CONFIG_BOOT_SIZE / page_size) {
					return 0;
				} else {
					printk
					    ("[%s] Un-Correctable HW ECC Error at page=%u\n",
					     __FUNCTION__, page);
					return -1;
				}
			}
	...
	if ((status & NDRS) == NDRS) {
		...
		if (this->erase_page_flag[chip_section + section] &
		    (1 << index)) {
			printk
			    ("[%s] kkkk: Correctable HW ECC Error at page=%u\n",
			     __FUNCTION__, page);
			;
		} else if (status & NRER) {
			error_count = (status & 0xf0) >> 4;
			...
			if (error_count > 4) {
				printk
				    ("[%s:%d] R: Un-Correctable HW ECC Error at page=%u, status=0x%08X\n",
				     __FUNCTION__, __LINE__, page, status);
				//   120229
				status &= 0x0f;	//clear NECN
				rtk_writel(status, NASR);
				...
				printk
				    ("[%s:%d] R: Correctable HW ECC Error at page=%u error_count %d\n",
...
*/
			printk
			    ("[%s:%d] W: Correctable HW ECC Error at page=%u\n",
...
//      printk("%s\r\n",__FUNCTION__);
	...
	dram_addr =
	    (unsigned char *)kzalloc(sizeof(unsigned char) * (page_size) / 4,
				     GFP_KERNEL);
	if (!dram_addr) {
	...
	for (page_shift = 0; page_shift < 3; page_shift++) {
		...
		if (!NAND_ADDR_CYCLE) {
			flash_addr_t |=
			    (page_num[page_shift] << (12 + 8 * page_shift));
		} else {
			flash_addr_t |=
			    (page_num[page_shift] << (9 + 8 * page_shift));
		}
	}
	...
	for (index = 0; index < 16; index++) {
		printk("%d 0x%08X ", 4 * index,
		       rtk_readl(NAND_CTRL_BASE + index * 4));
		if ((index + 1) % 4 == 0)
			printk("\n");
	}
#endif
	...
	while (dma_counter > 0) {
		...
		if (oob_buf) {
			...
			oob_sa =
			    ((uint32_t) oob_buf +
			     buf_pos * oob_unit) & (~M_mask);
		} else {
			//dma_cache_wback_inv((unsigned long)(this->g_oobbuf+buf_pos*oob_unit),oob_unit);
			oob_sa =
			    ((uint32_t) (this->g_oobbuf) +
			     buf_pos * oob_unit) & (~M_mask);
		}
#else
		if (oob_buf) {
			memcpy(buf_temp,
			       ((uint32_t) oob_buf + buf_pos * oob_unit),
			       (oob_unit));
			//      printk("t:%x\r\n",buf_temp);
		...
		memcpy(((uint32_t) oob_buf + buf_pos * oob_unit), buf_temp,
		       oob_unit);
#if 0				//czyao debug
		if (OOB_DATA) {
		...
		if (rtk_check_pageData
		    (mtd, chipnr, page, buf_pos * (512 + 16), lastSec) == -1) {
			spin_unlock_irqrestore(&lock_nand, flags);
...
#if 0
static int rtk_read_ecc_page(struct mtd_info *mtd, u16 chipnr,
			     unsigned int page, u_char * data_buf,
			     u_char * oob_buf)
{
	...
	while (dma_counter > 0) {
		...
		//printk("[%s, line %d] \n",__FUNCTION__,__LINE__);
		//rtk_writel( (rtk_readl(NACR) |ECC_enable|RBO|WBO), NACR);     
		rtk_writel((rtk_readl(NACR) | ECC_enable & (~RBO) & (~WBO)),
			   NACR);

		//set DMA RAM start address, add 512 bytes      
		dram_sa =
		    ((uint32_t) (data_buf + buf_pos * page_unit) & (~M_mask));
		rtk_writel(dram_sa, NADRSAR);
		      ...
		      u_char * data_buf, u_char * oob_buf)
{
	...
	data_area =
	    (uint8_t *) ((uint32_t) (data_area0 + CACHELINE_SIZE - 4) &
			 0xFFFFFFF0);
	...
	for (page_shift = 0; page_shift < 3; page_shift++) {
		...
		if (nandflash_info.page_size == 2048) {
			...
			flash_addr_t |=
			    (page_num[page_shift] << (13 + 8 * page_shift));
	...
	while (dma_counter > 0) {
		...
		if ((return_value != -1) && (ecc_count != -1)) {
			return_value =
			    (ecc_count >
			     return_value) ? ecc_count : return_value;
		} else {
			return_value = -1;
		}
#endif
	...
	if (block > 0) {
		...
		chunk_buf[nandflash_info.bbi_dma_offset] =
		    spare->u8_oob[nandflash_info.bbi_swap_offset];
...
*/
	...
	while (dma_counter > 0) {
		...
		if (rtk_check_pageData
			...
			spin_unlock_irqrestore(&lock_nand, flags);
			printk
			    ("%s: rtk_check_pageData return fail...page %d, buf_pos %d\n",
...
//ccwei 111116
	...
	if (chipnr == 0 && page >= 0
	    && page <
	    ((CONFIG_BOOT_SIZE -
	      (2 * BACKUP_BBT * this->block_size)) / page_size)) {
#else
	if (chipnr == 0 && page >= 0 && page < (CONFIG_BOOT_SIZE / page_size)) {
#endif
		if (isBBT && page < 3 * ppb) {
	...
	while (dma_counter > 0) {
		...
		if (!oob_buf && this->g_oobbuf) {
			...
			oob_sa =
			    ((uint32_t) (this->g_oobbuf + buf_pos * oob_unit) &
			     (~M_mask));
		} else {
...
static int rtk_nand_profile(void)
{
	...
	if (!(rtk_mtd->writesize & (0x200 - 1))) ;	//rtk_writel( rtk_mtd->oobblock >> 9, REG_PAGE_LEN);
	else {
	...
	if (mtd_device_parse_register
	    (rtk_mtd, NULL, NULL, rtl8196_parts1, ARRAY_SIZE(rtl8196_parts1))) {
	...
	if (pnum <= 0) {
		...
		if (add_mtd_device(rtk_mtd)) {
			printk(KERN_WARNING
			       "RTK: Failed to register new nand device\n");
			return -EAGAIN;
		}
	} else {
...
#else
int rtk_read_proc_nandinfo(char *buf, char **start, off_t offset, int len,
			   int *eof, void *data)
{
	struct nand_chip *this = (struct nand_chip *)rtk_mtd->priv;
	int wlen = 0;
...
static int __init rtk_nand_init(void)
{
	...
	if (rtk_readl(0xb800000c) & 0x00800000) {
		...
		printk(KERN_ERR
		       "Nand Flash Clock is NOT Open. Installing fails.\n");
		return -1;
	}
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,10,0)
	create_proc_read_entry("nandinfo", 0, NULL, rtk_read_proc_nandinfo,
			       NULL);
#else
		...
		if (rtk_mtd) {
			...
			if (this->erase_page_flag) {
				unsigned int flag_size =
				    (this->numchips * this->page_num) >> 3;
				unsigned int mempage_order =
				    get_order(flag_size);
				free_pages((unsigned long)this->erase_page_flag,
					   mempage_order);
			}
			kfree(rtk_mtd);
		}
