...
 static void parallel_nand_set_chip_param(unsigned int id_chain) 
{
	...
	 for (i = 0; i < sizeof(nand_chip_id) / sizeof(nand_chip_param_T);
		i++)
					...
					REG_WRITE_U32(REG_MULTI_CHNL_MODE,
						       NF_MULTI_CHNL_MODE_edo
						       (0x00));
				}
			}
			 rtkn->chip_param.id = nand_chip_id[i].id;
			rtkn->chip_param.isLastPage =
			...
			rtkn->chip_param.g_enReadRetrial =
				...
				rtkn->chip_param.set_chip_param =
				    &rtk_set_feature_micron;
				rtkn->chip_param.set_chip_terminate_param =
				    &rtk_set_feature_micron;
			    ...
			    unsigned char *oobBuf, int page) 
{
		...
		if (is_sync_mode == 0x2)
		...
		dma_cache_wback_inv((unsigned long)oobBuf,
		    ...
		    REG_WRITE_U32(REG_AUTO_TRIG,
				...
				if (rtkn->chip_param.read_retry_cnt <=
				     rtkn->chip_param.max_read_retry_cnt) {
					if (rtkn->chip_param.set_chip_param !=
					     NULL)
...
 int parallel_nand_write_page(struct rtknflash *rtkn, unsigned char *buf,
				unsigned char *oobBuf, int page) 
{
