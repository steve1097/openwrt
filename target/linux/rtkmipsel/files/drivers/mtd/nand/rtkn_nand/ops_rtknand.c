...
#define NAND_STATUS_WP		0x80
	...
	rtkn->status =
	    NAND_STATUS_TRUE_READY | NAND_STATUS_READY | NAND_STATUS_WP;
#elif defined(CONFIG_PARALLEL_NAND_FLASH)
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,10,0)
static int rtkn_ecc_read_page(struct mtd_info *mtd,
...
static int rtkn_jffs2_write_patch(struct mtd_info *mtd,
				  struct nand_chip *chip,
				  ...
				  int page)
{
...

	{
		...
		for (i = 0; i < ppb; i++) {
			memset(rtkn->tmpBuf, 0xff, MAX_RTKN_BUF_SIZE);
			if (memcmp
			    (blocktmp + i * (pagesize + oobsize),
			     rtkn->tmpBuf, pagesize + oobsize) == 0)
				continue;
...
int rtkn_ecc_write_page(struct mtd_info *mtd, struct nand_chip *chip,
			const uint8_t * buf, int oob_required)
#else
static int rtkn_ecc_write_page(struct mtd_info *mtd,
			       struct nand_chip *chip,
			       const uint8_t * buf, int oob_required)
#endif
{
	...
	if (oob_required) {
		...
		for (i = 0; i < spare_step; i++)
			memcpy(rtkn->buf + chunk_size + 8 * i,
			       oobBuf + 6 * i, 6);
	}

	if (parallel_nand_write_page
...
static int rtkn_ecc_read_oob(struct mtd_info *mtd,
...
static int rtkn_ecc_write_oob(struct mtd_info *mtd,
			      struct nand_chip *chip, int page)
{
	struct rtknflash *rtkn = (struct rtknflash *)chip->priv;
...
static int rtk_nand_register_init(void)
{
	...
	REG32(REG_PINMUX_06) =
	    (REG32(REG_PINMUX_06) & (~((15 << 28) | (15 << 24)))) |
	    ((3 << 28) | (3 << 24));
	REG32(REG_PINMUX_15) =
	    (REG32(REG_PINMUX_15) &
	     (~((15 << 28) | (15 << 24) | (15 << 16)))) | ((3 << 28) |
							   (3 << 24) |
							   (3 << 16));

	/*enable spi nand clock */
	if ((REG32(REG_CLK_MANAGE2) & (RTK_BIT(19) | RTK_BIT(20))) !=
	    (RTK_BIT(19) | RTK_BIT(20))) {
		printk("SPI NAND clock not enable\n");
		REG32(REG_CLK_MANAGE2) =
		    REG32(REG_CLK_MANAGE2) | RTK_BIT(19) | RTK_BIT(20);
	}
...
static int rtk_nand_register_init(void)
{
	...
	REG_WRITE_U32(REG_CLK_MANAGE2,
		      ((REG_READ_U32(REG_CLK_MANAGE2) & 0xFFBFFFFF) |
		       (1 << 21)));
	/* set pin mux */
	REG_WRITE_U32(REG_PIN_MUX00,
		      REG_READ_U32(REG_PIN_MUX00) & (~0xffff0000));
	REG_WRITE_U32(REG_PIN_MUX01,
		      REG_READ_U32(REG_PIN_MUX01) & (~0xffff0000));
	REG_WRITE_U32(REG_PIN_MUX02,
		      REG_READ_U32(REG_PIN_MUX02) & (~0xffff000));
	REG_WRITE_U32(REG_PINMUX_15,
		      REG_READ_U32(REG_PINMUX_15) & (~0xffff0000));
...
#else
int rtk_read_proc_nandinfo(char *buf, char **start, off_t offset,
...
#if !(defined(__UBOOT__) || defined(__RTK_BOOT__))
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,10,0)
	create_proc_read_entry("nandinfo", 0, NULL,
			       rtk_read_proc_nandinfo, NULL);
#else
