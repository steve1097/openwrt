...
 static int rtk_create_bbt(struct mtd_info *mtd, int page)
{
	...
	if (rtk_scan_erase_bbt(mtd, page)) {
		    ...
		    //mark as bad block;
		    if (isLastPage) {
			rtk_scan_write_bbt(mtd, (page + ppb - 1), page_size,
					    temp_BBT, &rtkn->tmpBuf[page_size]);
			rtk_scan_write_bbt(mtd, (page + ppb - 2), page_size,
					    temp_BBT, &rtkn->tmpBuf[page_size]);
		} else {
			rtk_scan_write_bbt(mtd, (page), page_size, temp_BBT,
					    &rtkn->tmpBuf[page_size]);
			rtk_scan_write_bbt(mtd, (page + 1), page_size,
					    temp_BBT, &rtkn->tmpBuf[page_size]);
	    ...
	    memcpy(temp_BBT, rtkn->bbt, sizeof(BB_t) * RBA);
		     ...
		     (mtd, (page + page_counter), page_size,
		      ...
		      &rtkn->tmpBuf[page_size])) {
			    ...
			    //mark as bad block;
			    if (isLastPage) {
				rtk_scan_write_bbt(mtd, (page + ppb - 1),
						    page_size, temp_BBT,
						    &rtkn->tmpBuf[page_size]);
				rtk_scan_write_bbt(mtd, (page + ppb - 2),
						    page_size, temp_BBT,
						    &rtkn->tmpBuf[page_size]);
			} else {
				rtk_scan_write_bbt(mtd, (page), page_size,
						    temp_BBT,
						    &rtkn->tmpBuf[page_size]);
...
static int rtk_nand_scan_bbt(struct mtd_info *mtd)
{
	...
	for (i = 0; i < BACKUP_BBT; i++) {
		    ...
		    rtk_scan_read_oob(mtd, rtkn->tmpBuf, (bbt_page + (i * ppb)),
				      page_size);
	...
	//is_first_boot = 1;
		...
		if (!rc) {
			...
			rc = rtk_scan_read_oob(mtd, rtkn->tmpBuf,
					       (bbt_page + (i * ppb)),
					       page_size);
...
static int rtk_scan_v2r_bbt(struct mtd_info *mtd)
{
	...
	if (!temp_BBT) {
	    ...
	    for (i = 0; i < BACKUP_BBT; i++) {
		    ...
		    rtk_scan_read_oob(mtd, rtkn->tmpBuf,
	...
	memcpy(temp_BBT, bbt, sizeof(BB_t) * rtkn->RBA);
		...
		page_counter = 0;
			...
			/*erase fail: mean this block is bad, so do not write data!!! */ 
				...
				rtk_scan_write_bbt(mtd, (bbt_page + (ppb * i)),
						    page_size, temp_BBT,
						    &rtkn->tmpBuf[page_size]);
...
 static rtk_remapBBT_read(struct mtd_info *mtd, uint8_t * buf,
...
 static rtk_remapBBT_write(struct mtd_info *mtd, const uint8_t * buf,
			     ...
			     const uint8_t * buf, int oob_required) 
{
	...
	 int block_remap = 0x12345678, i, ppb =
		...
		rtk_scan_read_oob(mtd, rtkn->tmpBuf, (block * ppb + i),
				   page_size);
		rtk_scan_write_bbt(mtd, (block_remap * ppb + i), page_size,
				    rtkn->tmpBuf, &rtkn->tmpBuf[page_size]);
		...
		rtk_scan_write_bbt(mtd, (block * ppb + ppb - 1), page_size,
				    rtkn->tmpBuf, &rtkn->tmpBuf[page_size]);
		rtk_scan_write_bbt(mtd, (block * ppb + ppb - 2), page_size,
				    rtkn->tmpBuf, &rtkn->tmpBuf[page_size]);
	} else {
		rtk_scan_erase_bbt(mtd, (block * ppb));
		rtk_scan_write_bbt(mtd, (block * ppb), page_size, rtkn->tmpBuf,
				    &rtkn->tmpBuf[page_size]);
		rtk_scan_write_bbt(mtd, (block * ppb + 1), page_size,
				    rtkn->tmpBuf, &rtkn->tmpBuf[page_size]);
...
static int rtk_create_normal_bbt(struct mtd_info *mtd, int page)
{
	...
	unsigned int page_size = (1 << this->page_shift), ppb =
	    ...
	    memset(rtkn->tmpBuf, 0xff, MAX_RTKN_BUF_SIZE);
		     ...
		     (mtd, (page + page_counter), page_size,
		      ...
		      &rtkn->tmpBuf[page_size])) {
				...
				rtk_scan_write_bbt(mtd, (page),
						    page_size,
						    rtkn->tmpBuf,
						    &rtkn->tmpBuf[page_size]);
				rtk_scan_write_bbt(mtd, (page + 1),
						    page_size,
						    rtkn->tmpBuf,
						    &rtkn->tmpBuf[page_size]);
...
int rtkn_scan_bbt(struct mtd_info *mtd) 
{
	    ...
	    ("[%s, line %d], RBA=%d, this->RBA_PERCENT = %d,block_v2r_num=%d\n",
...
  int rtkn_block_markbad(struct mtd_info *mtd, loff_t ofs) 
{
	...
	int block_remap = 0x12345678;
		...
		rtk_scan_write_bbt(mtd, (block * ppb + ppb - 1),
				    page_size, rtkn->tmpBuf,
				    &rtkn->tmpBuf[page_size]);
		rtk_scan_write_bbt(mtd, (block * ppb + ppb - 2),
				    page_size, rtkn->tmpBuf,
				    &rtkn->tmpBuf[page_size]);
	} else {
		rtk_scan_erase_bbt(mtd, block * ppb);
		rtk_scan_write_bbt(mtd, (block * ppb), page_size,
				    rtkn->tmpBuf, &rtkn->tmpBuf[page_size]);
