...
#if 1				//def CONFIG_MTD_NAND
	struct mtd_info *mtd = rtk_get_nand_mtd();
	rtk_nand_read(mtd, (uint8_t *) boot, FLASH_BOOTINFO_OFFSET,
		      sizeof(BOOTINFO_T));
#endif
...
 static ssize_t bootoffset_write(struct file *file,
...
 static void rtk_init_bootinfo_proc(struct proc_dir_entry *flash_proc_dir)
{
	...
	if (!bootmode_proc_entry)
		return;
#ifdef CONFIG_WRT_BARRIER_BREAKER
	bootoffset_proc_entry =
	    proc_create(BOOTOFFSET_NAME, 0, flash_proc_dir, &bootoffset_fops);
	
#else /*  */
	bootoffset_proc_entry =
	    create_proc_entry(BOOTOFFSET_NAME, 0, flash_proc_dir);
	bootoffset_proc_entry->read_proc = bootoffset_show;
	bootoffset_proc_entry->write_proc = bootoffset_write;
	
#endif /*  */
	    if (!bootoffset_proc_entry)
