...
void rtk_flash_write_data(unsigned int flash_addr, unsigned int len,
			  unsigned char *data)
{
#if 0				//def CONFIG_RTL819X_SPI_FLASH
	extern unsigned int ComSrlCmd_ComWriteData(unsigned char ucChip,
						   unsigned int uiAddr,
						   unsigned int uiLen,
...
{
#if 0				//def CONFIG_RTL819X_SPI_FLASH
	rtk_read_bootinfo_from_flash(FLASH_MEM_MAP_ADDR + FLASH_BOOTINFO_OFFSET,
				     boot);
#endif
#if 1				//def CONFIG_MTD_NAND
	struct mtd_info *mtd = rtk_get_nand_mtd();
			       ...
			       unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, 32)) {
		...
		rtk_write_bootinfo_to_flash(FLASH_MEM_MAP_ADDR_W +
					    FLASH_BOOTINFO_OFFSET,
					    &bootinfo_ram,
					    rtk_flash_write_data);
	}
...
#else //old proc files
static int bootbank_proc_read(char *page, char **start, off_t off, int count,
			      int *eof, void *data)
{
	int len;
	BOOTINFO_P bootinfo_p;
...
static int bootcnt_proc_read(char *page, char **start, off_t off, int count,
			     int *eof, void *data)
{
	int len;
	BOOTINFO_P bootinfo_p;
...
static int bootcntmax_proc_read(char *page, char **start, off_t off, int count,
				int *eof, void *data)
{
	int len;
	BOOTINFO_P bootinfo_p;
...
static int bootmode_proc_read(char *page, char **start, off_t off, int count,
			      int *eof, void *data)
{
	int len;
	BOOTINFO_P bootinfo_p;
...
#ifdef CONFIG_WRT_BARRIER_BREAKER
	bootbank_proc_entry =
	    proc_create(BOOTBANK_NAME, 0, flash_proc_dir, &bootbank_fops);
#else
	bootbank_proc_entry =
	    create_proc_entry(BOOTBANK_NAME, 0, flash_proc_dir);
...
#ifdef CONFIG_WRT_BARRIER_BREAKER
	bootcnt_proc_entry =
	    proc_create(BOOTCNT_NAME, 0, flash_proc_dir, &bootcnt_fops);
#else
...
#ifdef CONFIG_WRT_BARRIER_BREAKER
	bootcntmax_proc_entry =
	    proc_create(BOOTMAXCNT_NAME, 0, flash_proc_dir, &bootcntmax_fops);
#else
	bootcntmax_proc_entry =
	    create_proc_entry(BOOTMAXCNT_NAME, 0, flash_proc_dir);
...
#ifdef CONFIG_WRT_BARRIER_BREAKER
	bootmode_proc_entry =
	    proc_create(BOOTMODE_NAME, 0, flash_proc_dir, &bootmode_fops);
#else
	bootmode_proc_entry =
	    create_proc_entry(BOOTMODE_NAME, 0, flash_proc_dir);
			   ...
			   struct mtd_partition *rtl819x_parts)
{
	...
	int page_size = mtd->writesize, oob_size = mtd->oobsize, ppb =
	...
	while ((offset + page_size) < mtd_size) {
		...
		if (*((__u32 *) data_buf) == SQUASHFS_MAGIC_LE) {
			...
			rtl819x_parts[3].size =
			    (mtd_size + active_bank_offset) -
			    rtl819x_parts[3].offset;
			...
			rtl819x_parts[2].size =
			    (mtd_size + active_bank_offset) -
			    rtl819x_parts[2].offset;
	...
	rtl819x_parts[4].size =
	    (mtd_size + back_bank_offset) - rtl819x_parts[4].offset;
#endif
...
static void rtknflash_ops_read(struct mtd_info *mtd, uint8_t * buf, int len)
{
	struct nand_chip *this = (struct nand_chip *)mtd->priv;
	...
	if (!NAND_ADDR_CYCLE) {
		...
		rtk_writel((enNextAD | AD2EN | AD1EN | AD0EN | flash_addr1),
			   NAADR);
...
static void rtknflash_ops_read_buf(struct mtd_info *mtd, uint8_t * buf, int len)
{
	struct nand_chip *nand_chip = (struct nand_chip *)mtd->priv;
	struct rtknflash *rtkn = (struct rtknflash *)nand_chip->priv;
 ...
 * NAND chip ops
 **************************************************/
static void rtknflash_ops_select_chip(struct mtd_info *mtd, int chip)
{
...
int rtk_check_pageData(struct mtd_info *mtd, u16 chipnr, int page, int offset,
		       int isLastSector)
{
	struct nand_chip *this = (struct nand_chip *)mtd->priv;
...
#endif
{
	...
	while (dma_counter > 0) {
		...
		if (rtk_check_pageData
		    (mtd, 0, realpage, buf_pos * (512 + 16), lastSec) == -1) {
			goto Error;
		}
		//copy data
...
static int rtkn_jffs2_write_patch_check(struct mtd_info *mtd,
				  ...
				  int page)
{
...

	{
		...
		blocktmp =
		    (unsigned char *)kmalloc((pagesize + oobsize) * ppb,
					     GFP_KERNEL);
		if (!blocktmp) {
			return -1;
		}
		...
		for (i = 0; i < ppb; i++) {
			memset(rtkn->buf, 0xff, MAX_RTKN_BUF_SIZE);
			if (memcmp
			    (blocktmp + i * (pagesize + oobsize), rtkn->buf,
			     pagesize + oobsize) == 0)
				continue;

			res =
			    rtk_scan_write_bbt(mtd, page + i, pagesize,
					       blocktmp + i * (pagesize +
							       oobsize),
					       blocktmp + i * (pagesize +
							       oobsize) +
			       ...
			       const uint8_t * buf, int oob_required)
{
	...
	memset(rtkn->buf, 0xff, MAX_RTKN_BUF_SIZE);
	if (memcmp(rtkn->buf, buf, mtd->writesize) == 0 && oob_required == 0) {
		spin_unlock_irqrestore(&lock_nand, flags_nand);
		return res;
	}
#ifdef CONFIG_RTK_REMAP_BBT
	...
	if (res < 0) {
		...
		res =
		    rtkn_jffs2_write_patch(mtd, chip, buf, oob_required, page);
		return res;
	}
...
static int rtkn_ecc_write_oob(struct mtd_info *mtd, struct nand_chip *chip,
			      int page)
{
	struct rtknflash *rtkn = (struct rtknflash *)chip->priv;
...
int rtknflash_lowinit(struct mtd_info *mtd, struct nand_chip *nand)
{
	...
	if (mtd_device_parse_register
	    (mtd, NULL, NULL, rtl8196_parts1, ARRAY_SIZE(rtl8196_parts1))) {
