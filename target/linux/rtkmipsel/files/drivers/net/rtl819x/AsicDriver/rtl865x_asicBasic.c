...
#endif
int32 _rtl8651_forceAddAsicEntry(uint32 tableType, uint32 eidx,
				 void *entryContent_P)
{
...
int32 _rtl8651_readAsicEntry(uint32 tableType, uint32 eidx,
			     void *entryContent_P)
{
	uint32 *entryAddr;
	...
	ASSERT_CSP(entryContent_P);
	entryAddr = (uint32 *) ((uint32)
				rtl8651_asicTableAccessAddrBase(tableType) + (eidx << 5	/*RTL8651_ASICTABLE_ENTRY_LENGTH */
				));
	...
	do {
		for (entryContentIdx = 0;
			...
			if (_rtl8651_asicTableSize[tableType] > 8) {
				 ...
				 */
				entryContent[entryContentIdx][8] =
				    *(entryAddr + 8192);
				entryContent[entryContentIdx][9] =
				    *(entryAddr + 8193);
				entryContent[entryContentIdx][10] =
				    *(entryAddr + 8194);
			}
#endif
...
retry:
	...
	} while ((needRetry == TRUE) && (entryCompare_max_count > 0));
	...
	for (i = 0; i < _rtl8651_asicTableSize[tableType]; i++) {
		*((uint32 *) entryContent_P + i) =
		    entryContent[entryContent_new][i];
	}
#else
...
#ifdef RTL865X_FAST_ASIC_ACCESS
	{
		...
		for (index = 0; index < _rtl8651_asicTableSize[tableType];
			...
			if (index < 8)
				*((uint32 *) entryContent_P + index) =
				    READ_MEM32((uint32) (entryAddr + index));
			else
				*((uint32 *) entryContent_P + index) =
				    READ_MEM32((uint32)
					       (entryAddr + index - 8 + 8192));
#else
			*((uint32 *) entryContent_P + index) =
			    READ_MEM32((uint32) (entryAddr + index));
#endif
	...
	*((uint32 *) entryContent_P + 7) = *(entryAddr + 7);
	...
	tmp = *(uint32 *) ((uint32)
			   rtl8651_asicTableAccessAddrBase(TYPE_ACL_RULE_TABLE)
			   + 1024 * RTL8651_ASICTABLE_ENTRY_LENGTH);
#ifdef RTL865XC_ASIC_READ_PROTECTION
	if (RTL819X_TLU_CHECK) {	/* No need to stop HW table lookup process */
		WRITE_MEM32(SWTCR0, ~EN_STOP_TLU & READ_MEM32(SWTCR0));
	}
...
int32 _rtl8651_readAsicEntryStopTLU(uint32 tableType, uint32 eidx,
				    void *entryContent_P)
{
	uint32 *entryAddr;
	uint32 tmp;		/* dummy variable, don't remove it */
...
void rtl819x_poll_sw(void)
{
	...
	if (((rtl819x_checkSwCoreTimer++) % 20) == 0) {
		...
		if (port6Queue0Cgst == 0) {
			...
			if ((rtl819x_lastTxDesc == 0)
			    || (rtl819x_lastRxDesc == 0)) {
				rtl819x_lastRxDesc = curRing0RxDesc;
				rtl819x_lastTxDesc = curRing0TxDesc;
				rtl819x_swHangCnt = 0;
			} else {
				if ((rtl819x_lastRxDesc == curRing0RxDesc)
				    && (rtl819x_lastTxDesc == curRing0TxDesc)) {
					rtl819x_swHangCnt++;
				} else {
					rtl819x_swHangCnt = 0;
