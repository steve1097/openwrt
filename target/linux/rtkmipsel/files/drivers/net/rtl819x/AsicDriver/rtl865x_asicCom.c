...
static int rtl8651_getAsicVlanIndex(rtl865xc_tblAsic_vlanTable_t * entry,
				    uint16 * vid)
{
	int i;
	int ret = FAILED;
	...
	for (i = 0; i < RTL865XC_VLANTBL_SIZE; i++) {
		if ((rtl8651_getAsicVlan(i, &vlan) == SUCCESS)
		    && (entry->vid == vlan.vid)) {
			if ((entry->memberPort != vlan.memberPortMask)
			    || (entry->egressUntag != vlan.untagPortMask)
			    || (entry->fid != vlan.fid)) {
				*vid = i;
				return SUCCESS;
			} else {
...
int32 rtl8651_setAsicVlan(uint16 vid, rtl865x_tblAsicDrv_vlanParam_t * vlanp)
{
	...
	if (vlanp->memberPortMask > RTL8651_PHYSICALPORTMASK)
		entry.extMemberPort =
		    vlanp->memberPortMask >> RTL8651_PORT_NUMBER;
	if (vlanp->untagPortMask > RTL8651_PHYSICALPORTMASK)
		entry.extEgressUntag =
		    vlanp->untagPortMask >> RTL8651_PORT_NUMBER;
...
int32 rtl8651_getAsicVlan(uint16 vid, rtl865x_tblAsicDrv_vlanParam_t * vlanp)
{
	...
	vlanp->memberPortMask =
	    (entry.extMemberPort << RTL8651_PORT_NUMBER) | entry.memberPort;
	vlanp->untagPortMask =
	    (entry.extEgressUntag << RTL8651_PORT_NUMBER) | entry.egressUntag;
	vlanp->fid = entry.fid;
...
@comm
 */
int32 rtl8651_defineProtocolBasedVLAN(uint32 ruleNo, uint8 ProtocolType,
...
@comm
 */
int32 rtl8651_setProtocolBasedVLAN(uint32 ruleNo, uint32 port, uint8 valid,
				   uint16 vlanId)
{
	uint32 addr;
	uint32 value;
...
@comm
 */
int32 rtl8651_getProtocolBasedVLAN(uint32 ruleNo, uint32 port, uint8 * valid,
				   uint32 * vlanId)
{
	uint32 value;
...
int32 rtl865xC_setNetDecisionPolicy(enum ENUM_NETDEC_POLICY policy)
{
	if (policy == NETIF_PORT_BASED)
		WRITE_MEM32(SWTCR0,
			    (READ_MEM32(SWTCR0) & ~LIMDBC_MASK) | LIMDBC_PORT);
	else if (policy == NETIF_MAC_BASED)
		WRITE_MEM32(SWTCR0,
			    (READ_MEM32(SWTCR0) & ~LIMDBC_MASK) | LIMDBC_MAC);
	else
		WRITE_MEM32(SWTCR0,
			    (READ_MEM32(SWTCR0) & ~LIMDBC_MASK) | LIMDBC_VLAN);
...
RTL865xC supports Multilayer-Decision-Base for interface lookup.
 */
int32 rtl865x_setDefACLForNetDecisionMiss(uint8 start_ingressAclIdx,
					  uint8 end_ingressAclIdx,
					  uint8 start_egressAclIdx,
					  uint8 end_egressAclIdx)
{
	if (start_ingressAclIdx >= RTL8651_ACLHWTBL_SIZE
	    || end_ingressAclIdx >= RTL8651_ACLHWTBL_SIZE
	    || start_egressAclIdx >= RTL8651_ACLHWTBL_SIZE
	    || end_egressAclIdx >= RTL8651_ACLHWTBL_SIZE)
		return FAILED;
#if 0
	WRITE_MEM32(DACLRCR,
		    start_ingressAclIdx | end_ingressAclIdx << 7 |
		    start_egressAclIdx << 14 | end_egressAclIdx << 21);
#else
	WRITE_MEM32(DACLRCR,
		    start_ingressAclIdx | end_ingressAclIdx << ACLI_EDA_OFFSET |
		    start_egressAclIdx << ACLO_STA_OFFSET | end_egressAclIdx <<
		    ACLO_EDA_OFFSET);
#endif
	return SUCCESS;
}
...
int32 rtl865x_getDefACLForNetDecisionMiss(uint8 * start_ingressAclIdx,
					  uint8 * end_ingressAclIdx,
					  uint8 * start_egressAclIdx,
					  uint8 * end_egressAclIdx)
{
...
int32 rtl865x_delNetInterfaceByVid(uint16 vid)
{
	...
	if (netIfIdx < RTL865XC_NETIFTBL_SIZE) {
		memset(&entry, 0, sizeof(entry));
		retVal =
		    _rtl8651_forceAddAsicEntry(TYPE_NETINTERFACE_TABLE,
					       netIfIdx, &entry);
	}
...
int32 rtl8651_setAsicNetInterface(uint32 idx,
				  rtl865x_tblAsicDrv_intfParam_t * intfp)
{
	rtl865xc_tblAsic_netifTable_t entry;
	uint32 i;
	...
	if (idx == RTL865XC_NETIFTBL_SIZE) {
		...
		for (i = 0; i < RTL865XC_NETIFTBL_SIZE; i++) {
			_rtl8651_readAsicEntry(TYPE_NETINTERFACE_TABLE, i,
					       &entry);
...
/*2007-12-19*/
	...
	entry.vid = intfp->vid;
	entry.mac47_19 =
	    (intfp->macAddr.octet[0] << 21) | (intfp->macAddr.octet[1] << 13) |
	    (intfp->macAddr.octet[2] << 5)
	    | (intfp->macAddr.octet[3] >> 3);
	entry.mac18_0 =
	    (intfp->macAddr.octet[3] << 16) | (intfp->
					       macAddr.octet[4] << 8) | (intfp->
									 macAddr.octet
									 [5]);
...
#else
	entry.enHWRouteV6 =
	    (rtl8651_getAsicOperationLayer() > 2) ? (intfp->enableRouteV6 ==
						     TRUE ? 1 : 0) : 0;
#endif

	entry.enHWRoute =
	    (rtl8651_getAsicOperationLayer() > 2) ? (intfp->enableRoute ==
						     TRUE ? 1 : 0) : 0;
 ...
 */
int32 rtl8651_getAsicNetInterface(uint32 idx,
				  rtl865x_tblAsicDrv_intfParam_t * intfp)
{
	rtl865xc_tblAsic_netifTable_t entry;
	uint32 i;
...
found:
	...
	intfp->macAddr.octet[3] =
	    ((entry.mac47_19 & 0x3f) << 3) | (entry.mac18_0 >> 16);
...
int32 rtl865xC_setDefaultACLReg(uint32 isIngress, uint32 start, uint32 end)
{
	...
	if (isIngress == TRUE) {
		WRITE_MEM32(DACLRCR,
			    (READ_MEM32(DACLRCR) & (~ACLI_STA_MASK)) | start);
		WRITE_MEM32(DACLRCR,
			    (READ_MEM32(DACLRCR) & (~ACLI_EDA_MASK)) | (end <<
									ACLI_EDA_OFFSET));
	} else {		/* egress */

		WRITE_MEM32(DACLRCR,
			    (READ_MEM32(DACLRCR) & (~ACLO_STA_MASK)) | (start <<
									ACLO_STA_OFFSET));
		WRITE_MEM32(DACLRCR,
			    (READ_MEM32(DACLRCR) & (~ACLO_EDA_MASK)) | (end <<
									ACLO_EDA_OFFSET));
	}
	return SUCCESS;
}
...
int32 rtl865xC_getDefaultACLReg(uint32 isIngress, uint32 * start, uint32 * end)
{
	...
	if (isIngress == TRUE) {
		...
		*start =
		    (READ_MEM32(DACLRCR) & ACLO_STA_MASK) >> ACLO_STA_OFFSET;
...
{
	if (index >= RTL8651_ACLTBL_SIZE + RTL8651_ACLTBL_RESERV_SIZE
	    || rule == NULL)
		return FAILED;
...
void rtl8651_clearRegister(void)
{
	...
	WRITE_MEM32(QIDDPCR,
		    (0x1 << NAPT_PRI_OFFSET) | (0x1 << ACL_PRI_OFFSET) | (0x1 <<
									  DSCP_PRI_OFFSET)
		    | (0x1 << BP8021Q_PRI_OFFSET) | (0x1 << PBP_PRI_OFFSET));
	...
	WRITE_MEM32(SBFCR1,
		    (0x0190 << S_DSC_FCOFF_OFFSET) | (0x01CC <<
						      S_DSC_FCON_OFFSET));
	WRITE_MEM32(SBFCR2,
		    (0x0050 << S_Max_SBuf_FCOFF_OFFSET) | (0x006C <<
							   S_Max_SBuf_FCON_OFFSET));
	/* Port Based Flow Control Threshold Register */
	WRITE_MEM32(PBFCR0,
		    (0x003C << P_MaxDSC_FCOFF_OFFSET) | (0x005A <<
							 P_MaxDSC_FCON_OFFSET));
	WRITE_MEM32(PBFCR1,
		    (0x003C << P_MaxDSC_FCOFF_OFFSET) | (0x005A <<
							 P_MaxDSC_FCON_OFFSET));
	WRITE_MEM32(PBFCR2,
	...
	WRITE_MEM32(ILBPCR1,
		    (0 << UpperBound_OFFSET) | (0 << LowerBound_OFFSET));
	WRITE_MEM32(ILBPCR2,
		    (0x33 << ILB_feedToken_OFFSET) | (0x26 << ILB_Tick_OFFSET));
	for (i = 0; i < 42; i++)
		WRITE_MEM32(P0Q0RGCR + i * 4,
			    (7 << PPR_OFFSET) | (0xff << L1_OFFSET) | (0x3FFF <<
								       APR_OFFSET));
	for (i = 0; i < 7; i++) {
#if defined(CONFIG_RTL_8197F)
	...
	/* CPU NIC */
	WRITE_MEM32(CPUICR,
		    READ_MEM32(CPUICR) & ~(LBMODE | LB10MHZ | MITIGATION |
					   EXCLUDE_CRC));
...
int32 rtl8651_clearAsicCommTable(void)
{
	...
	rtl8651_clearSpecifiedAsicTable(TYPE_NETINTERFACE_TABLE,
	...
	while ((REG32(MEMCR) & ((1 << 10) | (1 << 13))) !=
	       ((1 << 10) | (1 << 13))) {
	};
#else
	rtl8651_clearSpecifiedAsicTable(TYPE_VLAN_TABLE, RTL865XC_VLANTBL_SIZE);
	rtl8651_clearSpecifiedAsicTable(TYPE_ACL_RULE_TABLE,
					RTL8651_ACLTBL_SIZE);
#endif
...

	{
		...
		for (aclIdx = 0; aclIdx <= RTL8651_ACLHWTBL_SIZE; aclIdx++)
			_rtl8651_forceAddAsicEntry(TYPE_ACL_RULE_TABLE, aclIdx,
						   &rule);
...
{
	WRITE_MEM32(TEACR,
		    (READ_MEM32(TEACR) & ~0x3) | (l2Enable ==
						  TRUE ? 0x0 : 0x1) | (l4Enable
								       ==
								       TRUE ?
								       0x0 :
								       0x2));
...
void rtl865x_start(void)
{
	...
	REG32(CPUIIMR) =
	    RX_DONE_IE_ALL | LINK_CHANGE_IE | PKTHDR_DESC_RUNOUT_IE_ALL |
...
uint64 rtl865xC_returnAsicCounter64(uint32 offset)
{
	...
	return (READ_MEM32(MIB_COUNTER_BASE + offset) +
		((uint64) READ_MEM32(MIB_COUNTER_BASE + offset + 4) << 22));
}
...
int32 rtl8651_resetAsicMIBCounter(uint32 port, uint32 inCounterReset,
				  uint32 outCounterReset)
{
	if (port == 0xffffffff) {
		...
		case 0:
			...
			if (outCounterReset == TRUE) {
				WRITE_MEM32(MIB_CONTROL,
					    OUT_COUNTER_RESTART_P0);
			}
			break;
...
int32 rtl8651_getSimpleAsicMIBCounter(uint32 port,
...
int32 rtl8651_getAdvancedMIBCounter(uint32 port, uint32 asicCounterType,
				    ...
				    advancedCounter)
{
	...
	memset(advancedCounter, 0,
	       sizeof(rtl865x_tblAsicDrv_advancedCounterParam_t));

	if ((asicCounterType & ASIC_IN_COUNTERS) != 0) {
		advancedCounter->ifInOctets =
		    rtl865xC_returnAsicCounter64(OFFSET_IFINOCTETS_P0 +
						 addrOffset_fromP0);
		advancedCounter->ifInUcastPkts =
		    rtl8651_returnAsicCounter(OFFSET_IFINUCASTPKTS_P0 +
					      addrOffset_fromP0);
		advancedCounter->etherStatsOctets =
		    rtl865xC_returnAsicCounter64(OFFSET_ETHERSTATSOCTETS_P0 +
						 addrOffset_fromP0);
		advancedCounter->etherStatsUndersizePkts =
		    rtl8651_returnAsicCounter(OFFSET_ETHERSTATSUNDERSIZEPKTS_P0
					      + addrOffset_fromP0);
		advancedCounter->etherStatsFraments =
		    rtl8651_returnAsicCounter(OFFSET_ETHERSTATSFRAGMEMTS_P0 +
					      addrOffset_fromP0);
		advancedCounter->etherStatsPkts64Octets =
		    rtl8651_returnAsicCounter(OFFSET_ETHERSTATSPKTS64OCTETS_P0 +
					      addrOffset_fromP0);
		advancedCounter->etherStatsPkts65to127Octets =
		    rtl8651_returnAsicCounter
		    (OFFSET_ETHERSTATSPKTS65TO127OCTETS_P0 + addrOffset_fromP0);
		advancedCounter->etherStatsPkts128to255Octets =
		    rtl8651_returnAsicCounter
		    (OFFSET_ETHERSTATSPKTS128TO255OCTETS_P0 +
		     addrOffset_fromP0);
		advancedCounter->etherStatsPkts256to511Octets =
		    rtl8651_returnAsicCounter
		    (OFFSET_ETHERSTATSPKTS256TO511OCTETS_P0 +
		     addrOffset_fromP0);
		advancedCounter->etherStatsPkts512to1023Octets =
		    rtl8651_returnAsicCounter
		    (OFFSET_ETHERSTATSPKTS512TO1023OCTETS_P0 +
		     addrOffset_fromP0);
		advancedCounter->etherStatsPkts1024to1518Octets =
		    rtl8651_returnAsicCounter
		    (OFFSET_ETHERSTATSPKTS1024TO1518OCTETS_P0 +
		     addrOffset_fromP0);
		advancedCounter->etherStatsOversizePkts =
		    rtl8651_returnAsicCounter(OFFSET_ETHERSTATSOVERSIZEPKTS_P0 +
					      addrOffset_fromP0);
		advancedCounter->etherStatsJabbers =
		    rtl8651_returnAsicCounter(OFFSET_ETHERSTATSJABBERS_P0 +
					      addrOffset_fromP0);
		advancedCounter->etherStatsMulticastPkts =
		    rtl8651_returnAsicCounter(OFFSET_ETHERSTATSMULTICASTPKTS_P0
					      + addrOffset_fromP0);
		advancedCounter->etherStatsBroadcastPkts =
		    rtl8651_returnAsicCounter(OFFSET_ETHERSTATSBROADCASTPKTS_P0
					      + addrOffset_fromP0);
		advancedCounter->dot1dTpPortInDiscards =
		    rtl8651_returnAsicCounter(OFFSET_DOT1DTPPORTINDISCARDS_P0 +
					      addrOffset_fromP0);
		advancedCounter->etherStatusDropEvents =
		    rtl8651_returnAsicCounter(OFFSET_ETHERSTATSDROPEVENTS_P0 +
					      addrOffset_fromP0);
		advancedCounter->dot3FCSErrors =
		    rtl8651_returnAsicCounter(OFFSET_DOT3STATSFCSERRORS_P0 +
					      addrOffset_fromP0);
		advancedCounter->dot3StatsSymbolErrors =
		    rtl8651_returnAsicCounter(OFFSET_DOT3STATSSYMBOLERRORS_P0 +
					      addrOffset_fromP0);
		advancedCounter->dot3ControlInUnknownOpcodes =
		    rtl8651_returnAsicCounter
		    (OFFSET_DOT3CONTROLINUNKNOWNOPCODES_P0 + addrOffset_fromP0);
		advancedCounter->dot3InPauseFrames =
		    rtl8651_returnAsicCounter(OFFSET_DOT3INPAUSEFRAMES_P0 +
					      addrOffset_fromP0);
	}

	if ((asicCounterType & ASIC_OUT_COUNTERS) != 0) {
		advancedCounter->ifOutOctets =
		    rtl865xC_returnAsicCounter64(OFFSET_IFOUTOCTETS_P0 +
						 addrOffset_fromP0);
		advancedCounter->ifOutUcastPkts =
		    rtl8651_returnAsicCounter(OFFSET_IFOUTUCASTPKTS_P0 +
					      addrOffset_fromP0);
		advancedCounter->ifOutMulticastPkts =
		    rtl8651_returnAsicCounter(OFFSET_IFOUTMULTICASTPKTS_P0 +
					      addrOffset_fromP0);
		advancedCounter->ifOutBroadcastPkts =
		    rtl8651_returnAsicCounter(OFFSET_IFOUTBROADCASTPKTS_P0 +
					      addrOffset_fromP0);
		advancedCounter->ifOutDiscards =
		    rtl8651_returnAsicCounter(OFFSET_IFOUTDISCARDS +
					      addrOffset_fromP0);
		advancedCounter->dot3StatsSingleCollisionFrames =
		    rtl8651_returnAsicCounter
		    (OFFSET_DOT3STATSSINGLECOLLISIONFRAMES_P0 +
		     addrOffset_fromP0);
		advancedCounter->dot3StatsMultipleCollisionFrames =
		    rtl8651_returnAsicCounter
		    (OFFSET_DOT3STATSMULTIPLECOLLISIONFRAMES_P0 +
		     addrOffset_fromP0);
		advancedCounter->dot3StatsDefferedTransmissions =
		    rtl8651_returnAsicCounter
		    (OFFSET_DOT3STATSDEFERREDTRANSMISSIONS_P0 +
		     addrOffset_fromP0);
		advancedCounter->dot3StatsLateCollisions =
		    rtl8651_returnAsicCounter(OFFSET_DOT3STATSLATECOLLISIONS_P0
					      + addrOffset_fromP0);
		advancedCounter->dot3StatsExcessiveCollisions =
		    rtl8651_returnAsicCounter
		    (OFFSET_DOT3STATSEXCESSIVECOLLISIONS_P0 +
		     addrOffset_fromP0);
		advancedCounter->dot3OutPauseFrames =
		    rtl8651_returnAsicCounter(OFFSET_DOT3OUTPAUSEFRAMES_P0 +
					      addrOffset_fromP0);
		advancedCounter->dot1dBasePortDelayExceededDiscards =
		    rtl8651_returnAsicCounter
		    (OFFSET_DOT1DBASEPORTDELAYEXCEEDEDDISCARDS_P0 +
		     addrOffset_fromP0);
		advancedCounter->etherStatsCollisions =
	...
	if ((asicCounterType & ASIC_WHOLE_SYSTEM_COUNTERS) != 0) {
		advancedCounter->dot1dTpLearnedEntryDiscards =
		    rtl8651_returnAsicCounter(MIB_ADDROFFSETBYPORT);
		advancedCounter->etherStatsCpuEventPkts =
		    rtl8651_returnAsicCounter(MIB_ADDROFFSETBYPORT);
	}
...
int32 rtl865xC_dumpAsicCounter(void)
{
	...
	for (i = 0; i <= RTL8651_PORT_NUMBER; i++) {
		...
		rtlglue_printf
		    ("Rx %llu Bytes, Rx %u Pkts, Drop %u pkts, Rx Pause %u pkts\n",
		     rtl865xC_returnAsicCounter64(OFFSET_IFINOCTETS_P0 +
						  addrOffset_fromP0),
		     rtl8651_returnAsicCounter(OFFSET_IFINUCASTPKTS_P0 +
					       addrOffset_fromP0) +
...
int32 rtl865xC_dumpAsicDiagCounter(char *page, int *ret_len)
{
		...
		len +=
		    sprintf(page + len,
			    ...
			    (OFFSET_DOT3CONTROLINUNKNOWNOPCODES_P0 +
			     addrOffset_fromP0),
			    rtl8651_returnAsicCounter
			    (OFFSET_DOT3INPAUSEFRAMES_P0 + addrOffset_fromP0));
	...
	len +=
	    sprintf(page + len, "   CpuEvent %u pkts\n",
		    rtl8651_returnAsicCounter(OFFSET_ETHERSTATSCPUEVENTPKT));
	*ret_len = len;
	return SUCCESS;
}
...
int32 rtl8651_clearAsicSpecifiedCounter(uint32 counterIdx)
{
	...
	uint32 reg;
	rtlglue_printf
	    ("attention!this function is obsolete, please use new api:rtl8651_resetAsicMIBCounter()  or rtl8651_clearAsicCounter()\n");
	return FAILED;
	switch (counterIdx) {
	case 0:
...
int32 rtl8651_addAsicCounterMemberPort(uint32 counterIdx, uint32 port)
{
	uint32 reg, portMask;
	rtlglue_printf
	    ("attention!this function is obsolete, it shouldn't be used any more\n");
	return FAILED;
	...
	if (port < RTL8651_PORT_NUMBER)
		...
		portMask =
		    1 << (port - RTL8651_PORT_NUMBER +
			  EXT_PORT_FOR_COUNTING_OFFSET);
	else
...
@comm
	...
	for guarantee following read can get correct value. 
*/
int32 rtl8651_getAsicCounter(uint32 counterIdx,
			     rtl865x_tblAsicDrv_basicCounterParam_t *
			     basicCounter)
{

	rtlglue_printf
	    ("attention!this function is obsolete, please use new api:rtl8651_getSimpleAsicMIBCounter()  or rtl8651_getAdvancedMIBCounter()\n");
	return FAILED;
	_rtl8651_initialRead();
	switch (counterIdx) {
	case 0:
