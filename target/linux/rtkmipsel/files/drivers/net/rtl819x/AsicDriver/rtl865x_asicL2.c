...
static uint32 _rtl865xB_BandwidthCtrlPerPortConfiguration[RTL8651_PORT_NUMBER]
    [_RTL865XB_BANDWIDTHCTRL_CFGTYPE /* Ingress (0), Egress (1) */ ];
...
void set_ram_code(void)
{
	...
	rtl8651_setAsicEthernetPHYReg(4, 0x19,
				      ((reg & ~(P4R25_rg_eeeprg_en)) |
				       P4R25_rg_eeeprg_rst));
...
void cmp_eee_ram_code(int mode)
{
	...
	for (i = 0; i < 5; i++) {
		...
		rtl8651_setAsicEthernetPHYReg(i, 0x19,
					      ((reg & ~(P4R25_rg_eeeprg_en)) |
					       P4R25_rg_eeeprg_rst));
	}
	...
	for (i = 0; i < len; i++) {
		...
		if (mode == 0)
			...
			if (reg != phy_data_b[i])
				printk
				    (" => different: idx= %d, readback: %x, expect: %x\n",
				     i, reg, phy_data_b[i]);
		}
	}
...
//      reg = reg & 0xF9FF | P4R25_rg_dacquiet_en | P4R25_rg_ldvquiet_en;
	reg |=
	    (P4R25_rg_dacquiet_en | P4R25_rg_ldvquiet_en | P4R25_rg_eeeprg_en);
...
void eee_phy_enable(void)
{
	...
	for (i = 0; i < MAX_PORT_NUMBER; i++) {
		...
		reg |=
		    (P4R16_eee_nway_en | P4R16_tx_quiet_en | P4R16_rx_quiet_en);
...
//              reg = reg & 0xF9FF | P4R25_rg_dacquiet_en | P4R25_rg_ldvquiet_en;
		reg |=
		    (P4R25_rg_dacquiet_en | P4R25_rg_ldvquiet_en |
		     P4R25_rg_eeeprg_en);
...
void set_phy_pwr_save(int id, int val)
{
	...
	for (i = start; i <= end; i++) {
		...
		if (val == 1)
			rtl8651_setAsicEthernetPHYReg(i, 24,
						      (reg_val | BIT(15)));
		else
			rtl8651_setAsicEthernetPHYReg(i, 24,
						      (reg_val & (~BIT(15))));
...
uint32 rtl8651_filterDbIndex(ether_addr_t * macAddr, uint16 fid)
{
	return (macAddr->octet[0] ^ macAddr->octet[1] ^
		...
		macAddr->
		octet[4] ^ macAddr->octet[5] ^ fidHashTable[fid]) & 0xFF;
}

int32 rtl8651_setAsicL2Table(uint32 row, uint32 column,
			     rtl865x_tblAsicDrv_l2Param_t * l2p)
{
	...
	if ((row >= RTL8651_L2TBL_ROW) || (column >= RTL8651_L2TBL_COLUMN)
	    || (l2p == NULL))
		return FAILED;
	if (l2p->macAddr.octet[5] !=
	    ((row ^ (fidHashTable[l2p->fid]) ^ l2p->macAddr.
	      octet[0] ^ l2p->macAddr.octet[1] ^ l2p->macAddr.octet[2] ^ l2p->
	      macAddr.octet[3] ^ l2p->macAddr.octet[4]) & 0xff))
		return FAILED;
	...
	if (l2p->memberPortMask > RTL8651_PHYSICALPORTMASK)	//this MAC is on extension port
		entry.extMemberPort =
		    (l2p->memberPortMask >> RTL8651_PORT_NUMBER);
	...
	entry.agingTime = (l2p->ageSec > 300) ? 0x03 : (l2p->ageSec <= 300
							&& l2p->ageSec >
							150) ? 0x02
	    : (l2p->ageSec <= 150 && l2p->ageSec > 0) ? 0x01 : 0x00;
...
/* Should be fixed 		WRITE_MEM32(TEACR,READ_MEM32(TEACR)|0x1);ASIC patch: disable L2 Aging while reading L2 table */
		_rtl8651_readAsicEntry(TYPE_L2_SWITCH_TABLE, row << 2 | column,
				       &entry);
/*		WRITE_MEM32(TEACR,READ_MEM32(TEACR)&~0x1); ASIC patch: enable L2 Aging aftrer reading L2 table */
		...
		if (dmac[0] == entry.mac47_40 &&
		    ...
		    dmac[5] ==
		    (row ^ dmac[0] ^ dmac[1] ^ dmac[2] ^ dmac[3] ^ dmac[4])) {

			cachedDA = *((ether_addr_t *) dmac);
			cachedMbr =
			    (entry.extMemberPort << RTL8651_PORT_NUMBER) |
			    entry.memberPort;
			return cachedMbr;
		}
	}
...
int32 rtl8651_getAsicL2Table(uint32 row, uint32 column,
			     rtl865x_tblAsicDrv_l2Param_t * l2p)
{
	rtl865xc_tblAsic_l2Table_t entry;
...
/*	RTL865XC should fix this problem.WRITE_MEM32(TEACR,READ_MEM32(TEACR)|0x1); ASIC patch: disable L2 Aging while reading L2 table */
	...
	l2p->macAddr.octet[5] =
	    row ^ l2p->macAddr.octet[0] ^ l2p->macAddr.octet[1] ^ l2p->
	    macAddr.octet[2] ^ l2p->macAddr.octet[3] ^ l2p->
	    macAddr.octet[4] ^ (fidHashTable[entry.fid]);
	...
	l2p->memberPortMask =
	    (entry.extMemberPort << RTL8651_PORT_NUMBER) | entry.memberPort;
...
int32 rtl8651_setAsicPortMirror(uint32 mRxMask, uint32 mTxMask,
				uint32 mPortMask)
{
	uint32 pmcr = 0;
...
int32 rtl8651_getAsicPortMirror(uint32 * mRxMask, uint32 * mTxMask,
				uint32 * mPortMask)
{
	...
	if (mPortMask) {
		*mPortMask =
		    (pmcr & MirrorPortMsk_MASK) >> MirrorPortMsk_OFFSET;
	}

	if (mRxMask) {
		*mRxMask =
		    (pmcr & MirrorRxPrtMsk_MASK) >> MirrorRxPrtMsk_OFFSET;
	}

	if (mTxMask) {
		*mTxMask =
		    (pmcr & MirrorTxPrtMsk_MASK) >> MirrorTxPrtMsk_OFFSET;
	}
...
{
	rtl8651_clearSpecifiedAsicTable(TYPE_L2_SWITCH_TABLE,
					RTL8651_L2TBL_ROW *
					RTL8651_L2TBL_COLUMN);
	rtl8651_clearSpecifiedAsicTable(TYPE_RATE_LIMIT_TABLE,
					RTL8651_RATELIMITTBL_SIZE);
	return SUCCESS;
}

inline int32 convert_setAsicL2Table(uint32 row, uint32 column,
				    ether_addr_t * mac, int8 cpu, int8 srcBlk,
				    uint32 mbr, uint32 ageSec, int8 isStatic,
				    int8 nhFlag, int8 fid, int8 auth)
{
 ...
 */
int32 rtl8651_setAsicL2Table_Patch(uint32 row, uint32 column,
				   ether_addr_t * mac, int8 cpu, int8 srcBlk,
				   uint32 mbr, uint32 ageSec, int8 isStatic,
				   int8 nhFlag, int8 fid, int8 auth)
{
	...
	if (memcmp(&bcast_mac, mac, sizeof(ether_addr_t)) == 0
	    || memcmp(&cpu_mac, mac, sizeof(ether_addr_t)) == 0) {
		return convert_setAsicL2Table(row, column, mac, TRUE,	/* Set CPU bit to TRUE */
					      FALSE, mbr, 500, isStatic,	/* No one will be broadcast/multicast source */
					      nhFlag,	/* No one will be broadcast/multicast source */
					      TRUE);
	}
#endif
	if (mac->octet[0] & 0x1) {
		return convert_setAsicL2Table(row, column, mac, cpu, FALSE, mbr, ageSec, isStatic,	/* No one will be broadcast/multicast source */
					      nhFlag,	/* No one will be broadcast/multicast source */
					      fid, TRUE);
	} else {
		int8 dStatic =
		    isStatic /*, dnhFlag=(isStatic==TRUE? TRUE: FALSE) */ ;
		int8 dnhFlag = nhFlag;
...
#endif
		return convert_setAsicL2Table(row,
					      column,
					      mac,
					      cpu,
					      srcBlk,
...
int getL2MbrByMac(unsigned char *mac)
{
	...
	idx =
	    (((fidHashTable[0]) ^ mac[0] ^ mac[1] ^ mac[2] ^ mac[3] ^ mac[4] ^
	      mac[5]) & 0xff);
...
#if 0
int32 rtl8651_getAsicL2Table_Patch(uint32 row, uint32 column,
				   ether_addr_t * mac, int8 * cpu,
				   int8 * srcBlk, int8 * isStatic, uint32 * mbr,
				   uint32 * ageSec, int8 * nhFlag)
{
	rtl865x_tblAsicDrv_l2Param_t l2;

	int32 retval = rtl8651_getAsicL2Table(row, column, &l2);
	if (mac)
		memcpy(mac, &l2.macAddr, 6);
	if (cpu)
		*cpu = l2.cpu;
	if (srcBlk)
		*srcBlk = l2.srcBlk;
	if (isStatic)
		*isStatic = l2.isStatic;
	if (mbr)
		*mbr = l2.memberPortMask;
	if (ageSec)
		*ageSec = l2.ageSec;
	if (nhFlag)
		*nhFlag = l2.nhFlag;
	if (isStatic != NULL)
		*isStatic = TRUE;	/* patch!!, always TRUE(static entry */
	if (nhFlag != NULL)
		*nhFlag = FALSE;	/* always false */
	return retval;
}
#else

int32 rtl8651_getAsicL2Table_Patch(uint32 row, uint32 column,
				   rtl865x_tblAsicDrv_l2Param_t * asic_l2_t)
{
...
static inline unsigned int rtl865x_probeP1toP4GigaPHYChip(void)
{
	...
	{
		rtl8651_getAsicEthernetPHYReg(CONFIG_EXTRTL8212_PHYID_P1 + i, 2,
					      &tmp);
		uid = tmp << 16;
		rtl8651_getAsicEthernetPHYReg(CONFIG_EXTRTL8212_PHYID_P1 + i, 3,
					      &tmp);
		uid = uid | tmp;
...
static void rtl865x_fix8214Bug(void)
{
	...
	rtl8651_setAsicEthernetPHYReg(EXTRTL_8214_REGBASE_1, PAGE_SELECT_REGID,
				      0x0009);
	rtl8651_setAsicEthernetPHYReg(EXTRTL_8214_REGBASE_1, 21, 0xDD0A);
	/*      52_phy_write 0x13 9 21 0xDD0A   */
	rtl8651_setAsicEthernetPHYReg(EXTRTL_8214_REGBASE_1 + 1,
				      PAGE_SELECT_REGID, 0x0009);
	rtl8651_setAsicEthernetPHYReg(EXTRTL_8214_REGBASE_1 + 1, 21, 0xDD0A);
	/*      52_phy_write 0x14 8 28 0x0003   */
	rtl8651_setAsicEthernetPHYReg(EXTRTL_8214_REGBASE_3, PAGE_SELECT_REGID,
				      0x0008);
	 ...
	 */
	rtl8651_setAsicEthernetPHYReg(EXTRTL_8214_REGBASE_3 + 1,
				      PAGE_SELECT_REGID, 0x0002);
...
#if 0
	/* Test */
	rtl8651_getAsicEthernetPHYReg(EXTRTL_8214_REGBASE_1 + 1,
				      PAGE_SELECT_REGID, &rData);
...
static int32 _rtl8651_initAsicPara(rtl8651_tblAsic_InitPara_t * para)
{
	...
	if (para) {
		...
		if (para->externalPHYProperty &
		    RTL8651_TBLASIC_EXTPHYPROPERTY_PORT1234_RTL8212) {
			...
			ASICDRV_ASSERT(para->externalPHYId[2] ==
				       (para->externalPHYId[1] + 1));
			ASICDRV_ASSERT(para->externalPHYId[4] ==
				       (para->externalPHYId[3] + 1));
		}
		if (para->externalPHYProperty &
		    RTL8651_TBLASIC_EXTPHYPROPERTY_PORT5_RTL8211B) {
			ASICDRV_ASSERT(para->externalPHYId[5] != 0);
		}

		/* ============= Check passed : set it =============  */
		memcpy(&rtl8651_tblAsicDrvPara, para,
		       sizeof(rtl8651_tblAsic_InitPara_t));
	}
...
void set_gray_code_by_port(int port)
{
	...
	if (port < 4)
		rtl8651_setAsicEthernetPHYReg(port, 19, 0x5440);
	if (port < 3)
		rtl8651_setAsicEthernetPHYReg(port, 19, 0x54c0);
	if (port < 2)
		rtl8651_setAsicEthernetPHYReg(port, 19, 0x5480);
	if (port < 1)
		rtl8651_setAsicEthernetPHYReg(port, 19, 0x5580);
	rtl8651_setAsicEthernetPHYReg(port, 31, 0);
...
void Setting_RTL8198_GPHY(void)
{
	...
	if (REG32(BSP_REVR) == BSP_RTL8198_REVISION_A) {
		...
		for (port = 0; port < 5; port++) {
			for (i = 0; i < len; i = i + 2) {
				rtl8651_setAsicEthernetPHYReg(port, phy_para[i],
							      phy_para[i + 1]);
			}
		}
...
int Setting_RTL8197D_PHY(void)
{
	...
	REG32(MACCR) =
	    (REG32(MACCR) & ~(CF_RXIPG_MASK | SELIPG_MASK)) | (0x05 |
							       SELIPG_11);
	...
	REG32(MACCR) =
	    (REG32(MACCR) & ~CF_FCDSC_MASK) | (0x30 << CF_FCDSC_OFFSET);
...
void Sram98C(uint32 phyid, uint32 oper, uint32 RegAddr, uint32 sramdata)
{
	...
	for (; i < wphyid_end; i++) {
		...
		if (oper == 0) {	// read
			...
			rtl8651_setAsicEthernetPHYReg(wphyid, 27, RegAddr);
			rtl8651_setAsicEthernetPHYReg(wphyid, 28,
						      ((sramdata << 8) |
						       (srdata & 0x00ff)));
			rtl8651_setAsicEthernetPHYReg(wphyid, 31, 0);
		}
	...
	for (i = 0; i < 5; i++) {
		...
		if ((phy_link_sts & (1 << i)) != (cur_link_sts & (1 << i))) {
			...
			if (phyData & (1 << 2)) {	// link up
				...
				rtl8651_getAsicEthernetPHYReg(phyid, 26,
							      &data2);
...
int phy_refine(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		for (j = 0; j < len; j = j + 2) {
			rtl8651_setAsicEthernetPHYReg(pid, phy_98c_para[j],
						      phy_98c_para[j + 1]);
		}
...
void Ado_RamCode(int phyport)
{
	...
	for (j = 0; j < len; j = j + 2) {
		Set_GPHYWB(phyport, 0, phy_ado_patch[j], 0,
			   phy_ado_patch[j + 1]);
	}
...
void ado_modified(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		Set_GPHYWB(phyport, 0xa43, 28, (0xffff & ~(1 << 15)),
			   (0 << 15));
		...
		Set_GPHYWB(phyport, 0xa81, 16, (0xffff & ~(0x1f << 0)),
			   (0 << 0));
		Set_GPHYWB(phyport, 0xa81, 16, (0xffff & ~(0xf << 6)),
			   (0xf << 6));
...
void ado_refine(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		for (j = 0; j < len; j = j + 2) {
			Set_GPHYWB(phyport, 0, phy_ado_data[j], 0,
				   phy_ado_data[j + 1]);
		}
	}
}
...
int Setting_RTL8198C_GPHY(void)
{
	 ...
	 */
	REG32(MACCR) =
	    (REG32(MACCR) & ~(CF_SYSCLK_SEL_MASK)) | (0x01 <<
						      CF_SYSCLK_SEL_OFFSET);
...
#ifdef RTL8198C_GPHY_CALIBRATION
	{
		...
		for (i = 0; i < 5; i++) {
			...
			if ((cali_rc[i] != 0) && (cali_rc[i] != 0xF)) {
				rtl8651_setAsicEthernetPHYReg(phyid, 0x16,
							      (cali_rc[i] |
							       (cali_rc[i] << 4)
							       | (cali_rc[i] <<
								  8) |
							       (cali_rc[i] <<
								12)));
				rtl8651_setAsicEthernetPHYReg(phyid, 0x17,
							      (cali_rc[i] |
							       (cali_rc[i] << 4)
							       | (cali_rc[i] <<
								  8) |
			...
			if ((cali_r[i] != 0) && (cali_r[i] != 0xF)) {
				rtl8651_setAsicEthernetPHYReg(phyid, 0x10,
							      (cali_r[i] |
							       (cali_r[i] << 4)
							       | (cali_r[i] <<
								  8) |
							       (cali_r[i] <<
								12)));
				rtl8651_setAsicEthernetPHYReg(phyid, 0x11,
							      (cali_r[i] |
							       (cali_r[i] << 4)
							       | (cali_r[i] <<
								  8) |
		...
		for (i = 0; i < 5; i++) {
			if ((efuse_value[i] & 0x80)) {	// bit7 = 1, efuse has been calibrated
				...
				Set_GPHYWB(i, 0xbcf, 22, 0,
					   (efuse_value[i] >> 8) & 0xffff);
			}
		}
	}
#endif
...
int Setting_RTL8197F_PHY(void)
{
	...
	REG32(MACCR) =
	    (REG32(MACCR) & ~CF_FCDSC_MASK) | (0x48 << CF_FCDSC_OFFSET);
...
void enable_EEE(void)
{
	...
	Set_GPHYWB(999, 4, 16, 0xffff - ((0x3 << 12) | (0x3 << 8)),
		   ((0x3 << 12) | (0x3 << 8)));
...
void set_8198C_EEE(int mode, uint32 port_mask)
{
	...
	for (i = 0; i < 4; i++) {
		if (((1 << i) & port_mask) != 0) {	// need to set port i
			if (mode == 0) {	// disable
				Set_GPHYWB(i, 0xa43, 25, 0xffff - (1 << 4),
					   0 << 4);
				Set_GPHYWB(i, 0xa5d, 16, 0xffff - (0x3 << 1),
					   0);
...
#else
		REG32(PIN_MUX_SEL2) =
		    (REG32(PIN_MUX_SEL2) & (~(0xe38))) | (0x618);
...
#define GPIO_RESET	26	// GPIO_H2
	...
	REG32(PIN_MUX_SEL14) =
	    (REG32(PIN_MUX_SEL14) & ~(0xF << 28)) | (2 << 28);
...
int init_p0(void)
{
	...
	REG32(PCRP0) = (REG32(PCRP0) & ~(0x1F << ExtPHYID_OFFSET))
	    | ((PORT0_RGMII_PHYID << ExtPHYID_OFFSET) | MIIcfg_RXER |
	       EnablePHYIf | MacSwReset);
	REG32(MACCR) |= (1 << 12);
	REG32(P0GMIICR) =
	    (REG32(P0GMIICR) & ~((1 << 4) | (7 << 0))) | ((1 << 4) | (5 << 0));
#ifndef CONFIG_PARALLEL_NAND_FLASH
	REG32(PIN_MUX_SEL) = 0;	// TODO: need to find out
	REG32(PIN_MUX_SEL1) = 0;	// TODO: need to find out
...
int init_p0(void)
{
	...
	REG32(PCRP0) |=
	    (PORT0_RGMII_PHYID << ExtPHYID_OFFSET) | MIIcfg_RXER | EnablePHYIf |
	    MacSwReset;
	REG32(MACCR) |= (1 << 12);
	REG32(P0GMIICR) =
	    (REG32(P0GMIICR) & ~((1 << 4) | (7 << 0))) | ((1 << 4) | (3 << 0));

#if defined(CONFIG_RTL_8881A)
	// 0xb8000040 bit [4:2] Configure P0 mdc/mdio PAD as P0-MDIO (3'b100)
	// 0xb8000040 bit [9:7] Configure P0 PAD as P0-MII (3'b000)
	REG32(PIN_MUX_SEL) =
	    (REG32(PIN_MUX_SEL) & ~((7 << 7) | (7 << 2))) | (4 << 2);
#endif
...
#define _98C_P5_PHY_ID			5
	...
	REG32(P5GMIICR) =
	    (REG32(P5GMIICR) & ~((1 << 4) | (7 << 0))) |
	    ((_98C_P5_RGMII_TX_DELAY << 4) | (_98C_P5_RGMII_RX_DELAY << 0));
	REG32(PCRP5) = (REG32(PCRP5) & ~(0x1f << ExtPHYID_OFFSET))
	    | ((_98C_P5_PHY_ID << ExtPHYID_OFFSET) | MIIcfg_RXER | EnablePHYIf |
	       MacSwReset | AutoNegoSts_MASK);
...
static int rtl819x_8367r_reset_pin_probe(struct platform_device *dev)
{
	...
	ret =
	    devm_gpio_request_one(&dev->dev, pdata->gpio,
				  GPIOF_OUT_INIT_HIGH | GPIOF_EXPORT_DIR_FIXED,
				  pdata->name);
#else
	if (!
	    (ret =
	     gpio_request_one(pdata->gpio, GPIOF_OUT_INIT_HIGH, pdata->name)))
...
{
	REG32(PCRP0) =
	    (REG32(PCRP0) & ~(ExtPHYID_MASK)) | (5 << ExtPHYID_OFFSET)
	    | MIIcfg_RXER | EnablePHYIf | MacSwReset;
	REG32(PCRP0) =
	    (REG32(PCRP0) & ~(AutoNegoSts_MASK)) | (EnForceMode | ForceLink |
						    ForceSpeed1000M |
						    ForceDuplex);

	REG32(P0GMIICR) = (REG32(P0GMIICR) & ~(3 << 23)) | (LINK_RGMII << 23);
	REG32(P0GMIICR) =
	    (REG32(P0GMIICR) & ~((1 << 4) | (7 << 0))) | ((0 << 4) | (5 << 0) |
							  (3 <<
							   CF_SEL_RGTXC_OFFSET));
	...
	REG32(PAD_CTRL_1) =
	    (REG32(PAD_CTRL_1) &
	     ~(PAD_P0_RGMII_DN_MASK | PAD_P0_RGMII_DP_MASK |
	       PAD_P0_RGMII_MODE_MASK | PAD_P0_TX_E2_MASK))
...
inline void set_nway_ability(int port)
{
	if (fe_ads[port].down_speed == 0)
		REG32(PCRP0 + (port * 4)) =
		    (REG32(PCRP0 + (port * 4)) & ~AutoNegoSts_MASK) |
		    (NwayAbility100MF | NwayAbility100MH | NwayAbility10MF |
		     NwayAbility10MH);
	else
		REG32(PCRP0 + (port * 4)) =
		    (REG32(PCRP0 + (port * 4)) & ~AutoNegoSts_MASK) |
...
void ads_debug(void)
{
	...
	for (i = ADS_PORT_START; i < 5; i++) {
		rtlglue_printf(" => [%d] time_flag= %u\n", i,
			       fe_ads[i].time_flag);
		rtlglue_printf
		    ("       fail_counter= %u, anerr_down_speed= %d\n",
		     fe_ads[i].fail_counter, fe_ads[i].down_speed);
		rtlglue_printf
		    ("       state= %u, anerr= %d, cb_snr_down_speed= %d\n",
		     fe_ads[i].state, fe_ads[i].anerr,
		     fe_ads[i].cb_snr_down_speed);
...
inline void ads_check(void)
{
	...
	for (i = ADS_PORT_START; i < RTL8651_PHY_NUMBER; i++) {
		...
		if (fe_ads[i].anerr) {
			...
			if (fe_ads[i].fail_counter >
			    ADS_FAILED_COUNT_TO_DOWN_SPEED) {
...
{
	if (rtl8651_tblAsicDrvPara.externalPHYProperty &
	    RTL8651_TBLASIC_EXTPHYPROPERTY_PORT1234_RTL8212) {
		 ...
		 */
		rtl865xC_setAsicEthernetRGMIITiming(0, RGMII_TCOMP_0NS,
						    RGMII_RCOMP_0NS);

		/* Set P1 - P4 to SerDes Interface. */
		WRITE_MEM32(PITCR,
			    Port4_TypeCfg_SerDes | Port3_TypeCfg_SerDes |
			    Port2_TypeCfg_SerDes | Port1_TypeCfg_SerDes);
	} else if (rtl8651_tblAsicDrvPara.externalPHYProperty &
		   RTL8651_TBLASIC_EXTPHYPROPERTY_PORT5_RTL8211B) {
		/* Patch for 'RGMII port does not get descritpors'. Set to MII PHY mode first and later we'll change to RGMII mode again. */
		rtl865xC_setAsicEthernetMIIMode(RTL8651_MII_PORTNUMBER,
						LINK_MII_PHY);
		...
		/*
		   ...
		   - RGMII Input Timing compensation control : 0 ns
		 */
		rtl865xC_setAsicEthernetRGMIITiming(RTL8651_MII_PORTNUMBER,
						    RGMII_TCOMP_0NS,
						    RGMII_RCOMP_0NS);
	}
#if defined(CONFIG_RTL_8198C)
...
#else
	memset(&rtl8651AsicEthernetTable[0], 0,
	       (RTL8651_PORT_NUMBER +
		rtl8651_totalExtPortNum) * sizeof(rtl8651_tblAsic_ethernet_t));
	...
	if (rtl8651_tblAsicDrvPara.externalPHYProperty &
	    RTL8651_TBLASIC_EXTPHYPROPERTY_PORT1234_RTL8212) {
		rtl8651AsicEthernetTable[1].phyId =
		    rtl8651_tblAsicDrvPara.externalPHYId[1];
		rtl8651AsicEthernetTable[2].phyId =
		    rtl8651_tblAsicDrvPara.externalPHYId[2];
		rtl8651AsicEthernetTable[3].phyId =
		    rtl8651_tblAsicDrvPara.externalPHYId[3];
		rtl8651AsicEthernetTable[4].phyId =
		    rtl8651_tblAsicDrvPara.externalPHYId[4];
		rtl8651AsicEthernetTable[1].isGPHY = TRUE;
		rtl8651AsicEthernetTable[2].isGPHY = TRUE;
		rtl8651AsicEthernetTable[3].isGPHY = TRUE;
		rtl8651AsicEthernetTable[4].isGPHY = TRUE;
	} else {		/* USE internal 10/100 PHY */
		rtl8651AsicEthernetTable[1].phyId = 1;	/* Default value of port 1's embedded phy id -- 1 */
		rtl8651AsicEthernetTable[2].phyId = 2;	/* Default value of port 2's embedded phy id -- 2 */
		rtl8651AsicEthernetTable[3].phyId = 3;	/* Default value of port 3's embedded phy id -- 3 */
		rtl8651AsicEthernetTable[4].phyId = 4;	/* Default value of port 4's embedded phy id -- 4 */
	...
	if (rtl8651_tblAsicDrvPara.externalPHYProperty &
	    RTL8651_TBLASIC_EXTPHYPROPERTY_PORT5_RTL8211B) {
		rtl8651AsicEthernetTable[RTL8651_MII_PORTNUMBER].phyId =
		    rtl8651_tblAsicDrvPara.externalPHYId[5];
		rtl8651AsicEthernetTable[RTL8651_MII_PORTNUMBER].isGPHY = TRUE;
		rtl8651_setAsicEthernetMII(rtl8651AsicEthernetTable
					   [RTL8651_MII_PORTNUMBER].phyId,
					   P5_LINK_RGMII, TRUE);
	}
#endif
...
int32 rtl865x_initAsicL2(rtl8651_tblAsic_InitPara_t * para)
{
	...
	REG32(DIRECTLCR) =
	    (REG32(DIRECTLCR) & ~LEDONSCALEP0_MASK) | (7 <<
						       LEDONSCALEP0_OFFSET);
...
/*	WRITE_MEM32(MISCCR,READ_MEM32(MISCCR)&~FRAG2CPU);//IP fragment packet does not need to send to CPU when initial ASIC
	...
	for (index = 0; index < RTL8651_PORT_NUMBER + rtl8651_totalExtPortNum;
	     index++) {

		rtl865xC_setAsicSpanningTreePortState(index,
						      RTL8651_PORTSTA_FORWARDING);

		rtl8651_setAsicEthernetBandwidthControl(index, TRUE,
							RTL8651_BC_FULL);
		rtl8651_setAsicEthernetBandwidthControl(index, FALSE,
							RTL8651_BC_FULL);
	}
	...
	/* Initiate Bandwidth control backward compatible mode : Set all of them to FULL-Rate */
	{
		...
		for (portIdx = 0; portIdx < RTL8651_PORT_NUMBER; portIdx++) {
			for (typeIdx = 0;
			     typeIdx < _RTL865XB_BANDWIDTHCTRL_CFGTYPE;
			     typeIdx++) {
				_rtl865xB_BandwidthCtrlPerPortConfiguration
				    [portIdx][typeIdx] = BW_FULL_RATE;
			}
		}
	   ...
	   =============================== */
	{
		...
		maxPort =
		    (rtl8651_tblAsicDrvPara.externalPHYProperty &
		     RTL8651_TBLASIC_EXTPHYPROPERTY_PORT5_RTL8211B) ?
		    RTL8651_MAC_NUMBER : RTL8651_PHY_NUMBER;
...
#define DEFAULT_ILB_LBOUND 0x3FBC
		...
		WRITE_MEM32(ILBPCR1,
			    DEFAULT_ILB_UBOUND << UpperBound_OFFSET |
			    DEFAULT_ILB_LBOUND << LowerBound_OFFSET);
		for (i = 0; i <= (RTL8651_PHY_NUMBER / 2); i++)	/*Current Token Register is 2 bytes per port */
			WRITE_MEM32(ILB_CURRENT_TOKEN + 4 * i,
				    DEFAULT_ILB_UBOUND << UpperBound_OFFSET |
				    DEFAULT_ILB_UBOUND);
	 ...
	 */
	{
		...
		for (index = PHY0; index <= CPU; index++) {
			/*
			   Init QUEUE Number configuration for RTL865xC : For Port 0~5 and CPU Port - All ports have 1 queue for each.
			 */
			rtl8651_setAsicOutputQueueNumber(index, QNUM1
							 /* According to DataSheet of QNUMCR : All ports use 1 queue by default */
			    );
		}

		for (index = 0; index <= 8; index++) {
			REG32(V4VLDSCPCR0 + (index * 4)) =
			    (REG32(V4VLDSCPCR0 + (index * 4)) &
			     ~(CF_IPM4DSCP_ACT_MASK | CF_IPM4PRI_ACT_MASK))
			    |
			    ((CF_IPM4DSCP_ACT_REMARK << CF_IPM4DSCP_ACT_OFFSET)
			     | (CF_IPM4PRI_ACT_ORIG << CF_IPM4PRI_ACT_OFFSET));
		}
#elif !defined(CONFIG_RTL_819XD) && !defined(CONFIG_RTL_8196E)
		...
		/* Enable Flow Control for each QUEUE / Port */
		{
			...
			for (port = PHY0; port <= CPU; port++) {
				...
				/*
				   Init QUEUE Number configuration for RTL865xC : For Port 0~5 and CPU Port - All ports have 1 queue for each.
				 */
				rtl8651_setAsicOutputQueueNumber(port, QNUM1
								 /* According to DataSheet of QNUMCR : All ports use 1 queue by default */
				    );
...

	{
		...
		for (index = 0; index <= CPU; index++)
			rtl8651_setAsicPortBasedFlowControlRegister(PHY0 +
								    index,
								    rtl_portFCON,
								    rtl_portFCOFF);
...
#define GMII_PIN_MUX 0xf00
	...
	if (ExtP5GigaPhyMode) {
		...
		REG32(PCRP5) =
		    REG32(PCRP5) | (GIGA_P5_PHYID << ExtPHYID_OFFSET) |
		    EnablePHYIf | MacSwReset | 1 << 20;
...
#define GPIO_RESET   24		// GPIO_ H0
	...
	// force mode
	REG32(PCRP5) =
	    (REG32(PCRP5) & ~AutoNegoSts_MASK) | (EnForceMode | ForceLink |
						  ForceSpeed1000M |
						  ForceDuplex);
	REG32(PCRP5) = (REG32(PCRP5) & ~(0x1f << ExtPHYID_OFFSET))
	    | ((0x5 << ExtPHYID_OFFSET) | MIIcfg_RXER | EnablePHYIf |
	       MacSwReset);
	...
	// P5txdly = 1; P5rxdly = 5
	REG32(P5GMIICR) =
	    (REG32(P5GMIICR) & ~((1 << 4) | (7 << 0))) | ((1 << 4) | (5 << 0));
	REG32(PCRP5) =
	    (REG32(PCRP5) & ~AutoNegoSts_MASK) | (EnForceMode | ForceLink |
						  ForceSpeed1000M |
						  ForceDuplex);
	REG32(PCRP5) = (REG32(PCRP5) & ~(0x1f << ExtPHYID_OFFSET))
	    | ((0x15 << ExtPHYID_OFFSET) | MIIcfg_RXER | EnablePHYIf |
	       MacSwReset);
	REG32(P5GMIICR) |= (Conf_done);
#endif
#if defined(CONFIG_RTL_8211F_SUPPORT)
...
#elif defined(CONFIG_RTL_819XD) || defined(CONFIG_RTL_8196E)
	{
		...
		} else		//external phy
		{
			...
			REG32(PCRP0) =
			    (REG32(PCRP0) & ~(ExtPHYID_MASK)) | (P0_EXT_PHY_ID
								 <<
								 ExtPHYID_OFFSET)
...
/* 	system clock = 50MHz --> token=0x40, tick=0x0b, l2=3*token=0xC0
	...
	WRITE_MEM32(PCRP0,
		    (READ_MEM32(PCRP0) &
		     (0xFFFFFFFF - (0x00400000 | MacSwReset))));
	...
	REG32(CPUICR1) |=
	    ((1 << CF_NIC_LITTLE_ENDIAN_OFFSET) | (1 << CF_TXRX_DIV_LX_OFFSET) |
	     (1 << CF_TSO_ID_SEL));
	REG32(MACCTRL1) |= CF_CMAC_CLK_SEL;
...
int32 rtl8651_setAsicPortPatternMatch(uint32 port, uint32 pattern,
				      uint32 patternMask, int32 operation)
{
	//not for ext port
...
int32 rtl8651_getAsicPortPatternMatch(uint32 port, uint32 * pattern,
				      uint32 * patternMask, int32 * operation)
{
	//not for ext port
	if (port >= RTL8651_PORT_NUMBER)
		return FAILED;
	if (((READ_MEM32(PPMAR) & (1 << 13)) == 0)
	    || ((READ_MEM32(PPMAR) & (1 << (26 + port))) == 0))
		return FAILED;
	if (pattern)
...
int32 rtl8651_getAsicSpanningEnable(int8 * spanningTreeEnabled)
{
	if (spanningTreeEnabled == NULL)
		return FAILED;
	*spanningTreeEnabled =
	    (READ_MEM32(MSCR) & (EN_STP)) == (EN_STP) ? TRUE : FALSE;
	return SUCCESS;
}
...
int32 rtl865xC_setAsicSpanningTreePortState(uint32 port, uint32 portState)
{
	...
	case RTL8651_PORTSTA_DISABLED:
		WRITE_MEM32(PCRP0 + offset,
			    (READ_MEM32(PCRP0 + offset) & (~STP_PortST_MASK)) |
			    STP_PortST_DISABLE);
		break;
 ...
 */
int32 rtl8651_getAsicMulticastSpanningTreePortState(uint32 port,
						    uint32 * portState)
{
	uint32 reg;
	uint32 offset = port * 4;
...
int32 rtl8651_getAsicEthernetPHYReg(uint32 phyId, uint32 regId, uint32 * rData)
{
	...
	WRITE_MEM32(MDCIOCR,
		    COMMAND_READ | (phyId << PHYADD_OFFSET) | (regId <<
							       REGADD_OFFSET));
...
{
	WRITE_MEM32(MDCIOCR,
		    COMMAND_WRITE | (phyId << PHYADD_OFFSET) | (regId <<
								REGADD_OFFSET) |
		    wData);
...
int32 rtl865xC_setAsicPortPauseFlowControl(uint32 port, uint8 rxEn, uint8 txEn)
{
	...
	WRITE_MEM32(PCRP0 + offset,
		    (~(PauseFlowControl_MASK) & (READ_MEM32(PCRP0 + offset))) |
		    pauseFC);
...
int32 rtl865xC_getAsicPortPauseFlowControl(uint32 port, uint8 * rxEn,
					   uint8 * txEn)
{
	uint32 offset = port << 2;
	uint32 pauseFC = 0;
...
int32 rtl865xC_setAsicEthernetMIIMode(uint32 port, uint32 mode)
{
	...
	if (port == 0) {
		...
		WRITE_MEM32(P0GMIICR,
			    (READ_MEM32(P0GMIICR) & ~CFG_GMAC_MASK) | (mode <<
								       LINKMODE_OFFSET));
	} else {
		/* MII port MAC interface mode configuration */
		WRITE_MEM32(P5GMIICR,
			    (READ_MEM32(P5GMIICR) & ~CFG_GMAC_MASK) | (mode <<
								       LINKMODE_OFFSET));
	}
	return SUCCESS;
...
int32 rtl865xC_setAsicEthernetRGMIITiming(uint32 port, uint32 Tcomp,
					  uint32 Rcomp)
{
	if (port != 0 && port != RTL8651_MII_PORTNUMBER)
		return FAILED;
	if (Tcomp < RGMII_TCOMP_0NS || Tcomp > RGMII_TCOMP_7NS
	    || Rcomp < RGMII_RCOMP_0NS || Rcomp > RGMII_RCOMP_2DOT5NS)
		return FAILED;

	if (port == 0) {
		WRITE_MEM32(P0GMIICR,
			    (((READ_MEM32(P0GMIICR) & ~RGMII_TCOMP_MASK) |
			      Tcomp) & ~RGMII_RCOMP_MASK) | Rcomp);
	} else {
		WRITE_MEM32(P5GMIICR,
			    (((READ_MEM32(P5GMIICR) & ~RGMII_TCOMP_MASK) |
			      Tcomp) & ~RGMII_RCOMP_MASK) | Rcomp);
	}
...
int32 rtl8651_setAsicEthernetMII(uint32 phyAddress, int32 mode, int32 enabled)
{
	...
	if (phyAddress < 0 || phyAddress > 31)
		return FAILED;
	if (mode != P5_LINK_RGMII && mode != P5_LINK_MII_MAC
	    && mode != P5_LINK_MII_PHY)
		return FAILED;
...
/*vlan remark*/
int32 rtl8651_setAsicVlanRemark(enum PORTID port, enum PRIORITYVALUE priority,
				int remark)
{
	int regValue;
	if ((port < PHY0) || (port > CPU) || (priority < PRI0)
	    || (priority > PRI7) || (remark < PRI0) || (remark > PRI7))
		return FAILED;

	WRITE_MEM32(RLRC, (READ_MEM32(RLRC) & ~(0x7)) | 0x7);

	regValue =
	    READ_MEM32(RMCR1P) & ~((0x7 << (3 * priority)) |
				   (0x1 << (24 + port)));
...
int32 rtl8651_getAsicVlanRemark(enum PORTID port, enum PRIORITYVALUE priority,
				int *remark)
{
	int regValue;
	if ((port < PHY0) || (port > CPU) || (priority < PRI0)
	    || (priority > PRI7) || (remark == NULL))
		return FAILED;
...
/*vlan remark*/
int32 rtl8651_setAsicDscpRemark(enum PORTID port, enum PRIORITYVALUE priority,
				int remark)
{
	int regValue0;
	if ((port < PHY0) || (port > CPU) || (priority < PRI0)
	    || (priority > PRI7) || (remark < 0) || (remark > 63))
		return FAILED;
	...
	if (priority < 5) {
		...
		regValue0 =
		    READ_MEM32(DSCPRM1) & ~((0x3f << (6 * (priority - 5))) |
					    (0x1 << (23 + port)));
		regValue0 |=
		    ((remark << (6 * (priority - 5))) | (0x1 << (23 + port)));
		WRITE_MEM32(DSCPRM1, regValue0);
	}
...
int32 rtl8651_getAsicDscpRemark(enum PORTID port, enum PRIORITYVALUE priority,
				int *remark)
{
 ...
 */
#if defined(CONFIG_RTL_8197F)
int32 rtl8651_setAsicPriorityDecision(uint32 portpri, uint32 dot1qpri,
				      uint32 dscppri, uint32 aclpri,
				      uint32 natpri, uint32 vidpri)
#else
int32 rtl8651_setAsicPriorityDecision(uint32 portpri, uint32 dot1qpri,
				      uint32 dscppri, uint32 aclpri,
				      uint32 natpri)
#endif
{
	...
	if ((portpri < 0) || (portpri > 0xF) || (dot1qpri < 0)
	    || (dot1qpri > 0xF) || (dscppri < 0) || (dscppri > 0xF)
	    || (aclpri < 0) || (aclpri > 0xF) || (natpri < 0) || (natpri > 0xF))
		return FAILED;
...
#if defined(CONFIG_RTL_8197F)
	WRITE_MEM32(QIDDPCR,
		    (portpri << PBP_PRI_OFFSET) | (dot1qpri <<
						   BP8021Q_PRI_OFFSET) |
...
#if defined(CONFIG_RTL_8197F)
int32 rtl8651_getAsicPriorityDecision(uint32 * portpri, uint32 * dot1qpri,
				      uint32 * dscppri, uint32 * aclpri,
				      uint32 * natpri, uint32 * vidpri)
#else
int32 rtl8651_getAsicPriorityDecision(uint32 * portpri, uint32 * dot1qpri,
...
{
	uint32 temp;
	if ((portpri == NULL) || (dot1qpri == NULL) || (dscppri == NULL)
	    || (aclpri == NULL) || (natpri == NULL))
		return FAILED;
...
int32 rtl8651_setAsicQueueFlowControlConfigureRegister(enum PORTID port,
						       enum QUEUEID queue,
						       uint32 enable)
{
	switch (port) {
	case PHY0:
		WRITE_MEM32(FCCR0,
			    (READ_MEM32(FCCR0) &
			     ~(0x1 << (queue + Q_P0_EN_FC_OFFSET))) | (enable <<
								       (queue +
									Q_P0_EN_FC_OFFSET)));
		break;
	case PHY1:
		WRITE_MEM32(FCCR0,
			    (READ_MEM32(FCCR0) &
			     ~(0x1 << (queue + Q_P1_EN_FC_OFFSET))) | (enable <<
								       (queue +
									Q_P1_EN_FC_OFFSET)));
		break;
	case PHY2:
		WRITE_MEM32(FCCR0,
			    (READ_MEM32(FCCR0) &
			     ~(0x1 << (queue + Q_P2_EN_FC_OFFSET))) | (enable <<
								       (queue +
									Q_P2_EN_FC_OFFSET)));
		break;
	case PHY3:
		WRITE_MEM32(FCCR0,
			    (READ_MEM32(FCCR0) &
			     ~(0x1 << (queue + Q_P3_EN_FC_OFFSET))) | (enable <<
								       (queue +
									Q_P3_EN_FC_OFFSET)));
		break;
	case PHY4:
		WRITE_MEM32(FCCR1,
			    (READ_MEM32(FCCR1) &
			     ~(0x1 << (queue + Q_P4_EN_FC_OFFSET))) | (enable <<
								       (queue +
									Q_P4_EN_FC_OFFSET)));
		break;
	case PHY5:
		WRITE_MEM32(FCCR1,
			    (READ_MEM32(FCCR1) &
			     ~(0x1 << (queue + Q_P5_EN_FC_OFFSET))) | (enable <<
								       (queue +
									Q_P5_EN_FC_OFFSET)));
		break;
	case CPU:
		WRITE_MEM32(FCCR1,
			    (READ_MEM32(FCCR1) &
			     ~(0x1 << (queue + Q_P6_EN_FC_OFFSET))) | (enable <<
								       (queue +
									Q_P6_EN_FC_OFFSET)));
		break;
	default:
...
int32 rtl8651_getAsicQueueFlowControlConfigureRegister(enum PORTID port,
						       enum QUEUEID queue,
						       uint32 * enable)
{
	if (enable != NULL) {
		...
		case PHY0:
			*enable =
			    (READ_MEM32(FCCR0) &
			     (0x1 << (queue + Q_P0_EN_FC_OFFSET))) >> (queue +
								       Q_P0_EN_FC_OFFSET);
			break;
		case PHY1:
			*enable =
			    (READ_MEM32(FCCR0) &
			     (0x1 << (queue + Q_P1_EN_FC_OFFSET))) >> (queue +
								       Q_P1_EN_FC_OFFSET);
			break;
		case PHY2:
			*enable =
			    (READ_MEM32(FCCR0) &
			     (0x1 << (queue + Q_P2_EN_FC_OFFSET))) >> (queue +
								       Q_P2_EN_FC_OFFSET);
			break;
		case PHY3:
			*enable =
			    (READ_MEM32(FCCR0) &
			     (0x1 << (queue + Q_P3_EN_FC_OFFSET))) >> (queue +
								       Q_P3_EN_FC_OFFSET);
			break;
		case PHY4:
			*enable =
			    (READ_MEM32(FCCR1) &
			     (0x1 << (queue + Q_P4_EN_FC_OFFSET))) >> (queue +
								       Q_P4_EN_FC_OFFSET);
			break;
		case PHY5:
			*enable =
			    (READ_MEM32(FCCR1) &
			     (0x1 << (queue + Q_P5_EN_FC_OFFSET))) >> (queue +
								       Q_P5_EN_FC_OFFSET);
			break;
		case CPU:
			*enable =
			    (READ_MEM32(FCCR1) &
			     (0x1 << (queue + Q_P6_EN_FC_OFFSET))) >> (queue +
								       Q_P6_EN_FC_OFFSET);
			break;
		default:
			return FAILED;
...
{
	WRITE_MEM32(ELBPCR,
		    (READ_MEM32(ELBPCR) & ~(Token_MASK | Tick_MASK)) | (token <<
									Token_OFFSET)
		    | (tick << Tick_OFFSET));
	WRITE_MEM32(ELBTTCR, (READ_MEM32(ELBTTCR) & ~0xFFFF /*L2_MASK */ ) | (hiThreshold
									      <<
									      L2_OFFSET));
	WRITE_MEM32(ILBPCR2,
		    (READ_MEM32(ILBPCR2) &
		     ~(ILB_feedToken_MASK | ILB_Tick_MASK)) | (token <<
							       ILB_feedToken_OFFSET)
		    | (tick << ILB_Tick_OFFSET));
	return SUCCESS;
}
...
@comm
 */
int32 rtl8651_getAsicLBParameter(uint32 * pToken, uint32 * pTick,
				 uint32 * pHiThreshold)
{
	uint32 regValue;
...
/*port based priority*/
int32 rtl8651_setAsicPortBasedPriority(enum PORTID port,
				       enum PRIORITYVALUE priority)
{
	/* Invalid input parameter */
	if ((priority < PRI0) || (priority > PRI7) || (port < PHY0)
	    || (port > EXT3))
		return FAILED;

	WRITE_MEM32(PBPCR,
		    (READ_MEM32(PBPCR) & ~(0x7 << (port * 3))) | (priority <<
								  (port * 3)));

	return SUCCESS;
}

int32 rtl8651_getAsicPortBasedPriority(enum PORTID port,
				       enum PRIORITYVALUE * priority)
{
	/* Invalid input parameter */
...
@comm
 */
int32 rtl8651_setAsicQueueRate(enum PORTID port, enum QUEUEID queueid,
			       uint32 pprTime, uint32 aprBurstSize, uint32 apr)
{
	uint32 reg1, regValue;

	if ((port < PHY0) || (port > CPU) || (queueid < QUEUE0)
	    || (queueid > MAX_QUEUE))
		return FAILED;
	...
	regValue |=
	    ((pprTime << PPR_OFFSET) | (aprBurstSize << L1_OFFSET) |
	     (apr << APR_OFFSET));
...
@comm
 */
int32 rtl8651_getAsicQueueRate(enum PORTID port, enum QUEUEID queueid,
			       uint32 * pPprTime, uint32 * pAprBurstSize,
			       uint32 * pApr)
{
	uint32 reg1, regValue;
...
int32 rtl8651_setAsicPortIngressBandwidth(enum PORTID port, uint32 bandwidth)
{
	...
	if (port % 2) {		/* ODD-port */
		WRITE_MEM32(reg1,
			    ((READ_MEM32(reg1) & ~(IBWC_ODDPORT_MASK)) |
			     ((bandwidth << IBWC_ODDPORT_OFFSET) &
			      IBWC_ODDPORT_MASK)));
	} else {		/* EVEN-port */
		WRITE_MEM32(reg1,
			    ((READ_MEM32(reg1) & ~(IBWC_EVENPORT_MASK)) |
			     ((bandwidth << IBWC_EVENPORT_OFFSET) &
			      IBWC_EVENPORT_MASK)));
	}
	...
	WRITE_MEM32(IBCR3,
		    ((READ_MEM32(IBCR3) &
		      (~(IBCR3_PORT_MASK << IBCR3_PORT_OFFSET(port)))) |
		     ((bandwidth_high & IBCR3_PORT_MASK) <<
...
int32 rtl8651_getAsicPortIngressBandwidth(enum PORTID port, uint32 * pBandwidth)
{
	...
	if (pBandwidth != NULL) {
		*pBandwidth = (port % 2) ?
		    /* Odd port */
		    ((regValue & IBWC_ODDPORT_MASK) >> IBWC_ODDPORT_OFFSET) :
		    /* Even port */
		    ((regValue & IBWC_EVENPORT_MASK) >> IBWC_EVENPORT_OFFSET);
	}
#if defined(CONFIG_RTL_8197F)
	regValue = READ_MEM32(IBCR3);
	pBandwidth_high =
	    (regValue & (IBCR3_PORT_MASK << IBCR3_PORT_OFFSET(port))) >>
	    IBCR3_PORT_OFFSET(port);
...
int32 rtl8651_setAsicPortEgressBandwidth(enum PORTID port, uint32 bandwidth)
{
	...
	WRITE_MEM32(reg1,
		    (READ_MEM32(reg1) & ~(APR_MASK)) | (bandwidth <<
							APR_OFFSET));
...
int32 rtl8651_setAsicQueueStrict(enum PORTID port, enum QUEUEID queueid,
				 enum QUEUETYPE queueType)
{
	...
	if (queueid < QUEUE6) {
		...
		regValue =
		    READ_MEM32(reg1) & ~((WEIGHT0_MASK | SCHE0_MASK) <<
					 regOFFSET);
	...
	regValue =
	    READ_MEM32(reg1) & ~((WEIGHT0_MASK | SCHE0_MASK) << regOFFSET);
	regValue |= (queueType << (SCHE0_OFFSET + regOFFSET));
	WRITE_MEM32(reg1, regValue);
#endif
	return SUCCESS;
...
int32 rtl8651_getAsicQueueStrict(enum PORTID port, enum QUEUEID queueid,
				 enum QUEUETYPE * pQueueType)
{
	uint32 reg1, regOFFSET, regValue;

	if ((port < PHY0) || (port > CPU) || (queueid < QUEUE0)
	    || (queueid > MAX_QUEUE) || (pQueueType == NULL))
		return FAILED;
	...
	if (queueid < QUEUE6) {
		...
		if (pQueueType != NULL)
			*pQueueType =
			    ((regValue & (SCHE0_MASK << regOFFSET)) >>
			     SCHE0_OFFSET) >> regOFFSET;
	} else {
		reg1 = WFQWCR1P0_98C + (port * 0x4) + (((queueid - QUEUE6) >> 2) * 0x4);	/* offset to get corresponding register */
		regOFFSET = ((queueid - QUEUE6) % 4) * 0x8;	/* used to offset register value */
		regValue = READ_MEM32(reg1);
	...
	if (pQueueType != NULL)
		*pQueueType =
		    ((regValue & (SCHE0_MASK << regOFFSET)) >> SCHE0_OFFSET) >>
		    regOFFSET;
#endif
	return SUCCESS;
}
...
@comm
 */
int32 rtl8651_setAsicQueueWeight(enum PORTID port, enum QUEUEID queueid,
				 enum QUEUETYPE queueType, uint32 weight)
{
	uint32 reg1, regOFFSET, regValue;
	...
	regValue =
	    READ_MEM32(reg1) & ~((WEIGHT0_MASK | SCHE0_MASK) << regOFFSET);
	regValue |=
	    ((queueType << (SCHE0_OFFSET + regOFFSET)) |
	     (weight << (WEIGHT0_OFFSET + regOFFSET)));
	WRITE_MEM32(reg1, regValue);
...
@comm
 */
int32 rtl8651_getAsicQueueWeight(enum PORTID port, enum QUEUEID queueid,
				 enum QUEUETYPE * pQueueType, uint32 * pWeight)
{
	uint32 reg1, regOFFSET, regValue;
	...
	if (pWeight != NULL)
		*pWeight =
		    ((regValue & (WEIGHT0_MASK << regOFFSET)) >> WEIGHT0_OFFSET)
		    >> regOFFSET;
...
int32 rtl8651_setAsicOutputQueueNumber(enum PORTID port, enum QUEUENUM qnum)
{
	...
	WRITE_MEM32(QNUMCR,
		    (READ_MEM32(QNUMCR) & ~(0x7 << (3 * port))) | ((qnum - 1) <<
								   (3 * port)));
#else
	...
	orgQnum = (READ_MEM32(QNUMCR) >> (3 * port)) & 0x7;
	WRITE_MEM32(QNUMCR,
		    (READ_MEM32(QNUMCR) & ~(0x7 << (3 * port))) | (qnum <<
								   (3 * port)));
#endif
...
@comm
 */
int32 rtl8651_setAsicPriorityToQIDMappingTable(enum QUEUENUM qnum,
					       enum PRIORITYVALUE priority,
					       enum QUEUEID qid)
{
	/* Invalid input parameter */
	...
	switch (qnum) {
	case QNUM1:
		WRITE_MEM32(UPTCMCR0,
			    (READ_MEM32(UPTCMCR0) & ~(0x7 << (priority * 3))) |
			    (qid << (priority * 3)));
		break;
	case QNUM2:
		WRITE_MEM32(UPTCMCR1,
			    (READ_MEM32(UPTCMCR1) & ~(0x7 << (priority * 3))) |
			    (qid << (priority * 3)));
		break;
	case QNUM3:
		WRITE_MEM32(UPTCMCR2,
			    (READ_MEM32(UPTCMCR2) & ~(0x7 << (priority * 3))) |
			    (qid << (priority * 3)));
		break;
	case QNUM4:
		WRITE_MEM32(UPTCMCR3,
			    (READ_MEM32(UPTCMCR3) & ~(0x7 << (priority * 3))) |
			    (qid << (priority * 3)));
		break;
	case QNUM5:
		WRITE_MEM32(UPTCMCR4,
			    (READ_MEM32(UPTCMCR4) & ~(0x7 << (priority * 3))) |
			    (qid << (priority * 3)));
		break;
	case QNUM6:
		WRITE_MEM32(UPTCMCR5,
			    (READ_MEM32(UPTCMCR5) & ~(0x7 << (priority * 3))) |
			    (qid << (priority * 3)));
		break;
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
	case QNUM7:
		WRITE_MEM32(UPTCMCR6,
			    (READ_MEM32(UPTCMCR6) & ~(0x7 << (priority * 3))) |
			    (qid << (priority * 3)));
		break;
	case QNUM8:
		WRITE_MEM32(UPTCMCR7,
			    (READ_MEM32(UPTCMCR7) & ~(0x7 << (priority * 3))) |
			    (qid << (priority * 3)));
		break;
#endif
	default:
...
int32 rtl8651_getAsicPriorityToQIDMappingTable(enum QUEUENUM qnum,
					       enum PRIORITYVALUE priority,
					       enum QUEUEID * qid)
{
	/* Invalid input parameter */
...
@comm
 */
int32 rtl8651_setAsicCPUPriorityToQIDMappingTable(enum PORTID port,
						  enum PRIORITYVALUE priority,
						  enum QUEUEID qid)
{
	uint32 reg;
	...
	WRITE_MEM32(reg,
		    (READ_MEM32(reg) & ~(0x7 << (priority << 2))) | (qid <<
								     (priority
								      << 2)));
...
int32 rtl8651_setAsicSystemBasedFlowControlRegister(uint32 sharedON,
						    uint32 sharedOFF,
						    uint32 fcON, uint32 fcOFF,
						    uint32 drop)
{
	/* Invalid input parameter */
	...
	WRITE_MEM32(SBFCR0,
		    (READ_MEM32(SBFCR0) & ~(S_DSC_RUNOUT_MASK)) | (drop <<
								   S_DSC_RUNOUT_OFFSET));
	WRITE_MEM32(SBFCR1,
		    (READ_MEM32(SBFCR1) & ~(S_DSC_FCON_MASK | S_DSC_FCOFF_MASK))
		    | (fcON << S_DSC_FCON_OFFSET) | (fcOFF <<
						     S_DSC_FCOFF_OFFSET));
	WRITE_MEM32(SBFCR2,
		    (READ_MEM32(SBFCR2) &
		     ~(S_Max_SBuf_FCON_MASK | S_Max_SBuf_FCOFF_MASK)) |
		    (sharedON << S_Max_SBuf_FCON_OFFSET) | (sharedOFF <<
							    S_Max_SBuf_FCOFF_OFFSET));
	return SUCCESS;
}

int32 rtl8651_setAsicQueueDescriptorBasedFlowControlRegister(enum PORTID port,
							     enum QUEUEID queue,
							     uint32 fcON,
							     uint32 fcOFF)
{
	...
	switch (queue) {
	case QUEUE0:
		WRITE_MEM32((QDBFCRP0G0 + (port * 0xC)),
			    (READ_MEM32(QDBFCRP0G0 + (port * 0xC)) &
			     ~(QG_DSC_FCON_MASK | QG_DSC_FCOFF_MASK)) | (fcON <<
									 QG_DSC_FCON_OFFSET)
			    | (fcOFF << QG_DSC_FCOFF_OFFSET));
		break;
	case QUEUE1:
	case QUEUE2:
	case QUEUE3:
	case QUEUE4:
		WRITE_MEM32((QDBFCRP0G1 + (port * 0xC)),
			    (READ_MEM32(QDBFCRP0G1 + (port * 0xC)) &
			     ~(QG_DSC_FCON_MASK | QG_DSC_FCOFF_MASK)) | (fcON <<
									 QG_DSC_FCON_OFFSET)
			    | (fcOFF << QG_DSC_FCOFF_OFFSET));
		break;
	case QUEUE5:
		WRITE_MEM32((QDBFCRP0G2 + (port * 0xC)),
			    (READ_MEM32(QDBFCRP0G2 + (port * 0xC)) &
			     ~(QG_DSC_FCON_MASK | QG_DSC_FCOFF_MASK)) | (fcON <<
									 QG_DSC_FCON_OFFSET)
			    | (fcOFF << QG_DSC_FCOFF_OFFSET));
		break;
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
	case QUEUE6:
		WRITE_MEM32((QDBFCRP0G3 + (port * 0x8)),
			    (READ_MEM32(QDBFCRP0G3 + (port * 0x8)) &
			     ~(QG_DSC_FCON_MASK | QG_DSC_FCOFF_MASK)) | (fcON <<
									 QG_DSC_FCON_OFFSET)
			    | (fcOFF << QG_DSC_FCOFF_OFFSET));
		break;
	case QUEUE7:
		WRITE_MEM32((QDBFCRP0G4 + (port * 0x8)),
			    (READ_MEM32(QDBFCRP0G3 + (port * 0x8)) &
			     ~(QG_DSC_FCON_MASK | QG_DSC_FCOFF_MASK)) | (fcON <<
									 QG_DSC_FCON_OFFSET)
			    | (fcOFF << QG_DSC_FCOFF_OFFSET));
...
int32 rtl8651_setAsicQueuePacketBasedFlowControlRegister(enum PORTID port,
							 enum QUEUEID queue,
							 uint32 fcON,
							 uint32 fcOFF)
{
	...
	switch (queue) {
	case QUEUE0:
		WRITE_MEM32((QPKTFCRP0G0 + (port * 0xC)),
			    (READ_MEM32(QPKTFCRP0G0 + (port * 0xC)) &
			     ~(QG_QLEN_FCON_MASK | QG_QLEN_FCOFF_MASK)) | (fcON
									   <<
									   QG_QLEN_FCON_OFFSET)
			    | (fcOFF << QG_QLEN_FCOFF_OFFSET));
		break;
	case QUEUE1:
	case QUEUE2:
	case QUEUE3:
	case QUEUE4:
		WRITE_MEM32((QPKTFCRP0G1 + (port * 0xC)),
			    (READ_MEM32(QPKTFCRP0G1 + (port * 0xC)) &
			     ~(QG_QLEN_FCON_MASK | QG_QLEN_FCOFF_MASK)) | (fcON
									   <<
									   QG_QLEN_FCON_OFFSET)
			    | (fcOFF << QG_QLEN_FCOFF_OFFSET));
		break;
	case QUEUE5:
		WRITE_MEM32((QPKTFCRP0G2 + (port * 0xC)),
			    (READ_MEM32(QPKTFCRP0G2 + (port * 0xC)) &
			     ~(QG_QLEN_FCON_MASK | QG_QLEN_FCOFF_MASK)) | (fcON
									   <<
									   QG_QLEN_FCON_OFFSET)
			    | (fcOFF << QG_QLEN_FCOFF_OFFSET));
...
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
	case QUEUE6:
		WRITE_MEM32((QPKTFCRP0G3 + (port * 0x8)),
			    (READ_MEM32(QPKTFCRP0G3 + (port * 0x8)) &
			     ~(QG_QLEN_FCON_MASK | QG_QLEN_FCOFF_MASK)) | (fcON
									   <<
									   QG_QLEN_FCON_OFFSET)
			    | (fcOFF << QG_QLEN_FCOFF_OFFSET));
		break;
	case QUEUE7:
		WRITE_MEM32((QPKTFCRP0G4 + (port * 0x8)),
			    (READ_MEM32(QPKTFCRP0G4 + (port * 0x8)) &
			     ~(QG_QLEN_FCON_MASK | QG_QLEN_FCOFF_MASK)) | (fcON
									   <<
									   QG_QLEN_FCON_OFFSET)
			    | (fcOFF << QG_QLEN_FCOFF_OFFSET));
...
int32 rtl8651_setAsicPortBasedFlowControlRegister(enum PORTID port, uint32 fcON,
						  uint32 fcOFF)
{
	/* Invalid input parameter */
	...
	case PHY0:
		WRITE_MEM32(PBFCR0,
			    (READ_MEM32(PBFCR0) &
			     ~(P_MaxDSC_FCON_MASK | P_MaxDSC_FCOFF_MASK)) |
			    (fcON << P_MaxDSC_FCON_OFFSET) | (fcOFF <<
...
int32 rtl8651_setAsicPerQueuePhysicalLengthGapRegister(uint32 gap)
{
	...
	WRITE_MEM32(PQPLGR,
		    (READ_MEM32(PQPLGR) & ~(QLEN_GAP_MASK)) | (gap <<
							       QLEN_GAP_OFFSET));
	return SUCCESS;
}
...
int32 rtl819x_setQosThreshold(uint32 old_sts, uint32 new_sts)
{
	...
	if (link_up_ports <= DYN_THR_LINK_UP_PORTS) {
		WRITE_MEM32(SBFCR1,
			    (READ_MEM32(SBFCR1) &
			     ~(S_DSC_FCON_MASK | S_DSC_FCOFF_MASK)) |
			    (DYN_THR_AGG_fcON << S_DSC_FCON_OFFSET) |
			    (DYN_THR_AGG_fcOFF << S_DSC_FCOFF_OFFSET));
		WRITE_MEM32(SBFCR2,
			    (READ_MEM32(SBFCR2) &
			     ~(S_Max_SBuf_FCON_MASK | S_Max_SBuf_FCOFF_MASK)) |
			    (DYN_THR_AGG_sharedON << S_Max_SBuf_FCON_OFFSET) |
			    (DYN_THR_AGG_sharedOFF << S_Max_SBuf_FCOFF_OFFSET));
	} else {
		WRITE_MEM32(SBFCR1,
			    (READ_MEM32(SBFCR1) &
			     ~(S_DSC_FCON_MASK | S_DSC_FCOFF_MASK)) |
			    (DYN_THR_DEF_fcON << S_DSC_FCON_OFFSET) |
			    (DYN_THR_DEF_fcOFF << S_DSC_FCOFF_OFFSET));
		WRITE_MEM32(SBFCR2,
			    (READ_MEM32(SBFCR2) &
			     ~(S_Max_SBuf_FCON_MASK | S_Max_SBuf_FCOFF_MASK)) |
			    (DYN_THR_DEF_sharedON << S_Max_SBuf_FCON_OFFSET) |
			    (DYN_THR_DEF_sharedOFF << S_Max_SBuf_FCOFF_OFFSET));
	}
...
static int32 _rtl865x_setQosThresholdByQueueIdx(uint32 qidx)
{
	...
	retval =
	    rtl8651_setAsicSystemBasedFlowControlRegister(outputQueuePara
							  [qidx].systemSBFCON,
							  outputQueuePara
							  [qidx].systemSBFCOFF,
							  outputQueuePara
	...
	for (i = 0; i < RTL8651_OUTPUTQUEUE_SIZE; i++) {
		retval =
		    rtl8651_setAsicQueueDescriptorBasedFlowControlRegister(0, i,
									   outputQueuePara
									   [qidx].queueDescFCON,
									   outputQueuePara
									   [qidx].queueDescFCOFF);
		...
		for (j = 1; j <= CPU; j++)
			rtl8651_setAsicQueueDescriptorBasedFlowControlRegister
			    (PHY0 + j, i, outputQueuePara[qidx].queueDescFCON,
			     outputQueuePara[qidx].queueDescFCOFF);

		retval =
		    rtl8651_setAsicQueuePacketBasedFlowControlRegister(0, i,
								       outputQueuePara
								       [qidx].queuePktFCON,
								       outputQueuePara
								       [qidx].queuePktFCOFF);
		if (retval != SUCCESS) {
			rtlglue_printf
			    ("Set Queue Packet Base Flow Control Para Error.\n");
			return retval;
		}
		for (j = 1; j <= CPU; j++)
			rtl8651_setAsicQueuePacketBasedFlowControlRegister(PHY0
									   + j,
									   i,
									   outputQueuePara
									   [qidx].queuePktFCON,
	...
	retval =
	    rtl8651_setAsicPortBasedFlowControlRegister(0,
							outputQueuePara
							[qidx].portFCON,
							outputQueuePara
							[qidx].portFCOFF);
	...
	for (j = 1; j <= CPU; j++)
		rtl8651_setAsicPortBasedFlowControlRegister(PHY0 + j,
							    outputQueuePara
							    [qidx].portFCON,
							    outputQueuePara
							    [qidx].portFCOFF);
	...
	retval =
	    rtl8651_setAsicPerQueuePhysicalLengthGapRegister(outputQueuePara
							     [qidx].gap);
	if (retval != SUCCESS) {
...
int32 rtl865xC_waitForOutputQueueEmpty(void)
{
...
#endif
#if defined(CONFIG_RTL_8196C)
	while ((READ_MEM32(LAGCR1) & OUTPUTQUEUE_STAT_MASK_CR1) !=
	       OUTPUTQUEUE_STAT_MASK_CR1) ;
#elif defined(CONFIG_RTL_8198C)	|| defined(CONFIG_RTL_8197F)
	while (((READ_MEM32(LAGCR1) & OUTPUTQUEUE_STAT_MASK_CR1) ^
		OUTPUTQUEUE_STAT_MASK_CR1)
	       || ((READ_MEM32(LAGCR2) & OUTPUTQUEUE_STAT_MASK_CR2) ^
		   OUTPUTQUEUE_STAT_MASK_CR2)) {
		/*free rx ring when cpu port queue not empty */
		currtime = jiffies;
		if ((time_before_eq(latetime + 5 * HZ, currtime))
		    && ((READ_MEM32(LAGCR2) & OUTPUTQUEUE_STAT_MASK_CPU) ^
			OUTPUTQUEUE_STAT_MASK_CPU)) {
			swNic_freeRxRing();
		}
#if defined (CONFIG_RTL_HW_QOS_DEBUG)
...
#else
	while (((READ_MEM32(LAGCR0) & OUTPUTQUEUE_STAT_MASK_CR0) ^
		OUTPUTQUEUE_STAT_MASK_CR0)
	       || ((READ_MEM32(LAGCR1) & OUTPUTQUEUE_STAT_MASK_CR1) ^
		   OUTPUTQUEUE_STAT_MASK_CR1)) {
#if defined (CONFIG_RTL_HW_QOS_DEBUG)
	...
	i = 0;
	while (((currentDescUsage =
		 ((READ_MEM32(GDSR0) & USEDDSC_MASK) >> USEDDSC_OFFSET)) >
		_rtl865xC_QM_orgDescUsage) && (i < cnt)) {
#if defined (CONFIG_RTL_HW_QOS_DEBUG)
		if (net_ratelimit())
			printk
			    ("Waiting for input queue empty.  ==> currently used %d-%d=%d.\n",
			     currentDescUsage, _rtl865xC_QM_orgDescUsage,
			     (currentDescUsage - _rtl865xC_QM_orgDescUsage));
#else
		;		/* do nothing */
	...
	while (((currentDescUsage =
		...
		if (net_ratelimit() && i < 120)
			rtlglue_printf
			    ("Waiting for input queue empty.  ==> currently used %d.\n",
			     (currentDescUsage - _rtl865xC_QM_orgDescUsage));
#else
		;		/* do nothing */
#endif
...
int32 rtl8651_resetAsicOutputQueue(void)
{
	...
	case 0:
		pauseTicks = 6250000;
		break;		//lexra clock 50Mhz
	case 1:
		pauseTicks = 12500000;
		break;		//lexra clock 100Mhz
	default:
		pauseTicks = 6250000;
		break;		//lexra clock 50Mhz             
	}
...
static void _rtl8651_syncToAsicEthernetBandwidthControl(void)
{
	...
	for (port = 0; port < RTL8651_PORT_NUMBER; port++) {
		for (cfgTypeIdx = 0;
		     cfgTypeIdx < _RTL865XB_BANDWIDTHCTRL_CFGTYPE;
		     cfgTypeIdx++) {
			...
			/*
			   ...
			   1                            1                               x8
			 */
			if (_rtl865xB_BandwidthCtrlMultiplier & _RTL865XB_BANDWIDTHCTRL_X8) {	/* case {b'10, b'11} */
				currentSwBandwidthCtrlMultiplier = 8;
			} else if (_rtl865xB_BandwidthCtrlMultiplier & _RTL865XB_BANDWIDTHCTRL_X4) {	/* case {b'01} */
			...
			currentSwBandwidthCtrlBasicSetting =
			    ((_rtl865xC_BandwidthCtrlNum
			      [_rtl865xB_BandwidthCtrlPerPortConfiguration[port]
			       [cfgTypeIdx]]) *
			     (currentSwBandwidthCtrlMultiplier));
			currentSwBandwidthCtrlSetting = (cfgTypeIdx == 0) ?
			    /* Ingress */
			    (((currentSwBandwidthCtrlBasicSetting %
			       RTL865XC_INGRESS_16KUNIT) <
			      (RTL865XC_INGRESS_16KUNIT >> 1))
			     ? (currentSwBandwidthCtrlBasicSetting /
				RTL865XC_INGRESS_16KUNIT)
			     : ((currentSwBandwidthCtrlBasicSetting /
				 RTL865XC_INGRESS_16KUNIT) + 1)) :
			    /* Egress */
			    (((currentSwBandwidthCtrlBasicSetting %
			       RTL865XC_EGRESS_64KUNIT) <
			      (RTL865XC_EGRESS_64KUNIT >> 1))
			     ? (currentSwBandwidthCtrlBasicSetting /
				RTL865XC_EGRESS_64KUNIT)
			     : ((currentSwBandwidthCtrlBasicSetting /
				 RTL865XC_EGRESS_64KUNIT) + 1));
				...
				retval = (cfgTypeIdx == 0) ?
				    ...
				    (rtl8651_setAsicPortIngressBandwidth(port,
									 (currentSwBandwidthCtrlSetting
									  == 0)
									 ? (0
									    /* For Ingress Bandwidth control, 0 means "disabled" */
									 ) :
									 (currentSwBandwidthCtrlSetting)))
				    :
				    /* Egress */
				    (rtl8651_setAsicPortEgressBandwidth(port,
									...
									?
									(0xfffff
									 /* For Egress Bandwidth control, 0xfffff means "disabled" */
									) :
									(currentSwBandwidthCtrlSetting)));
				if (retval != SUCCESS) {
					assert(0);
			...
			if ((!
			     ((currentSwBandwidthCtrlSetting) == 0
			      && (currentAsicBandwidthCtrlSetting ==
				  0x3fff)) /* for FULL Rate case */ ) ||
			    (currentSwBandwidthCtrlSetting !=
			     currentAsicBandwidthCtrlSetting)) {
#if 0
				if (cfgTypeIdx == 0) {
					rtlglue_printf
					    ("set ingress bandwidth port %d, %d.\n",
					     port,
					     (currentSwBandwidthCtrlSetting ==
					      0) ? (0
						    /* For Ingress Bandwidth control, 0 means "disabled" */
					...
					rtlglue_printf
					    ("set ingress bandwidth port %d, %d.\n",
					     ...
					     (currentSwBandwidthCtrlSetting ==
					      0) ? (0x3fff
						    /* For Egress Bandwidth control, 0x3fff means "disabled" */
					     ) :
					     (currentSwBandwidthCtrlSetting));
				}
#endif
 ...
 */
int32 rtl8651_setAsicEthernetBandwidthControl(uint32 port, int8 input,
					      uint32 rate)
{
	...
	currentConfig_p = &(_rtl865xB_BandwidthCtrlPerPortConfiguration[port]
			    [(input) ? 0 /* Ingress */ : 1 /* Egress */ ]);
...
int32 rtl8651_setAsicSystemInputFlowControlRegister(uint32 fcON, uint32 fcOFF)
{
	...
	WRITE_MEM32(IQFCTCR,
		    ((READ_MEM32(IQFCTCR) &
		      ~(IQ_DSC_FCON_MASK | IQ_DSC_FCOFF_MASK)) | (fcON <<
								  IQ_DSC_FCON_OFFSET)
...
Set input-queue flow control threshold on RTL865xC platform.
 */
int32 rtl8651_getAsicSystemInputFlowControlRegister(uint32 * fcON,
						    uint32 * fcOFF)
{
	uint32 iqfctcr;
...
int32 rtl865xC_setAsicEthernetForceModeRegs(uint32 port, uint32 enForceMode,
					    uint32 forceLink, uint32 forceSpeed,
					    uint32 forceDuplex)
{
	uint32 offset = port * 4;
	...
	if (enForceMode) {
		...
		PCR =
		    (PCR & ~(PollLinkStatus)) | (ForceLink | AutoNegoSts_MASK);
...
static int32 rtl8651_perPortStormControl(uint32 type, uint32 portNum,
					 uint32 enable)
{
	uint32 regAddress;
	uint32 oldRegValue;
...
static int32 rtl865x_setPerPortExtendStormControl(uint32 type, uint32 port,
						  uint32 enable)
{
	uint32 regAddress;
	uint32 oldRegValue;
	...
	if (enable == TRUE) {
		...
		counter =
		    (port & 0x1) ? (counter << OPUUSC_OFFSET) : (counter <<
								 EPUUSC_OFFSET);
...
int32 rtl865x_setExtendStormControl(uint32 type, uint32 enable,
				    uint32 percentage)
{
	uint32 port;
...
int32 rtl8651_setAsic802D1xMacBaseAbility(enum PORTID port, uint32 isEnable)
{
	...
	switch (port) {
	case PHY0:
		WRITE_MEM32(DOT1XMACCR,
			    isEnable ==
			    TRUE ? (READ_MEM32(DOT1XMACCR) | (Dot1xMAC_P0En))
	...
	case PHY1:
		WRITE_MEM32(DOT1XMACCR,
			    isEnable ==
			    TRUE ? (READ_MEM32(DOT1XMACCR) | (Dot1xMAC_P1En))
	...
	case PHY2:
		WRITE_MEM32(DOT1XMACCR,
			    isEnable ==
			    TRUE ? (READ_MEM32(DOT1XMACCR) | (Dot1xMAC_P2En))
	...
	case PHY3:
		WRITE_MEM32(DOT1XMACCR,
			    isEnable ==
			    TRUE ? (READ_MEM32(DOT1XMACCR) | (Dot1xMAC_P3En))
	...
	case PHY4:
		WRITE_MEM32(DOT1XMACCR,
			    isEnable ==
			    TRUE ? (READ_MEM32(DOT1XMACCR) | (Dot1xMAC_P4En))
	...
	case PHY5:
		WRITE_MEM32(DOT1XMACCR,
			    isEnable ==
			    TRUE ? (READ_MEM32(DOT1XMACCR) | (Dot1xMAC_P5En))
	...
	case CPU:
		WRITE_MEM32(DOT1XMACCR,
			    isEnable ==
			    TRUE ? (READ_MEM32(DOT1XMACCR) | (Dot1xMAC_P6En))
	...
	case EXT1:
		WRITE_MEM32(DOT1XMACCR,
			    isEnable ==
			    TRUE ? (READ_MEM32(DOT1XMACCR) | (Dot1xMAC_P7En))
	...
	case EXT2:
		WRITE_MEM32(DOT1XMACCR,
			    isEnable ==
			    TRUE ? (READ_MEM32(DOT1XMACCR) | (Dot1xMAC_P8En))
...
int32 rtl8651_setAsic802D1xMacBaseDirection(int32 dir)
{
	...
	if (dir == Dot1xMAC_OPDIR_IN) {
		WRITE_MEM32(DOT1XMACCR,
			    (READ_MEM32(DOT1XMACCR)) | Dot1xMAC_OPDIR);
	} else {
		WRITE_MEM32(DOT1XMACCR,
			    (READ_MEM32(DOT1XMACCR)) & (~Dot1xMAC_OPDIR));
	}
...
{
	/* Invalid input parameter */
	if ((process < Dot1xUNAUTHBH_DROP)
	    || (process > Dot1xUNAUTHBH_RESERVED))
		return FAILED;
...
@comm
 */
int32 rtl8651_setAsicDot1qAbsolutelyPriority(enum PRIORITYVALUE srcpriority,
					     enum PRIORITYVALUE priority)
{
	/* Invalid input parameter */
	if ((srcpriority < PRI0) || (srcpriority > PRI7) || (priority < PRI0)
	    || (priority > PRI7))
		return FAILED;

	switch (srcpriority) {
	case PRI0:
		WRITE_MEM32(LPTM8021Q,
			    (READ_MEM32(LPTM8021Q) & ~(EN_8021Q2LTMPRI0_MASK)) |
			    (priority << EN_8021Q2LTMPRI0));
		break;
	case PRI1:
		WRITE_MEM32(LPTM8021Q,
			    (READ_MEM32(LPTM8021Q) & ~(EN_8021Q2LTMPRI1_MASK)) |
			    (priority << EN_8021Q2LTMPRI1));
		break;
	case PRI2:
		WRITE_MEM32(LPTM8021Q,
			    (READ_MEM32(LPTM8021Q) & ~(EN_8021Q2LTMPRI2_MASK)) |
			    (priority << EN_8021Q2LTMPRI2));
		break;
	case PRI3:
		WRITE_MEM32(LPTM8021Q,
			    (READ_MEM32(LPTM8021Q) & ~(EN_8021Q2LTMPRI3_MASK)) |
			    (priority << EN_8021Q2LTMPRI3));
		break;
	case PRI4:
		WRITE_MEM32(LPTM8021Q,
			    (READ_MEM32(LPTM8021Q) & ~(EN_8021Q2LTMPRI4_MASK)) |
			    (priority << EN_8021Q2LTMPRI4));
		break;
	case PRI5:
		WRITE_MEM32(LPTM8021Q,
			    (READ_MEM32(LPTM8021Q) & ~(EN_8021Q2LTMPRI5_MASK)) |
			    (priority << EN_8021Q2LTMPRI5));
		break;
	case PRI6:
		WRITE_MEM32(LPTM8021Q,
			    (READ_MEM32(LPTM8021Q) & ~(EN_8021Q2LTMPRI6_MASK)) |
			    (priority << EN_8021Q2LTMPRI6));
		break;
	case PRI7:
		WRITE_MEM32(LPTM8021Q,
			    (READ_MEM32(LPTM8021Q) & ~(EN_8021Q2LTMPRI7_MASK)) |
			    (priority << EN_8021Q2LTMPRI7));
		break;
	}
...
@comm
 */
int32 rtl8651_getAsicDot1qAbsolutelyPriority(enum PRIORITYVALUE srcpriority,
...
static uint32 _rtl8651_findAsicLinkupPortmask(uint32 portMask)
{
	...
	for (port = 0; port < RTL8651_PORT_NUMBER + rtl8651_totalExtPortNum;
	     port++) {
		if ((portMask & (1 << port))
		    && (rtl8651AsicEthernetTable[port].linkUp == FALSE)) {
...
//      uint32 portmask;
	...
	notify =
	    (rtl8651AsicEthernetTable[port].linkUp !=
	     ((linkUp == TRUE) ? TRUE : FALSE)) ? TRUE : FALSE;
	...
	if (portmask & (1 << port)) {
		...
		uint32 lnkUp_portMask =
		    _rtl8651_findAsicLinkupPortmask(portmask);
...
int32 rtl8651_setAsicRateLimitTable(uint32 index,
...
int32 rtl8651_getAsicRateLimitTable(uint32 index,
				    rtl865x_tblAsicDrv_rateLimitParam_t *
				    rateLimit_t)
{
	rtl8651_tblAsic_rateLimitTable_t entry;
...
int32 rtl8651_setPortFlowControlConfigureRegister(enum PORTID port,
						  uint32 enable)
{
	int enable_port = 0;
	if (enable)
	...
	case PHY0:
		WRITE_MEM32(FCCR0,
			    (READ_MEM32(FCCR0) & ~(0x3F << Q_P0_EN_FC_OFFSET)) |
			    (enable_port << Q_P0_EN_FC_OFFSET));
		break;
	case PHY1:
		WRITE_MEM32(FCCR0,
			    (READ_MEM32(FCCR0) & ~(0x3F << Q_P1_EN_FC_OFFSET)) |
			    (enable_port << Q_P1_EN_FC_OFFSET));
		break;
	case PHY2:
		WRITE_MEM32(FCCR0,
			    (READ_MEM32(FCCR0) & ~(0x3F << Q_P2_EN_FC_OFFSET)) |
			    (enable_port << Q_P2_EN_FC_OFFSET));
		break;
	case PHY3:
		WRITE_MEM32(FCCR0,
			    (READ_MEM32(FCCR0) & ~(0x3F << Q_P3_EN_FC_OFFSET)) |
			    (enable_port << Q_P3_EN_FC_OFFSET));
		break;
	case PHY4:
		WRITE_MEM32(FCCR1,
			    (READ_MEM32(FCCR1) & ~(0x3F << Q_P4_EN_FC_OFFSET)) |
			    (enable_port << Q_P4_EN_FC_OFFSET));
		break;
	case PHY5:
		WRITE_MEM32(FCCR1,
			    (READ_MEM32(FCCR1) & ~(0x3F << Q_P5_EN_FC_OFFSET)) |
			    (enable_port << Q_P5_EN_FC_OFFSET));
		break;
	case CPU:
		WRITE_MEM32(FCCR1,
			    (READ_MEM32(FCCR1) & ~(0x1 << Q_P6_EN_FC_OFFSET)) |
			    (enable_port << Q_P6_EN_FC_OFFSET));
		break;
	default:
		return FAILED;
	}
...
int32 rtl8651_setAsicPortPriority(enum PORTID port, enum PRIORITYVALUE priority)
{
	...
	case PHY0:
		WRITE_MEM32(PBPCR,
			    (READ_MEM32(PBPCR) & ~(PBPRI_P0_MASK)) | (priority
								      <<
								      PBPRI_P0_OFFSET));
		break;
	case PHY1:
		WRITE_MEM32(PBPCR,
			    (READ_MEM32(PBPCR) & ~(PBPRI_P1_MASK)) | (priority
								      <<
								      PBPRI_P1_OFFSET));
		break;
	case PHY2:
		WRITE_MEM32(PBPCR,
			    (READ_MEM32(PBPCR) & ~(PBPRI_P2_MASK)) | (priority
								      <<
								      PBPRI_P2_OFFSET));
		break;
	case PHY3:
		WRITE_MEM32(PBPCR,
			    (READ_MEM32(PBPCR) & ~(PBPRI_P3_MASK)) | (priority
								      <<
								      PBPRI_P3_OFFSET));
		break;
	case PHY4:
		WRITE_MEM32(PBPCR,
			    (READ_MEM32(PBPCR) & ~(PBPRI_P4_MASK)) | (priority
								      <<
								      PBPRI_P4_OFFSET));
		break;
	case PHY5:
		WRITE_MEM32(PBPCR,
			    (READ_MEM32(PBPCR) & ~(PBPRI_P5_MASK)) | (priority
								      <<
								      PBPRI_P5_OFFSET));
		break;
	case EXT1:
		WRITE_MEM32(PBPCR,
			    (READ_MEM32(PBPCR) & ~(PBPRI_P6_MASK)) | (priority
								      <<
								      PBPRI_P6_OFFSET));
		break;
	case EXT2:
		WRITE_MEM32(PBPCR,
			    (READ_MEM32(PBPCR) & ~(PBPRI_P7_MASK)) | (priority
								      <<
								      PBPRI_P7_OFFSET));
		break;
	case EXT3:
		WRITE_MEM32(PBPCR,
			    (READ_MEM32(PBPCR) & ~(PBPRI_P8_MASK)) | (priority
								      <<
								      PBPRI_P8_OFFSET));
		break;
	case CPU:		/* fall thru */
...
@comm
 */
int32 rtl8651_getAsicPortPriority(enum PORTID port,
				  enum PRIORITYVALUE * pPriority)
{
	if (pPriority != NULL) {
		switch (port) {
		case PHY0:
			*pPriority =
			    (READ_MEM32(PBPCR) & PBPRI_P0_MASK) >>
			    PBPRI_P0_OFFSET;
			break;
		case PHY1:
			*pPriority =
			    (READ_MEM32(PBPCR) & PBPRI_P1_MASK) >>
			    PBPRI_P1_OFFSET;
			break;
		case PHY2:
			*pPriority =
			    (READ_MEM32(PBPCR) & PBPRI_P2_MASK) >>
			    PBPRI_P2_OFFSET;
			break;
		case PHY3:
			*pPriority =
			    (READ_MEM32(PBPCR) & PBPRI_P3_MASK) >>
			    PBPRI_P3_OFFSET;
			break;
		case PHY4:
			*pPriority =
			    (READ_MEM32(PBPCR) & PBPRI_P4_MASK) >>
			    PBPRI_P4_OFFSET;
			break;
		case PHY5:
			*pPriority =
			    (READ_MEM32(PBPCR) & PBPRI_P5_MASK) >>
			    PBPRI_P5_OFFSET;
			break;
		case EXT1:
			*pPriority =
			    (READ_MEM32(PBPCR) & PBPRI_P6_MASK) >>
			    PBPRI_P6_OFFSET;
			break;
		case EXT2:
			*pPriority =
			    (READ_MEM32(PBPCR) & PBPRI_P7_MASK) >>
			    PBPRI_P7_OFFSET;
			break;
		case EXT3:
			*pPriority =
			    (READ_MEM32(PBPCR) & PBPRI_P8_MASK) >>
			    PBPRI_P8_OFFSET;
			break;
		case CPU:	/* fall thru */
		default:
...
int32 rtl8651_setAsicDscpPriority(uint32 dscp, enum PRIORITYVALUE priority)
{
	...
	if ((0 <= dscp) && (dscp <= 9))
		WRITE_MEM32(DSCPCR0,
			    (READ_MEM32(DSCPCR0) & ~(0x7 << (dscp * 3))) |
			    (priority << (dscp * 3)));
	else if ((10 <= dscp) && (dscp <= 19))
		WRITE_MEM32(DSCPCR1,
			    (READ_MEM32(DSCPCR1) & ~(0x7 << ((dscp % 10) * 3)))
			    | (priority << ((dscp % 10) * 3)));
	else if ((20 <= dscp) && (dscp <= 29))
		WRITE_MEM32(DSCPCR2,
			    (READ_MEM32(DSCPCR2) & ~(0x7 << ((dscp % 10) * 3)))
			    | (priority << ((dscp % 10) * 3)));
	else if ((30 <= dscp) && (dscp <= 39))
		WRITE_MEM32(DSCPCR3,
			    (READ_MEM32(DSCPCR3) & ~(0x7 << ((dscp % 10) * 3)))
			    | (priority << ((dscp % 10) * 3)));
	else if ((40 <= dscp) && (dscp <= 49))
		WRITE_MEM32(DSCPCR4,
			    (READ_MEM32(DSCPCR4) & ~(0x7 << ((dscp % 10) * 3)))
			    | (priority << ((dscp % 10) * 3)));
	else if ((50 <= dscp) && (dscp <= 59))
		WRITE_MEM32(DSCPCR5,
			    (READ_MEM32(DSCPCR5) & ~(0x7 << ((dscp % 10) * 3)))
			    | (priority << ((dscp % 10) * 3)));
	else if ((60 <= dscp) && (dscp <= 63))
		WRITE_MEM32(DSCPCR6,
			    (READ_MEM32(DSCPCR6) & ~(0x7 << ((dscp % 10) * 3)))
			    | (priority << ((dscp % 10) * 3)));
...
int32 rtl8651_getAsicDscpPriority(uint32 dscp, enum PRIORITYVALUE * pPriority)
{
	...
	if (pPriority != NULL) {
		if ((0 <= dscp) && (dscp <= 9))
			*pPriority =
			    (READ_MEM32(DSCPCR0) & (0x7 << (dscp * 3))) >> (dscp
									    *
									    3);
		else if ((10 <= dscp) && (dscp <= 19))
			*pPriority =
...
int32 rtl8651_setLAHashIndexToPortMapping(uint32 hash_index, uint32 port_num)
{
	...
	if ((hash_index >= RTL865XC_LAGHASHIDX_NUMBER)
	    || (port_num >= RTL8651_AGGREGATOR_NUMBER)) {
		return FAILED;
	}
