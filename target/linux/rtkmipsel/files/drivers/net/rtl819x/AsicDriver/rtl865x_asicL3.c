...
int32 rtl8651_setAsicExtIntIpTable(uint32 index,
				   rtl865x_tblAsicDrv_extIntIpParam_t *
				   extIntIpp)
{
	rtl8651_tblAsic_extIpTable_t entry;

	if ((index >= RTL8651_IPTABLE_SIZE) || (extIntIpp == NULL) || ((extIntIpp->localPublic == TRUE) && (extIntIpp->nat == TRUE)))	//Local public IP and NAT property cannot co-exist
		return FAILED;
...
int32 rtl8651_getAsicExtIntIpTable(uint32 index,
				   rtl865x_tblAsicDrv_extIntIpParam_t *
				   extIntIpp)
{
	rtl8651_tblAsic_extIpTable_t entry;
...
int32 rtl8651_setAsicPppoe(uint32 index,
			   rtl865x_tblAsicDrv_pppoeParam_t * pppoep)
{
	rtl8651_tblAsic_pppoeTable_t entry;

	if ((index >= RTL8651_PPPOETBL_SIZE) || (pppoep == NULL)
	    || (pppoep->sessionId == 0xffff))
		return FAILED;
...
int32 rtl8651_getAsicPppoe(uint32 index,
			   rtl865x_tblAsicDrv_pppoeParam_t * pppoep)
{
	rtl8651_tblAsic_pppoeTable_t entry;
...
int32 rtl8651_setAsicNextHopTable(uint32 index,
				  rtl865x_tblAsicDrv_nextHopParam_t * nextHopp)
{
...
int32 rtl8651_getAsicNextHopTable(uint32 index,
				  rtl865x_tblAsicDrv_nextHopParam_t * nextHopp)
{
	rtl8651_tblAsic_nextHopTable_t entry;
	if ((index >= RTL8651_NEXTHOPTBL_SIZE) || (nextHopp == NULL))
...
int32 rtl8651_setAsicRouting(uint32 index,
			     rtl865x_tblAsicDrv_routingParam_t * routingp)
{
	...
	case 0:		//PPPoE
		...
		entry.linkTo.PPPoEEntry.nextHop =
		    (routingp->nextHopRow << 2) | routingp->nextHopColumn;
	...
	case 5:		//NAPT NextHop
		...
		case 2:
			entry.linkTo.NxtHopEntry.nhNum = 0;
			break;
		case 4:
			entry.linkTo.NxtHopEntry.nhNum = 1;
			break;
		case 8:
			entry.linkTo.NxtHopEntry.nhNum = 2;
			break;
		case 16:
			entry.linkTo.NxtHopEntry.nhNum = 3;
			break;
		case 32:
			entry.linkTo.NxtHopEntry.nhNum = 4;
			break;
		default:
			return FAILED;
		}
...
int32 rtl8651_getAsicRouting(uint32 index,
			     rtl865x_tblAsicDrv_routingParam_t * routingp)
{
	uint32 i;
	rtl865xc_tblAsic_l3RouteTable_t entry;
	...
	for (i = 0, routingp->ipMask = 0; i <= entry.linkTo.ARPEntry.IPMask;
	     i++)
...
uint32 rtl8651_ipMulticastTableIndex(ipaddr_t srcAddr, ipaddr_t dstAddr)
{
	...
	if (currHashMethod == HASH_METHOD_SIP) {
		...
		hash[7] =
		    sip[0] ^ sip[8] ^ sip[16] ^ sip[24] ^ dip[7] ^ dip[15] ^
		    dip[23] ^ dip[31];
		hash[6] =
		    sip[1] ^ sip[9] ^ sip[17] ^ sip[25] ^ dip[6] ^ dip[14] ^
		    dip[22] ^ dip[30];
		hash[5] =
		    sip[2] ^ sip[10] ^ sip[18] ^ sip[26] ^ dip[5] ^ dip[13] ^
		    dip[21] ^ dip[29];
		hash[4] =
		    sip[3] ^ sip[11] ^ sip[19] ^ sip[27] ^ dip[4] ^ dip[12] ^
		    dip[20] ^ dip[28];
		hash[3] =
		    sip[4] ^ sip[12] ^ sip[20] ^ sip[28] ^ dip[3] ^ dip[11] ^
		    dip[19] ^ dip[27];
		hash[2] =
		    sip[5] ^ sip[13] ^ sip[21] ^ sip[29] ^ dip[2] ^ dip[10] ^
		    dip[18] ^ dip[26];
		hash[1] =
		    sip[6] ^ sip[14] ^ sip[22] ^ sip[30] ^ dip[1] ^ dip[9] ^
		    dip[17] ^ dip[25];
		hash[0] =
		    sip[7] ^ sip[15] ^ sip[23] ^ sip[31] ^ dip[0] ^ dip[8] ^
		    dip[16] ^ dip[24];
		...
		tempPtr = (uint8 *) (&temp);
		idx =
		    (uint32) (tempPtr[0] ^ tempPtr[1] ^ tempPtr[2] ^
			      tempPtr[3]);
#endif
	...
	if (currHashMethod == HASH_METHOD_SIP) {	/* hash method:  SIP[6:0] */
		...
		hash[0] =
		    sip[0] ^ sip[7] ^ sip[14] ^ sip[21] ^ sip[28] ^ dip[1] ^
		    dip[8] ^ dip[15] ^ dip[22] ^ dip[29];
		hash[1] =
		    sip[1] ^ sip[8] ^ sip[15] ^ sip[22] ^ sip[29] ^ dip[2] ^
		    dip[9] ^ dip[16] ^ dip[23] ^ dip[30];
		hash[2] =
		    sip[2] ^ sip[9] ^ sip[16] ^ sip[23] ^ sip[30] ^ dip[3] ^
		    dip[10] ^ dip[17] ^ dip[24] ^ dip[31];
		hash[3] =
		    sip[3] ^ sip[10] ^ sip[17] ^ sip[24] ^ sip[31] ^ dip[4] ^
		    dip[11] ^ dip[18] ^ dip[25];
		hash[4] =
		    sip[4] ^ sip[11] ^ sip[18] ^ sip[25] ^ dip[5] ^ dip[12] ^
		    dip[19] ^ dip[26];
		hash[5] =
		    sip[5] ^ sip[12] ^ sip[19] ^ sip[26] ^ dip[6] ^ dip[13] ^
		    dip[20] ^ dip[27];
		hash[6] =
		    sip[6] ^ sip[13] ^ sip[20] ^ sip[27] ^ dip[0] ^ dip[7] ^
		    dip[14] ^ dip[21] ^ dip[28];
		...
		hashSrcAddr =
		    srcAddr ^ (srcAddr >> 7) ^ (srcAddr >> 14) ^ (srcAddr >> 21)
		    ^ ((srcAddr >> 28) & 0xF);
		hashDstAddr =
		    ((dstAddr >> 1) ^ (dstAddr >> 8) ^ (dstAddr >> 15) ^
		     (dstAddr >> 22)) ^ ((dstAddr >> 29) & 0x7) ^ ((dstAddr &
								    0x1) << 6);
...
int32 rtl8651_setAsicIpMulticastTable(rtl865x_tblAsicDrv_multiCastParam_t *
				      mCast_t)
{
	uint32 idx;
	...
	if (mCast_t->dip >> 28 != 0xe
	    || mCast_t->port >= RTL8651_PORT_NUMBER + rtl8651_totalExtPortNum)
...
int32 rtl8651_getAsicIpMulticastTable(uint32 index,
...
int32 rtl865x_set_mcastMacClone(uint32 enabled, unsigned char *macAddr)
{
	...
	if (enabled) {
		WRITE_MEM32(FFCR,
			    ((READ_MEM32(FFCR) & (~CF_IPMMAC_CLONE_EN)) |
			     CF_IPMMAC_CLONE_EN));
		if (macAddr) {
...
{
	if (port >= RTL8651_PORT_NUMBER + rtl8651_totalExtPortNum) {	/* Invalid port */
		return FAILED;
	}
	...
	if (isInternal == TRUE) {
		WRITE_MEM32(SWTCR0,
			    READ_MEM32(SWTCR0) |
			    (((1 << port) & MCAST_PORT_EXT_MODE_MASK) <<
			     MCAST_PORT_EXT_MODE_OFFSET));
	} else {
		WRITE_MEM32(SWTCR0,
			    READ_MEM32(SWTCR0) &
			    ~(((1 << port) & MCAST_PORT_EXT_MODE_MASK) <<
...
{
	if (mcastMTU & ~(MultiCastMTU_MASK)) {	/* multicast MTU overflow */
		return FAILED;
	}
...
int32 rtl8651_getAsicMulticastMTU(uint32 * mcastMTU)
{
	...
	*mcastMTU =
	    GET_MEM32_VAL(ALECR, MultiCastMTU_MASK, MultiCastMTU_OFFSET);
...
int32 rtl865x_setAsicMCastHashMethod(unsigned int hashMethod)
{
	...
	if (oldHashMethod != currHashMethod) {	/* set FFCR Register bit 5~6 */
		WRITE_MEM32(FFCR,
			    (READ_MEM32(FFCR) & 0xFFFFFF9F) | (currHashMethod <<
							       5));
	}
	return SUCCESS;
}
...
{
	if (mcastMTU & ~(MultiCastv6MTU_MASK)) {	/* multicast MTU overflow */
		return FAILED;
	}

	UPDATE_MEM32(IPV6CR0, mcastMTU, MultiCastv6MTU_MASK,
		     MultiCastv6MTU_OFFSET);
...
int32 rtl8198C_getAsicMulticastv6MTU(uint32 * mcastMTU)
{
	...
	*mcastMTU =
	    GET_MEM32_VAL(IPV6CR0, MultiCastv6MTU_MASK, MultiCastv6MTU_OFFSET);
...
int32 rtl8198C_setAsicMCastv6HashMethod(unsigned int hashMethod)	//to be checked
{
	...
	if (oldHashMethod != hashMethod) {	/* set IPV6CR Register bit 17~18 and flush multicastv6 table */
		WRITE_MEM32(IPV6CR0,
			    (READ_MEM32(IPV6CR0) & 0xFFF9FFFF) | (currHashMethod
								  << 17));
	}
	return SUCCESS;
}
...
//6rd
///////////////
int32 rtl8198C_setAsic6rdTable(uint32 index,
			       rtl8198C_tblAsicDrv_6rdParam_t * six_rd_entry)
{
	uint32 asicMask = 0;
	...
	if ((six_rd_entry->six_rd_prefix_len == 0)
	    || (six_rd_entry->br_ip_mask_len == 0)) {
		return FAILED;
	}
...
#if 0
	for (i = 0; i < 32; i++)
		if (six_rd_entry->ce_ip_mask & (1 << i))
			break;
	asicMask = i;
	...
	entry.six_rd_pfx25_0 = six_rd_entry->six_rd_prefix.v6_addr32[1]
	    & 0x3FFFFFF;
	entry.six_rd_pfx57_26 =
	    ((six_rd_entry->six_rd_prefix.
	      v6_addr32[0] << 6) | (six_rd_entry->six_rd_prefix.v6_addr32[1] >>
				    26));
	entry.six_rd_pfx63_58 =
	    (six_rd_entry->six_rd_prefix.v6_addr32[0] >> 26);
	...
	if (1 /*six_rd_entry->br_ip_mask */ ) {
		for (i = 0; i < 32; i++)
			if (six_rd_entry->br_ip_mask & (1 << i))
				break;
		///leroy////asicMask = 31 - i;
		if (i != 0)
			asicMask = i - 1;
...
int32 rtl8198C_getAsic6rdTable(uint32 index,
			       rtl8198C_tblAsicDrv_6rdParam_t * six_rd_entry)
{
	rtl8198C_tblAsic_6rdTable_t entry;
	if ((index >= RTL8198C_6RDTBL_SIZE) || (six_rd_entry == NULL))
	...
	six_rd_entry->six_rd_prefix.v6_addr32[0] =
	    ((entry.six_rd_pfx57_26 & (~0x3F)) >> 6)
	    | ((entry.six_rd_pfx63_58 & 0x3F) << 26);
	six_rd_entry->six_rd_prefix_len = entry.pfx_mask + 1;
	six_rd_entry->br_ip_addr =
	    entry.br_ip_addr19_0 | (entry.br_ip_addr31_20 << 20);
...
//dslite
///////////////
int32 rtl8198C_setAsicDsliteTable(uint32 index,
				  ...
				  dslite_entry)
{
	...
	entry.aftr_ipv6_addr24_0 =
	    dslite_entry->aftr_ipv6_addr.v6_addr32[3] & 0x1FFFFFF;
	entry.aftr_ipv6_addr56_25 =
	    ((dslite_entry->aftr_ipv6_addr.
	      v6_addr32[2] << 7) | (dslite_entry->aftr_ipv6_addr.v6_addr32[3] >>
				    25));
	entry.aftr_ipv6_addr88_57 =
	    ((dslite_entry->aftr_ipv6_addr.
	      v6_addr32[1] << 7) | (dslite_entry->aftr_ipv6_addr.v6_addr32[2] >>
				    25));
	entry.aftr_ipv6_addr120_89 =
	    ((dslite_entry->aftr_ipv6_addr.
	      v6_addr32[0] << 7) | (dslite_entry->aftr_ipv6_addr.v6_addr32[1] >>
				    25));
	entry.aftr_ipv6_addr127_121 =
	    (dslite_entry->aftr_ipv6_addr.v6_addr32[0] >> 25);
...
int32 rtl8198C_getAsicDsliteTable(uint32 index,
				  rtl8198C_tblAsicDrv_dsliteParam_t *
				  dslite_entry)
{
	rtl8198C_tblAsic_dsliteTable_t entry;
	...
	dslite_entry->aftr_ipv6_addr.v6_addr32[2] =
	    (entry.
	     aftr_ipv6_addr88_57 << 25) | (entry.aftr_ipv6_addr56_25 >> 7);

	dslite_entry->aftr_ipv6_addr.v6_addr32[1] =
	    (entry.
	     aftr_ipv6_addr120_89 << 25) | (entry.aftr_ipv6_addr88_57 >> 7);

	dslite_entry->aftr_ipv6_addr.v6_addr32[0] =
	    (entry.
	     aftr_ipv6_addr127_121 << 25) | (entry.aftr_ipv6_addr120_89 >> 7);
...
int32 rtl8198C_setAsicRoutingv6(uint32 index,
				rtl8198C_tblAsicDrv_routingv6Param_t * routingp)
{
...
#endif
	return _rtl8651_forceAddAsicEntry(TYPE_L3_V6_ROUTING_TABLE, index,
					  &entry);
...
int32 rtl8198C_getAsicRoutingv6(uint32 index,
				rtl8198C_tblAsicDrv_routingv6Param_t * routingp)
{
	rtl8198C_tblAsic_l3v6RouteTable_t entry;
...
int32 rtl8198C_Arpv6TableIndex(rtl8198C_tblAsicDrv_arpV6Param_t * arpp)
{
	...
	arpv6_hash_idx[0] =
	    id[0] ^ id[6] ^ id[12] ^ id[18] ^ id[24] ^ id[30] ^ id[36] ^ id[42]
	    ^ id[48] ^ id[54] ^ id[60] ^ id[66];
	arpv6_hash_idx[1] =
	    id[1] ^ id[7] ^ id[13] ^ id[19] ^ id[25] ^ id[31] ^ id[37] ^ id[43]
	    ^ id[49] ^ id[55] ^ id[61];
	arpv6_hash_idx[2] =
	    id[2] ^ id[8] ^ id[14] ^ id[20] ^ id[26] ^ id[32] ^ id[38] ^ id[44]
	    ^ id[50] ^ id[56] ^ id[62];
	arpv6_hash_idx[3] =
	    id[3] ^ id[9] ^ id[15] ^ id[21] ^ id[27] ^ id[33] ^ id[39] ^ id[45]
	    ^ id[51] ^ id[57] ^ id[63];
	arpv6_hash_idx[4] =
	    id[4] ^ id[10] ^ id[16] ^ id[22] ^ id[28] ^ id[34] ^ id[40] ^ id[46]
	    ^ id[52] ^ id[58] ^ id[64];
	arpv6_hash_idx[5] =
	    id[5] ^ id[11] ^ id[17] ^ id[23] ^ id[29] ^ id[35] ^ id[41] ^ id[47]
	    ^ id[53] ^ id[59] ^ id[65];
...
int32 rtl8198C_addAsicArpV6(uint32 index,
			    rtl8198C_tblAsicDrv_arpV6Param_t * arpp)
{
	uint32 found = 0;
	...
	for (colIdx = 0; colIdx < 4; colIdx++) {
		...
		if ((rtl8198C_getAsicArpV6(rowIdx << 2 | colIdx, &tbl_entry)) !=
		    SUCCESS) {
	...
	if (found == TRUE) {
		if (rtl8198C_setAsicArpV6(rowIdx << 2 | colIdx, arpp) ==
		    SUCCESS) ;
		return SUCCESS;
	}
...
int32 rtl8198C_setAsicArpV6(uint32 index,
			    rtl8198C_tblAsicDrv_arpV6Param_t * arpp)
{
	...
	entry.host_id_49_18 =
	    (arpp->hostid.v6_addr32[2] << 14) | (arpp->
						 hostid.v6_addr32[3] >> 18);
...
int32 rtl8198C_getAsicArpV6(uint32 index,
			    rtl8198C_tblAsicDrv_arpV6Param_t * arpp)
{
	rtl8198C_tblAsic_arpV6Table_t entry;
...
int32 rtl8198C_setAsicNextHopTableV6(uint32 index,
				     ...
				     nextHopp)
{
	...
	entry.dstVid = nextHopp->dvid;
	entry.type = nextHopp->isPppoe == TRUE ? 1 : 0;
	return _rtl8651_forceAddAsicEntry(TYPE_NEXT_HOP_V6_TABLE, index,
					  &entry);
}
...
int32 rtl8198C_getAsicNextHopTableV6(uint32 index,
				     rtl8198C_tblAsicDrv_nextHopV6Param_t *
				     nextHopp)
{
	rtl8198C_tblAsic_nextHopV6Table_t entry;
...
/*=========================================
  ...
  *=========================================*/
uint32 rtl8198C_ipMulticastv6TableIndex(uint32 hash_type, inv6_addr_t srcAddr,
					inv6_addr_t dstAddr)
{
	...
	sip_hash[0] =
	    sip[7] ^ sip[15] ^ sip[23] ^ sip[31] ^ sip[39] ^ sip[47] ^ sip[55] ^
	    sip[63] ^ sip[71] ^ sip[79] ^ sip[87] ^ sip[95] ^ sip[103] ^
	    sip[111] ^ sip[119] ^ sip[127];
	sip_hash[1] =
	    sip[6] ^ sip[14] ^ sip[22] ^ sip[30] ^ sip[38] ^ sip[46] ^ sip[54] ^
	    sip[62] ^ sip[70] ^ sip[78] ^ sip[86] ^ sip[94] ^ sip[102] ^
	    sip[110] ^ sip[118] ^ sip[126];
	sip_hash[2] =
	    sip[5] ^ sip[13] ^ sip[21] ^ sip[29] ^ sip[37] ^ sip[45] ^ sip[53] ^
	    sip[61] ^ sip[69] ^ sip[77] ^ sip[85] ^ sip[93] ^ sip[101] ^
	    sip[109] ^ sip[117] ^ sip[125];
	sip_hash[3] =
	    sip[4] ^ sip[12] ^ sip[20] ^ sip[28] ^ sip[36] ^ sip[44] ^ sip[52] ^
	    sip[60] ^ sip[68] ^ sip[76] ^ sip[84] ^ sip[92] ^ sip[100] ^
	    sip[108] ^ sip[116] ^ sip[124];
	sip_hash[4] =
	    sip[3] ^ sip[11] ^ sip[19] ^ sip[27] ^ sip[35] ^ sip[43] ^ sip[51] ^
	    sip[59] ^ sip[67] ^ sip[75] ^ sip[83] ^ sip[91] ^ sip[99] ^ sip[107]
	    ^ sip[115] ^ sip[123];
	sip_hash[5] =
	    sip[2] ^ sip[10] ^ sip[18] ^ sip[26] ^ sip[34] ^ sip[42] ^ sip[50] ^
	    sip[58] ^ sip[66] ^ sip[74] ^ sip[82] ^ sip[90] ^ sip[98] ^ sip[106]
	    ^ sip[114] ^ sip[122];
	sip_hash[6] =
	    sip[1] ^ sip[9] ^ sip[17] ^ sip[25] ^ sip[33] ^ sip[41] ^ sip[49] ^
	    sip[57] ^ sip[65] ^ sip[73] ^ sip[81] ^ sip[89] ^ sip[97] ^ sip[105]
	    ^ sip[113] ^ sip[121];
	sip_hash[7] =
	    sip[0] ^ sip[8] ^ sip[16] ^ sip[24] ^ sip[32] ^ sip[40] ^ sip[48] ^
	    sip[56] ^ sip[64] ^ sip[72] ^ sip[80] ^ sip[88] ^ sip[96] ^ sip[104]
	    ^ sip[112] ^ sip[120];

	dip_hash[7] =
	    dip[7] ^ dip[15] ^ dip[23] ^ dip[31] ^ dip[39] ^ dip[47] ^ dip[55] ^
	    dip[63] ^ dip[71] ^ dip[79] ^ dip[87] ^ dip[95] ^ dip[103] ^
	    dip[111] ^ dip[119] ^ dip[127];
	dip_hash[6] =
	    dip[6] ^ dip[14] ^ dip[22] ^ dip[30] ^ dip[38] ^ dip[46] ^ dip[54] ^
	    dip[62] ^ dip[70] ^ dip[78] ^ dip[86] ^ dip[94] ^ dip[102] ^
	    dip[110] ^ dip[118] ^ dip[126];
	dip_hash[5] =
	    dip[5] ^ dip[13] ^ dip[21] ^ dip[29] ^ dip[37] ^ dip[45] ^ dip[53] ^
	    dip[61] ^ dip[69] ^ dip[77] ^ dip[85] ^ dip[93] ^ dip[101] ^
	    dip[109] ^ dip[117] ^ dip[125];
	dip_hash[4] =
	    dip[4] ^ dip[12] ^ dip[20] ^ dip[28] ^ dip[36] ^ dip[44] ^ dip[52] ^
	    dip[60] ^ dip[68] ^ dip[76] ^ dip[84] ^ dip[92] ^ dip[100] ^
	    dip[108] ^ dip[116] ^ dip[124];
	dip_hash[3] =
	    dip[3] ^ dip[11] ^ dip[19] ^ dip[27] ^ dip[35] ^ dip[43] ^ dip[51] ^
	    dip[59] ^ dip[67] ^ dip[75] ^ dip[83] ^ dip[91] ^ dip[99] ^ dip[107]
	    ^ dip[115] ^ dip[123];
	dip_hash[2] =
	    dip[2] ^ dip[10] ^ dip[18] ^ dip[26] ^ dip[34] ^ dip[42] ^ dip[50] ^
	    dip[58] ^ dip[66] ^ dip[74] ^ dip[82] ^ dip[90] ^ dip[98] ^ dip[106]
	    ^ dip[114] ^ dip[122];
	dip_hash[1] =
	    dip[1] ^ dip[9] ^ dip[17] ^ dip[25] ^ dip[33] ^ dip[41] ^ dip[49] ^
	    dip[57] ^ dip[65] ^ dip[73] ^ dip[81] ^ dip[89] ^ dip[97] ^ dip[105]
	    ^ dip[113] ^ dip[121];
	dip_hash[0] =
	    dip[0] ^ dip[8] ^ dip[16] ^ dip[24] ^ dip[32] ^ dip[40] ^ dip[48] ^
	    dip[56] ^ dip[64] ^ dip[72] ^ dip[80] ^ dip[88] ^ dip[96] ^ dip[104]
	    ^ dip[112] ^ dip[120];
...
int32 rtl8198C_setAsicIpMulticastv6Table(uint32 hash_type,
					 ...
					 * mCast_t)
{
	...
	if (mCast_t->dip.v6_addr32[0] >> 24 != 0xFF
	    || mCast_t->port >= RTL8651_PORT_NUMBER + rtl8651_totalExtPortNum) {
		return FAILED;	//Non-IP multicast destination address
	}
	...
	idx =
	    rtl8198C_ipMulticastv6TableIndex(hash_type, mCast_t->sip,
					     mCast_t->dip);
...
int32 rtl8198C_delAsicIpMulticastv6Table(uint32 index)
{
	rtl8198C_tblAsic_ipMulticastv6Table_t entry;
	memset(&entry, 0, sizeof(entry));
	return _rtl8651_forceAddAsicEntry(TYPE_MULTICAST_V6_TABLE, index,
					  &entry);
}

int32 rtl8198C_getAsicIpMulticastv6Table(uint32 index,
					 rtl8198C_tblAsicDrv_multiCastv6Param_t
					 * mCast_t)
{
	rtl8198C_tblAsic_ipMulticastv6Table_t entry;
...
int32 rtl865x_initAsicL3(void)
{
	...
	rtl8651_clearSpecifiedAsicTable(TYPE_MULTICAST_TABLE,
					RTL8651_IPMULTICASTTBL_SIZE);
#endif
	rtl8651_clearSpecifiedAsicTable(TYPE_EXT_INT_IP_TABLE,
					RTL8651_IPTABLE_SIZE);
	rtl8651_clearSpecifiedAsicTable(TYPE_PPPOE_TABLE,
					RTL8651_PPPOETBL_SIZE);
	rtl8651_clearSpecifiedAsicTable(TYPE_NEXT_HOP_TABLE,
					RTL8651_NEXTHOPTBL_SIZE);
	rtl8651_clearSpecifiedAsicTable(TYPE_L3_ROUTING_TABLE,
					RTL8651_ROUTINGTBL_SIZE);

#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
	rtl8651_clearSpecifiedAsicTable(TYPE_DS_LITE_TABLE,
					RTL8198C_6RDTBL_SIZE);
	rtl8651_clearSpecifiedAsicTable(TYPE_6RD_TABLE,
					RTL8198C_DSLITETBL_SIZE);
	rtl8651_clearSpecifiedAsicTable(TYPE_L3_V6_ROUTING_TABLE,
					RTL8198C_ROUTINGV6TBL_SIZE);
	rtl8651_clearSpecifiedAsicTable(TYPE_NEXT_HOP_V6_TABLE,
					RTL8198C_MULTICASTV6TBL_SIZE);
	rtl8651_clearSpecifiedAsicTable(TYPE_ARP_V6_TABLE,
					RTL8198C_ARPV6TBL_SIZE);
	rtl8651_clearSpecifiedAsicTable(TYPE_MULTICAST_V6_TABLE,
					RTL8198C_NEXTHOPV6TBL_SIZE);
#endif
	...
	WRITE_MEM32(V6DSLITECTR1,
		    (CF_HOP_LIMIT << CF_DSL_HOPLIMIT_OFFSET &
		     CF_DSL_HOPLIMIT_MASK));
#endif
#ifdef CONFIG_RTL_HW_6RD_SUPPORT
	REG32(V6RDCTR0) = 0;
	WRITE_MEM32(V6RDCTR0,
		    (CF_6RD_EN_MASK | CF_6RDTOS_MASK | CF_UKN_6RDDIP_CPU_MASK));
	REG32(V6RDCTR1) = 0;
	WRITE_MEM32(V6RDCTR1,
		    (CF_DSLITE_EN_MASK | CF_DSLTC_EN_MASK |
		     CF_UKN_DSLDIP_CPU_MASK | CF_DSLITE_NAT_MASK));
#endif
#endif
	//Enable TTL-1
