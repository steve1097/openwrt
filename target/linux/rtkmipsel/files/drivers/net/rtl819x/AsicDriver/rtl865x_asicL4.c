 ...
 */
uint32 rtl8651_naptTcpUdpTableIndex(int8 isTCP, ipaddr_t srcAddr,
				    uint16 srcPort, ipaddr_t destAddr,
				    uint16 destPort)
{
	uint32 eidx;
	...
	if (destAddr == 0 && destPort == 0 && ((isTCP & HASH_FOR_VERI) == 0)) {
		 ...
		 */
		eidx =
		    (srcAddr & 0x3ff) ^ ((srcAddr >> 10) & 0x3ff) ^
		    ((srcAddr >> 20) & 0x3ff) ^ (srcPort & 0x3ff) ^
		    (((isTCP & 1) << 8) | ((srcAddr & 0xc0000000) >> 24) |
		     ((srcPort >> 10) & 0x3f));
	 ...
	 */
	eidx =
	    (srcPort & 0x3ff) ^ (((srcPort & 0xfc00) >> 10) |
				 ((srcAddr & 0xf) << 6)) ^ ((srcAddr >> 4) &
							    0x3ff) ^ ((srcAddr
								       >> 14) &
								      0x3ff) ^
	    (((srcAddr & 0xff000000) >> 24) | ((isTCP & 1) << 8) |
	     ((destPort & 1) << 9)) ^ ((destPort >> 1) & 0x3ff) ^
	    (((destPort >> 11) & 0x1f) | ((destAddr & 0x1f) << 5)) ^
	    ((destAddr >> 5) & 0x3ff) ^ ((destAddr >> 15) & 0x3ff) ^
	    ((destAddr >> 25) & 0x7f);
	assert(eidx < RTL8651_TCPUDPTBL_SIZE);
	return eidx;
}
...
int32 rtl8651_setAsicLiberalNaptTcpUdpTable(int8 forced, uint16 index,
					    ...
					    uint32 ageSec, int8 entryType,
					    int8 isTcp, int8 isCollision,
					    int8 isCollision2, int8 isValid)
{
	...
	if (isValid) {
		 ...
		 */
		tcpQuiet =
		    (ISSET(entryType, RTL8651_NAPT_SYNFIN_QUIET)) ? 1 : 0;
		directional =
		    (ISSET(entryType, RTL8651_NAPT_UNIDIRECTIONAL_FLOW)) ? 1 :
		    0;
		outbound = (directional
			    && ISSET(entryType,
				     RTL8651_NAPT_OUTBOUND_FLOW)) ? 1 : 0;
		...
		entry.TCPFlag =
		    (tcpQuiet << 2) | (directional << 1) | (outbound);
		entry.isTCP = isTcp == TRUE ? 1 : 0;
	} else {
		entry.isStatic = entry.dedicate = entry.intIPAddr =
		    entry.intPort = 0;
		entry.agingTime = entry.selEIdx = entry.selIPIdx =
		    entry.TCPFlag = 0;
	}

	if (forced == TRUE)
		return _rtl8651_forceAddAsicEntry(TYPE_L4_TCP_UDP_TABLE, index,
						  &entry);
...
int32 rtl8651_setAsicNaptTcpUdpTable(int8 forced, uint32 index,
				     ...
				     naptTcpUdpp)
{
	...
	if (forced == TRUE) {
		_rtl8651_forceAddAsicEntry(TYPE_L4_TCP_UDP_TABLE, index,
					   &entry);
	} else if (_rtl8651_addAsicEntry(TYPE_L4_TCP_UDP_TABLE, index, &entry))
		return FAILED;
	return SUCCESS;
...
{
	rtl865xc_tblAsic_naptTcpUdpTable_t *entry =
	    (rtl865xc_tblAsic_naptTcpUdpTable_t *) vEntry;
...
int32 rtl8651_getAsicNaptTcpUdpTable(uint32 index,
				     rtl865x_tblAsicDrv_naptTcpUdpParam_t *
				     naptTcpUdpp)
{
	rtl865xc_tblAsic_naptTcpUdpTable_t entry;
...
{
	if ((start >= RTL8651_TCPUDPTBL_SIZE)
	    || (end >= RTL8651_TCPUDPTBL_SIZE))
		return FAILED;
...
int32 rtl8651_setAsicNaptIcmpTimeout(uint32 timeout)
{
	...
	WRITE_MEM32(TEATCR,
		    (READ_MEM32(TEATCR) & ~timeMask) | (timeUnit << 24));
	return SUCCESS;
}
...
int32 rtl8651_setAsicNaptUdpTimeout(uint32 timeout)
{
	...
	timeMask = 0x3f << 18;
	WRITE_MEM32(TEATCR,
		    (READ_MEM32(TEATCR) & ~timeMask) | (timeUnit << 18));
...
int32 rtl8651_setAsicNaptTcpLongTimeout(uint32 timeout)
{
	...
	timeMask = 0x3f << 12;
	WRITE_MEM32(TEATCR,
		    (READ_MEM32(TEATCR) & ~timeMask) | (timeUnit << 12));
...
{
	/*clear asic table */
	rtl8651_clearSpecifiedAsicTable(TYPE_L4_TCP_UDP_TABLE,
					RTL8651_TCPUDPTBL_SIZE);
