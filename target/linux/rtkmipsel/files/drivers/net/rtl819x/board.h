...
typedef enum { BAND_11B = 1, BAND_11G = 2, BAND_11BG = 3, BAND_11A = 4
} BAND_TYPE_T;
typedef enum { AP_MODE = 0, CLIENT_MODE = 1, WDS_MODE = 2, AP_WDS_MODE = 3
} WLAN_MODE_T;
typedef enum _wlan_mac_state {
	STATE_DISABLED =
	    0, STATE_IDLE, STATE_SCANNING, STATE_STARTED, STATE_CONNECTED
} wlan_mac_state;
...
typedef struct romeCfgParam_s {
	...
	serverpCfgParam_t serverpCfgParam[MAX_PPPOE_SESSION][MAX_SERVER_PORT];
	 specialapCfgParam_t
	    specialapCfgParam[MAX_PPPOE_SESSION][MAX_SPECIAL_AP];
	...
	tbldrvCfgParam_t tbldrvCfgParam;
	 urlfilterCfgParam_t
	    urlfilterCfgParam[MAX_PPPOE_SESSION][MAX_URL_FILTER];
...
//#endif
	...
	pptpServerCfgParam_t pptpServerCfgParam;
	 urlfilterTrustedUserCfgParam_t
	    urlfilterTrustedUserCfgParam[URL_FILTER_MAX_TRUSTED_USERS];
	...
	cwmpDHCPServ_t cwmpDHCPServerConfCfgParam;
	 cwmpNotifyConfig_t
	    cwmpNotifyConfigCfgParam[CWMP_MAX_NOTIFY_CONFIG_INFO];
