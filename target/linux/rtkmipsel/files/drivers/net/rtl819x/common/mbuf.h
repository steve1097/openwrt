...
extern uint32 mBuf_driverGet(uint32 Nmbuf, struct rtl_mBuf **ppFirstMbuf,
			     struct rtl_mBuf **ppLastMbuf);
/*
...
extern struct rtl_mBuf *mBuf_getm(struct rtl_mBuf *m, uint32 len, int32 how,
				  int32 unused);
...
extern uint32 mBuf_driverGetPkthdr(uint32 Npkthdr,
				   struct rtl_pktHdr **ppHeadPkthdr,
				   struct rtl_pktHdr **ppTailPkthdr);
/*
...
extern int32 mBuf_attachCluster(struct rtl_mBuf *m, void *buffer, uint32 id,
				uint32 size, uint16 datalen, uint16 align);
/*
...
extern int32 mBuf_freeOneMbufPkthdr(struct rtl_mBuf *m, void **buffer,
				    uint32 * id, uint16 * size);
/*
...
uint32 mBuf_driverFreePkthdr(struct rtl_pktHdr *ph, uint32 Npkthdr,
			     struct rtl_pktHdr **ppHeadPkthdr);
/*
...
extern int32 mBuf_copyToMbuf(struct rtl_mBuf *, uint32 offset, uint32 len,
			     int8 * cp);
...
extern int32 mBuf_copyToUserBuffer(struct rtl_mBuf *m, uint32 off, uint32 len,
				   int8 * cp);
...
extern struct rtl_mBuf *mBuf_cloneMbufChain(struct rtl_mBuf *pThisMbuf,
					    int32 iOffset, int32 iLength,
					    int32 iWait);
...
extern struct rtl_mBuf *mBuf_dupMbufChain(struct rtl_mBuf *pMbufChain,
					  int32 iOffset, int32 iLength,
					  int32 flag);
...
extern struct rtl_mBuf *mBuf_prepend(struct rtl_mBuf *m, uint32 plen,
				     int32 how);
...
extern struct rtl_mBuf *mBuf_padding(struct rtl_mBuf *m, uint32 plen,
				     int32 how);
/*
...
struct rtl_mBuf *mBuf_split(register struct rtl_mBuf *m0, uint32 len0,
			    int32 wait);
...
void mBuf_getBMjmpTable(uint8 * pat, uint16 * jump_tbl, uint16 patLen,
			uint8 caseSensitive);
int32 mBuf_BMpatternMatch(struct rtl_mBuf *m, uint32 len, uint8 * delimiter,
			  uint32 delimitLen, uint16 * jmp_tbl,
			  uint8 caseSensitive);
...
struct rtl_mBuf *mBuf_attachHeader(void *buffer, uint32 id, uint32 bufsize,
				   uint32 datalen, uint16 align);
struct rtl_mBuf *mBuf_attachHeaderJumbo(void *buffer, uint32 id, uint32 bufsize,
					uint32 datalen);
