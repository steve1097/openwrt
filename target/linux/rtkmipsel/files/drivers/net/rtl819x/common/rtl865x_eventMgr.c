...
 int32 rtl865x_initEventMgr(rtl865x_eventMgr_param_t * param) 
{
		...
		CTAILQ_INSERT_HEAD(&eventsTables.freeList.eventHead,
				    &eventPool[i], next);
		}
	 for (i = 0; i < eventMgrParam.eventListCnt; i++)
		 {
		CTAILQ_INSERT_HEAD(&eventsTables.freeList.eventLayerListHead,
...
  static int32 rtl865x_flushEventLayerList(rtl865x_eventLayerList_t *
					      eventLayerList) 
{
		...
		CTAILQ_INSERT_HEAD(&eventsTables.freeList.eventHead, event,
				    next);
...
 int32 rtl865x_reInitEventMgr(void)
{
		...
		 CTAILQ_REMOVE(&eventsTables.inuseList.eventLayerListHead,
				 eventLayerList, next);
...
 static rtl865x_event_t *rtl865x_searchEvent(rtl865x_eventLayerList_t *
					       eventList,
...
 static void rtl865x_insertEventLayerList(rtl865x_eventLayerList_t *
					    eventLayerList) 
{
	...
	 CTAILQ_FOREACH(tmpEventLayerList,
			  &eventsTables.inuseList.eventLayerListHead, next)  {
		if (tmpEventLayerList->eventLayerId >=
		     eventLayerList->eventLayerId)
			 {
			CTAILQ_INSERT_BEFORE(&eventsTables.inuseList.
					      eventLayerListHead,
		...
		CTAILQ_INSERT_TAIL(&eventsTables.inuseList.eventLayerListHead,
				    eventLayerList, next);
		}
	return;
 }

 static void rtl8651_insertEvent(rtl865x_eventLayerList_t * eventLayerList,
				   rtl865x_event_t * event) 
{
			...
			CTAILQ_INSERT_BEFORE(&eventLayerList->eventHead,
					      tmpEvent, event, next);
...
 int32 rtl865x_registerEvent(rtl865x_event_Param_t * eventParam) 
{
		...
		CTAILQ_REMOVE(&eventsTables.freeList.eventLayerListHead,
			       eventLayerList, next);
...
 int32 rtl865x_unRegisterEvent(rtl865x_event_Param_t * eventParam) 
{
	       ...
	       (eventParam->event_action_fn == NULL))
			...
			CTAILQ_REMOVE(&eventsTables.inuseList.
				       eventLayerListHead, eventLayerList,
				       next);
			eventLayerList->eventLayerId = 0;
			CTAILQ_INIT(&eventLayerList->eventHead);
			CTAILQ_INSERT_HEAD(&eventsTables.freeList.
					    eventLayerListHead, eventLayerList,
					    next);
