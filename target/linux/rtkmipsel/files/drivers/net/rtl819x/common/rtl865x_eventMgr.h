...
int32 rtl865x_registEvent(int32 event, int32 priority,
			  void (*event_action_fn) (void *param));
int32 rtl865x_unRegistEvent(int32 event, int32 priority,
			    void (*event_action_fn) (void *param));
...
typedef CTAILQ_HEAD(_eventLayerList_head_s,
		    rtl865x_eventLayerList_s) rtl865x_eventLayerList_Head_t;
...
#else
int32 rtl865x_event_proc_read(char *page, char **start, off_t off, int count,
			      int *eof, void *data);
int32 rtl865x_event_proc_write(struct file *filp, const char *buff,
			       unsigned long len, void *data);
#endif
