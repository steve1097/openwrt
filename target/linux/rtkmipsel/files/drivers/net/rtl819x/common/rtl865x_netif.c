...
static int (*rtl_get_drv_netifName_by_psName) (const char *psName,
					       char *netifName);
...
static int32 _rtl865x_regist_aclChain(char *netifName, int32 priority,
				      uint32 flag);
...
int32 rtl865x_dump_AclRule(rtl865x_AclRule_t * rule)
{
	...
	int8 *actionT[] =
	    { "permit", "redirect to ether", "drop", "to cpu", "legacy drop",
		"drop for log", "mirror", "redirect to pppoe",
		"default redirect", "mirror keep match",
		"drop rate exceed pps", "log rate exceed pps",
		"drop rate exceed bps", "log rate exceed bps", "priority "
	};

	switch (rule->ruleType_) {
	case RTL865X_ACL_MAC:
		printk(" [%d] rule type: %s   rule action: %s\n", rule->aclIdx,
		       "Ethernet", actionT[rule->actionType_]);
		printk("\tether type: %x   ether type mask: %x\n",
		       rule->typeLen_, rule->typeLenMask_);
		printk("\tDMAC: %x:%x:%x:%x:%x:%x  DMACM: %x:%x:%x:%x:%x:%x\n",
		       rule->dstMac_.octet[0], rule->dstMac_.octet[1],
		       rule->dstMac_.octet[2], rule->dstMac_.octet[3],
		       rule->dstMac_.octet[4], rule->dstMac_.octet[5],
		       rule->dstMacMask_.octet[0], rule->dstMacMask_.octet[1],
		       rule->dstMacMask_.octet[2], rule->dstMacMask_.octet[3],
		       rule->dstMacMask_.octet[4], rule->dstMacMask_.octet[5]
		    );

		printk("\tSMAC: %x:%x:%x:%x:%x:%x  SMACM: %x:%x:%x:%x:%x:%x\n",
		       rule->srcMac_.octet[0], rule->srcMac_.octet[1],
		       rule->srcMac_.octet[2], rule->srcMac_.octet[3],
		       rule->srcMac_.octet[4], rule->srcMac_.octet[5],
		       rule->srcMacMask_.octet[0], rule->srcMacMask_.octet[1],
		       rule->srcMacMask_.octet[2], rule->srcMacMask_.octet[3],
		       rule->srcMacMask_.octet[4], rule->srcMacMask_.octet[5]
		    );
		break;

	case RTL865X_ACL_IP:
		printk(" [%d] rule type: %s   rule action: %s\n", rule->aclIdx,
		       "IP", actionT[rule->actionType_]);
		printk("\tdip: %d.%d.%d.%d dipM: %d.%d.%d.%d\n",
		       (rule->dstIpAddr_ >> 24),
		       ((rule->dstIpAddr_ & 0x00ff0000) >> 16),
		       ((rule->dstIpAddr_ & 0x0000ff00) >> 8),
		       (rule->dstIpAddr_ & 0xff), (rule->dstIpAddrMask_ >> 24),
		       ((rule->dstIpAddrMask_ & 0x00ff0000) >> 16),
		       ((rule->dstIpAddrMask_ & 0x0000ff00) >> 8),
		       (rule->dstIpAddrMask_ & 0xff)
		    );
		printk("\tsip: %d.%d.%d.%d sipM: %d.%d.%d.%d\n",
		       (rule->srcIpAddr_ >> 24),
		       ((rule->srcIpAddr_ & 0x00ff0000) >> 16),
		       ((rule->srcIpAddr_ & 0x0000ff00) >> 8),
		       (rule->srcIpAddr_ & 0xff), (rule->srcIpAddrMask_ >> 24),
		       ((rule->srcIpAddrMask_ & 0x00ff0000) >> 16),
		       ((rule->srcIpAddrMask_ & 0x0000ff00) >> 8),
		       (rule->srcIpAddrMask_ & 0xff)
		    );
		printk
		    ("\tTos: %x   TosM: %x   ipProto: %x   ipProtoM: %x   ipFlag: %x   ipFlagM: %x\n",
		     rule->tos_, rule->tosMask_, rule->ipProto_,
		     rule->ipProtoMask_, rule->ipFlag_, rule->ipFlagMask_);

		printk
		    ("\t<FOP:%x> <FOM:%x> <http:%x> <httpM:%x> <IdentSdip:%x> <IdentSdipM:%x> \n",
		     rule->ipFOP_, rule->ipFOM_, rule->ipHttpFilter_,
		     rule->ipHttpFilterM_, rule->ipIdentSrcDstIp_,
	...
	case RTL865X_ACL_IP_RANGE:
		printk(" [%d] rule type: %s   rule action: %s\n", rule->aclIdx,
		       "IP Range", actionT[rule->actionType_]);
	...
	case RTL865X_ACL_ICMP:
		...
		printk
		    ("\tTos: %x   TosM: %x   type: %x   typeM: %x   code: %x   codeM: %x\n",
		     rule->tos_, rule->tosMask_, rule->icmpType_,
		     rule->icmpTypeMask_, rule->icmpCode_, rule->icmpCodeMask_);
		break;
	case RTL865X_ACL_ICMP_IPRANGE:
		printk(" [%d] rule type: %s   rule action: %s\n", rule->aclIdx,
		       "ICMP IP RANGE", actionT[rule->actionType_]);
		printk("\tdipU: %d.%d.%d.%d dipL: %d.%d.%d.%d\n",
		       (rule->dstIpAddr_ >> 24),
		       ((rule->dstIpAddr_ & 0x00ff0000) >> 16),
		       ((rule->dstIpAddr_ & 0x0000ff00) >> 8),
	...
	case RTL865X_ACL_IGMP:
		...
		printk("\tTos: %x   TosM: %x   type: %x   typeM: %x\n",
		       rule->tos_, rule->tosMask_, rule->igmpType_,
		       rule->igmpTypeMask_);
		break;

	case RTL865X_ACL_IGMP_IPRANGE:
		printk(" [%d] rule type: %s   rule action: %s\n", rule->aclIdx,
		       "IGMP IP RANGE", actionT[rule->actionType_]);
		printk("\tdip: %d.%d.%d.%d dipM: %d.%d.%d.%d\n",
		       (rule->dstIpAddr_ >> 24),
		       ((rule->dstIpAddr_ & 0x00ff0000) >> 16),
		       ((rule->dstIpAddr_ & 0x0000ff00) >> 8),
	...
	case RTL865X_ACL_TCP:
		...
		printk
		    ("\tTos:%x  TosM:%x  sportL:%d  sportU:%d  dportL:%d  dportU:%d\n",
		     rule->tos_, rule->tosMask_, rule->tcpSrcPortLB_,
		     rule->tcpSrcPortUB_, rule->tcpDstPortLB_,
		     rule->tcpDstPortUB_);
		printk
		    ("\tflag: %x  flagM: %x  <URG:%x> <ACK:%x> <PSH:%x> <RST:%x> <SYN:%x> <FIN:%x>\n",
		     rule->tcpFlag_, rule->tcpFlagMask_, rule->tcpURG_,
		     rule->tcpACK_, rule->tcpPSH_, rule->tcpRST_, rule->tcpSYN_,
		     rule->tcpFIN_);
		break;
	case RTL865X_ACL_TCP_IPRANGE:
		printk(" [%d] rule type: %s   rule action: %s\n", rule->aclIdx,
		       "TCP IP RANGE", actionT[rule->actionType_]);
		printk("\tdipU: %d.%d.%d.%d dipL: %d.%d.%d.%d\n",
		       (rule->dstIpAddr_ >> 24),
		       ((rule->dstIpAddr_ & 0x00ff0000) >> 16),
		       ((rule->dstIpAddr_ & 0x0000ff00) >> 8),
	...
	case RTL865X_ACL_UDP:
		...
		printk
		    ("\tTos:%x  TosM:%x  sportL:%d  sportU:%d  dportL:%d  dportU:%d\n",
		     rule->tos_, rule->tosMask_, rule->udpSrcPortLB_,
		     rule->udpSrcPortUB_, rule->udpDstPortLB_,
		     rule->udpDstPortUB_);
		break;
	case RTL865X_ACL_UDP_IPRANGE:
		printk(" [%d] rule type: %s   rule action: %s\n", rule->aclIdx,
		       "UDP IP RANGE", actionT[rule->actionType_]);
		printk("\tdipU: %d.%d.%d.%d dipL: %d.%d.%d.%d\n",
		       (rule->dstIpAddr_ >> 24),
		       ((rule->dstIpAddr_ & 0x00ff0000) >> 16),
		       ((rule->dstIpAddr_ & 0x0000ff00) >> 8),
	...
	case RTL865X_ACL_SRCFILTER:
		printk(" [%d] rule type: %s   rule action: %s\n", rule->aclIdx,
		       "Source Filter", actionT[rule->actionType_]);
		printk("\tSMAC: %x:%x:%x:%x:%x:%x  SMACM: %x:%x:%x:%x:%x:%x\n",
		       rule->srcFilterMac_.octet[0],
		       rule->srcFilterMac_.octet[1],
		       rule->srcFilterMac_.octet[2],
		       rule->srcFilterMac_.octet[3],
		       rule->srcFilterMac_.octet[4],
		       rule->srcFilterMac_.octet[5],
		       rule->srcFilterMacMask_.octet[0],
		       rule->srcFilterMacMask_.octet[1],
		       rule->srcFilterMacMask_.octet[2],
		       rule->srcFilterMacMask_.octet[3],
		       rule->srcFilterMacMask_.octet[4],
		       rule->srcFilterMacMask_.octet[5]
		    );
		printk
		    ("\tsvidx: %d   svidxM: %x   sport: %d   sportM: %x   ProtoType: %x\n",
		     rule->srcFilterVlanIdx_, rule->srcFilterVlanIdxMask_,
		     rule->srcFilterPort_, rule->srcFilterPortMask_,
		     (rule->srcFilterIgnoreL3L4_ ==
		      TRUE ? 2 : (rule->srcFilterIgnoreL4_ == 1 ? 1 : 0))
		    );
		printk("\tsip: %d.%d.%d.%d   sipM: %d.%d.%d.%d\n",
		       (rule->srcFilterIpAddr_ >> 24),
		       ((rule->srcFilterIpAddr_ & 0x00ff0000) >> 16),
		       ((rule->srcFilterIpAddr_ & 0x0000ff00) >> 8),
		       (rule->srcFilterIpAddr_ & 0xff),
		       (rule->srcFilterIpAddrMask_ >> 24),
		       ((rule->srcFilterIpAddrMask_ & 0x00ff0000) >> 16),
		       ((rule->srcFilterIpAddrMask_ & 0x0000ff00) >> 8),
		       (rule->srcFilterIpAddrMask_ & 0xff)
		    );
		printk("\tsportL: %d   sportU: %d\n",
		       rule->srcFilterPortLowerBound_,
		       rule->srcFilterPortUpperBound_);
		break;

	case RTL865X_ACL_SRCFILTER_IPRANGE:
		printk(" [%d] rule type: %s   rule action: %s\n", rule->aclIdx,
		       "Source Filter(IP RANGE)", actionT[rule->actionType_]);
		printk("\tSMAC: %x:%x:%x:%x:%x:%x  SMACM: %x:%x:%x:%x:%x:%x\n",
		       rule->srcFilterMac_.octet[0],
		       rule->srcFilterMac_.octet[1],
		       rule->srcFilterMac_.octet[2],
	...
	case RTL865X_ACL_DSTFILTER:
		printk(" [%d] rule type: %s   rule action: %s\n", rule->aclIdx,
		       "Deatination Filter", actionT[rule->actionType_]);
		printk("\tDMAC: %x:%x:%x:%x:%x:%x  DMACM: %x:%x:%x:%x:%x:%x\n",
		       rule->dstFilterMac_.octet[0],
		       rule->dstFilterMac_.octet[1],
		       rule->dstFilterMac_.octet[2],
		       rule->dstFilterMac_.octet[3],
		       rule->dstFilterMac_.octet[4],
		       rule->dstFilterMac_.octet[5],
		       rule->dstFilterMacMask_.octet[0],
		       rule->dstFilterMacMask_.octet[1],
		       rule->dstFilterMacMask_.octet[2],
		       rule->dstFilterMacMask_.octet[3],
		       rule->dstFilterMacMask_.octet[4],
		       rule->dstFilterMacMask_.octet[5]
		    );
		printk
		    ("\tdvidx: %d   dvidxM: %x  ProtoType: %x   dportL: %d   dportU: %d\n",
		     rule->dstFilterVlanIdx_, rule->dstFilterVlanIdxMask_,
		     (rule->dstFilterIgnoreL3L4_ ==
		      TRUE ? 2 : (rule->dstFilterIgnoreL4_ == 1 ? 1 : 0)),
		     rule->dstFilterPortLowerBound_,
		     rule->dstFilterPortUpperBound_);
		printk("\tdip: %d.%d.%d.%d   dipM: %d.%d.%d.%d\n",
		       (rule->dstFilterIpAddr_ >> 24),
		       ((rule->dstFilterIpAddr_ & 0x00ff0000) >> 16),
		       ((rule->dstFilterIpAddr_ & 0x0000ff00) >> 8),
		       (rule->dstFilterIpAddr_ & 0xff),
		       (rule->dstFilterIpAddrMask_ >> 24),
		       ((rule->dstFilterIpAddrMask_ & 0x00ff0000) >> 16),
		       ((rule->dstFilterIpAddrMask_ & 0x0000ff00) >> 8),
	...
	case RTL865X_ACL_MIRROR_KEEP_MATCH:
		printk("\tnetifIdx: %d   pppoeIdx: %d   l2Idx:%d  ",
		       rule->netifIdx_, rule->pppoeIdx_, rule->L2Idx_);
		break;
...
int32 rtl865x_show_allAclChains(struct seq_file * s)
{
...
#endif
	{
		if (netifTbl[i].valid == 1) {
			netif = &netifTbl[i];
			for (j = RTL865X_ACL_INGRESS; j <= RTL865X_ACL_EGRESS;
			     j++) {
					...
					while (rule) {
						...
						case RTL865X_ACL_SRCFILTER:
							...
							seq_printf(s,
								   ...
								   srcFilterVlanIdx_,
								   rule->
								   srcFilterVlanIdxMask_,
								   rule->
								   srcFilterPort_,
								   rule->
								   srcFilterPortMask_,
								   (rule->
								    srcFilterIgnoreL3L4_
								    ==
								    TRUE ? 2
								    : (rule->
								       srcFilterIgnoreL4_
							...
							seq_printf(s,
								   "\tsportL: %d   sportU: %d\n",
								   rule->
								   srcFilterPortLowerBound_,
								   rule->
								   srcFilterPortUpperBound_);
							break;
						...
						case RTL865X_ACL_DSTFILTER:
							...
							seq_printf(s,
								   ...
								   dstFilterVlanIdx_,
								   rule->
								   dstFilterVlanIdxMask_,
								   (rule->
								    dstFilterIgnoreL3L4_
								    ==
								    TRUE ? 2
								    : (rule->
								       dstFilterIgnoreL4_
								       ...
								       0)),
								   rule->
								   dstFilterPortLowerBound_,
								   rule->
								   dstFilterPortUpperBound_);
							seq_printf(s,
								   "\tdip: %d.%d.%d.%d   dipM: %d.%d.%d.%d\n",
								   (rule->
								    dstFilterIpAddr_
...
static int32 _rtl865x_print_allChain_allAcl(rtl865x_netif_local_t * netif)
{
	...
	for (i = RTL865X_ACL_INGRESS; i <= RTL865X_ACL_EGRESS; i++) {
		...
		while (chain) {
			...
			while (rule) {
				printk
				    ("\tIdx%d:  aclIdx(%d),ruleType(%d),action(%d),direction(%d),pktOpApp(%d)\n",
				     rule->aclIdx, rule->aclIdx,
				     rule->ruleType_, rule->actionType_,
				     rule->direction_, rule->pktOpApp_);
				rule = rule->next;
			}
			printk("===========================\n");
			chain = chain->nextChain;
...
static int32 _rtl865x_setAsicNetif(rtl865x_netif_local_t * entry)
{
...
#ifdef CONFIG_RTL_8367R_SUPPORT
	{
		 ...
		 */
		extern void set_8367r_L2(uint8 * mac, int intf_wan,
					 int is_static);
...
rtl865x_netif_local_t *_rtl865x_getSWNetifByName(char *name)
{
...
#endif
	{
		...
		if (netifTbl[i].valid == 1
		    && strlen(name) == strlen(netifTbl[i].name)
...
#if defined (CONFIG_RTL_LOCAL_PUBLIC)
	if (virtualNetIf.valid == 1 && strlen(name) == strlen(virtualNetIf.name)
	    && memcmp(virtualNetIf.name, name, strlen(name)) == 0) {
		netif = &virtualNetIf;
	}
#endif
...
int32 _rtl865x_addNetif(rtl865x_netif_t * netif)
{
	...
	for (asicIdx = 0; asicIdx < RTL865XC_NETIFTBL_SIZE; asicIdx++) {
		_rtl8651_readAsicEntry(TYPE_NETINTERFACE_TABLE, asicIdx,
				       &asicEntry);
	...
	entry->enableRouteV6 =
	    (rtl8651_getAsicOperationLayer() > 2) ? (netif->enableRouteV6) : 0;
#endif
#else
...
//      printk("netif:%s,%d,[%s]:[%d].\n",netif->name,netif->is_slave,__FUNCTION__,__LINE__);
	...
	entry->enableRoute =
	    (rtl8651_getAsicOperationLayer() > 2) ? (netif->enableRoute) : 0;
#else
	...
	if (entry->is_slave == 0) {
#if defined(CONFIG_RTL_ISP_MULTI_WAN_SUPPORT)
		{
			...
			for (hw_netif_idx = 0; hw_netif_idx < NETIF_NUMBER;
			     hw_netif_idx++) {
			...
			if (hw_netif_idx == NETIF_NUMBER) {
				printk
				    ("%s[%d] Warning , there is no more empty entry in Asic Netif Table\n ",
				     __FUNCTION__, __LINE__);
				return RTL_ENOFREEBUFFER;
			}
		...
		retval =
		    rtl865x_regist_aclChain(netif->name,
...
#endif
		retval =
		    rtl865x_regist_aclChain(netif->name, RTL865X_ACL_USER_USED,
					    RTL865X_ACL_INGRESS);
		retval =
		    rtl865x_regist_aclChain(netif->name, RTL865X_ACL_USER_USED,
					    RTL865X_ACL_EGRESS);
		...
		retval =
		    rtl865x_regist_aclChain(netif->name, RTL865X_ACL_QOS_USED0,
					    RTL865X_ACL_INGRESS);
		retval =
		    rtl865x_regist_aclChain(netif->name, RTL865X_ACL_QOS_USED1,
					    RTL865X_ACL_INGRESS);
#endif
#endif //CONFIG_RTL_LAYERED_DRIVER_ACL
	}
...
static int32 _rtl865x_delNetif(char *name)
{
	...
	if (entry->is_slave == 0) {
		...
		/*now delete all slave interface whose master is the deleting master interface */
		{
			...
			{
				if (netifTbl[i].valid == 1
				    && netifTbl[i].is_slave == 1
				    && netifTbl[i].master == entry)
		...
		if (netifTbl[i].valid == 1
			...
			if (netifTbl[i].is_slave == TRUE) {
				if (netifTbl[i].master)
					return
					    _rtl865x_getNetifIdxByNameExt
					    (netifTbl[i].master->name);
				else
					return -1;
			} else
				return i;
...
int32 _rtl865x_getAclFromAsic(int32 index, rtl865x_AclRule_t * rule)
{
	...
	if (index >= RTL865X_ACL_MAX_NUMBER + RTL865X_ACL_RESERVED_NUMBER
	    || rule == NULL)
		return FAILED;
	...
	case RTL865X_ACL_MAC:	/* Ethernet rule type     */
		...
		rule->dstMacMask_.octet[1] =
		    entry.is.ETHERNET.dMacM47_32 & 0xff;
		rule->dstMacMask_.octet[2] = entry.is.ETHERNET.dMacM31_16 >> 8;
		rule->dstMacMask_.octet[3] =
		    entry.is.ETHERNET.dMacM31_16 & 0xff;
		...
		rule->srcMacMask_.octet[1] =
		    entry.is.ETHERNET.sMacM47_32 & 0xff;
		rule->srcMacMask_.octet[2] = entry.is.ETHERNET.sMacM31_16 >> 8;
		rule->srcMacMask_.octet[3] =
		    entry.is.ETHERNET.sMacM31_16 & 0xff;
...
l3l4_shared:
	...
	case RTL865X_ACL_SRCFILTER_IPRANGE:
		rule->srcFilterMac_.octet[0] =
		    entry.is.SRC_FILTER.sMacP47_32 >> 8;
		rule->srcFilterMac_.octet[1] =
		    entry.is.SRC_FILTER.sMacP47_32 & 0xff;
		rule->srcFilterMac_.octet[2] =
		    entry.is.SRC_FILTER.sMacP31_16 >> 8;
		rule->srcFilterMac_.octet[3] =
		    entry.is.SRC_FILTER.sMacP31_16 & 0xff;
		rule->srcFilterMac_.octet[4] =
		    entry.is.SRC_FILTER.sMacP15_0 >> 8;
		rule->srcFilterMac_.octet[5] =
		    entry.is.SRC_FILTER.sMacP15_0 & 0xff;
		if (entry.is.SRC_FILTER.sMacM3_0 & 0x8) {
			...
			rule->srcFilterMacMask_.octet[5] =
			    0xF0 | entry.is.SRC_FILTER.sMacM3_0;
		} else {
		...
		if (entry.is.SRC_FILTER.protoType == 2)
			rule->srcFilterIgnoreL4_ = 1;
		else if (entry.is.SRC_FILTER.protoType == 1)
	...
	case RTL865X_ACL_DSTFILTER_IPRANGE:	/* Destination Filter(IP range) */
		rule->dstFilterMac_.octet[0] =
		    entry.is.DST_FILTER.dMacP47_32 >> 8;
		rule->dstFilterMac_.octet[1] =
		    entry.is.DST_FILTER.dMacP47_32 & 0xff;
		rule->dstFilterMac_.octet[2] =
		    entry.is.DST_FILTER.dMacP31_16 >> 8;
		rule->dstFilterMac_.octet[3] =
		    entry.is.DST_FILTER.dMacP31_16 & 0xff;
		rule->dstFilterMac_.octet[4] =
		    entry.is.DST_FILTER.dMacP15_0 >> 8;
		rule->dstFilterMac_.octet[5] =
		    entry.is.DST_FILTER.dMacP15_0 & 0xff;
		if (entry.is.DST_FILTER.dMacM3_0 & 0x8) {
			...
			rule->dstFilterMacMask_.octet[5] =
			    0xF0 | entry.is.DST_FILTER.dMacM3_0;
		} else {
	...
	case RTL865X_ACL_IPV6_RANGE:
		...
		{
			rule->srcIpV6Addr_.v6_addr32[3] =
			    entry.is.L3V6.is.entry0.sip_addr31_0;
			rule->srcIpV6Addr_.v6_addr32[2] =
			    entry.is.L3V6.is.entry0.sip_addr63_32;
			rule->srcIpV6Addr_.v6_addr32[1] =
			    entry.is.L3V6.is.entry0.sip_addr95_64;
			rule->srcIpV6Addr_.v6_addr32[0] =
			    entry.is.L3V6.is.entry0.sip_addr127_96;

			rule->srcIpV6AddrMask_.v6_addr32[3] =
			    entry.is.L3V6.is.entry0.sip_mask31_0;
			rule->srcIpV6AddrMask_.v6_addr32[2] =
			    entry.is.L3V6.is.entry0.sip_mask63_32;
			rule->srcIpV6AddrMask_.v6_addr32[1] =
			    entry.is.L3V6.is.entry0.sip_mask95_64;

			rule->srcIpV6AddrMask_.v6_addr32[0] =
			    entry.is_d.ipv6.is.entry0.sip_mask119_96;
			rule->srcIpV6AddrMask_.v6_addr32[0] |=
			    entry.is_d.ipv6.is.entry0.sip_mask127_120 << 24;
			rule->ipv6FlowLabel_ =
			    entry.is_d.ipv6.is.entry0.flowLabel;
			rule->ipv6FlowLabelM_ =
			    entry.is_d.ipv6.is.entry0.flowLabelM3_0;
			rule->ipv6FlowLabelM_ |=
			    entry.is_d.ipv6.is.entry0.flowLabelM19_4 << 4;
		} else {
			rule->dstIpV6Addr_.v6_addr32[3] =
			    entry.is.L3V6.is.entry1.dip_addr31_0;
			rule->dstIpV6Addr_.v6_addr32[2] =
			    entry.is.L3V6.is.entry1.dip_addr63_32;
			rule->dstIpV6Addr_.v6_addr32[1] =
			    entry.is.L3V6.is.entry1.dip_addr95_64;
			rule->dstIpV6Addr_.v6_addr32[0] =
			    entry.is.L3V6.is.entry1.dip_addr127_96;

			rule->dstIpV6AddrMask_.v6_addr32[3] =
			    entry.is.L3V6.is.entry1.dip_mask31_0;
			...
			rule->dstIpV6AddrMask_.v6_addr32[2] =
			    entry.is.L3V6.is.entry1.dip_mask63_32;
			rule->dstIpV6AddrMask_.v6_addr32[1] =
			    entry.is.L3V6.is.entry1.dip_mask95_64;

			rule->dstIpV6AddrMask_.v6_addr32[0] =
			    entry.is_d.ipv6.is.entry1.dip_mask119_96;
			rule->dstIpV6AddrMask_.v6_addr32[0] |=
			    entry.is_d.ipv6.is.entry1.dip_mask127_120 << 24;
			rule->ipv6TrafficClass_ =
			    entry.is_d.ipv6.is.entry1.trafficClass;
			rule->ipv6TrafficClassM_ =
			    entry.is_d.ipv6.is.entry1.trafficClassM;
			rule->ipv6NextHeader_ =
			    entry.is_d.ipv6.is.entry1.nextHeader;
			rule->ipv6NextHeaderM_ =
			    entry.is_d.ipv6.is.entry1.nextHeaderM;
			rule->ipv6HttpFilter_ = entry.is_d.ipv6.is.entry1.HTTPP;
			rule->ipv6HttpFilterM_ =
			    entry.is_d.ipv6.is.entry1.HTTPM;
			rule->ipv6IdentSrcDstIp_ =
			    entry.is_d.ipv6.is.entry1.identSDIPP;
			rule->ipv6IdentSrcDstIpM_ =
			    entry.is_d.ipv6.is.entry1.identSDIPM;
		}
	...
	case RTL865X_ACL_IPV6_RANGE:
		...
		if (entry.ipv6ETY0)	//first entry
		{
			...
			rule->srcIpV6AddrMask_.v6_addr32[0] =
			    entry.ipv6.is.entry0.sip_mask119_96;
			rule->srcIpV6AddrMask_.v6_addr32[0] |=
			    entry.ipv6.is.entry0.sip_mask127_120 << 24;
			rule->ipv6FlowLabel_ = entry.ipv6.is.entry0.flowLabel;
			rule->ipv6FlowLabelM_ =
			    entry.ipv6.is.entry0.flowLabelM3_0;
			rule->ipv6FlowLabelM_ |=
			    entry.ipv6.is.entry0.flowLabelM19_4 << 4;
		} else {
			rule->dstIpV6Addr_.v6_addr32[3] =
			    entry.is.L3V6.is.entry1.dip_addr31_0;
			rule->dstIpV6Addr_.v6_addr32[2] =
			...
			rule->dstIpV6AddrMask_.v6_addr32[0] =
			    entry.ipv6.is.entry1.dip_mask119_96;
			rule->dstIpV6AddrMask_.v6_addr32[0] |=
			    entry.ipv6.is.entry1.dip_mask127_120 << 24;
			rule->ipv6TrafficClass_ =
			    entry.ipv6.is.entry1.trafficClass;
			rule->ipv6TrafficClassM_ =
			    entry.ipv6.is.entry1.trafficClassM;
			rule->ipv6NextHeader_ = entry.ipv6.is.entry1.nextHeader;
			rule->ipv6NextHeaderM_ =
			    entry.ipv6.is.entry1.nextHeaderM;
			rule->ipv6HttpFilter_ = entry.ipv6.is.entry1.HTTPP;
			rule->ipv6HttpFilterM_ = entry.ipv6.is.entry1.HTTPM;
			rule->ipv6IdentSrcDstIp_ =
			    entry.ipv6.is.entry1.identSDIPP;
			rule->ipv6IdentSrcDstIpM_ =
			    entry.ipv6.is.entry1.identSDIPM;
		}
	...
	case RTL865X_ACL_PM:	/* acl     pattern match */
		if (entry.ipv6ETY0)	//first entry
		{
			...
			rule->pmPatternMask3_ =
			    (entry.is_d.PM_D.is.entry0.
			     pm3_8_0 | (entry.is_d.PM_D.is.entry0.
					pm3_15_9 << 9));
			...
			rule->pmPattern6_ =
			    (entry.is.PM_U.is.entry1.
			     pattern6_8_0 | (entry.is.PM_U.is.entry1.
					     pattern6_15_9 << 9));
...
#endif
	default:
		return FAILED;	/* Unknown rule type */
	...
	case RTL865X_ACL_VID:
		...
		rule->aclfid_ =
		    ((entry.PPPoEIndex << 1) | (entry.vid >> 2)) & 0x3;
		break;
...
static int32 _rtl865x_setAclToAsic(int32 startIdx, rtl865x_AclRule_t * rule)
{
	...
	if (rule->aclIdx >= RTL865X_ACL_MAX_NUMBER + RTL865X_ACL_RESERVED_NUMBER
	    || rule == NULL)
		return FAILED;
	...
	case RTL865X_ACL_MAC:	/* Etnernet Rule Type : 0x0000 */
		entry.is.ETHERNET.dMacP47_32 =
		    rule->dstMac_.octet[0] << 8 | rule->dstMac_.octet[1];
		entry.is.ETHERNET.dMacP31_16 =
		    rule->dstMac_.octet[2] << 8 | rule->dstMac_.octet[3];
		entry.is.ETHERNET.dMacP15_0 =
		    rule->dstMac_.octet[4] << 8 | rule->dstMac_.octet[5];
		entry.is.ETHERNET.dMacM47_32 =
		    rule->dstMacMask_.octet[0] << 8 | rule->dstMacMask_.
		    octet[1];
		entry.is.ETHERNET.dMacM31_16 =
		    rule->dstMacMask_.octet[2] << 8 | rule->dstMacMask_.
		    octet[3];
		entry.is.ETHERNET.dMacM15_0 =
		    rule->dstMacMask_.octet[4] << 8 | rule->dstMacMask_.
		    octet[5];
		entry.is.ETHERNET.sMacP47_32 =
		    rule->srcMac_.octet[0] << 8 | rule->srcMac_.octet[1];
		entry.is.ETHERNET.sMacP31_16 =
		    rule->srcMac_.octet[2] << 8 | rule->srcMac_.octet[3];
		entry.is.ETHERNET.sMacP15_0 =
		    rule->srcMac_.octet[4] << 8 | rule->srcMac_.octet[5];
		entry.is.ETHERNET.sMacM47_32 =
		    rule->srcMacMask_.octet[0] << 8 | rule->srcMacMask_.
		    octet[1];
		entry.is.ETHERNET.sMacM31_16 =
		    rule->srcMacMask_.octet[2] << 8 | rule->srcMacMask_.
		    octet[3];
		entry.is.ETHERNET.sMacM15_0 =
		    rule->srcMacMask_.octet[4] << 8 | rule->srcMacMask_.
		    octet[5];
	...
	case RTL865X_ACL_IP_RANGE:	/* 0x0010 */
		...
		entry.is.L3L4.is.IP.HTTPP = entry.is.L3L4.is.IP.HTTPM =
		    rule->ipHttpFilter_;
		entry.is.L3L4.is.IP.identSDIPP =
		    entry.is.L3L4.is.IP.identSDIPM = rule->ipIdentSrcDstIp_;
...
l3l4_shared:
	...
	case RTL865X_ACL_SRCFILTER_IPRANGE:	/* 0x000B */
		rule->srcFilterMac_.octet[0] =
		    rule->srcFilterMac_.octet[0] & rule->srcFilterMacMask_.
		    octet[0];
		rule->srcFilterMac_.octet[1] =
		    rule->srcFilterMac_.octet[1] & rule->srcFilterMacMask_.
		    octet[1];
		rule->srcFilterMac_.octet[2] =
		    rule->srcFilterMac_.octet[2] & rule->srcFilterMacMask_.
		    octet[2];
		rule->srcFilterMac_.octet[3] =
		    rule->srcFilterMac_.octet[3] & rule->srcFilterMacMask_.
		    octet[3];
		rule->srcFilterMac_.octet[4] =
		    rule->srcFilterMac_.octet[4] & rule->srcFilterMacMask_.
		    octet[4];
		rule->srcFilterMac_.octet[5] =
		    rule->srcFilterMac_.octet[5] & rule->srcFilterMacMask_.
		    octet[5];

		entry.is.SRC_FILTER.sMacP47_32 =
		    rule->srcFilterMac_.octet[0] << 8 | rule->srcFilterMac_.
		    octet[1];
		entry.is.SRC_FILTER.sMacP31_16 =
		    rule->srcFilterMac_.octet[2] << 8 | rule->srcFilterMac_.
		    octet[3];
		entry.is.SRC_FILTER.sMacP15_0 =
		    rule->srcFilterMac_.octet[4] << 8 | rule->srcFilterMac_.
		    octet[5];
		entry.is.SRC_FILTER.sMacM3_0 =
		    rule->srcFilterMacMask_.octet[5] & 0xf;

		rule->srcFilterVlanId_ =
		    rule->srcFilterVlanId_ & rule->srcFilterVlanIdMask_;
	...
	case RTL865X_ACL_DSTFILTER_IPRANGE:	/* 0x0001 */
		entry.is.DST_FILTER.dMacP47_32 =
		    rule->dstFilterMac_.octet[0] << 8 | rule->dstFilterMac_.
		    octet[1];
		entry.is.DST_FILTER.dMacP31_16 =
		    rule->dstFilterMac_.octet[2] << 8 | rule->dstFilterMac_.
		    octet[3];
		entry.is.DST_FILTER.dMacP15_0 =
		    rule->dstFilterMac_.octet[4] << 8 | rule->dstFilterMac_.
		    octet[5];
		entry.is.DST_FILTER.dMacM3_0 =
		    rule->dstFilterMacMask_.octet[5] & 0xf;
	...
	case RTL865X_ACL_IPV6_RANGE:	/* 0x001A */
		...
		if (rule->ipv6EntryType_ == 1)	//first entry
		{
			...
			entry.is_d.ipv6.is.entry0.sip_mask119_96 =
			    rule->srcIpV6AddrMask_.v6_addr32[0] & 0xFFFFFF;
			entry.is_d.ipv6.is.entry0.sip_mask127_120 =
			    rule->srcIpV6AddrMask_.v6_addr32[0] >> 24;
			entry.is_d.ipv6.is.entry0.flowLabel =
			    rule->ipv6FlowLabel_;
			entry.is_d.ipv6.is.entry0.flowLabelM3_0 =
			    rule->ipv6FlowLabelM_ & 0xF;
			entry.is_d.ipv6.is.entry0.flowLabelM19_4 =
			    rule->ipv6FlowLabelM_ >> 4;
		} else {
			entry.is.L3V6.is.entry1.dip_addr31_0 =
			    rule->dstIpV6Addr_.v6_addr32[3];
			entry.is.L3V6.is.entry1.dip_addr63_32 =
			...
			entry.is_d.ipv6.is.entry1.dip_mask119_96 =
			    rule->dstIpV6AddrMask_.v6_addr32[0] & 0xFFFFFF;
			entry.is_d.ipv6.is.entry1.dip_mask127_120 =
			    rule->dstIpV6AddrMask_.v6_addr32[0] >> 24;
			entry.is_d.ipv6.is.entry1.trafficClass =
			    rule->ipv6TrafficClass_;
			entry.is_d.ipv6.is.entry1.trafficClassM =
			    rule->ipv6TrafficClassM_;
			...
			entry.is_d.ipv6.is.entry1.nextHeaderM =
			    rule->ipv6NextHeaderM_;
			entry.is_d.ipv6.is.entry1.HTTPP =
			    entry.is_d.ipv6.is.entry1.HTTPM =
			    rule->ipv6HttpFilter_;
			entry.is_d.ipv6.is.entry1.identSDIPP =
			    entry.is_d.ipv6.is.entry1.identSDIPM =
			    rule->ipv6IdentSrcDstIp_;
		}
	...
	case RTL865X_ACL_IPV6_RANGE:	/* 0x001A */
		...
		if (rule->ipv6EntryType_ == 1)	//first entry
		{
			...
			entry.ipv6.is.entry0.sip_mask119_96 =
			    rule->srcIpV6AddrMask_.v6_addr32[0] & 0xFFFFFF;
			entry.ipv6.is.entry0.sip_mask127_120 =
			    rule->srcIpV6AddrMask_.v6_addr32[0] >> 24;
			entry.ipv6.is.entry0.flowLabel = rule->ipv6FlowLabel_;
			entry.ipv6.is.entry0.flowLabelM3_0 =
			    rule->ipv6FlowLabelM_ & 0xF;
			entry.ipv6.is.entry0.flowLabelM19_4 =
			    rule->ipv6FlowLabelM_ >> 4;
		} else {
			entry.is.L3V6.is.entry1.dip_addr31_0 =
			    rule->dstIpV6Addr_.v6_addr32[3];
			...
			entry.ipv6.is.entry1.dip_mask119_96 =
			    rule->dstIpV6AddrMask_.v6_addr32[0] & 0xFFFFFF;
			entry.ipv6.is.entry1.dip_mask127_120 =
			    rule->dstIpV6AddrMask_.v6_addr32[0] >> 24;
			entry.ipv6.is.entry1.trafficClass =
			    rule->ipv6TrafficClass_;
			entry.ipv6.is.entry1.trafficClassM =
			...
			entry.ipv6.is.entry1.nextHeaderM =
			    rule->ipv6NextHeaderM_;
			entry.ipv6.is.entry1.HTTPP =
			    entry.ipv6.is.entry1.HTTPM = rule->ipv6HttpFilter_;
			entry.ipv6.is.entry1.identSDIPP =
			    entry.ipv6.is.entry1.identSDIPM =
			    rule->ipv6IdentSrcDstIp_;
		}
	...
	case RTL865X_ACL_PM:	/* IP Rule Type: 0x0012 */
		...
		if (rule->ipv6EntryType_ == 1)	//first entry
		{
			...
			entry.is.PM_U.is.entry0.protocol =
			    rule->un_ty.PM._protocol;

			entry.is.PM_U.is.entry0.offset0 =
			    rule->un_ty.PM._offset0;
			entry.is.PM_U.is.entry0.pattern0 =
			    rule->un_ty.PM._pattern0;
			entry.is.PM_U.is.entry0.pm0_15_1 =
			    rule->un_ty.PM._pm0 >> 1;
			entry.is.PM_U.is.entry0.pm0_0 =
			    rule->un_ty.PM._pm0 & 0x1;
			entry.is.PM_U.is.entry0.or0 = rule->un_ty.PM._or0;

			entry.is.PM_U.is.entry0.offset1 =
			    rule->un_ty.PM._offset1;
			entry.is.PM_U.is.entry0.pattern1_8_0 =
			    rule->un_ty.PM._pattern1 & 0x1ff;
			entry.is.PM_U.is.entry0.pattern1_15_9 =
			    rule->un_ty.PM._pattern1 >> 9;
			entry.is.PM_U.is.entry0.pm1 = rule->un_ty.PM._pm1;
			entry.is.PM_U.is.entry0.or1 = rule->un_ty.PM._or1;

			entry.is.PM_U.is.entry0.offset2 =
			    rule->un_ty.PM._offset2;
			entry.is.PM_U.is.entry0.pattern2_0 =
			    rule->un_ty.PM._pattern2 & 0x1;
			entry.is.PM_U.is.entry0.pattern2_15_1 =
			    rule->un_ty.PM._pattern2 >> 1;
			entry.is.PM_U.is.entry0.pm2 = rule->un_ty.PM._pm2;
			entry.is.PM_U.is.entry0.or2 = rule->un_ty.PM._or2;

			entry.is_d.PM_D.is.entry0.offset3 =
			    rule->un_ty.PM._offset3;
			entry.is_d.PM_D.is.entry0.pattern3 =
			    rule->un_ty.PM._pattern3;
			entry.is_d.PM_D.is.entry0.pm3_8_0 =
			    rule->un_ty.PM._pm3 & 0x1ff;
			entry.is_d.PM_D.is.entry0.pm3_15_9 =
			    rule->un_ty.PM._pm3 >> 9;
			entry.is_d.PM_D.is.entry0.or3 = rule->un_ty.PM._or3;
		} else {
			entry.is.PM_U.is.entry1.offset4 =
			    rule->un_ty.PM._offset4;
			entry.is.PM_U.is.entry1.pattern4 =
			    rule->un_ty.PM._pattern4;
			entry.is.PM_U.is.entry1.pm4_8_0 =
			    rule->un_ty.PM._pm4 & 0x1ff;
			entry.is.PM_U.is.entry1.pm4_15_9 =
			    rule->un_ty.PM._pm4 >> 9;
			entry.is.PM_U.is.entry1.or4 = rule->un_ty.PM._or4;

			entry.is.PM_U.is.entry1.offset5 =
			    rule->un_ty.PM._offset5;
			entry.is.PM_U.is.entry1.pattern5 =
			    rule->un_ty.PM._pattern5;
			entry.is.PM_U.is.entry1.pm5_0 =
			    rule->un_ty.PM._pm5 & 0x1;
			entry.is.PM_U.is.entry1.pm5_15_1 =
			    rule->un_ty.PM._pm5 >> 1;
			entry.is.PM_U.is.entry1.or5 = rule->un_ty.PM._or5;

			entry.is.PM_U.is.entry1.offset6 =
			    rule->un_ty.PM._offset6;
			entry.is.PM_U.is.entry1.pattern6_8_0 =
			    rule->un_ty.PM._pattern6 & 0x1ff;
			entry.is.PM_U.is.entry1.pattern6_15_9 =
			    rule->un_ty.PM._pattern6 >> 9;
			entry.is.PM_U.is.entry1.pm6 = rule->un_ty.PM._pm6;
			entry.is.PM_U.is.entry1.or6 = rule->un_ty.PM._or6;

			entry.is.PM_U.is.entry1.offset7 =
			    rule->un_ty.PM._offset7;
			entry.is.PM_U.is.entry1.pattern7_0 =
			    rule->un_ty.PM._pattern7 & 0x1;
			entry.is.PM_U.is.entry1.pattern7_15_1 =
			    rule->un_ty.PM._pattern7 >> 1;
			entry.is.PM_U.is.entry1.pm7 = rule->un_ty.PM._pm7;
			entry.is.PM_U.is.entry1.or7 = rule->un_ty.PM._or7;

			entry.is.PM_U.is.entry1.specialop =
			    rule->un_ty.PM._specialop;
			...
			entry.is.PM_U.is.entry1.pppctlm =
			    rule->un_ty.PM._pppctlm;
			entry.is.PM_U.is.entry1.pppctl_or =
			    rule->un_ty.PM._pppctlor;
		}
...
#endif
	default:
		return FAILED;	/* Unknown rule type: -1 */
	...
	return _rtl8651_forceAddAsicEntry(TYPE_ACL_RULE_TABLE,
					  startIdx + rule->aclIdx - 1, &entry);
}
...
#ifdef CONFIG_RTL_LAYERED_ASIC_DRIVER
	rtl865x_setDefACLForNetDecisionMiss(RTL865X_ACLTBL_PERMIT_ALL,
...
*======================================*/
static int32 _rtl865x_setDefACLForAllNetif(uint8 start_ingressAclIdx,
					   uint8 end_ingressAclIdx,
					   uint8 start_egressAclIdx,
...
#if defined (CONFIG_RTL_LOCAL_PUBLIC)
	rtl865x_setDefACLForNetDecisionMiss(start_ingressAclIdx,
					    end_ingressAclIdx,
					    start_egressAclIdx,
					    end_egressAclIdx);
#endif
...
int32 rtl865x_setDefACLForAllNetif(uint8 start_ingressAclIdx,
				   uint8 end_ingressAclIdx,
				   uint8 start_egressAclIdx,
				   uint8 end_egressAclIdx)
{
	_rtl865x_setDefACLForAllNetif(start_ingressAclIdx, end_ingressAclIdx,
				      start_egressAclIdx, end_egressAclIdx);
	return SUCCESS;
}

static int8 _rtl865x_sameAclRule(rtl865x_AclRule_t * rule1,
				 rtl865x_AclRule_t * rule2)
{

	if (rule1->actionType_ != rule2->actionType_
	    || rule1->ruleType_ != rule2->ruleType_)
		return FALSE;
	...
	case RTL865X_ACL_MAC:
		if (rule1->typeLen_ != rule2->typeLen_
			...
			return FALSE;
		if (memcmp
		    (&rule1->dstMac_, &rule2->dstMac_, sizeof(ether_addr_t))
		    || memcmp(&rule1->dstMacMask_, &rule2->dstMacMask_,
			      sizeof(ether_addr_t))
		    || memcmp(&rule1->srcMac_, &rule2->srcMac_,
			      sizeof(ether_addr_t))
		    || memcmp(&rule1->srcMacMask_, &rule2->srcMacMask_,
			      sizeof(ether_addr_t)))
			return FALSE;
		return TRUE;
	case RTL865X_ACL_IP:
	case RTL865X_ACL_IP_RANGE:
		if (rule1->ipProto_ != rule2->ipProto_
		    || rule1->ipProtoMask_ != rule2->ipProtoMask_
		    || rule1->ipFlag_ != rule2->ipFlag_
		    || rule1->ipFlagMask_ != rule2->ipFlagMask_)
			return FALSE;
		break;

	case RTL865X_ACL_ICMP:
	case RTL865X_ACL_ICMP_IPRANGE:
		if (rule1->icmpType_ != rule2->icmpType_
		    || rule1->icmpTypeMask_ != rule2->icmpTypeMask_
		    || rule1->icmpCode_ != rule2->icmpCode_
		    || rule1->icmpCodeMask_ != rule2->icmpCodeMask_)
			return FALSE;
		break;

	case RTL865X_ACL_IGMP:
	case RTL865X_ACL_IGMP_IPRANGE:
		if (rule1->igmpType_ != rule2->igmpType_
		    || rule1->igmpTypeMask_ != rule2->igmpTypeMask_)
			return FALSE;
		break;
	case RTL865X_ACL_TCP:
	case RTL865X_ACL_TCP_IPRANGE:
		if (rule1->tcpFlag_ != rule2->tcpFlag_
		    || rule1->tcpFlagMask_ != rule2->tcpFlagMask_
		    || rule1->tcpSrcPortUB_ != rule2->tcpSrcPortUB_
		    || rule1->tcpSrcPortLB_ != rule2->tcpSrcPortLB_
		    || rule1->tcpDstPortUB_ != rule2->tcpDstPortUB_
		    || rule1->tcpDstPortLB_ != rule2->tcpDstPortLB_)
			return FALSE;
		break;
	case RTL865X_ACL_UDP:
	case RTL865X_ACL_UDP_IPRANGE:
		if (rule1->udpSrcPortUB_ != rule2->udpSrcPortUB_
		    || rule1->udpSrcPortLB_ != rule2->udpSrcPortLB_
		    || rule1->udpDstPortUB_ != rule2->udpDstPortUB_
		    || rule1->udpDstPortLB_ != rule2->udpDstPortLB_)
			return FALSE;
		break;
	...
	case RTL865X_ACL_SRCFILTER_IPRANGE:
		if ((rule1->srcFilterPort_ != rule2->srcFilterPort_) ||
		    memcmp(&rule1->srcFilterMac_, &rule2->srcFilterMac_,
			   sizeof(ether_addr_t)) != 0
		    || memcmp(&rule1->srcFilterMacMask_,
			      &rule2->srcFilterMacMask_,
			      sizeof(ether_addr_t)) != 0
		    || (rule1->srcFilterVlanIdx_ != rule2->srcFilterVlanIdx_)
		    || (rule1->srcFilterVlanIdxMask_ !=
			rule2->srcFilterVlanIdxMask_)
		    || (rule1->srcFilterIgnoreL3L4_ !=
			rule2->srcFilterIgnoreL3L4_)
		    || (rule1->srcFilterIgnoreL4_ != rule2->srcFilterIgnoreL4_)) {
			return FALSE;
		}

		if (rule1->srcFilterIgnoreL4_ == 0
		    && rule1->srcFilterIgnoreL3L4_ == 0) {
			if ((rule1->srcFilterPortUpperBound_ !=
			     rule2->srcFilterPortUpperBound_)
			    || (rule1->srcFilterPortLowerBound_ !=
				rule2->srcFilterPortLowerBound_))
				return FALSE;
		}

		if (rule1->srcFilterIgnoreL3L4_ == 0) {
			if ((rule1->srcFilterIpAddr_ != rule2->srcFilterIpAddr_)
			    || (rule2->srcFilterIpAddrMask_ !=
				rule2->srcFilterIpAddrMask_))
				return FALSE;
		}
	...
	case RTL865X_ACL_DSTFILTER_IPRANGE:
		if (memcmp
		    (&rule1->dstFilterMac_, &rule2->dstFilterMac_,
		     sizeof(ether_addr_t)) != 0
		    || memcmp(&rule1->dstFilterMacMask_,
			      &rule2->dstFilterMacMask_,
			      sizeof(ether_addr_t)) != 0
		    || (rule1->dstFilterVlanIdx_ != rule2->dstFilterVlanIdx_)
		    || (rule1->dstFilterVlanIdxMask_ !=
			rule2->dstFilterVlanIdxMask_)
		    || (rule1->dstFilterIgnoreL3L4_ !=
			rule2->dstFilterIgnoreL3L4_)
		    || (rule1->dstFilterIgnoreL4_ != rule2->dstFilterIgnoreL4_)) {
			return FALSE;
		}

		if (rule1->dstFilterIgnoreL4_ == 0
		    && rule1->dstFilterIgnoreL4_ == 0) {
			if ((rule1->dstFilterPortUpperBound_ !=
			     rule2->dstFilterPortUpperBound_)
			    || (rule1->dstFilterPortLowerBound_ !=
				rule2->dstFilterPortLowerBound_))
				return FALSE;
		}

		if (rule1->dstFilterIgnoreL3L4_ == 0) {
			if ((rule1->dstFilterIpAddr_ != rule2->dstFilterIpAddr_)
			    || (rule2->dstFilterIpAddrMask_ !=
				rule2->dstFilterIpAddrMask_))
				return FALSE;
		}
	...
	/* Compare common part */
	if (rule1->srcIpAddr_ != rule2->srcIpAddr_
	    || rule1->srcIpAddrMask_ != rule2->srcIpAddrMask_
	    || rule1->dstIpAddr_ != rule2->dstIpAddr_
	    || rule1->dstIpAddrMask_ != rule2->dstIpAddrMask_
	    || rule1->tos_ != rule2->tos_ || rule1->tosMask_ != rule2->tosMask_)
		return FALSE;
	return TRUE;
}

static int32 _rtl865x_addAclToChain(rtl865x_AclRule_t * rule,
				    rtl865x_AclRule_t ** head,
				    rtl865x_AclRule_t ** tail)
{
...
static int32 _rtl865x_delAclFromChain(rtl865x_AclRule_t * rule,
				      rtl865x_AclRule_t ** head,
				      rtl865x_AclRule_t ** tail)
{
...
#endif
{
	...
	_rtl865x_setDefACLForAllNetif(RTL865X_ACLTBL_PERMIT_ALL,
				      RTL865X_ACLTBL_PERMIT_ALL,
				      RTL865X_ACLTBL_PERMIT_ALL,
				      RTL865X_ACLTBL_PERMIT_ALL);
	startIdx = 0;
		...
		if (netif->valid) {
			...
			while (chain) {
				...
				while (rule) {
					...
					if ((preRule == NULL)
					    ||
					    (_rtl865x_sameAclRule(preRule, rule)
					     == FALSE)) {
						_rtl865x_setAclToAsic(startIdx,
								      rule);
						addCnt++;
					}
			...
			if (totalAddCnt > 0) {
				netif->inAclEnd =
				    netif->inAclStart + totalAddCnt - 1;
//                              startIdx += addCnt;

				if (netif->inAclEnd <
				    (RTL865X_ACL_MAX_NUMBER - 1)) {
					if (_rtl865x_checkDefAclAvailable
					    (endRule) == 0) {
			...
			if (totalAddCnt > 0) {
				netif->outAclEnd =
				    netif->outAclStart + totalAddCnt - 1;
//                              startIdx += addCnt;
				if (netif->outAclEnd <
				    (RTL865X_ACL_MAX_NUMBER - 1)) {
					if (_rtl865x_checkDefAclAvailable
			...
			netif->enableRoute =
			    (rtl8651_getAsicOperationLayer() > 2) ? 1 : 0;
#endif
...
//                              startIdx += addCnt;
			...
			rtl865x_setDefACLForNetDecisionMiss(netif->inAclStart,
							    netif->inAclEnd,
							    netif->outAclStart,
							    netif->outAclEnd);
		}
...
static int32 _rtl865x_unRegist_aclChain(char *netifName, int32 priority,
					uint32 flag)
{
	rtl865x_netif_local_t *netif;
	...
	while (aclRule) {
		_rtl865x_delAclFromChain(aclRule, &chainEntry->head,
					 &chainEntry->tail);
		aclRule = chainEntry->head;
		chainEntry->ruleCnt--;
	}
...
static int32 _rtl865x_flush_allAcl_fromChain(char *netifName, int32 priority,
					     uint32 flag)
{
	rtl865x_netif_local_t *netif;
	rtl865x_AclRule_t *aclRule;
	...
	if (netifName && netifName[0] != '\0') {
...
#endif
		{
			...
			if (netif->valid) {
				...
				aclRule = chainEntry->head;
				while (aclRule) {
					_rtl865x_delAclFromChain(aclRule,
								 &chainEntry->
								 head,
								 &chainEntry->
								 tail);
...
static rtl865x_AclRule_t *_rtl865x_matched_layer4_aclChain(char *netifName,
							   int32 priority,
							   uint32 flag,
							   rtl865x_AclRule_t *
							   match)
{
	...
	if (netifName && netifName[0] != '\0') {
		...
		/*check all aclrule */
		for (aclRule = chainEntry->head; aclRule;
		     aclRule = aclRule->next) {
			isRange = FALSE;
			switch (aclRule->ruleType_) {
			case RTL865X_ACL_TCP_IPRANGE:
				isRange = TRUE;
			case RTL865X_ACL_TCP:
				if ((aclRule->tcpSrcPortLB_ <=
				     match->tcpSrcPortLB_)
				    && (aclRule->tcpSrcPortUB_ >=
					match->tcpSrcPortLB_)
				    && (aclRule->tcpDstPortLB_ <=
					match->tcpDstPortLB_)
				    && (aclRule->tcpDstPortUB_ >=
					match->tcpDstPortLB_)
			...
			case RTL865X_ACL_UDP_IPRANGE:
				isRange = TRUE;
			case RTL865X_ACL_UDP:
				if ((aclRule->udpSrcPortLB_ <=
				     match->tcpSrcPortLB_)
				    && (aclRule->udpSrcPortUB_ >=
					match->tcpSrcPortLB_)
				    && (aclRule->udpDstPortLB_ <=
					match->tcpDstPortLB_)
				    && (aclRule->udpDstPortUB_ >=
					match->tcpDstPortLB_)
			...
			case RTL865X_ACL_IP_RANGE:
				isRange = TRUE;
			case RTL865X_ACL_IP:
				if (((aclRule->tos_ & aclRule->tosMask_) ==
				     (match->tos_ & aclRule->tosMask_))
				    &&
				    ((aclRule->ipProto_ & aclRule->
				      ipProtoMask_) ==
				     (match->ipProto_ & aclRule->ipProtoMask_))
				    &&
				    ((aclRule->ipFlag_ & aclRule->
				      ipFlagMask_) ==
				     (match->ipFlag_ & aclRule->ipFlagMask_))) {
					break;
				} else
					continue;
			default:
			...
			if (isRange) {
				if ((aclRule->srcIpAddrStart_ <=
				     match->srcIpAddr_)
				    && (aclRule->srcIpAddrEnd_ >=
					match->srcIpAddr_)
				    && (aclRule->dstIpAddrStart_ <=
					match->dstIpAddr_)
				    && (aclRule->dstIpAddrEnd_ >=
					match->dstIpAddr_)) {
					break;
				} else
					continue;
			} else {
				if (((aclRule->srcIpAddr_ & aclRule->
				      srcIpAddrMask_)
				     == (match->srcIpAddr_ & aclRule->
					 srcIpAddrMask_))
				    &&
				    ((aclRule->dstIpAddr_ & aclRule->
				      dstIpAddrMask_)
				     == (match->dstIpAddr_ & aclRule->
					 dstIpAddrMask_))) {
					break;
				} else
					continue;
...
static inline int _rtl865x_cmpMacAddr(ether_addr_t * mac1, ether_addr_t * mac2,
				      ether_addr_t * mask)
{
	int i;

	for (i = 0; i < ETHER_ADDR_LEN; i++) {
		if ((mac1->octet[i] & mask->octet[i]) !=
		    (mac2->octet[i] & mask->octet[i]))
			return (SUCCESS + i + 1);
	}
	return SUCCESS;
}

static rtl865x_AclRule_t *_rtl865x_matched_layer2_aclChain(char *netifName,
							   int32 priority,
							   uint32 flag,
							   rtl865x_AclRule_t *
							   match)
{
	...
	if (netifName && netifName[0] != '\0') {
		...
		for (aclRule = chainEntry->head; aclRule;
			...
			{
				if (_rtl865x_cmpMacAddr
				    (&aclRule->srcMac_, &match->srcMac_,
				     &aclRule->srcMacMask_) == SUCCESS
				    && _rtl865x_cmpMacAddr(&aclRule->dstMac_,
							   &match->dstMac_,
							   &aclRule->
							   dstMacMask_)
				    == SUCCESS
				    &&
				    ((aclRule->typeLen_ & aclRule->
				      typeLenMask_) ==
				     (match->typeLen_ & aclRule->typeLenMask_))
...
static int32 _rtl865x_unRegister_all_aclChain(char *netifName)
{
	...
	for (aclDir = RTL865X_ACL_INGRESS; aclDir <= RTL865X_ACL_EGRESS;
	     aclDir++) {
		chainEntry = netif->chainListHead[aclDir];
		while (chainEntry) {
			/*remove all aclrule */
...
static rtl865x_acl_chain_t
    * _rtl865x_find_aclChain_byPriority(rtl865x_acl_chain_t * head,
					int32 priority)
{
	rtl865x_acl_chain_t *entry;
	entry = head;
...
static int32 _rtl865x_add_acl(rtl865x_AclRule_t * rule, char *netifName,
			      int32 chainNo)
{
	rtl865x_netif_local_t *netif = NULL;
	...
	if (netifName && netifName[0] != '\0' && (rule->inv_flag == 0)) {
		...
		chain =
		    _rtl865x_find_aclChain_byPriority(netif->chainListHead
						      [rule->direction_],
						      chainNo);
		if (chain == NULL)
			return RTL_EENTRYNOTFOUND;
		...
		retval =
		    _rtl865x_addAclToChain(rule, &(chain->head),
					   &(chain->tail));
		if (retval == SUCCESS) {
			isSynAsic = 1;
			chain->ruleCnt++;
...
#endif
		{
			...
			{
				chain =
				    _rtl865x_find_aclChain_byPriority
				    (netif->chainListHead[rule->direction_],
				     chainNo);
				if (chain == NULL)
					continue;
...
static int32 _rtl865x_del_acl(rtl865x_AclRule_t * rule, char *netifName,
			      int32 chainNo)
{
	rtl865x_netif_local_t *netif = NULL;
	rtl865x_acl_chain_t *chain = NULL;
	...
	if (netif != NULL) {
		...
		retval =
		    _rtl865x_delAclFromChain(rule, &(chain->head),
					     &(chain->tail));
		if (retval == SUCCESS) {
			isSynAsic = 1;
			chain->ruleCnt--;
...
static int32 _rtl865x_flush_allAcl_sw_fromChain(char *netifName, int32 priority,
						uint32 flag)
{
	rtl865x_netif_local_t *netif;
	rtl865x_AclRule_t *aclRule;
...
int32 rtl865x_flush_allAcl_sw_fromChain(char *netifName, int32 priority,
					uint32 flag)
{
	return _rtl865x_flush_allAcl_sw_fromChain(netifName, priority, flag);
}

static int32 _rtl865x_add_sw_acl(rtl865x_AclRule_t * rule, char *netifName,
				 int32 chainNo)
{
	rtl865x_netif_local_t *netif = NULL;
	rtl865x_acl_chain_t *chain = NULL;
...
int32 rtl865x_add_sw_acl(rtl865x_AclRule_t * rule, char *netifName,
			 int32 priority)
{
	int32 retval = FAILED;
...
rtl865x_AclRule_t *rtl865x_matched_layer4_aclChain(char *netifName,
						   int32 priority, uint32 flag,
						   rtl865x_AclRule_t * match)
{
	return _rtl865x_matched_layer4_aclChain(netifName, priority, flag,
						match);
}

rtl865x_AclRule_t *rtl865x_matched_layer2_aclChain(char *netifName,
						   int32 priority, uint32 flag,
						   rtl865x_AclRule_t * match)
{
	return _rtl865x_matched_layer2_aclChain(netifName, priority, flag,
						match);
}

rtl865x_acl_chain_t *rtl865x_find_aclChain_byPriority(rtl865x_acl_chain_t *
						      head, int32 priority)
{
	rtl865x_acl_chain_t *entry;
	entry = head;
...
int32 rtl865x_enable_acl(uint32 enable)
{
	if (enable) {
		...
		_rtl865x_setDefACLForAllNetif(RTL865X_ACLTBL_ALL_TO_CPU,
					      RTL865X_ACLTBL_ALL_TO_CPU,
					      RTL865X_ACLTBL_PERMIT_ALL,
					      RTL865X_ACLTBL_PERMIT_ALL);
	}
	return SUCCESS;
...
int rtl865x_add_pattern_acl_for_contentFilter(rtl865x_AclRule_t * rule,
					      char *netifName)
{
	uint32 i;
	...
	for (i = 0; i < RTL8651_PORT_NUMBER; i++) {
		if (vlan->memberPortMask & 1 << i) {
			if (rtl8651_setAsicPortPatternMatch
			    (i, u.pattern, 0xffffffff, 0x2) != SUCCESS)
				return FAILED;
		}
...
int rtl865x_del_pattern_acl_for_contentFilter(rtl865x_AclRule_t * rule,
					      char *netifName)
{
	union {
		char pat[4];
...
int32 rtl865x_addVirtualNetif(rtl865x_netif_t * netif)
{
	...
	/*register 2 ingress chains: system/user */
	retval =
	    rtl865x_regist_aclChain(netif->name, RTL865X_ACL_SYSTEM_USED,
				    RTL865X_ACL_INGRESS);
	retval =
	    rtl865x_regist_aclChain(netif->name, RTL865X_ACL_USER_USED,
				    RTL865X_ACL_INGRESS);
...
int32 rtl865x_config_callback_for_get_drv_netifName(int (*fun)
						    (const char *psName,
						     char *netifName))
{
	...
	case 0:
		{
			...
			int32 cnt = 0;
			retval =
			    _rtl865x_regist_aclChain(RTL_DRV_LAN_NETIF_NAME,
						     -500, RTL865X_ACL_INGRESS);
			...
			retval =
			    _rtl865x_unRegist_aclChain(RTL_DRV_LAN_NETIF_NAME,
						       -500,
						       RTL865X_ACL_INGRESS);
	...
	case 1:
		{
			...
			_rtl865x_unRegist_aclChain(RTL_DRV_LAN_NETIF_NAME, -500,
						   RTL865X_ACL_INGRESS);
			printk("now the information of netif(%s) is:\n\n",
			       netif->name);
			_rtl865x_print_allChain_allAcl(netif);
			...
			_rtl865x_unRegister_all_aclChain
	...
	case 5:
		{
			...
			for (i = 0; i < RTL8651_PORT_NUMBER; i++) {
				if (rtl8651_setAsicPortPatternMatch
				    (i, u.pattern, 0xffffffff,
				     0x2 /* fwd to CPU */ ) != SUCCESS)
					return FAILED;
			}
...
int rtl865x_show_all_netif(void)
{
	...
	for (i = 0; i < NETIF_NUMBER; i++) {
		if (netifTbl[i].valid == 1) {
			netif = &netifTbl[i];
			printk
			    ("idx(%d),valid(%d),name(%s),vid(%d),is_slave(%d),type(%d)\n",
			     i, netif->valid, netif->name, netif->vid,
			     netif->is_slave, netif->if_type);
		}
	}
...
int rtl865x_isWanNetifByIdx(int netifIdx)
{
	...
	if (netifTbl[netifIdx].valid) {
		if (strncmp
		    (netifTbl[netifIdx].name, RTL_DRV_LAN_NETIF_NAME,
		     strlen(RTL_DRV_LAN_NETIF_NAME)) == 0)
			return 0;
		else if (strncmp
			 (netifTbl[netifIdx].name, RTL_DRV_WAN0_NETIF_NAME,
			  strlen(RTL_DRV_WAN0_NETIF_NAME)) == 0)
			return 1;
	}
...
int32 sw_netif_read(struct seq_file * s, void *v)
{
...
#endif
	{
		...
		if (intf->valid) {
			...
			seq_printf(s,
				   "[%d]netif:%s	VID[%d] %x:%x:%x:%x:%x:%x",
				   i, intf->name, intf->vid, mac[0], mac[1],
			...
			if (RTL8651_ACLTBL_DROP_ALL <= intf.inAclStart) {
				if (intf.inAclStart ==
				    RTL8651_ACLTBL_PERMIT_ALL)
					seq_printf(s, "permit all,");
				if (intf.inAclStart ==
				    RTL8651_ACLTBL_ALL_TO_CPU)
...
#else
int32 sw_netif_read(char *page, char **start, off_t off, int count, int *eof,
		    void *data)
{
	int len;
	uint8 *mac;
	...
	for (i = 0; i < RTL865XC_NETIFTBL_SIZE; i++) {
		...
		if (intf->valid) {
			...
			len +=
			    sprintf(page + len,
				    "[%d]  %s	VID[%d] %x:%x:%x:%x:%x:%x", i,
				    intf->name, intf->vid, mac[0], mac[1],
				    mac[2], mac[3], mac[4], mac[5]);
			len +=
			    sprintf(page + len, "  Routing %s \n",
				    intf->enableRoute ==
				    TRUE ? "enabled" : "disabled");

			len +=
			    sprintf(page + len, "is slave %d		",
				    intf->is_slave);
			if (intf->is_slave) {
				len +=
				    sprintf(page + len, "master netif:%s",
					    intf->master->name);
			}
			...
			if (RTL8651_ACLTBL_DROP_ALL <= intf.inAclStart) {
				if (intf.inAclStart ==
				    RTL8651_ACLTBL_PERMIT_ALL)
					len +=
					    sprintf(page + len, "permit all,");
				if (intf.inAclStart ==
				    RTL8651_ACLTBL_ALL_TO_CPU)
					len +=
					    sprintf(page + len, "all to cpu,");
				if (intf.inAclStart == RTL8651_ACLTBL_DROP_ALL)
					len += sprintf(page + len, "drop all,");
			} else
				len +=
				    sprintf(page + len, "ACL %d-%d, ",
