...
__inline__ void __nano_delay__(unsigned long nsecs)
{
	...
	lo = (((nsecs * 0x000010c6) >> 12) *
	      ((__loops_per_jiffy * 100) >> 20)) / 1000;
	__delay__(lo);
}
...
void _smi_start(void)
{
	...
	_rtl865x_initGpioPin(smi_SDA, GPIO_PERI_GPIO, GPIO_DIR_OUT,
			     GPIO_INT_DISABLE);
	_rtl865x_initGpioPin(smi_SCK, GPIO_PERI_GPIO, GPIO_DIR_OUT,
			     GPIO_INT_DISABLE);
...
void _smi_readBit(rtk_uint32 bitLen, rtk_uint32 * rData)
{
	...
	_rtl865x_initGpioPin(smi_SDA, GPIO_PERI_GPIO, GPIO_DIR_IN,
			     GPIO_INT_DISABLE);
...
void _smi_stop(void)
{
	...
	_rtl865x_initGpioPin(smi_SDA, GPIO_PERI_GPIO, GPIO_DIR_IN,
			     GPIO_INT_DISABLE);
	_rtl865x_initGpioPin(smi_SCK, GPIO_PERI_GPIO, GPIO_DIR_IN,
			     GPIO_INT_DISABLE);
...
rtk_int32 smi_reset(rtk_uint32 port, rtk_uint32 pinRST)
{
	...
	res =
	    _rtl865x_initGpioPin(gpioId, GPIO_PERI_GPIO, GPIO_DIR_OUT,
				 GPIO_INT_DISABLE);
	if (res != RT_ERR_OK)
		return res;
	smi_RST = gpioId;
	...
	/* change GPIO pin to Input only */
	_rtl865x_initGpioPin(smi_RST, GPIO_PERI_GPIO, GPIO_DIR_IN,
			     GPIO_INT_DISABLE);
#endif
	return RT_ERR_OK;
}
