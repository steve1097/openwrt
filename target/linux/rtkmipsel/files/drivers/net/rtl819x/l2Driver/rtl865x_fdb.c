...
#ifdef __KERNEL__
	TBL_MEM_ALLOC(sw_FDB_Table.filterDB, rtl865x_filterDbTable_t,
		      RTL865x_FDB_NUMBER);
#else
	sw_FDB_Table.filterDB =
	    (rtl865x_filterDbTable_t *) malloc(RTL865x_FDB_NUMBER *
					       sizeof(rtl865x_filterDbTable_t));
#endif
	{			//Initial free filter database entry
		rtl865x_filterDbTableEntry_t *tempFilterDb = NULL;
#ifdef __KERNEL__
		TBL_MEM_ALLOC(tempFilterDb, rtl865x_filterDbTableEntry_t,
			      RTL8651_L2TBL_ROW);
#else
#endif
		SLIST_INIT(&sw_FDB_Table.freefdbList.filterDBentry);
		for (index = 0; index < RTL8651_L2TBL_ROW; index++)
			SLIST_INSERT_HEAD(&sw_FDB_Table.
					  freefdbList.filterDBentry,
					  &tempFilterDb[index], nextFDB);
	}
...
int32 _rtl865x_layer2_patch(void)
{
	...
	retval =
	    _rtl865x_addFilterDatabaseEntry(RTL865x_L2_TYPEII, RTL_LAN_FID,
					    &mac, FDB_TYPE_TRAPCPU, portmask,
					    TRUE, FALSE);
#if defined (CONFIG_RTL_IVL_SUPPORT)
	retval =
	    _rtl865x_addFilterDatabaseEntry(RTL865x_L2_TYPEII, RTL_WAN_FID,
					    &mac, FDB_TYPE_TRAPCPU, portmask,
					    TRUE, FALSE);
#endif
	...
	retval =
	    _rtl865x_addFilterDatabaseEntry(RTL865x_L2_TYPEII, RTL_LAN_FID,
					    &cpu_mac, FDB_TYPE_TRAPCPU, 0, TRUE,
					    FALSE);
#if defined (CONFIG_RTL_IVL_SUPPORT)
	retval =
	    _rtl865x_addFilterDatabaseEntry(RTL865x_L2_TYPEII, RTL_WAN_FID,
...
int32 _rtl865x_fdb_collect(void)
{
	...
	for (index = 0, filterDbPtr = &sw_FDB_Table.filterDB[0];
	     index < RTL865x_FDB_NUMBER; index++, filterDbPtr++) {
		for (index1 = 0; index1 < RTL8651_L2TBL_ROW; index1++)
			while (SLIST_FIRST(&(filterDbPtr->database[index1]))) {
				tempFilterDbPtr =
				    SLIST_FIRST(&
						(filterDbPtr->database
						 [index1]));
				SLIST_REMOVE_HEAD(&
						  (filterDbPtr->database
						   [index1]), nextFDB);
...
int32 rtl865x_setHWL2Table(uint32 row, uint32 column,
			   rtl865x_tblAsicDrv_l2Param_t * l2p)
{
	return (rtl8651_setAsicL2Table(row, column, l2p));
}

int32 rtl865x_getHWL2Table(uint32 row, uint32 column,
			   rtl865x_tblAsicDrv_l2Param_t * l2p)
{
...
int32 rtl_get_hw_fdb_age(uint32 fid, ether_addr_t * mac, uint32 flags)
{
	...
	for (colIdx = 0; colIdx < RTL8651_L2TBL_COLUMN; colIdx++) {
		...
		if (retval != SUCCESS || memcmp(&(L2buff.macAddr), mac, 6) != 0)
			continue;
		if (((flags & FDB_DYNAMIC) && !L2buff.isStatic)
		    || ((flags & FDB_STATIC) && L2buff.isStatic)) {
...
int32 rtl_get_hw_fdb_port(uint32 fid, ether_addr_t * mac, uint32 flags,
			  uint8 * port)
{
	int i;
	uint32 rowIdx;
...
int32 rtl865x_getPortNum(const unsigned char *addr)
{
	...
	if (rtl865x_Lookup_fdb_entry
	    (RTL_LAN_FID, macAddr, FDB_DYNAMIC, &column,
	     &fdbEntry) == SUCCESS) {
		/*find the fdb entry */
		port_num =
		    rtl865x_ConvertPortMasktoPortNum(fdbEntry.memberPortMask);
	} else {
		/*can't find */
	}
	return port_num;
...
int32 rtl865x_Lookup_fdb_entry(uint32 fid, ether_addr_t * mac, uint32 flags,
			       uint32 * col_num,
			       rtl865x_tblAsicDrv_l2Param_t * L2buff)
{
	uint32 rowIdx;
				     ...
				     uint32 portmask, uint32 type)
{
	...
	if (type != FDB_TYPE_FWD && type != FDB_TYPE_SRCBLK
	...
	retval =
	    _rtl865x_addFilterDatabaseEntry(RTL865x_L2_TYPEII, fid, mac, type,
					    portmask, TRUE, FALSE);
#else
	retval =
	    _rtl865x_addFilterDatabaseEntry(RTL865x_L2_TYPEII, fid, mac, type,
					    portmask, FALSE, FALSE);
#endif
	/*l2 unlock */
	//rtl_up(&l2_sem);
...
int32 rtl865x_addFilterDatabaseEntryExtension(uint16 fid,
					      rtl865x_filterDbTableEntry_t *
					      L2entry)
{
	int32 retval;
	//unsigned long flags=0;
	if (L2entry->process != FDB_TYPE_FWD
	    && L2entry->process != FDB_TYPE_SRCBLK
	    && L2entry->process != FDB_TYPE_TRAPCPU)
		return RTL_EINVALIDINPUT;	/* Invalid parameter */
				     ...
				     uint32 isStatic)
{
	...
	retval =
	    _rtl865x_addFilterDatabaseEntry(isStatic ==
					    TRUE ? RTL865x_L2_TYPEII :
					    RTL865x_L2_TYPEI, fid, mac, type,
					    portmask, TRUE, FALSE);
#else
	retval =
	    _rtl865x_addFilterDatabaseEntry(isStatic ==
...
int32 _rtl865x_removeFilterDatabaseEntry(uint16 fid, ether_addr_t * mac,
					 uint32 rowIdx)
{
	...
	if (SLIST_FIRST(&(fdb_t->database[rowIdx]))) {
		SLIST_FOREACH(l2entry_t, &(fdb_t->database[rowIdx]), nextFDB) {
			if (memcmp(&(l2entry_t->macAddr), mac, 6) == 0) {
				SLIST_REMOVE(&(fdb_t->database[rowIdx]),
					     l2entry_t,
				...
				rtl865x_raiseEvent(EVENT_DEL_FDB,
						   (void *)(l2entry_t));
...
int32 rtl865x_lookup_FilterDatabaseEntry(uint16 fid, ether_addr_t * mac,
					 rtl865x_filterDbTableEntry_t *
					 l2_entry)
{
	rtl865x_filterDbTable_t *fdb_t = &sw_FDB_Table.filterDB[fid];
...
int32 _rtl865x_addFilterDatabaseEntry(uint16 l2Type, uint16 fid,
				      ether_addr_t * macAddr, uint32 type,
				      uint32 portMask, uint8 auth, uint8 SrcBlk)
{
	uint32 rowIdx = 0;
	uint32 colIdx = 0;
	uint32 col_num = 0;
	...
	if (found == FALSE) {
		...
		if (flag == FALSE) {
			...
			_rtl865x_removeFilterDatabaseEntry(fid,
							   &(L2buff->macAddr),
							   rowIdx);

			/*overwrite asic entry */
			rtl8651_setAsicL2Table_Patch(rowIdx,
						     col_num,
						     macAddr,
						     toCpu,
						     SrcBlk,
		...
		SLIST_FOREACH(tmpL2, &(fdb_t->database[rowIdx]), nextFDB) {
			if (memcmp(&(tmpL2->macAddr), macAddr, 6) == 0) {
				if ((tmpL2->auth != auth) ||
					...
					SLIST_INSERT_HEAD
					    (&sw_FDB_Table.freefdbList.filterDBentry,
					     l2entry_t, nextFDB);
					break;
/*					tmpL2 ->refCount = 1;*/
				} else {
...
/*		l2entry_t ->refCount = 1;*/
		SLIST_INSERT_HEAD(&(fdb_t->database[rowIdx]), l2entry_t,
				  nextFDB);
	}
...
*/
int32 rtl865x_delFilterDatabaseEntry(uint16 l2Type, uint16 fid,
				     ether_addr_t * macAddr)
{
	int32 retval;
	unsigned long flags = 0;
...
/*	L2 lock		*/
	...
	retval =
	    _rtl865x_delFilterDatabaseEntry(RTL865x_L2_TYPEII, fid, macAddr);
/*	L2 unlock		*/
	//rtl_up(&l2_sem);
...
int32 _rtl865x_delFilterDatabaseEntry(uint16 l2Type, uint16 fid,
				      ether_addr_t * macAddr)
{
	uint32 rowIdx = 0;
	uint32 colIdx = 0;
...
int32 rtl865x_arrangeFdbEntry(const unsigned char *timeout_addr, int32 * port)
{
	...
	found =
	    rtl865x_Lookup_fdb_entry(RTL_LAN_FID, macAddr, FDB_DYNAMIC, &column,
				     &fdbEntry);
	if (found != SUCCESS) {
		if (rtl865x_lookup_FilterDatabaseEntry
		    (RTL_LAN_FID, macAddr, &l2temp_entry) == SUCCESS) {
...
void update_hw_l2table(const char *srcName, const unsigned char *addr)
{
	...
	if (memcmp(srcName, RTL_WLAN_NAME, 4) == 0) {
...
#endif
		{
			...
			if ((fdbEntry.memberPortMask & RTL8651_PHYSICALPORTMASK)
			    != 0) {
#if defined (CONFIG_RTL_LAYERED_DRIVER)
#if defined (CONFIG_RTL_LAYERED_DRIVER_L2)
				ret =
				    rtl865x_delFilterDatabaseEntry
				    (RTL865x_L2_TYPEII, RTL_LAN_FID, macAddr);
#endif
#else
				ret =
...
#if defined (CONFIG_RTL_LAYERED_DRIVER_L2)
		if (rtl865x_Lookup_fdb_entry
		    (RTL_WAN_FID, macAddr, FDB_DYNAMIC, &column,
		     &fdbEntry) == SUCCESS)
#endif
#endif
		{
...
int32 rtl865x_addFDBEntry(const unsigned char *addr)
{
	...
	if (found == SUCCESS) {
		...
		if (rtl865x_lookup_FilterDatabaseEntry
		    (fdbEntry.fid, macAddr, &l2temp_entry) != SUCCESS) {
			l2temp_entry.l2type =
			    (fdbEntry.nhFlag ==
...
/*			l2temp_entry.SrcBlk = FALSE;*/
			memcpy(&(l2temp_entry.macAddr), macAddr,
			       sizeof(ether_addr_t));
			ret =
			    _rtl865x_addFilterDatabaseEntry(l2temp_entry.l2type,
							    fdbEntry.fid,
							    &
...
int32 rtl865x_addExtFDBEntry(const unsigned char *addr, int extPort)
{
	...
	found =
	    rtl865x_lookup_FilterDatabaseEntry(RTL_LAN_FID, macAddr, &l2_entry);
	if (found == SUCCESS) {
#if 0
		port_num =
		    rtl865x_ConvertPortMasktoPortNum(fdbEntry.memberPortMask);
		...
		if (extPort == PKTHDR_EXTPORT_P3)
			ret =
			    _rtl865x_addFilterDatabaseEntry(RTL865x_L2_TYPEIII,
							    RTL_LAN_FID,
							    macAddr,
							    FDB_TYPE_FWD,
							    (1 <<
	...
	for (index = 0, filterDbPtr = &sw_FDB_Table.filterDB[0];
		...
		for (index1 = 0; index1 < RTL8651_L2TBL_ROW; index1++) {
			if (SLIST_FIRST(&(filterDbPtr->database[index1]))) {
				SLIST_FOREACH(tempFilterDbPtr,
					      &(filterDbPtr->database[index1]),
					      nextFDB) {
					if (tempFilterDbPtr->l2type == l2Type) {
						_rtl865x_delFilterDatabaseEntry
						    (tempFilterDbPtr->l2type,
						     RTL_LAN_FID,
						     &tempFilterDbPtr->macAddr);
					}
				}
			}
		}
...
int32 _rtl865x_addAuthFilterDatabaseEntry(uint16 l2Type, uint16 fid,
					  ether_addr_t * macAddr, uint32 type,
...
int32 _rtl865x_addAuthSWl2Entry(uint16 l2Type, uint16 fid,
				ether_addr_t * macAddr, uint32 type, int32 port,
				uint8 auth, uint8 SrcBlk)
{
	uint32 rowIdx = 0;
	int32 nexthp_flag = FALSE;
	rtl865x_filterDbTableEntry_t *l2entry_t = NULL;
...
int32 rtl865x_addAuthFilterDatabaseEntryExtension(uint16 fid,
						  rtl865x_filterDbTableEntry_t *
						  L2entry)
{
	int32 retval;
...
@rvalue RTL_EINVALIDINPUT | Invalid input parameter.
*/
int32 rtl865x_delAuthFilterDatabaseEntry(uint16 l2Type, uint16 fid,
					 ether_addr_t * macAddr)
{
	int32 retval;
	unsigned long flags = 0;
...
/*	L2 lock		*/
	...
	SMP_LOCK_ETH(flags);
	retval =
	    _rtl865x_delAuthFilterDatabaseEntry(RTL865x_L2_TYPEII, fid,
						macAddr);
/*	L2 unlock		*/
	//rtl_up(&l2_sem);
	SMP_UNLOCK_ETH(flags);
...
int32 _rtl865x_delAuthFilterDatabaseEntry(uint16 l2Type, uint16 fid,
					  ether_addr_t * macAddr)
{
	uint32 rowIdx = 0;
	uint32 colIdx = 0;
	...
	SLIST_FOREACH(l2entry_t, &(fdb_t->database[rowIdx]), nextFDB) {
		...
		if (!memcmp(&l2entry_t->macAddr, macAddr, sizeof(ether_addr_t))) {
			...
			SLIST_REMOVE(&(fdb_t->database[rowIdx]),
				     l2entry_t,
				     rtl865x_filterDbTableEntry_s, nextFDB);
			SLIST_INSERT_HEAD(&sw_FDB_Table.
					  freefdbList.filterDBentry, l2entry_t,
					  nextFDB);
			if (found == TRUE) {
				rtl8651_delAsicL2Table(rowIdx, colIdx);
			}
			/*printk("l2entry_t->srcblk:%d,l2entry_t->auth:%d\n", l2entry_t->SrcBlk,l2entry_t->auth); */
			if ((l2entry_t->SrcBlk == FALSE)
			    && (l2entry_t->auth == TRUE)) {
...
#if defined (CONFIG_RTL865X_LANPORT_RESTRICTION) && defined(CONFIG_RTL_LAYERED_DRIVER) && defined(CONFIG_RTL_LAYERED_DRIVER_L2)&& defined(CONFIG_RTL865X_SYNC_L2)
				{
					...
					if (port_num != FAILED) {
						...
						if ((lan_restrict_getBlockAddr
							 ...
							 */
							rtl865x_addAuthFDBEntry
							    (swap_addr, TRUE,
							     port_num, FALSE);
						}

					} else {	/*port_num==FAILED */
					}
				}
			...
			rtl865x_raiseEvent(EVENT_DEL_AUTHED_FDB,
					   (void *)(l2entry_t));
					...
					const unsigned char *macAddr)
{
	...
	for (rowIdx = 0; rowIdx < RTL8651_L2TBL_ROW; rowIdx++) {
		if (SLIST_FIRST(&(fdb_t->database[rowIdx]))) {
			...
			SLIST_FOREACH(l2entry_t, &(fdb_t->database[rowIdx]),
				      nextFDB) {
				if ((l2entry_t->memberPortMask &
				     (1 << port_num))
				    && (l2entry_t->auth == FALSE)
				    && (l2entry_t->SrcBlk == TRUE)) {

					memcpy((ether_addr_t *) macAddr,
					       &(l2entry_t->macAddr),
					       sizeof(ether_addr_t));
#if 0
					printk
...
/*			l2temp_entry.SrcBlk = FALSE;*/
			memcpy(&(l2temp_entry.macAddr), macAddr,
			       sizeof(ether_addr_t));
			ret =
			    _rtl865x_addAuthFilterDatabaseEntry
			    (l2temp_entry.l2type, fdbEntry.fid,
			     &(l2temp_entry.macAddr), l2temp_entry.process,
			     l2temp_entry.memberPortMask, auth, srcblk);

		}
#if 1
		else {
			if ((l2temp_entry.auth == FALSE)
...
int32 _rtl865x_ClearFDBEntryByPort(int32 port_num)
{
	...
	for (i = 0; i < RTL8651_L2TBL_ROW; i++)
		for (j = 0; j < RTL8651_L2TBL_COLUMN; j++) {
			...
			if ((rtl865x_ConvertPortMasktoPortNum
			     (L2buff->memberPortMask)) != port_num)
				continue;
			...
			if (name) {
				rtl_delFdbByMac((unsigned char
						 *)(&(L2buff->macAddr.octet)),
						name);
			}
#endif
			_rtl865x_removeFilterDatabaseEntry(RTL_LAN_FID,
							   &(L2buff->macAddr),
							   i);
		}
	...
	for (i = 0; i < RTL8651_MAC_NUMBER; i++) {
		...
		if ((status & PortStatusLinkUp) == FALSE) {
...
#if defined(CONFIG_RTL_HW_TX_CSUM)
			{
				...
				memset(hw_csum_cache_mac, 0,
				       sizeof(hw_csum_cache_mac));
			}
#endif
		} else {
			/* Link is up. */
...
//                                      seq_printf(s,"%s %s %s",tempFilterDbPtr->process?"CPU":"FWD", tempFilterDbPtr->l2type?"STA":"DYN",  tempFilterDbPtr->SrcBlk?"BLK":"");
					seq_printf(s, "%s ",
						   (tempFilterDbPtr->l2type !=
						    RTL865x_L2_TYPEI) ? "STA" :
						   "DYN");
					seq_printf(s, "%s ",
						   (tempFilterDbPtr->l2type ==
						    RTL865x_L2_TYPEII) ? "NH" :
						   "");

					seq_printf(s, "%s ",
						   tempFilterDbPtr->SrcBlk ?
						   "BLK" : "");

					if (tempFilterDbPtr->auth) {
						seq_printf(s, "AUTH:%d  ",
							   tempFilterDbPtr->auth);
					}
...
#else
int32 rtl865x_sw_l2_proc_read(char *page, char **start, off_t off, int count,
			      int *eof, void *data)
{
	int32 index = 0;
	int32 index1 = 0;
...
int32 rtl865x_sw_l2_proc_write(struct file * filp, const char *buff,
...
int32 rtl865x_updateAuthState(int32 auth, const unsigned char *addr)
{
	...
	if (found == SUCCESS) {
		...
		if (auth != fdbEntry.auth) {
			...
			//update sw l2
			if (SLIST_FIRST(&(fdb_t->database[rowIdx]))) {
				SLIST_FOREACH(tmpL2, &(fdb_t->database[rowIdx]),
					      nextFDB) {
					if (memcmp
					    (&(tmpL2->macAddr), macAddr,
					     6) == 0) {
						if ((tmpL2->auth != auth)) {
							tmpL2->auth = auth;
							break;
...
int32 rtl865x_isEthMac(void *mac)
{
	...
	for (i = 0; i < 2; i++) {
		...
		if (rtl865x_Lookup_fdb_entry
		    (fid, (ether_addr_t *) mac, fdb_type[i], &column,
		     &fdbEntry) != SUCCESS) {
			continue;
		}
...
int32 rtl819x_save_inused_l2_entry(ipaddr_t ip, ether_addr_t * mac, uint32 fid)
{
	...
	ret =
	    rtl865x_Lookup_fdb_entry(fid, mac, FDB_DYNAMIC, &column, &fdbEntry);
	if (ret == SUCCESS) {
		//found it , store it.
		for (i = 0; i < L2_BAKUP_ENTRY_MAX; i++) {
			//find empty entry to store
			if (!memcmp
			    (l2_entry_bak[i].macAddr.octet, empty_mac.octet,
			     ETHER_ADDR_LEN)) {
				memcpy((void *)&l2_entry_bak[i],
				       (void *)&fdbEntry, sizeof(fdbEntry));
...
int rtl819x_restore_hw_l2_table(void)
{
	...
	for (i = 0; i < L2_BAKUP_ENTRY_MAX; i++) {
		...
		if (memcmp
			...
			ret =
			    _rtl865x_addFilterDatabaseEntry((entry->nhFlag ==
							     0) ?
							    RTL865x_L2_TYPEI :
							    RTL865x_L2_TYPEII,
							    entry->fid,
...
int rtl_addFilterDatabaseEntryforMCast(unsigned char *mac,
				       unsigned int fwdPortMask)
{
	...
	macAddr.octet[4] = mac[4];
	macAddr.octet[5] = mac[5];
	return rtl865x_addFilterDatabaseEntry(0, &macAddr, fwdPortMask,
					      FDB_TYPE_FWD);
}
#endif
