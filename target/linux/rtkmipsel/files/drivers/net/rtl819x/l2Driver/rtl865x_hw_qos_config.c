...
// to_be_checked !!!
			    ...
			    ("Priority decision set to QIDDPCR register failed\n ");
					     ...
					     qos_data.queue_rate.queuemask) {
						    ...
						    rtl8651_setAsicQueueRate(i,
									     ...
									     [i]
									     [j], recv_data.qos_data.queue_rate.burst[i]
									     [j], recv_data.qos_data.queue_rate.apr[i]
									     [j]);
	...
	 rtk_nlsendmsg(pid, hw_qos_sk, sizeof(struct qos_cmd_info_s),
			 &send_data);
