...
static uint32 priority2HandleMapping[NETIF_NUMBER][TOTAL_VLAN_PRIORITY_NUM] =
    { {0} };
static rtl_qos_mark_info_t mark2Priority[NETIF_NUMBER][MAX_MARK_NUM_PER_DEV] =
    { {{0}} };
...
static uint8 vlanPri2SystemPri[TOTAL_VLAN_PRIORITY_NUM] =
    { 2, 0, 1, 3, 4, 5, 6, 7 };
#endif

int rtl_hw_qos_config_single_open(struct inode *inode, struct file *file);
static ssize_t rtl_hw_qos_config_single_write(struct file *file,
...
int32 rtl8367_qosSetIngressBandwidth(unsigned int memberPort,
				     unsigned int Kbps);

int rtl8367_qosProcessQueue(unsigned int memberPort, unsigned int queueNum,
			    unsigned int *queueFlag, unsigned int *queueId,
			    unsigned int *queueBw, unsigned int *queueWeight,
			    unsigned int wanPortMask);
...
int32 rtl865x_qosSetBandwidth(uint8 * netIfName, uint32 bps)
{
	...
	if (asicBandwidth > 0
	    && (bps & (1 << (EGRESS_BANDWIDTH_GRANULARITY_BITLEN - 1))) != 0) {
		asicBandwidth++;
	}
#if defined CONFIG_RTL_8367_QOS_SUPPORT
	...
	if (wanPortAsicBandwidth > 0
	    && (bps & (1 << (INGRESS_BANDWIDTH_GRANULARITY_BITLEN - 1))) != 0) {
...
int32 rtl_qosGetPriorityByMark(uint8 * netIfName, int32 mark)
{
	...
	for (i = 0; i < MAX_MARK_NUM_PER_DEV; i++) {
		...
		if ((mark2Priority[netifIdx][i].mark &
		     mark2Priority[netifIdx][i].mask) ==
		    (mark & mark2Priority[netifIdx][i].mask))
#else
...
#endif
{
	...
	for (i = 0; i < MAX_MARK_NUM_PER_DEV; i++) {
		...
		if ((mark2Priority[netifIdx][i].mark == mark)
			...
			if (enable == TRUE) {
				mark2Priority[netifIdx][i].priority =
				    _rtl865x_qosGetPriorityByHandle(netifIdx,
								    handler);
			} else {
#ifdef DEFAULT_PRIORITY_PATCH
				mark2Priority[netifIdx][i].mark = -1;
...
/*
	_rtl865x_compare2Entry Return Value:
		...
		=0		means entry1 = entry2
*/
static int8 _rtl865x_compare2Entry(rtl865x_qos_t * entry1,
...
#endif
int32 rtl865x_registerQosCompFunc(int8(*p_cmpFunc)
				  (rtl865x_qos_t * entry1,
				   rtl865x_qos_t * entry2))
{
	if (p_cmpFunc == NULL)
...
static int32 _rtl865x_qosArrangeQueue(rtl865x_qos_t * qosInfo)
{
	...
	/*      Process the queue type & ratio  */
	{
		...
		if ((qosInfo[0].flags & QOS_DEF_QUEUE) != 0)
			...
			divisor = qosInfo[0].bandwidth;
#endif
		for (queueNum = 0; queueNum < RTL8651_OUTPUTQUEUE_SIZE;
		     queueNum++) {
			if ((qosInfo[queueNum].flags & QOS_VALID_MASK) == 0)
				break;
			/*      Currently, we set all queue as WFQ              */
			if ((qosInfo[queueNum].flags & QOS_TYPE_MASK) ==
			    QOS_TYPE_WFQ) {
#if !defined (CONFIG_OPENWRT_SDK)
				if ((qosInfo[queueNum].flags & QOS_DEF_QUEUE) !=
				    0) {
					qosInfo[queueNum].bandwidth =
					    qosInfo[queueNum].ceil;
				}
#endif
				divisor =
				    my_gcd(qosInfo[queueNum].bandwidth,
					   divisor);
			}
		}
		...
		/*      process WFQ type ratio  */
		{
			if (divisor) {
				...
				if (maxBandwidth > EGRESS_WFQ_MAX_RATIO) {
					...
					divisor =
					    (maxBandwidth /
					     EGRESS_WFQ_MAX_RATIO)
					    +
					    ((maxBandwidth %
					      EGRESS_WFQ_MAX_RATIO) >
					     (EGRESS_WFQ_MAX_RATIO >> 1) ? 1 :
					     0);
					while (queueNum > 0) {
						if ((qosInfo[queueNum - 1].flags
						     & QOS_TYPE_MASK)
	...
	for (entry = 0; entry < RTL8651_OUTPUTQUEUE_SIZE;
	     entry++, outputQueue++) {
		...
		if ((outputQueue->flags & QOS_TYPE_MASK) == QOS_TYPE_WFQ) {
			...
			if (outputQueue->bandwidth >
			    ((WEIGHT0_MASK >> WEIGHT0_OFFSET) + 1))
				outputQueue->bandwidth =
				    (WEIGHT0_MASK >> WEIGHT0_OFFSET) + 1;
	...
	memset((void *)tmp_qosInfo, 0,
	       RTL8651_OUTPUTQUEUE_SIZE * sizeof(rtl865x_qos_t));
	...
	for (i = 0; i < mEnd; i++) {
		...
		for (j = 0; j < RTL8651_OUTPUTQUEUE_SIZE; j++) {
			...
			if (cnt == i) {
				memcpy(&tmp_qosInfo[j], &qosInfo[queueOrder[i]],
				       sizeof(rtl865x_qos_t));
				priority2HandleMapping[idx][queueMatrix
							    [queueNum - 1][j]] =
				    tmp_qosInfo[j].handle;
				tmp_qosInfo[j].queueId = j;
			}
		}
	}

	memcpy(qosInfo, tmp_qosInfo,
	       RTL8651_OUTPUTQUEUE_SIZE * sizeof(rtl865x_qos_t));
	...
	if (qosDev) {
#if !defined (CONFIG_OPENWRT_SDK)
		for (qosMarkNumIdx = 0; qosMarkNumIdx < MAX_MARK_NUM_PER_DEV;
		     qosMarkNumIdx++) {
#ifdef DEFAULT_PRIORITY_PATCH
			if (mark2Priority[idx][qosMarkNumIdx].mark != -1) {
				if (tc_getHandleByKey
				    (mark2Priority[idx][qosMarkNumIdx].mark,
				     &tmpHandle, qosDev, &tmpDev) == 0) {
					mark2Priority[idx]
					    [qosMarkNumIdx].priority =
					    _rtl865x_qosGetPriorityByHandle(idx,
									    tmpHandle);
				} else {
					mark2Priority[idx][qosMarkNumIdx].mark =
					    -1;
...
int32 rtl865x_qosProcessQueue(uint8 * netIfName, rtl865x_qos_t * qosInfo)
{
	...
	for (queue = 0; queue < RTL8651_OUTPUTQUEUE_SIZE; queue++) {
		...
		if (qosInfo[queue].ceil != 0) {
			queueBw[queue] =
			    ((qosInfo[queue].ceil) >>
			     (EGRESS_BANDWIDTH_GRANULARITY_BITLEN)) - 1;
			if ((qosInfo[queue].ceil) &
			    (1 << (EGRESS_BANDWIDTH_GRANULARITY_BITLEN - 1)))
				queueBw[queue] += 1;
	...
	rtl8367_qosProcessQueue(memberPort, queueNum, queueFlag, queueId,
				queueBw, queueWeight, RTL_WANPORT_MASK);
#else
	rtl865xC_lockSWCore();
	...
	for (port = 0; port <= CPU; port++) {
		...
		for (queue = 0; queue < RTL8651_OUTPUTQUEUE_SIZE; queue++) {
			...
			if ((qosInfo[queue].ceil) &
				...
				asicBandwidth += 1;
#if 0
			if (asicBandwidth < EGRESS_WFQ_MAX_RATIO
			    && ((qosInfo[queue].ceil << 3) &
				(1 <<
				 (EGRESS_BANDWIDTH_GRANULARITY_BITLEN - 1))) !=
			    0) {
				asicBandwidth++;
			...
			if (qosInfo[queue].bandwidth == 1) {
				//Default queue
				rtl8651_setAsicQueueRate(port,
							 qosInfo[queue].queueId,
							 PPR_MASK >> PPR_OFFSET,
							 L1_MASK >> L1_OFFSET,
							 asicBandwidth);
			} else
#endif
			if ((qosInfo[queue].flags & QOS_TYPE_MASK) ==
				    QOS_TYPE_STR) {
				rtl8651_setAsicQueueRate(port,
							 qosInfo[queue].queueId,
							 PPR_MASK >> PPR_OFFSET,
							 L1_MASK >> L1_OFFSET,
							 asicBandwidth);
				rtl8651_setAsicQueueWeight(port,
							   qosInfo
							   [queue].queueId,
							   STR_PRIO, 0);
			} else {
...
int32 rtl865x_qosArrangeRuleByNetif(void)
{
	...
	for (i = 0; i < NETIF_NUMBER; i++) {
		if (netIfNameArray[i][0] != 0) {
			rtl865x_flush_allAcl_fromChain(netIfNameArray[i],
						       RTL865X_ACL_QOS_USED0,
						       RTL865X_ACL_INGRESS);
			rtl865x_flush_allAcl_fromChain(netIfNameArray[i],
						       RTL865X_ACL_QOS_USED1,
						       RTL865X_ACL_INGRESS);
		}
	}
...
static int32 _rtl865x_qosArrangeRuleByNetif(uint8 * netIfName)
{
	...
	for (qosRule = rtl865x_qosRuleHead; qosRule; qosRule = qosRule->next) {
		...
		if (qosRule->outIfname[0] != '\0') {
			...
			tmp_idx =
			    _rtl865x_getNetifIdxByNameExt(qosRule->outIfname);
			if (tmp_idx != idx)
				continue;
		}
#if	defined(CONFIG_RTL_QOS_8021P_SUPPORT)
		if (qosRule->rule->ruleType_ == RTL865X_ACL_802D1P) {
			rtl8651_setAsicDot1qAbsolutelyPriority(qosRule->
							       rule->vlanTagPri_,
							       priority);
			continue;
		}
#endif
		{
			...
			if (qosRule->inIfname[0] != '\0') {
				...
				rtl865x_del_acl(qosRule->rule,
						qosRule->inIfname,
						RTL865X_ACL_QOS_USED0);
				rtl865x_add_acl(qosRule->rule,
						qosRule->inIfname,
						RTL865X_ACL_QOS_USED0);
#if defined (CONFIG_RTL_LOCAL_PUBLIC)
#if defined(CONFIG_RTL_PUBLIC_SSID)
				if (memcmp
				    (RTL_GW_WAN_DEVICE_NAME, qosRule->inIfname,
				     4) == 0
				    && qosRule->rule->direction_ ==
				    RTL_ACL_INGRESS)
#else
				if (memcmp
				    (RTL_DRV_WAN0_NETIF_NAME, qosRule->inIfname,
				     4) == 0
				    && qosRule->rule->direction_ ==
				    RTL_ACL_INGRESS)
#endif
				{
					rtl_checkLocalPublicNetifIngressRule
					    (qosRule->rule);
				}
#endif
			} else {
...
#ifndef DEFAULT_PRIORITY_PATCH
	{
		...
		for (i = 0; i < NETIF_NUMBER; i++) {
			if (netIfNameArray[i][0] != 0) {
				tmp_idx =
				    _rtl865x_getNetifIdxByNameExt(netIfNameArray
								  [i]);
				if (tmp_idx >= 0 && tmp_idx < NETIF_NUMBER
				    && tmp_idx != idx) {
					rtl865x_add_acl(&aclRule,
							netIfNameArray[i],
							RTL865X_ACL_QOS_USED1);
				}
			}
...
int32 rtl865x_qosCheckNaptPriority(rtl865x_AclRule_t * qosRule)
{
	...
	rule_p =
	    rtl865x_matched_layer4_aclChain(netIfNameArray[qosRule->netifIdx_],
					    RTL865X_ACL_QOS_USED0,
					    RTL865X_ACL_INGRESS, qosRule);

	if (!rule_p)
		rule_p =
		    rtl865x_matched_layer2_aclChain(netIfNameArray
						    [qosRule->netifIdx_],
						    RTL865X_ACL_QOS_USED0,
						    RTL865X_ACL_INGRESS,
						    qosRule);
...
int32 rtl865x_qosFlushMarkRuleByDev(uint8 * netIfName)
{
	...
	for (i = 0; i < NETIF_NUMBER; i++) {
		if (netIfNameArray[i][0] != 0
		    && (strcmp(netIfName, netIfNameArray[i]) != 0)) {
			_rtl865x_qosArrangeRuleByNetif(netIfNameArray[i]);
		}
	}
...
int32 rtl865x_closeQos(uint8 * netIfName)
{
	...
	for (i = 0; i < NETIF_NUMBER; i++) {
		if (netIfNameArray[i][0] != 0) {
			tmp_idx =
			    _rtl865x_getNetifIdxByNameExt(netIfNameArray[i]);
			if (tmp_idx >= 0 && tmp_idx < NETIF_NUMBER
			    && tmp_idx != idx) {
				rtl865x_flush_allAcl_fromChain(netIfNameArray
							       [i],
							       RTL865X_ACL_QOS_USED0,
							       RTL865X_ACL_INGRESS);
				rtl865x_flush_allAcl_fromChain(netIfNameArray
	...
	defPriority[idx] = 0;
	queueNumber[idx] = 0;
	memset(priority2HandleMapping[idx], 0,
	       TOTAL_VLAN_PRIORITY_NUM * sizeof(int));
	for (i = 0; i < MAX_MARK_NUM_PER_DEV; i++)
#ifdef DEFAULT_PRIORITY_PATCH
	...
	for (port = 0; port <= CPU; port++) {
		...
		for (i = 0; i < RTL8651_OUTPUTQUEUE_SIZE; i++) {
			...
			rtl8651_setAsicQueueRate(port, i,
						 PPR_MASK >> PPR_OFFSET,
						 L1_MASK >> L1_OFFSET,
						 APR_MASK >> APR_OFFSET);
		}
...
int __init rtl865x_initOutputQueue(uint8 ** netIfName)
{
	...
	for (i = 0; i < RTL8651_OUTPUTQUEUE_SIZE; i++) {
		for (j = 0; j < TOTAL_VLAN_PRIORITY_NUM; j++)
			rtl8651_setAsicPriorityToQIDMappingTable(i + 1, j,
								 priorityMatrix
								 [i][j]);
	}
#if 0
	for (i = CPU; i <= MULTEXT; i++) {
		...
		for (j = 0; j < TOTAL_VLAN_PRIORITY_NUM; j++)
			if (j < 4)
				rtl8651_setAsicCPUPriorityToQIDMappingTable(i,
									    j,
									    0);
			else
				rtl8651_setAsicCPUPriorityToQIDMappingTable(i,
	...
	rtl8651_setAsicPriorityDecision(priorityDecisionArray[PORT_BASE],
					priorityDecisionArray[D1P_BASE],
					priorityDecisionArray[DSCP_BASE],
					priorityDecisionArray[ACL_BASE],
					priorityDecisionArray[NAT_BASE],
	...
	for (i = 0; i < RTL8651_OUTPUTQUEUE_SIZE; i++) {
		for (j = PHY0; j <= CPU; j++) {
			 ...
			 * 2011-04-02, zj
			 */
			if (rtl8651_setAsicQueueFlowControlConfigureRegister
			    (j, i, TRUE) != SUCCESS) {
				QOS_DEBUGP
				    ("Set Queue Flow Control Para Error.\n");
			}
		}
	}
	...
	for (i = 0; i < NETIF_NUMBER; i++) {
		if (netIfNameArray[i][0] != 0) {
			rtl865x_regist_aclChain(netIfNameArray[i],
						RTL865X_ACL_QOS_USED0,
						RTL865X_ACL_INGRESS);
			rtl865x_regist_aclChain(netIfNameArray[i],
						RTL865X_ACL_QOS_USED1,
	...
	memset(priority2HandleMapping, 0,
	       NETIF_NUMBER * TOTAL_VLAN_PRIORITY_NUM * sizeof(int));
#ifdef DEFAULT_PRIORITY_PATCH
	for (i = 0; i < NETIF_NUMBER; i++) {
...
#else
	memset(mark2Priority, 0,
	       NETIF_NUMBER * MAX_MARK_NUM_PER_DEV *
	       sizeof(rtl_qos_mark_info_t));
#endif
	...
	proc_create_data("rtl_hw_qos_config", 0, &proc_root,
			 &rtl_hw_qos_config_proc_fops, NULL);
#else
	rtl_hw_qos_config_entry =
	    create_proc_entry("rtl_hw_qos_config", 0, NULL);
...
void __exit rtl865x_exitOutputQueue(void)
{
	...
	for (i = 0; i < NETIF_NUMBER; i++) {
		if (netIfNameArray[i][0] != 0) {
			rtl865x_unRegist_aclChain(netIfNameArray[i],
						  RTL865X_ACL_QOS_USED0,
						  RTL865X_ACL_INGRESS);
			rtl865x_unRegist_aclChain(netIfNameArray[i],
						  RTL865X_ACL_QOS_USED1,
...
void rtl865x_HwQosProcess(struct net_device *dev, int flag)
{
	...
	if (flag) {
		...
		if ((dev == NULL) || (netif_index < 0) || (netIf == NULL)
		    || (vlan == NULL)) {
			rtl865xC_lockSWCore();
			/* qos enabled */
			for (i = 0; i < NETIF_NUMBER; i++) {
			...
			for (port = PHY0; port <= CPU; port++) {
				...
				for (queue = QUEUE0; queue <= MAX_QUEUE;
				     queue++) {
					rtl8651_setAsicQueueWeight(port, queue,
								   STR_PRIO, 0);
					rtl8651_setAsicQueueRate(port, queue,
								 PPR_MASK >>
								 PPR_OFFSET,
								 L1_MASK >>
								 L1_OFFSET,
		...
		if (dev == NULL) {
			for (i = 0; i < NETIF_NUMBER; i++) {
				if (netIfNameArray[i][0] != 0) {
					rtl865x_unRegist_aclChain(netIfNameArray
								  [i],
								  RTL865X_ACL_QOS_USED0,
								  RTL865X_ACL_INGRESS);
					rtl865x_unRegist_aclChain(netIfNameArray
								  ...
								  RTL865X_ACL_QOS_USED1,
								  RTL865X_ACL_INGRESS);
					rtl865x_qosFlushBandwidth(netIfNameArray
								  [i]);
					rtl865x_closeQos(netIfNameArray[i]);
				}
			}
...
void rtl865x_reinitOutputQueuePatchForQoS(uint32 qosEnabled)
{
	...
	if (qosEnabled == TRUE) {
		...
		for (port = PHY0; port <= CPU; port++) {
			rtl8651_setAsicOutputQueueNumber(port,
							 MAX_QOS_PATCH_QUEUE_NUM);
	...
	for (qosRule = rtl865x_qosRuleHead; qosRule; qosRule = qosRule->next) {
		...
		case RTL865X_ACL_MAC:
			...
			seq_printf(s,
				   ...
				   rule->dstMac_.octet[0],
				   rule->dstMac_.octet[1],
				   rule->dstMac_.octet[2],
				   rule->dstMac_.octet[3],
				   rule->dstMac_.octet[4],
				   rule->dstMac_.octet[5],
				   rule->dstMacMask_.octet[0],
				   rule->dstMacMask_.octet[1],
				   rule->dstMacMask_.octet[2],
				   rule->dstMacMask_.octet[3],
				   rule->dstMacMask_.octet[4],
				   rule->dstMacMask_.octet[5]
			    );

			seq_printf(s,
				   "\tSMAC: %x:%x:%x:%x:%x:%x  SMACM: %x:%x:%x:%x:%x:%x\n",
				   rule->srcMac_.octet[0],
				   rule->srcMac_.octet[1],
				   rule->srcMac_.octet[2],
				   rule->srcMac_.octet[3],
				   rule->srcMac_.octet[4],
				   rule->srcMac_.octet[5],
				   rule->srcMacMask_.octet[0],
				   rule->srcMacMask_.octet[1],
				   rule->srcMacMask_.octet[2],
				   rule->srcMacMask_.octet[3],
				   rule->srcMacMask_.octet[4],
				   rule->srcMacMask_.octet[5]
			    );
			break;
		...
		case RTL865X_ACL_IP:
			...
			seq_printf(s, "\tdip: %d.%d.%d.%d dipM: %d.%d.%d.%d\n",
				   ...
				   (rule->dstIpAddr_ & 0xff),
				   (rule->dstIpAddrMask_ >> 24),
			...
			seq_printf(s, "\tsip: %d.%d.%d.%d sipM: %d.%d.%d.%d\n",
				   ...
				   (rule->srcIpAddr_ & 0xff),
				   (rule->srcIpAddrMask_ >> 24),
			...
			seq_printf(s,
				   ...
				   rule->ipProtoMask_, rule->ipFlag_,
				   rule->ipFlagMask_);

			seq_printf(s,
				   "\t<FOP:%x> <FOM:%x> <http:%x> <httpM:%x> <IdentSdip:%x> <IdentSdipM:%x> \n",
				   rule->ipFOP_, rule->ipFOM_,
				   rule->ipHttpFilter_, rule->ipHttpFilterM_,
				   rule->ipIdentSrcDstIp_,
		...
		case RTL865X_ACL_ICMP:
			...
			seq_printf(s,
				   ...
				   rule->icmpTypeMask_, rule->icmpCode_,
				   rule->icmpCodeMask_);
			break;
		case RTL865X_ACL_ICMP_IPRANGE:
			seq_printf(s, " [%d] rule type: %s   rule action: %s\n",
				   rule->aclIdx, "ICMP IP RANGE",
				   actionT[rule->actionType_]);
			seq_printf(s, "\tdipU: %d.%d.%d.%d dipL: %d.%d.%d.%d\n",
				   (rule->dstIpAddr_ >> 24),
				   ((rule->dstIpAddr_ & 0x00ff0000) >> 16),
		...
		case RTL865X_ACL_TCP:
			...
			seq_printf(s,
				   ...
				   rule->tcpSrcPortLB_, rule->tcpSrcPortUB_,
				   rule->tcpDstPortLB_, rule->tcpDstPortUB_);
			seq_printf(s,
				   "\tflag: %x  flagM: %x  <URG:%x> <ACK:%x> <PSH:%x> <RST:%x> <SYN:%x> <FIN:%x>\n",
				   rule->tcpFlag_, rule->tcpFlagMask_,
				   rule->tcpURG_, rule->tcpACK_, rule->tcpPSH_,
				   rule->tcpRST_, rule->tcpSYN_, rule->tcpFIN_);
			break;
		case RTL865X_ACL_TCP_IPRANGE:
			seq_printf(s, " [%d] rule type: %s   rule action: %s\n",
				   rule->aclIdx, "TCP IP RANGE",
				   actionT[rule->actionType_]);
			seq_printf(s, "\tdipU: %d.%d.%d.%d dipL: %d.%d.%d.%d\n",
				   (rule->dstIpAddr_ >> 24),
				   ((rule->dstIpAddr_ & 0x00ff0000) >> 16),
		...
		case RTL865X_ACL_SRCFILTER:
			...
			seq_printf(s,
				   ...
				   rule->srcFilterVlanIdx_,
				   rule->srcFilterVlanIdxMask_,
				   rule->srcFilterPort_,
				   rule->srcFilterPortMask_,
				   (rule->srcFilterIgnoreL3L4_ ==
				    TRUE ? 2 : (rule->srcFilterIgnoreL4_ ==
						1 ? 1 : 0))
			    );
			seq_printf(s,
				   "\tsip: %d.%d.%d.%d   sipM: %d.%d.%d.%d\n",
				   (rule->srcFilterIpAddr_ >> 24),
				   ((rule->srcFilterIpAddr_ & 0x00ff0000) >>
				    16),
				   ((rule->srcFilterIpAddr_ & 0x0000ff00) >> 8),
				   (rule->srcFilterIpAddr_ & 0xff),
				   (rule->srcFilterIpAddrMask_ >> 24),
				   ((rule->srcFilterIpAddrMask_ & 0x00ff0000) >>
				    16),
				   ((rule->srcFilterIpAddrMask_ & 0x0000ff00) >>
				    8), (rule->srcFilterIpAddrMask_ & 0xff)
			    );
		...
		case RTL865X_ACL_SRCFILTER_IPRANGE:
			seq_printf(s, " [%d] rule type: %s   rule action: %s\n",
				   rule->aclIdx, "Source Filter(IP RANGE)",
				   actionT[rule->actionType_]);
			seq_printf(s,
				   "\tSMAC: %x:%x:%x:%x:%x:%x  SMACM: %x:%x:%x:%x:%x:%x\n",
				   rule->srcFilterMac_.octet[0],
		...
		case RTL865X_ACL_DSTFILTER:
			seq_printf(s, " [%d] rule type: %s   rule action: %s\n",
				   rule->aclIdx, "Deatination Filter",
				   actionT[rule->actionType_]);
			seq_printf(s,
				   "\tDMAC: %x:%x:%x:%x:%x:%x  DMACM: %x:%x:%x:%x:%x:%x\n",
			...
			seq_printf(s,
				   ...
				   rule->dstFilterVlanIdx_,
				   rule->dstFilterVlanIdxMask_,
				   (rule->dstFilterIgnoreL3L4_ ==
				    TRUE ? 2 : (rule->dstFilterIgnoreL4_ ==
						1 ? 1 : 0)),
			...
			seq_printf(s,
				   ...
				   ((rule->dstFilterIpAddr_ & 0x00ff0000) >>
				    16),
				   ((rule->dstFilterIpAddr_ & 0x0000ff00) >> 8),
				   (rule->dstFilterIpAddr_ & 0xff),
				   (rule->dstFilterIpAddrMask_ >> 24),
				   ((rule->dstFilterIpAddrMask_ & 0x00ff0000) >>
				    16),
				   ((rule->dstFilterIpAddrMask_ & 0x0000ff00) >>
				    8), (rule->dstFilterIpAddrMask_ & 0xff)
			    );
			break;
		case RTL865X_ACL_DSTFILTER_IPRANGE:
			seq_printf(s, " [%d] rule type: %s   rule action: %s\n",
				   rule->aclIdx, "Deatination Filter(IP Range)",
				   actionT[rule->actionType_]);
			seq_printf(s,
				   "\tDMAC: %x:%x:%x:%x:%x:%x  DMACM: %x:%x:%x:%x:%x:%x\n",
				   rule->dstFilterMac_.octet[0],
		...
		seq_printf(s, "InDev: %s	OutDev: %s\n",
			   qosRule->inIfname[0] ==
			   0 ? "NULL" : qosRule->inIfname,
			   qosRule->outIfname[0] ==
			   0 ? "NULL" : qosRule->outIfname);
	...
	for (port = 0; port <= CPU; port++) {
		...
		for (qosMarkNumIdx = 0; qosMarkNumIdx < MAX_MARK_NUM_PER_DEV;
			...
			if (mark2Priority[netifIdx][qosMarkNumIdx].mark) {
				if (mark2Priority[netifIdx]
				    [qosMarkNumIdx].dscpRemark != 0xff)
					rtl8651_setAsicDscpRemark(port,
								  mark2Priority
								  [netifIdx]
								  [qosMarkNumIdx].priority, mark2Priority[netifIdx]
								  [qosMarkNumIdx].dscpRemark);

				if (mark2Priority[netifIdx]
				    [qosMarkNumIdx].vlanpriRemark != 0xff)
					rtl8651_setAsicVlanRemark(port,
								  mark2Priority
								  [netifIdx]
								  [qosMarkNumIdx].priority, mark2Priority[netifIdx]
								  [qosMarkNumIdx].vlanpriRemark);
			}
		}
	}
	return SUCCESS;
...
int rtl865x_setRemarkByMark(uint8 * netIfName, unsigned int markValue,
			    int remarkDscp, int remark8021p)
{
	rtl865x_netif_local_t *netIf;
	int qosMarkNumIdx;
	int netifIdx;
	...
	for (qosMarkNumIdx = 0; qosMarkNumIdx < MAX_MARK_NUM_PER_DEV;
		...
		if (mark2Priority[netifIdx][qosMarkNumIdx].mark == markValue) {
			if (remarkDscp != -1)
				mark2Priority[netifIdx]
				    [qosMarkNumIdx].dscpRemark = remarkDscp;
			if (remark8021p != -1)
				mark2Priority[netifIdx]
				    [qosMarkNumIdx].vlanpriRemark = remark8021p;
		}
	}
...
#if defined(CONFIG_RTL_HW_QOS_BRIDGE_FWD_SUPPORT)
static int rtl865x_setPortPriorityByClassId(int port, int netifIdx,
...
static int rtl865x_setVlanPriorityByClassId(int vlanPri, int netifIdx,
					    unsigned int classId)
{
	int priority = 0;
...
static int rtl865x_setDscpPriorityByClassId(int dscp, int netifIdx,
					    unsigned int classId)
{
	int priority = 0;
...
#else
static int32 rtl_hw_qos_config_read(char *page, char **start, off_t off,
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < RTL8651_PORT_NUMBER; j++) {
			if (classId2Port[i][j] != 0) {
				len +=
				    sprintf(page + len,
					    "\t\tPort[%d] mapping to classId[0x%x]\n",
					    j, classId2Port[i][j]);
			}
		}
		...
		for (j = 0; j < TOTAL_VLAN_PRIORITY_NUM; j++) {
			if (classId2VlanPriority[i][j] != 0) {
				len +=
				    sprintf(page + len,
					    "\t\tVlan Priority[%d] mapping to classId[0x%x]\n",
		...
		for (j = 0; j < TOTAL_DSCP_NUM; j++) {
			if (classId2Dscp[i][j] != 0) {
				len +=
				    sprintf(page + len,
					    "\t\tDscp[%d] mapping to classId[0x%x]\n",
					    j, classId2Dscp[i][j]);
			}
		}
#endif
...
*/
static int32 rtl_hw_qos_config_write(struct file *filp, const char *buff,
		...
		/*echo "add port 1 classId 1000 eth1" > /proc/rtl_hw_qos_config */
			...
			if (strncmp(cmdptr, "port", 4) == 0) {
				...
				if (strncmp(cmdptr, "classId", 7) == 0) {
					...
					if (strncmp
					    (cmdptr, RTL_DRV_LAN_NETIF_NAME,
					     strlen(RTL_DRV_LAN_NETIF_NAME)) ==
					    0)
						netifIdx =
						    _rtl865x_getNetifIdxByNameExt
					...
					rtl865x_setPortPriorityByClassId(tmp,
									 netifIdx,
									 classId);
				}
				...
				if (strncmp(cmdptr, "classId", 7) == 0) {
					...
					classId2VlanPriority[netifIdx][tmp] =
					    classId;
					rtl865x_setVlanPriorityByClassId(tmp,
									 netifIdx,
									 classId);
				}
				...
				if (strncmp(cmdptr, "classId", 7) == 0) {
					...
					rtl865x_setDscpPriorityByClassId(tmp,
									 netifIdx,
									 classId);
				}
			} else
			...
			if (strncmp(cmdptr, "port", 4) == 0) {
				...
				if (strncmp(cmdptr, "classId", 7) == 0) {
					...
					if (classId2VlanPriority[netifIdx][tmp]
					    == classId) {
						classId2VlanPriority[netifIdx]
						    [tmp] = 0;
			...
			if (strncmp(cmdptr, "port", 4) == 0) {
				...
				for (i = 0; i < NETIF_NUMBER; i++) {
					for (j = 0; j < TOTAL_VLAN_PRIORITY_NUM;
					     j++) {
						classId2VlanPriority[i][j] = 0;
					}
				}
