...
static int32 rtl8198c_ipv6_arp_callbackFn_for_del_fdb(void *param)
{
	...
	for (i = 0; i < RTL8198C_IPV6_ARPTBL_SIZE; i++) {
		...
		if ((ipv6ArpTables.mappings[i].valid == 1)
		    && memcmp(&(fdbEntry->macAddr),
			      &(ipv6ArpTables.mappings[i].mac), 6) == 0)
...
int32 rtl8198c_ipv6_arp_init(void)
{
	...
	memset(ipv6ArpTables.mappings, 0,
	       RTL8198C_IPV6_ARPTBL_SIZE *
	       sizeof(rtl8198c_ipv6_arpMapping_entry_t));
...
int32 rtl8198c_getIpv6ArpMapping(inv6_addr_t ip,
				 rtl8198c_ipv6_arpMapping_entry_t * arp_mapping)
{
	...
	if ((route->valid != 1) || (route->process != 2)
	    || (route->dstNetif == NULL))
		return FAILED;
	...
	for (hash_low = 0; hash_low < 4; hash_low++) {
		...
		if ((memcmp
		     (&ipv6ArpTables.mappings[hash].ip, &ip,
		      sizeof(inv6_addr_t)) == 0)
		    && (ipv6ArpTables.mappings[hash].subnetIdx ==
			route->arp.subnetIdx)) {
			memcpy(arp_mapping, &(ipv6ArpTables.mappings[hash]),
			       sizeof(rtl8198c_ipv6_arpMapping_entry_t));
			return SUCCESS;
		}
	}
...
static int32 rtl8198c_getIpv6ArpFid(inv6_addr_t ip, uint16 * fid)
{
	...
	if ((ip.v6_addr32[0] == 0) && (ip.v6_addr32[1] == 0)
	    && (ip.v6_addr32[2] == 0) && (ip.v6_addr32[3] == 0))
		return FAILED;
...
int32 rtl8198c_isIpv6EthArp(inv6_addr_t ip)
{
	...
	for (hash_low = 0; hash_low < 4; hash_low++) {
		hash = hash_high << 2 | hash_low;
		if ((ipv6ArpTables.mappings[hash].valid == 1)
		    && (ipv6ArpTables.mappings[hash].subnetIdx ==
			route->arp.subnetIdx)
		    &&
		    (memcmp
...
int32 rtl8198c_addIpv6Arp(inv6_addr_t ip, ether_addr_t * mac)
{
...
#if 0
	printk
	    ("%s.%d.ipAddr(0x%4x%4x%4x%4x%4x%4x%4x%4x),mac(%02x:%02x:%02x:%02x:%02x:%02x)****\n",
	     __FUNCTION__, __LINE__, ip.v6_addr16[0], ip.v6_addr16[1],
	     ip.v6_addr16[2], ip.v6_addr16[3], ip.v6_addr16[4], ip.v6_addr16[5],
	     ip.v6_addr16[6], ip.v6_addr16[7], mac->octet[0], mac->octet[1],
	...
	for (hash_low = 0; hash_low < 4; hash_low++) {
		...
		if ((ipv6ArpTables.mappings[hash].valid == 1)
		    ...
		    (memcmp
		     (&ipv6ArpTables.mappings[hash].ip, &ip,
		      sizeof(inv6_addr_t)) == 0)) {
			if (memcmp(&ipv6ArpTables.mappings[hash].mac, mac, 6) !=
			    0) {
				/*should clear old arp mapping before delete fdb entry and raise arp event */
				memcpy(&oldArpMapping,
				       &(ipv6ArpTables.mappings[hash]),
				       sizeof
				       (rtl8198c_ipv6_arpMapping_entry_t));
				rtl8198C_delAsicArpV6(hash);
				memset(&(ipv6ArpTables.mappings[hash]), 0,
				       sizeof
				       (rtl8198c_ipv6_arpMapping_entry_t));

				if (rtl8198c_getIpv6ArpFid
				    (oldArpMapping.ip, &fid) == SUCCESS)
					rtl865x_delFilterDatabaseEntry
					    (RTL865x_L2_TYPEI |
					     RTL865x_L2_TYPEII, fid,
					     &oldArpMapping.mac);

				rtl865x_raiseEvent(EVENT_DEL_IPV6_ARP,
						   (void *)(&oldArpMapping));
			} else {
				hash_found = hash;
			}
	...
	for (i = 0; i < 2; i++) {
		...
		printk("%s:%d\n,fid(%d),mac(%02x:%02x:%02x:%02x:%02x:%02x)\n",
		       __FUNCTION__, __LINE__, fid, mac->octet[0],
		       mac->octet[1], mac->octet[2], mac->octet[3],
		...
		memcpy(&(ipv6ArpTables.mappings[hash]), &newArpMapping,
		       sizeof(rtl8198c_ipv6_arpMapping_entry_t));
		ipv6ArpTables.mappings[hash].valid = 1;
		rtl865x_raiseEvent(EVENT_ADD_IPV6_ARP,
				   (void *)(&newArpMapping));
	...
	for (hash_low = 0; hash_low < 4; hash_low++) {
		...
		if ((ipv6ArpTables.mappings[hash].valid == 1)
		    ...
		    (memcmp
		     (&ipv6ArpTables.mappings[hash].ip, &ip,
		      sizeof(inv6_addr_t)) == 0)) {
			memcpy(&arpMapping, &ipv6ArpTables.mappings[hash],
			       sizeof(rtl8198c_ipv6_arpMapping_entry_t));
...
int32 rtl8198c_delIpv6ArpBySubnetIdx(uint32 subnetIdx)
{
	...
	for (i = 0; i < RTL8198C_IPV6_ARPTBL_SIZE; i++) {
		if ((ipv6ArpTables.mappings[i].valid == 1)
		    && (ipv6ArpTables.mappings[i].subnetIdx == subnetIdx)) {
			if (rtl8198c_getIpv6ArpFid
			    (ipv6ArpTables.mappings[i].ip, &fid) != SUCCESS)
				continue;

			rtl8198C_delAsicArpV6(i);
			rtl865x_delFilterDatabaseEntry(RTL865x_L2_TYPEII, fid,
						       &(ipv6ArpTables.mappings
							 [i].mac));
			memset(&ipv6ArpTables.mappings[i], 0,
			       sizeof(rtl8198c_ipv6_arpMapping_entry_t));
		}
	}
...
uint32 rtl8198c_ipv6ArpSync(inv6_addr_t ip, uint32 refresh)
{
	...
	if (rtl865x_getHWL2Table
	    (asicArpEntry.nextHopRow, asicArpEntry.nextHopColumn,
	     &l2entry) != SUCCESS)
	...
	age = l2entry.ageSec;
	if (refresh) {
		rtl865x_refleshHWL2Table(&(l2entry.macAddr),
					 FDB_DYNAMIC | FDB_STATIC, l2entry.fid);
		age = 150;
	} else {
		if (age >= 300) {
			age = age - 150;
...
delete_and_out:
	...
	if (rtl8198c_getIpv6ArpFid(ip, &fid) == SUCCESS) {
		...
		rtl865x_delFilterDatabaseEntry(RTL865x_L2_TYPEI |
					       RTL865x_L2_TYPEII, fid,
					       &arpMapping.mac);
...
int32 rtl8198c_changeIpv6ArpSubnetIdx(uint32 old_subnetIdx,
				      uint32 new_subnetIdx)
{
	...
	for (i = 0; i < RTL8198C_IPV6_ARPTBL_SIZE; i++) {
		if ((ipv6ArpTables.mappings[i].valid == 1)
		    && (ipv6ArpTables.mappings[i].subnetIdx == old_subnetIdx)) {
			if (rtl8198C_getAsicArpV6(i, &asicArpEntry) != SUCCESS) {
				printk
				    ("%s[%d], Waring ipv6 arp sw tables is not the same with hw table!\n",
				     __FUNCTION__, __LINE__);
				continue;
			}
...
int32 sw_arp6_read(struct seq_file * s, void *v)
{
	...
	for (i = 0; i < RTL8651_ARPTBL_SIZE; i++) {
		...
		if (arpEntry->valid == 0)
			continue;
		seq_printf(s,
			   "  [%d]  %d,ip:%x:%x:%x:%x,mac:%x-%x-%x-%x-%x-%x \n",
			   i, arpEntry->subnetIdx, arpEntry->ip.v6_addr32[0],
			   arpEntry->ip.v6_addr32[1], arpEntry->ip.v6_addr32[2],
			   arpEntry->ip.v6_addr32[3], arpEntry->mac.octet[0],
			   arpEntry->mac.octet[1], arpEntry->mac.octet[2],
			   arpEntry->mac.octet[3], arpEntry->mac.octet[4],
			   arpEntry->mac.octet[5]);
	}

	return 0;
}
#else
int32 sw_arp6_read(char *page, char **start, off_t off, int count, int *eof,
		   void *data)
{
	int len;
	rtl8198c_ipv6_arpMapping_entry_t *arpEntry = NULL;
	...
	for (i = 0; i < RTL8651_ARPTBL_SIZE; i++) {
		...
		len +=
		    sprintf(page + len,
			    "  [%d]  %d,ip:%x:%x:%x:%x,mac:%x-%x-%x-%x-%x-%x \n",
			    i, arpEntry->subnetIdx, arpEntry->ip.v6_addr32[0],
			    arpEntry->ip.v6_addr32[1],
			    arpEntry->ip.v6_addr32[2],
			    arpEntry->ip.v6_addr32[3], arpEntry->mac.octet[0],
			    arpEntry->mac.octet[1], arpEntry->mac.octet[2],
			    arpEntry->mac.octet[3], arpEntry->mac.octet[4],
			    arpEntry->mac.octet[5]);
...
int32 sw_arp6_write(struct file * filp, const char *buff, unsigned long len,
		    void *data)
{
	return len;
}
...
{
	memcpy(&ipv6ArpTablesBack, &ipv6ArpTables,
	       sizeof(struct rtl8198c_ipv6_arp_table));
...
int rtl819x_restore_hw_ipv6_arp_table(void)
{
	...
	for (i = 0; i < RTL8198C_IPV6_ARPTBL_SIZE; i++) {
		if ((ipv6ArpTablesBack.mappings[i].ip.v6_addr32[0]
		     && ipv6ArpTablesBack.mappings[i].ip.v6_addr32[1]
		     && ipv6ArpTablesBack.mappings[i].ip.v6_addr32[2]
		     && ipv6ArpTablesBack.mappings[i].ip.v6_addr32[3])
		    || memcmp(ipv6ArpTablesBack.mappings[i].mac.octet,
			      empty_mac.octet, ETHER_ADDR_LEN) != 0) {
			ret =
			    rtl8198c_addIpv6Arp(ipv6ArpTablesBack.
						mappings[i].ip,
						&ipv6ArpTablesBack.
						mappings[i].mac);
		}
