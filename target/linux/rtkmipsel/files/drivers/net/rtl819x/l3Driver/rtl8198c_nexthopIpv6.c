...
static void _print_softNxtHop(void) 
{
		    ...
		    ("idx(%d): valid(%d),dst(%s),nexthop(0x%x%x%x%x),nexthopType(%d),refCnt(%d)\n",
		     ...
		     entry->un.nexthop.v6_addr32[0],
		     entry->un.nexthop.v6_addr32[1],
		     entry->un.nexthop.v6_addr32[2],
		     entry->un.nexthop.v6_addr32[3], entry->nextHopType,
		     entry->refCnt);
...
{
	TBL_MEM_ALLOC(rtl865x_ipv6_nxtHopTable, rtl8198c_ipv6_nextHopEntry_t,
		       IPV6_NXTHOP_ENTRY_NUM);
	memset(rtl865x_ipv6_nxtHopTable, 0,
...
 static int32 _rtl8198c_synIpv6NxtHopToAsic(rtl8198c_ipv6_nextHopEntry_t *
					      entry_t) 
{
			...
			|| (entry_t->un.nexthop.v6_addr32[2] != 0)
			|| (entry_t->un.nexthop.v6_addr32[3] != 0))
			ret_arpFound =
			    rtl8198c_getIpv6ArpMapping(entry_t->un.nexthop,
						       &arp_t);
		 rtl865x_getVlanFilterDatabaseId(entry_t->dstNetif->vid,
						   &fid);
...
 static int32 _rtl8198c_addIpv6NxtHop(uint32 attr, void *ref_ptr,
...
extern int32 _rtl865x_eventHandle_addIpv6Arp(rtl8198c_ipv6_arpMapping_entry_t *
					     arp);
extern int32 _rtl865x_eventHandle_delIpv6Arp(rtl8198c_ipv6_arpMapping_entry_t *
					      arp);
