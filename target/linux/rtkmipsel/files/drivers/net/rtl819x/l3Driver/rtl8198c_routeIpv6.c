...
 static rtl8198c_ipv6_route_t *_rtl8198c_getIpv6RouteEntry(inv6_addr_t ipAddr,
							     int fc_len) 
{
...
 static int32 _rtl8198c_updateDefaultIpv6Route(rtl8198c_ipv6_route_t * rt,
						 int32 action) 
{
...
 static int32 _rtl8198c_arrangeIpv6Route(rtl8198c_ipv6_route_t * start_rt,
					   int32 start_idx) 
{
			    ...
			    if (rt->fc_dst_len == 0) {
				    ...
				    if (rt->process == IPV6_RT_ARP) {
							...
							rtl8198c_changeIpv6ArpSubnetIdx
							    (rt->arp.subnetIdx,
							     rt->asicIdx);
...
 static int32 _rtl8198c_delIpv6RouteFromInusedList(rtl8198c_ipv6_route_t *
						     rt) 
{
	int32 retval, start_idx;
	rtl8198c_ipv6_route_t * fore_rt = NULL, *entry = NULL, *start_rt =
	    NULL;
	    ...
	    if (fore_rt == NULL) {
		rtl8198c_ipv6_route_inusedHead =
			    ...
			    int line) 
{
		...
		ipAddr.v6_addr32[0], ipAddr.v6_addr32[1], ipAddr.v6_addr32[2],
...
 static rtl8198c_ipv6_route_t *_rtl8198c_getNewRouteEntry(inv6_addr_t ipAddr,
							    int fc_dst_len,
							    inv6_addr_t nextHop,
...
static int32 _rtl8198c_addIpv6Route(inv6_addr_t ipAddr, int fc_dst_len,
				     inv6_addr_t nextHop, int8 * ifName) 
{
	    ...
	    ("%s.%d.ipAddr(0x%8x%8x%8x%8x),fc_dst_len(%d)nextHop(0x%8x%8x%8x%8x),ifName(%s)\n",
	     __FUNCTION__, __LINE__, ipAddr.v6_addr32[0], ipAddr.v6_addr32[1],
	     ipAddr.v6_addr32[2], ipAddr.v6_addr32[3], fc_dst_len,
	     nextHop.v6_addr32[0], nextHop.v6_addr32[1], nextHop.v6_addr32[2],
	     nextHop.v6_addr32[3], ifName);
		 ...
		 && ((ipAddr.v6_addr32[0] >> 16) != 0x0 && (ipAddr.v6_addr32[0] >> 16) != 0xFE80))	//global ip
	{
		    ...
		    //printk("%s:%d ifName=%s\n",__FUNCTION__,__LINE__,ifName);
					...
					(&ifaAddr,
					 (struct in6_addr *)&rt->ipAddr,
					 rt->fc_dst_len)) {
					rtl8198c_ipv6_route_t * rt_pppGlo =
					    NULL;
	    ...
	    if (netif->if_type == IF_6RD)
		      ...
		      && (ipAddr.v6_addr32[2] == 0)
		      && (ipAddr.v6_addr32[3] ==
			  0))  &&((nextHop.v6_addr32[0] == 0)
	     ...
	     */ 
	    if (((nextHop.v6_addr32[0] == 0) && (nextHop.v6_addr32[1] == 0)
		  && (nextHop.v6_addr32[2] == 0) && (nextHop.v6_addr32[3] == 0))
		 && (fc_dst_len != 0)) {
...
 static int32 _rtl8198c_delIpv6Route(inv6_addr_t ipAddr, int fc_dst_len) 
{
		...
		 if (_rtl8198c_usedNetifInIpv6Route(entry->dstNetif->name) ==
		       FAILED)
...
int rtl819x_save_inused_ipv6_route(void) 
{
	...
	 memset(rtl8198c_ipv6_route_inused_back, 0,
		  sizeof(rtl8198c_ipv6_route_t) * IPV6_RT_DRV_ENTRY_NUM);
	while (rt)
		 {
		if (rt->valid == 1) {
			memcpy(&rtl8198c_ipv6_route_inused_back[i], rt,
