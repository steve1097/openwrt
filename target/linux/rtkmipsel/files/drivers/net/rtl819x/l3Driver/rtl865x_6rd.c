...
static int32 _rtl865x_updateIpv66RDEntry(rtl865x_6rd_s * tunnel_param,
					 int entryIdx);
...
int32 rtl8198c_initIpv66RDTable(void)
{
	...
	memset(rtl865x_ipv6_6RDTable, 0,
	       sizeof(rtl865x_6rd_s) * IPV6_6RD_ENTRY_NUM);
	br_ip_addr = 0;
	return SUCCESS;
}
...
static int32 _rtl865x_synIpv66RDToAsic(rtl865x_6rd_s * entry_t)
{
	...
	asic_entry.six_rd_prefix.v6_addr32[0] =
	    entry_t->six_rd_prefix.v6_addr32[0];
	asic_entry.six_rd_prefix.v6_addr32[1] =
	    entry_t->six_rd_prefix.v6_addr32[1];
	asic_entry.six_rd_prefix.v6_addr32[2] =
	    entry_t->six_rd_prefix.v6_addr32[2];
	asic_entry.six_rd_prefix.v6_addr32[3] =
	    entry_t->six_rd_prefix.v6_addr32[3];
...
static void _rtl865x_tunnelTo6RDparam(struct ip_tunnel *t,
				      rtl865x_6rd_s * tunnel_param)
{
	tunnel_param->ce_ip_addr = htonl(t->parms.iph.saddr);
	tunnel_param->ce_ip_mask_len = 0;
	tunnel_param->six_rd_prefix.v6_addr32[0] =
	    htonl(t->ip6rd.prefix.s6_addr32[0]);
	tunnel_param->six_rd_prefix.v6_addr32[1] =
	    htonl(t->ip6rd.prefix.s6_addr32[1]);
	tunnel_param->six_rd_prefix.v6_addr32[2] =
	    htonl(t->ip6rd.prefix.s6_addr32[2]);
	tunnel_param->six_rd_prefix.v6_addr32[3] =
	    htonl(t->ip6rd.prefix.s6_addr32[3]);
...
#if 0
	printk
	    ("[%s]:[%d].retVal:%d, index:%d, ce: %x/%d, 6rd prefix: %x-%x-%x-%x/%d, br: %x/%d, if: %s, mtu: %d\n",
	     __FUNCTION__, __LINE__, retVal, entryIdx, entry->ce_ip_addr,
	     entry->ce_ip_mask_len, entry->six_rd_prefix.v6_addr32[0],
	     entry->six_rd_prefix.v6_addr32[1],
	     entry->six_rd_prefix.v6_addr32[2],
	     entry->six_rd_prefix.v6_addr32[3], entry->six_rd_prefix_len,
	     entry->br_ip_addr, entry->br_ip_mask_len, entry->name, entry->mtu);
#endif
	return retVal;
}
					 ...
					 int entryIdx)
{
	...
	if (tunnel_param->six_rd_prefix.v6_addr32[0] != 0 ||
		...
		entry->six_rd_prefix.v6_addr32[0] =
		    tunnel_param->six_rd_prefix.v6_addr32[0];
		entry->six_rd_prefix.v6_addr32[1] =
		    tunnel_param->six_rd_prefix.v6_addr32[1];
		entry->six_rd_prefix.v6_addr32[2] =
		    tunnel_param->six_rd_prefix.v6_addr32[2];
		entry->six_rd_prefix.v6_addr32[3] =
		    tunnel_param->six_rd_prefix.v6_addr32[3];
	}
