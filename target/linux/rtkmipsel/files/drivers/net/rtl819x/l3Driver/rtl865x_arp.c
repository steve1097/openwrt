...
#endif
#if defined(CONFIG_RTL_MULTIPLE_WAN) ||defined(CONFIG_RTL_REDIRECT_ACL_SUPPORT_FOR_ISP_MULTI_WAN)
static int (*rtl_callback_for_ps_arp) (ipaddr_t ip,
...
static int32 rtl865x_arp_callbackFn_for_del_fdb(void *param)
{
	...
	for (i = 0; i < RTL8651_ARPTBL_SIZE; i++) {
		...
		if (memcmp
		    (&(fdbEntry->macAddr), &(arpTables.mappings[i].mac),
		     6) == 0) {
...
static int32 rtl865x_arp_callbackFn_for_add_fdb(void *param)
{
	...
	for (i = 0; i < RTL8651_ARPTBL_SIZE; i++) {
		/*be careful of dead loop, the delete fdb event maybe caused by arp time out */
		if (memcmp
		    (&(fdbEntry->macAddr), &(arpTables_all.mappings[i].mac),
		     6) == 0) {
#if 0
			printk
			    ("%s[%d], ip is 0x%x, mac is 0x%2x%2x%2x%2x%2x%2x\n",
			     __FUNCTION__, __LINE__,
			     arpTables_all.mappings[i].ip,
			     arpTables_all.mappings[i].mac.octet[0],
			     arpTables_all.mappings[i].mac.octet[1],
			     arpTables_all.mappings[i].mac.octet[2],
			     arpTables_all.mappings[i].mac.octet[3],
			     arpTables_all.mappings[i].mac.octet[4],
			     arpTables_all.mappings[i].mac.octet[5]);
#endif
			rtl865x_addArp(arpTables_all.mappings[i].ip,
				       &(arpTables_all.mappings[i].mac));
		}
	}
#endif
	return EVENT_CONTINUE_EXECUTE;
...
int32 rtl865x_arp_init(void)
{
	...
	memset(arpTables.mappings, 0,
	       RTL8651_ARPTBL_SIZE * sizeof(rtl865x_arpMapping_entry_t));
	for (i = 0; i < RTL8651_ARPTBL_SIZE; i++) {
	...
	memset(arpTables_all.mappings, 0,
	       RTL8651_ARPTBL_SIZE * sizeof(rtl865x_arpMapping_entry_t));
#endif
	return SUCCESS;
}
...
int32 rtl865x_arp_tbl_alloc(rtl865x_route_t * route)
{
	...
	if (netSize > 1 && ((bestSize << 3) >= netSize)) {
		...
		route->un.arp.arpend =
		    (bestStartPos + (netSize >> 3) -
		     ((netSize & 0x7) == 0 ? 1 : 0)) << 3;
		for (entry = route->un.arp.arpsta >> 3;
		     entry <= route->un.arp.arpend >> 3; entry++) {
	...
	for (i = route->un.arp.arpsta >> 3; i <= route->un.arp.arpend >> 3; i++) {
		...
		for (j = 0; j < 8; j++) {
			...
			if (arpTables.mappings[index].ip != 0) {
				...
				vlan =
				    _rtl8651_getVlanTableEntry(route->
							       dstNetif->vid);
				rtl865x_delFilterDatabaseEntry
				    (RTL865x_L2_TYPEII, vlan->fid,
				     &(arpTables.mappings[index].mac));
				memset(&(arpTables.mappings[index]), 0,
				       sizeof(rtl865x_arpMapping_entry_t));
			} else {
				memset(&(arpTables.mappings[index]), 0,
				       sizeof(rtl865x_arpMapping_entry_t));
			}
...
int32 rtl8198c_6rd_Ipv6ArpMapping(ipaddr_t ip,
			    ...
			    rtl865x_arpMapping_entry_t * arp_mapping)
{
	...
	if (arpTables.mappings[hash].ip == ip) {
		memcpy(arp_mapping, &(arpTables.mappings[hash]),
		       sizeof(rtl865x_arpMapping_entry_t));
	} else
		return FAILED;
...
int32 rtl865x_getAsicArpEntry(ipaddr_t ip,
			      rtl865x_tblAsicDrv_arpParam_t * asicArpEntry)
{
	...
	if ((route == NULL) || (route->valid != 1) || (route->process != 2)
	    || (route->dstNetif == NULL)) {
		return FAILED;
	}
...
int32 rtl865x_mapIpToMac(ipaddr_t ip, ether_addr_t * mac)
{
	...
	if (arpTables.mappings[hash].ip == ip) {
		memcpy(mac, &(arpTables.mappings[hash].mac),
		       sizeof(ether_addr_t));
		return SUCCESS;
	}
...
int32 rtl865x_isEthArp(ipaddr_t ip)
{
	...
	for (i = 0; i < 2; i++) {
		...
		if (rtl865x_Lookup_fdb_entry
		    (fid, &(arpTables.mappings[hash].mac), fdb_type[i], &column,
		     &fdbEntry) != SUCCESS) {
			continue;
		}
...
#if defined(CONFIG_RTL_MULTIPLE_WAN) || defined(CONFIG_RTL_REDIRECT_ACL_SUPPORT_FOR_ISP_MULTI_WAN)
int32 rtl_set_callback_for_ps_arp(int (*call_back_fn)
				  (u32 ip, rtl865x_arpMapping_entry_t * entry))
{
...
int32 rtl865x_addArp(ipaddr_t ip, ether_addr_t * mac)
{
	...
	if ((mac->octet[0] & mac->octet[1] & mac->
	     octet[2] & mac->octet[3] & mac->octet[4] & mac->octet[5]) == 0xff)
		return FAILED;

	//printk("%s:%d+++++++++++++++++++++++++++++\n",__FUNCTION__,__LINE__);

	/*check the old arp mapping first */
	memcpy(&oldArpMapping, &(arpTables.mappings[hash]),
	       sizeof(rtl865x_arpMapping_entry_t));
	if ((oldArpMapping.ip != ip)
	    || (memcmp(&(oldArpMapping.mac), mac, 6) != 0)) {
		...
		if (oldArpMapping.ip != 0) {
			...
			memset(&(arpTables.mappings[hash]), 0,
			       sizeof(rtl865x_arpMapping_entry_t));

			if (rtl865x_getArpFid(oldArpMapping.ip, &fid) ==
			    SUCCESS) {
			...
			rtl865x_raiseEvent(EVENT_DEL_ARP,
					   (void *)(&oldArpMapping));
		}
	}
	...
	for (i = 0; i < 2; i++) {
...
#if defined(CONFIG_RTL_EXT_PORT_SUPPORT)
		if (i == 0)
			memcpy(&(arpTables_all.mappings[hash]), &newArpMapping,
			       sizeof(rtl865x_arpMapping_entry_t));
#endif
		if (rtl865x_Lookup_fdb_entry
...
#endif
		{
			if (rtl865x_isEthPortMask(fdbEntry.memberPortMask) ==
			    FALSE) {
				continue;
			}
		}
		...
		if ((oldArpMapping.ip != ip)
			...
			memcpy(&(l2temp_entry.macAddr), mac,
			       sizeof(ether_addr_t));
...
//                      _rtl865x_addFilterDatabaseEntry((fdbEntry.nhFlag==0)?RTL865x_L2_TYPEI: RTL865x_L2_TYPEII, fid, mac, FDB_TYPE_FWD, fdbEntry.memberPortMask, fdbEntry.auth,fdbEntry.srcBlk);
		...
		memcpy(&(arpTables.mappings[hash]), &newArpMapping,
		       sizeof(rtl865x_arpMapping_entry_t));
#if defined(CONFIG_RTL_HW_NAPT_REFINE_KERNEL_HOOKS)
		arpTables.mappings[hash].exist_in_ps = 1;
...
int32 rtl865x_delArp(ipaddr_t ip)
{
	...
	//printk("%s:%d***************************************\n",__FUNCTION__,__LINE__);
	memcpy(&arpMapping, &arpTables.mappings[hash],
	       sizeof(rtl865x_arpMapping_entry_t));
	...
	memset(&arpTables.mappings[hash], 0,
	       sizeof(rtl865x_arpMapping_entry_t));
#if defined(CONFIG_RTL_EXT_PORT_SUPPORT)
	memset(&arpTables_all.mappings[hash], 0,
	       sizeof(rtl865x_arpMapping_entry_t));
#endif
...
void rtl819x_save_hw_arp_table(void)
{
	...
	for (i = 0; i < RTL8651_ARPTBL_SIZE; i++) {
		if ((arpTables_bak.mappings[i].ip != 0) ||
		    memcmp(arpTables_bak.mappings[i].mac.octet, empty_mac.octet,
			   ETHER_ADDR_LEN) != 0) {
			//panic_printk("%s %d ip=0x%x \n", __FUNCTION__, __LINE__, arpTables_bak.mappings[i].ip);
			if (rtl865x_getArpFid
			    (arpTables_bak.mappings[i].ip, &fid) != SUCCESS) {
				printk("%s %d ip=0x%x \n", __FUNCTION__,
				       __LINE__, arpTables_bak.mappings[i].ip);
			}
			rtl819x_save_inused_l2_entry(arpTables_bak.
						     mappings[i].ip,
						     (ether_addr_t *) &
						     arpTables_bak.
						     mappings[i].mac, fid);
	...
	for (i = 0; i < RTL865X_ARPTBL_SIZE; i++) {
		if ((arpTables_bak.mappings[i].ip != 0) ||
		    memcmp(arpTables_bak.mappings[i].mac.octet, empty_mac.octet,
			   ETHER_ADDR_LEN) != 0) {
			ret =
			    rtl865x_addArp(arpTables_bak.mappings[i].ip,
					   &arpTables_bak.mappings[i].mac);
...
int rtl_del_arp_not_exist_in_ps(void)
{
	...
	for (i = 0; i < RTL8651_ARPTBL_SIZE; i++) {
		if ((arpTables.mappings[i].valid == 1)
		    && (arpTables.mappings[i].exist_in_ps == 0)) {
