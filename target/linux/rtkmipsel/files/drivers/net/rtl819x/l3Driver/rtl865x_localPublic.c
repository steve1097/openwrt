...
typedef CTAILQ_HEAD(_rtl865x_localPublicList,
		    rtl865x_localPublic) rtl865x_localPublicList;
...
static int rtl865x_getAllHWLocalPublic(struct rtl865x_localPublic
				       localPublicArray[], int arraySize);
...
static struct rtl865x_localPublic *rtl865x_searchLocalPublic(unsigned int
							     ipAddr)
{
...
int rtl_checkLocalPublicNetifIngressRule(rtl865x_AclRule_t * rule)
{
	...
	memset(lp, 0,
	       sizeof(struct rtl865x_localPublic) * MAX_HW_LOCAL_PUBLIC_NUM);
	...
	case RTL865X_ACL_UDP:
		{
			for (i = 0; i < MAX_HW_LOCAL_PUBLIC_NUM; i++) {
				if (lp[i].hw
				    &&
				    (((lp[i].ipAddr & rule->dstIpAddrMask_) ==
				      (rule->
				       dstIpAddr_ & rule->dstIpAddrMask_)))) {
					rtl865x_del_acl(rule, lp[i].lpNetif,
							RTL865X_ACL_QOS_USED0);
					rtl865x_add_acl(rule, lp[i].lpNetif,
							RTL865X_ACL_QOS_USED0);
				}

				if ((rule->dstIpAddr_ & 0xf0000000) ==
				    0xe0000000) {
					rtl865x_del_acl(rule, "virtualNetif",
							RTL865X_ACL_QOS_USED0);
					rtl865x_add_acl(rule, "virtualNetif",
							RTL865X_ACL_QOS_USED0);
				}
			}
			break;
	...
	case RTL865X_ACL_UDP_IPRANGE:
		{
			for (i = 0; i < MAX_HW_LOCAL_PUBLIC_NUM; i++) {
				...
				if (((rule->dstIpAddrLB_ & 0xf0000000) ==
				     0xe0000000)
				    && ((rule->dstIpAddrUB_ & 0xf0000000) ==
					0xe0000000)) {
					rtl865x_del_acl(rule, "virtualNetif",
							RTL865X_ACL_QOS_USED0);
					rtl865x_add_acl(rule, "virtualNetif",
...
static int rtl865x_copyLocalPublic(struct rtl865x_localPublic *to,
				   struct rtl865x_localPublic *from)
{
...
static int rtl865x_addInboundAclToCpu(struct rtl865x_localPublic *entry)
{
	...
	if (policy != NETIF_MAC_BASED) {
		...
		ret =
		    rtl865x_add_acl(&rule, wanIfInfo.ifname,
...
static int rtl865x_delInboundAclToCpu(struct rtl865x_localPublic *entry)
{
	...
	if (policy != NETIF_MAC_BASED) {
		...
		ret =
		    rtl865x_del_acl(&rule, wanIfInfo.ifname,
				    RTL865X_ACL_SYSTEM_USED);
	}
	return ret;
...
static void rtl865x_addNetifMissedDefAclForLP(void)
{
	...
	for (i = 0; i < routeCnt; i++) {
		if ((routeTbl[i].ipMask != 0xFFFFFFFF)
		    && (routeTbl[i].ipMask != 0)) {
			bzero((void *)&rule, sizeof(rtl865x_AclRule_t));
...
static int rtl865x_addMCastNetif(void)
{
...
#if defined(CONFIG_RTK_VLAN_SUPPORT)
	{
		...
		rule.actionType_ = RTL865X_ACL_PERMIT;
		ret =
		    rtl865x_add_acl(&rule, mCastNetIf, RTL865X_ACL_SYSTEM_USED);
	}
#endif
...
static int rtl865x_delMCastNetif(void)
{
...
#if defined(CONFIG_RTK_VLAN_SUPPORT)
	{
		...
		rule.actionType_ = RTL865X_ACL_PERMIT;
		ret =
		    rtl865x_del_acl(&rule, mCastNetIf, RTL865X_ACL_SYSTEM_USED);
	}
#endif
...
static int rtl865x_addLocalPublicRoute(struct rtl865x_localPublic *entry)
{
	...
	ret =
	    rtl865x_addRoute(entry->ipAddr, 0xFFFFFFFF, entry->ipAddr,
			     lanIfInfo.ifname, 0);
	//printk("%s:%d,ret is %d \n",__FUNCTION__,__LINE__,ret);
	return SUCCESS;
...
static int rtl865x_addIpRangeAclToCpu(unsigned int srcIpStart,
...
static int rtl865x_delIpRangeAclToCpu(unsigned int srcIpStart,
				      unsigned int srcIpEnd,
				      unsigned int dstIpStart,
				      unsigned int dstIpEnd)
{
...
static int rtl865x_addOutboundToCpuAcl(struct rtl865x_localPublic *entry)
{
	...
	CTAILQ_FOREACH(lpEntry, &usedLocalPublicList, next) {
		...
		rtl865x_addIpRangeAclToCpu(entry->ipAddr, entry->ipAddr,
					   lpEntry->ipAddr, lpEntry->ipAddr);
	}
	...
	list_for_each_safe(p, n, &public_ssid_list_head) {
		ssidEntry =
		    list_entry(p, struct rtl865x_public_ssid_entry, list);
		if (ssidEntry->public_addr == entry->ipAddr) {
			continue;
		}
		rtl865x_addIpRangeAclToCpu(entry->ipAddr, entry->ipAddr,
					   ssidEntry->public_addr,
					   ssidEntry->public_addr);
	}
#endif
...
static int rtl865x_delOutboundToCpuAcl(struct rtl865x_localPublic *entry)
{
	...
	CTAILQ_FOREACH(lpEntry, &usedLocalPublicList, next) {
		...
		rtl865x_delIpRangeAclToCpu(entry->ipAddr, entry->ipAddr,
					   lpEntry->ipAddr, lpEntry->ipAddr);
	}
...
static int rtl865x_addOutboundRedirectAcl(struct rtl865x_localPublic *entry)
{
	...
	nextHopIndex =
	    rtl865x_getNxtHopIdx(NEXTHOP_DEFREDIRECT_ACL, localPublicInterface,
				 entry->defGateway);
	if (nextHopIndex == -1) {
		ret =
		    rtl865x_addNxtHop(NEXTHOP_DEFREDIRECT_ACL, NULL,
				      localPublicInterface, entry->defGateway);
		if (ret != SUCCESS) {
			return FAILED;
		}

		nextHopIndex =
		    rtl865x_getNxtHopIdx(NEXTHOP_DEFREDIRECT_ACL,
	...
	for (i = 0; i < routeCnt; i++) {
		...
		if (rt->ipMask != 0xFFFFFFFF && (rt->ipMask != 0)) {
			...
			/*-----------------------------------------------------------*/
				...
				ret =
				    rtl865x_add_acl(&rule, lanIfInfo.ifname,
						    RTL865X_ACL_SYSTEM_USED);
				//printk("%s:%d,ret is %d \n",__FUNCTION__,__LINE__,ret);
				if (ret != SUCCESS) {
					rtl865x_delNxtHop
...
static int rtl865x_delOutboundRedirectAcl(struct rtl865x_localPublic *entry)
{
	...
	for (i = 0; i < routeCnt; i++) {
		...
		if (rt->ipMask != 0xFFFFFFFF && (rt->ipMask != 0)) {
			...
			ret =
			    rtl865x_del_acl(&rule, lanIfInfo.ifname,
					    RTL865X_ACL_SYSTEM_USED);
			//printk("%s:%d,ret is %d \n",__FUNCTION__,__LINE__,ret);
			...
			rule.dstIpAddrLB_ =
			    (rt->ipAddr & rt->ipMask) + ((~rt->ipMask) + 1);
			rule.dstIpAddrUB_ = 0xFFFFFFFF;
...
static int rtl865x_addLocalPublicInterface(struct rtl865x_localPublic *entry)
{
	...
	if (entry->lpNetif[0] == 0) {
		...
		for (vid = LOCAL_PUBLIC_VLAN_START;
		     vid <= LOCAL_PUBLIC_VLAN_END; vid++) {
		...
		ret =
		    rtl865x_addVlanPortMember(entry->lpVid, wanIfInfo.memPort);
		//printk("%s:%d,ret is %d \n",__FUNCTION__,__LINE__,ret);
		ret =
		    rtl865x_setVlanFilterDatabase(entry->lpVid, wanIfInfo.fid);
...
#if defined(CONFIG_RTK_VLAN_SUPPORT)
		{
			...
			rule.actionType_ = RTL865X_ACL_PERMIT;
			ret =
			    rtl865x_add_acl(&rule, entry->lpNetif,
					    RTL865X_ACL_SYSTEM_USED);
		}
#endif
...
static int rtl865x_delLocalPublicInterface(struct rtl865x_localPublic *entry)
{
...
#if defined(CONFIG_RTK_VLAN_SUPPORT)
	{
		...
		rule.actionType_ = RTL865X_ACL_PERMIT;
		ret =
		    rtl865x_del_acl(&rule, entry->lpNetif,
				    RTL865X_ACL_SYSTEM_USED);
	}
#endif
...
static int rtl865x_updateLocalPublicMac(struct rtl865x_localPublic *entry,
...
#if defined(CONFIG_RTL_HW_QOS_SUPPORT)
				rtl865x_regist_aclChain(entry->lpNetif,
							RTL865X_ACL_QOS_USED0,
							RTL865X_ACL_INGRESS);
				rtl865x_regist_aclChain(entry->lpNetif,
...
static int rtl865x_updateDefaultGatewayMac(struct rtl865x_localPublic *entry,
					   unsigned char *newMac)
{
...
int rtl865x_delLocalPublic(struct rtl865x_localPublic *delEntry)
{
...
#if defined(CONFIG_RTL_HW_QOS_SUPPORT)
		rtl865x_unRegist_aclChain(delEntry->lpNetif,
					  RTL865X_ACL_QOS_USED0,
					  RTL865X_ACL_INGRESS);
		rtl865x_unRegist_aclChain(delEntry->lpNetif,
					  RTL865X_ACL_QOS_USED1,
	...
	while (entry) {
		...
		if (entry->ipAddr == delEntry->ipAddr) {
			...
			memset(entry, 0,
			       sizeof(struct rtl865x_localPublic) - 8);
				  ...
				  int count, int *eof, void *data)
{
	...
	CTAILQ_FOREACH(entry, &usedLocalPublicList, next) {
		...
		len +=
		    sprintf(page + len,
			    "[%2d]  ip:%d.%d.%d.%d, netmask:%d.%d.%d.%d, gateway:%d.%d.%d.%d,hw:%d, dev:%s, mac:0x%x:%x:%x:%x:%x:%x,gw mac:0x%x:%x:%x:%x:%x:%x, lpNetif:%s\n",
			    entryCnt, NIPQUAD(entry->ipAddr),
			    NIPQUAD(entry->netMask), NIPQUAD(entry->defGateway),
			    entry->hw, (entry->dev[0] ? entry->dev : null),
			    entry->mac[0], entry->mac[1], entry->mac[2],
			    entry->mac[3], entry->mac[4], entry->mac[5],
			    entry->defGwMac[0], entry->defGwMac[1],
			    entry->defGwMac[2], entry->defGwMac[3],
			    entry->defGwMac[4], entry->defGwMac[5],
			    entry->lpNetif[0] ? entry->lpNetif : null);
	}
...
static void rtl865x_saveNetDecPolicyAndDefACL(void)
{
	...
	rtl865x_getDefACLForNetDecisionMiss(&savedDefInAclStart,
					    &savedDefInAclEnd,
					    &savedDefOutAclStart,
					    &savedDefOutAclEnd);
}
...
static void rtl865x_restoreNetDecPolicyAndDefACL(void)
{
	...
	rtl865x_setDefACLForNetDecisionMiss(savedDefInAclStart,
					    savedDefInAclEnd,
					    savedDefOutAclStart,
					    savedDefOutAclEnd);
}

static ssize_t proc_write_local_public(struct file *file,
				       ...
				       loff_t * data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, 128)) {
		...
		int num =
		    sscanf(tmp, "%s %d.%d.%d.%d %d.%d.%d.%d %d.%d.%d.%d %d",
			   cmd,
			   &localPublicIp[0], &localPublicIp[1],
			   &localPublicIp[2], &localPublicIp[3],
			   &netMask[0], &netMask[1], &netMask[2], &netMask[3],
			   &defGateway[0], &defGateway[1], &defGateway[2],
			   &defGateway[3], &hw);
		...
		entry.ipAddr =
		    (localPublicIp[0] << 24) | (localPublicIp[1] << 16) |
		    (localPublicIp[2] << 8) | localPublicIp[3];
		entry.netMask =
		    (netMask[0] << 24) | (netMask[1] << 16) | (netMask[2] << 8)
		    | netMask[3];
		entry.defGateway =
		    (defGateway[0] << 24) | (defGateway[1] << 16) |
		    (defGateway[2] << 8) | defGateway[3];
		entry.hw = hw ? 1 : 0;
		...
		if ((strcmp(cmd, "add") == 0) || (strcmp(cmd, "ADD") == 0)) {
			if (totalCnt == 0) {
				...
				memset(&vituralNetif, 0,
				       sizeof(rtl865x_netif_t));
...
#if defined(CONFIG_RTL_HW_QOS_SUPPORT)
				rtl865x_regist_aclChain("virtualNetif",
							RTL865X_ACL_QOS_USED0,
							RTL865X_ACL_INGRESS);
				rtl865x_regist_aclChain("virtualNetif",
							RTL865X_ACL_QOS_USED1,
		...
		if (totalCnt > 0) {
			...
			rtl865x_delAllInboundAclToCpu();
#if defined(CONFIG_RTL_HW_QOS_SUPPORT)
			rtl865x_unRegist_aclChain("virtualNetif",
						  RTL865X_ACL_QOS_USED0,
						  RTL865X_ACL_INGRESS);
			rtl865x_unRegist_aclChain("virtualNetif",
						  RTL865X_ACL_QOS_USED1,
...
#if defined(CONFIG_RTL_PUBLIC_SSID)
static struct rtl865x_public_ssid_entry *rtl865x_get_public_ssid_entry(unsigned
								       int
								       ipAddr)
{
...
static ssize_t proc_write_public_SSID(struct file *file,
				      const char __user * buffer, size_t count,
				      loff_t * data)
{
	char tmpbuf[128];
	...
	if (buffer && !copy_from_user(tmpbuf, buffer, 128)) {
		...
		if (!strcmp(cmd_addr, "add") || !strcmp(cmd_addr, "ADD")) {
			num =
			    sscanf(tokptr, "%d.%d.%d.%d", &public_ssid_ip[0],
				   &public_ssid_ip[1], &public_ssid_ip[2],
				   &public_ssid_ip[3]);
			if (num != 4)
				goto errout;

			ipAddr =
			    public_ssid_ip[0] << 24 | public_ssid_ip[1] << 16 |
			    public_ssid_ip[2] << 8 | public_ssid_ip[3];
			rtl865x_add_public_ssid_entry(ipAddr, 0);
		} else if ((strcmp(cmd_addr, "del") == 0)
			   || (strcmp(cmd_addr, "DEL") == 0)) {
			num =
			    sscanf(tokptr, "%d.%d.%d.%d", &public_ssid_ip[0],
				   &public_ssid_ip[1], &public_ssid_ip[2],
				   &public_ssid_ip[3]);
		...
		} else if ((strcmp(cmd_addr, "pssid_netif") == 0)
			   || (strcmp(cmd_addr, "PSSID_NETIF") == 0)) {
			num = 0;
...
int rtl865x_initLocalPublic(struct rtl865x_localPublicPara *para)
{
	...
	TBL_MEM_ALLOC(localPublicPool, struct rtl865x_localPublic,
		      maxLocalPublicNum)
	if (localPublicPool != NULL) {
		memset(localPublicPool, 0,
		       maxLocalPublicNum * sizeof(struct rtl865x_localPublic));
	} else {
		return FAILED;
	}

	for (i = 0; i < maxLocalPublicNum; i++) {
		CTAILQ_INSERT_HEAD(&freeLocalPublicList, &localPublicPool[i],
				   next);
	}

	localPublicProcEntry = create_proc_entry("localPublic", 0, NULL);
	if (localPublicProcEntry != NULL) {
		localPublicProcEntry->proc_fops =
		    &localpublic_single_seq_file_operations;
	}
...
static int rtl865x_checkIpHdr(struct rtl865x_pktInfo *pktInfo)
{
	...
	if (iph->protocol == 0x11) {
		...
		if ((udph->source == CLIENT_PORT)
			...
			if (srcIpAddr == 0) {
				requestedIp =
				    get_dhcpOption(dhcph, DHCP_REQUESTED_IP);
				if (requestedIp != NULL) {
			...
			if (entry != NULL) {
				...
				if (pktInfo->action == RX_LAN_PACKET) {
					...
					if ((entry->hw)
					    &&
					    (strncmp
					     (pktInfo->dev, RTL_WLAN_NAME,
					      4) == 0)) {
						rtl865x_disableLocalPublicHwFwd
						    (entry);
						entry->hw = 0;
					}
					rtl865x_updateLocalPublicMac(entry,
								     dhcph->chaddr);
				}
				//printk("%s:%d, match dhcp src ip:%d.%d.%d.%d\n",__FUNCTION__,__LINE__,NIPQUAD(srcIpAddr));
				return SUCCESS;
			}
		} else if ((udph->source == SERVER_PORT)
			   && (udph->dest == CLIENT_PORT)) {
			/*server--->client */
			ptr = ptr + 8;
			dhcph = (struct dhcpMessage *)ptr;
...
static int rtl865x_checkArpHdr(struct rtl865x_pktInfo *pktInfo)
{
	...
	if (entry != NULL) {
		...
		if (pktInfo->action == RX_LAN_PACKET) {
			...
			if ((entry->hw)
			    && (strncmp(pktInfo->dev, RTL_WLAN_NAME, 4) == 0)) {
				rtl865x_disableLocalPublicHwFwd(entry);
				entry->hw = 0;
			}
		...
		if ((pktInfo->action == RX_WAN_PACKET) && (arph->ar_op == 0x02)) {
			...
			CTAILQ_FOREACH(entry, &usedLocalPublicList, next) {
				if ((entry->hw)
				    && (entry->defGateway == srcIpAddr)) {
					rtl865x_updateDefaultGatewayMac(entry,
									ar_sha);
				}
...
int rtl865x_getLocalPublicArpMapping(unsigned int ip,
...
int rtl865x_getLocalPublicInfo(unsigned int ipAddr,
...
int rtl865x_setLpIfInfo(struct rtl865x_interface_info *ifInfo)
{
	...
	if (ifInfo->isWan) {
		memcpy(&wanIfInfo, ifInfo,
		       sizeof(struct rtl865x_interface_info));
	} else {
		memcpy(&lanIfInfo, ifInfo,
		       sizeof(struct rtl865x_interface_info));
	}
	return SUCCESS;
}

int rtl865x_getAllLocalPublic(struct rtl865x_localPublic localPublicArray[],
			      int arraySize)
{
	struct rtl865x_localPublic *entry;
	int cnt = 0;
