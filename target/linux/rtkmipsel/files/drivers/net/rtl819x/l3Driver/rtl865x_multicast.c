...
static int32 _rtl865x_initMCastFwdDescPool(void) 
{
		...
		MC_LIST_INSERT_HEAD(&free_mcast_fwd_descriptor_head,
...
 static int32 _rtl865x_freeMCastFwdDesc(rtl865x_mcast_fwd_descriptor_t *
					  descPtr) 
{
...
 static int32 _rtl865x_flushMCastFwdDescChain(mcast_fwd_descriptor_head_t *
						descChainHead) 
{
...
   static int32 _rtl865x_mCastFwdDescEnqueue(mcast_fwd_descriptor_head_t *
						 queueHead,
						 ...
						 * enqueueDesc) 
{
	...
	  for (curDesc = MC_LIST_FIRST(queueHead); curDesc != NULL;
		  curDesc = nextDesc)
...
  static int32 _rtl865x_mergeMCastFwdDescChain(mcast_fwd_descriptor_head_t *
						  targetChainHead,
						  ...
						  * srcChain) 
{
	...
	 for (curDesc = srcChain; curDesc != NULL;
...
    static int32 _rtl865x_initMCastEntryPool(void) 
{
	...
	 TBL_MEM_ALLOC(multiCast_t, rtl865x_tblDrv_mCast_t,
		...
		TAILQ_INSERT_HEAD(&mCastTbl.freeList.freeMultiCast,
				   &multiCast_t[index], nextMCast);
		}
	 TBL_MEM_ALLOC(multiCast_t, rtl865x_tblDrv_mCast_t,
			 RTL8651_MULTICASTTBL_SIZE);
	memset(multiCast_t, 0,
...
 static rtl865x_tblDrv_mCast_t *_rtl865x_allocMCastEntry(uint32 hashIndex) 
{
	...
	 TAILQ_INSERT_TAIL(&mCastTbl.inuseList.mCastTbl[hashIndex], newEntry,
			     nextMCast);
...
 static int32 _rtl865x_freeMCastEntry(rtl865x_tblDrv_mCast_t * mCastEntry,
					uint32 hashIndex) 
{
	...
	 TAILQ_REMOVE(&mCastTbl.inuseList.mCastTbl[hashIndex], mCastEntry,
			nextMCast);
	_rtl865x_flushMCastEntry(mCastEntry);
	TAILQ_INSERT_HEAD(&mCastTbl.freeList.freeMultiCast, mCastEntry,
			   nextMCast);
	return SUCCESS;
}

  static uint32 _rtl865x_doMCastEntrySrcVlanPortFilter(rtl865x_tblDrv_mCast_t
							  * mCastEntry) 
{
	...
	 for (curDesc = MC_LIST_FIRST(&mCastEntry->fwdDescChain);
...
#endif /*  */
 static uint32 rtl865x_genMCastEntryAsicFwdMask(rtl865x_tblDrv_mCast_t *
						  mCastEntry) 
{
...
 static uint16 rtl865x_genMCastEntryCpuFlag(rtl865x_tblDrv_mCast_t *
					      mCastEntry) 
{
...
static void _rtl865x_setASICMulticastPortStatus(void)
{
		...
		index++) {
		rtl8651_setAsicMulticastPortInternal(index,
...
 static inline void _rtl865x_patchPppoeWeak(rtl865x_tblDrv_mCast_t *
					      mCast_t) 
{
		...
		if (rtl865x_getNetifType(curDesc->netifName, &netifType) ==
		     SUCCESS)
...
static void _rtl865x_arrangeMulticast(uint32 entryIndex) 
{
	...
	 TAILQ_FOREACH(mCast_t, &mCastTbl.inuseList.mCastTbl[entryIndex],
			 nextMCast)  {
		...
		if ((swapOutEntry->unKnownMCast == FALSE)
		     && (select_t->unKnownMCast == TRUE))
	    ...
	    if (select_t)
			...
			TAILQ_REMOVE(&mCastTbl.inuseList.mCastTbl[entryIndex],
				      select_t, nextMCast);
			TAILQ_INSERT_HEAD(&mCastTbl.
...
  static void _rtl865x_mCastEntryReclaim(void) 
{
	    ...
	    for (index = 0; index < RTL8651_MULTICASTTBL_SIZE; index++)
		    ...
		    TAILQ_FIRST(&mCastTbl.inuseList.mCastTbl[index]);
					...
					_rtl865x_freeMCastEntry(curMCastEntry,
								 index);
						    ...
						    uint16 sport) 
{
		...
		if (mCast_t->dip == mAddr && mCast_t->sip == sip
		...
		TAILQ_FOREACH(mCast_t, &mCastTbl.inuseList.mCastTbl[entry],
			       nextMCast) {
...
int rtl865x_findEmptyCamEntry(void) 
{
	...
	 for (index = RTL8651_IPMULTICASTTBL_SIZE;
...
#endif /*  */
{
	...
	struct rtl_groupInfo groupInfo;
	    ...
	    TAILQ_FOREACH(mCast_t, &mCastTbl.inuseList.mCastTbl[hashIndex],
			  nextMCast)  {
		if (mCast_t->sip == sip && mCast_t->dip == mAddr
		     && mCast_t->svid == svid && mCast_t->port == sport)
	...
	 _rtl865x_mergeMCastFwdDescChain(&mCast_t->fwdDescChain,
					   newFwdDescChain);
...
/*the following function maybe used in future*/ 
    int32 rtl865x_addMulticastFwdDesc(ipaddr_t mcast_addr,
				       rtl865x_mcast_fwd_descriptor_t *
				       newFwdDesc) 
{
...
 int32 rtl865x_delMulticastFwdDesc(ipaddr_t mcast_addr,
				     rtl865x_mcast_fwd_descriptor_t *
				     deadFwdDesc) 
{
...
 int32 rtl865x_delMulticastUpStream(ipaddr_t mcast_addr, ipaddr_t sip,
				      uint16 svid, uint16 sport) 
{
		...
		TAILQ_FOREACH(mCast_t, &mCastTbl.inuseList.mCastTbl[index],
			       nextMCast)  {
...
  static int32 _rtl865x_subMCastFwdDescChain(mcast_fwd_descriptor_head_t *
						targetChainHead,
						rtl865x_mcast_fwd_descriptor_t *
						srcChain) 
{
...
 static int32 _rtl865x_mCastFwdDescDequeue(mcast_fwd_descriptor_head_t *
					     queueHead,
					     rtl865x_mcast_fwd_descriptor_t *
					     dequeueDesc) 
{
...
extern unsigned int rtl865x_getPhyFwdPortMask(struct net_bridge *br,
...
#endif	/* RTL8651_MCAST_ALWAYS2UPSTREAM */
				    rtl_getGroupInfo(mCastEntry->dip,
						      &groupInfo);
...
extern int rtl_addFilterDatabaseEntryforMCast(unsigned char *mac,
...
void _rtl865x_timeUpdateMulticast(uint32 secPassed) 
{
	    ...
	    for (entry = 0; entry < RTL8651_MULTICASTTBL_SIZE; entry++) {
			    ...
			    nextMCast_t = TAILQ_NEXT(mCast_t, nextMCast);
					...
					_rtl865x_freeMCastEntry(mCast_t,
								 entry);
    ...
    int32 rtl865x_initMulticast(rtl865x_mCastConfig_t * mCastConfigPtr) 
{
		...
		rtl865x_setMulticastExternalPortMask
	    ...
	    seq_printf(s,
		    ...
		    TAILQ_FIRST(&mCastTbl6.inuseList.mCastTbl[entry]);
			    ...
			    nextMCastv6_t = TAILQ_NEXT(mCastv6_t, nextMCast);
				    ...
				    entry, mCastv6_t->dip.v6_addr32[0],
				    mCastv6_t->dip.v6_addr32[1],
				    mCastv6_t->dip.v6_addr32[2],
				    mCastv6_t->dip.v6_addr32[3],
				    mCastv6_t->sip.v6_addr32[0],
				    mCastv6_t->sip.v6_addr32[1],
				    mCastv6_t->sip.v6_addr32[2],
    ...
    int32 rtl_dumpSwMulticastInfo(char *page, int *ret_len) 
{
	    ...
	    for (entry = 0; entry < RTL8651_MULTICASTTBL_SIZE; entry++) {
		    ...
		    mCast_t = TAILQ_FIRST(&mCastTbl.inuseList.mCastTbl[entry]);
			    ...
			    sprintf(page + len,
				    "\t      extIP:0x%x,age:%d, cpu:%d, count:%d, inAsic:%d, (%s)\n",
				    mCast_t->extIp, mCast_t->age, mCast_t->cpu,
...
#endif /*  */
#endif
int rtl865x_genVirtualMCastFwdDescriptor(unsigned int forceToCpu,
					  uint32 fwdPortMask,
					  rtl865x_mcast_fwd_descriptor_t *
					  fwdDescriptor)
{
...
 int rtl865x_setMCastHashMethod(unsigned int hashMethod) 
{
			...
			((hashMethod == HASH_METHOD_SIP_DIP0
			 ...
			 || (hashMethod == HASH_METHOD_SIP_DIP1
...
static int32 _rtl8198C_initMCastv6EntryPool(void) 
{
	...
	 TBL_MEM_ALLOC(multiCast_t, rtl8198c_tblDrv_mCastv6_t,
		...
		memset(&multiCast_t[index], 0,
			sizeof(rtl8198c_tblDrv_mCastv6_t));
		TAILQ_INSERT_HEAD(&mCastTbl6.freeList.freeMultiCast,
				   &multiCast_t[index], nextMCast);
...
 static int32 _rtl8198C_freeMCastv6FwdDesc(rtl8198c_mcast_fwd_descriptor6_t *
					     descPtr) 
{
...
  static rtl8198c_mcast_fwd_descriptor6_t
...
 static int32 _rtl8198C_mCastv6FwdDescEnqueue(mcast_fwd_descriptor_head6_t *
						queueHead,
						...
						* enqueueDesc) 
{
	    ...
	    newDesc = _rtl8198C_allocMCastv6FwdDesc();
		  ...
		  curDesc = nextDesc)
			...
			if (strcmp(curDesc->netifName, newDesc->netifName) ==
			     0)
				 {
				if (newDesc->descPortMask == 0)
					 {
					newDesc->descPortMask =
					    curDesc->descPortMask;
...
 static int32 _rtl8198C_flushMCastv6FwdDescChain(mcast_fwd_descriptor_head6_t
						   * descChainHead) 
{
...
  static int32 _rtl8198C_flushMCastv6Entry(rtl8198c_tblDrv_mCastv6_t *
					      mCastEntry) 
{
...
  static int32 _rtl8198C_freeMCastv6Entry(rtl8198c_tblDrv_mCastv6_t *
					     mCastEntry, uint32 hashIndex) 
{
	...
	 TAILQ_REMOVE(&mCastTbl6.inuseList.mCastTbl[hashIndex], mCastEntry,
			nextMCast);
	_rtl8198C_flushMCastv6Entry(mCastEntry);
	TAILQ_INSERT_HEAD(&mCastTbl6.freeList.freeMultiCast, mCastEntry,
			   nextMCast);
...
  static void _rtl8198C_arrangeMulticastv6(uint32 entryIndex) 
{
	...
	TAILQ_FOREACH(mCast_t, &mCastTbl6.inuseList.mCastTbl[entryIndex],
		       nextMCast)  {
...
//                      memcpy(&asic_mcast, select_t, (uint32)&(((rtl8198c_tblDrv_mCastv6_t *)0)->six_rd_idx));
			...
			TAILQ_REMOVE(&mCastTbl6.inuseList.mCastTbl[entryIndex],
				      select_t, nextMCast);
			TAILQ_INSERT_HEAD(&mCastTbl6.
					   inuseList.mCastTbl[entryIndex],
					   select_t, nextMCast);
			    ...
			    if (1)
					...
					rtl8198C_delAsicIpMulticastv6Table
					    (entryIndex);
...
 void _rtl8198C_timeUpdateMulticastv6(uint32 secPassed) 
{
	    ...
	    for (entry = 0; entry < RTL8651_MULTICASTTBL_SIZE; entry++) {
			    ...
			    nextMCast_t = TAILQ_NEXT(mCast_t, nextMCast);
					...
					_rtl8198C_freeMCastv6Entry(mCast_t,
								    entry);
...
 }  static void _rtl8198C_initMCastv6SysTimer(void) 
{
	...
	rtl8198c_mCast6SysTimer.function =
...
 static int32 _rtl8198C_initMCastv6FwdDescPool(void) 
{
	...
	TBL_MEM_ALLOC(rtl8198c_mcastFwdDescPool6,
		       rtl8198c_mcast_fwd_descriptor6_t,
		       MAX_MCASTV6_FWD_DESCRIPTOR_CNT);
	 if (rtl8198c_mcastFwdDescPool6 != NULL)
		 {
		memset(rtl8198c_mcastFwdDescPool6, 0,
			MAX_MCASTV6_FWD_DESCRIPTOR_CNT *
			sizeof(rtl8198c_mcast_fwd_descriptor6_t));
		...
		MC_LIST_INSERT_HEAD(&free_mcast_fwd_descriptor_head6,
				     &rtl8198c_mcastFwdDescPool6[i], next);
		}
	 return SUCCESS;
}

 static rtl8198c_tblDrv_mCastv6_t *_rtl8198C_allocMCastv6Entry(uint32
								 hashIndex) 
{
	...
	 TAILQ_INSERT_TAIL(&mCastTbl6.inuseList.mCastTbl[hashIndex], newEntry,
			     nextMCast);
	 return newEntry;
}

 static int32 _rtl8198C_mergeMCastv6FwdDescChain(mcast_fwd_descriptor_head6_t
						   * targetChainHead,
...
  static uint32 rtl8198C_genMCastv6EntryAsicFwdMask(rtl8198c_tblDrv_mCastv6_t
						       * mCastEntry) 
{
...
 static uint16 rtl8198C_genMCastv6EntryCpuFlag(rtl8198c_tblDrv_mCastv6_t *
						 mCastEntry) 
{
...
  static void _rtl8198C_mCastv6EntryReclaim(void) 
{
	    ...
	    for (index = 0; index < RTL8651_MULTICASTTBL_SIZE; index++)
		    ...
		    TAILQ_FIRST(&mCastTbl6.inuseList.mCastTbl[index]);
				...
				_rtl8198C_freeMCastv6Entry(curMCastEntry,
							    index);
...
 static inline void _rtl8198C_patchPppoeWeakv6(rtl8198c_tblDrv_mCastv6_t *
						 mCast_t) 
{
...
 static int32 rtl8198C_multicastv6CallbackFn(void *param) 
{
...
#endif /*  */
	{
			      ...
			      mCastEntry; mCastEntry = nextMCastEntry)
					...
					_rtl8198C_freeMCastv6Entry(mCastEntry,
								    index);
		    ...
		    ("%s:%d,multicast event from ethernet (%s),mcastEventContext.groupAddr[0] is 0x%x\n",
		     ...
		     mcastEventContext.groupAddr[0]);
			      ...
			      mCastEntry; mCastEntry = nextMCastEntry)
				    ...
				    /*update/replace old forward descriptor */ 
				    _rtl8198C_mergeMCastv6FwdDescChain
				    (&mCastEntry->fwdDescChain, &newFwdDesc);
...
 rtl8198c_tblDrv_mCastv6_t * rtl8198C_findMCastv6Entry(inv6_addr_t dip,
							 inv6_addr_t sip,
							 uint16 svid,
							 uint16 sport) 
{
		...
		if (is_ip6_addr_equal(mCast_t->dip, dip)
		...
		TAILQ_FOREACH(mCast_t, &mCastTbl6.inuseList.mCastTbl[entry],
			       nextMCast) {
...
#endif				/*  */
{
	...
	struct rtl_groupInfo groupInfo;
	    ...
	    TAILQ_FOREACH(mCast_t, &mCastTbl6.inuseList.mCastTbl[hashIndex],
			  nextMCast)  {
		if (is_ip6_addr_equal(mCast_t->sip, sip)
	...
	 _rtl8198C_mergeMCastv6FwdDescChain(&mCast_t->fwdDescChain,
					      newFwdDescChain);
...
int rtl8198C_genVirtualMCastv6FwdDescriptor(unsigned int forceToCpu,
					     unsigned int fwdPortMask,
					     rtl8198c_mcast_fwd_descriptor6_t *
					     fwdDescriptor) 
{
