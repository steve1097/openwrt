...
static int32 _rtl865x_addPpp(uint8 * ifname, ether_addr_t * mac,
...
int rtl819x_save_hw_ppp_table(void) 
{
	...
	 memset((void *)&rtl865x_pppTable_bak, 0,
		  sizeof(rtl865x_pppTable_bak));
	 for (i = 0; i < PPP_NUMBER; i++)
		 {
		if (rtl865x_pppTable[i].valid == 1)
			 {
			rtl865x_pppTable_bak[i].netif =
			    rtl865x_pppTable[i].netif;
			rtl865x_pppTable_bak[i].sessionId =
			    rtl865x_pppTable[i].sessionId;
			rtl865x_pppTable_bak[i].valid =
			    rtl865x_pppTable[i].valid;
			rtl865x_pppTable_bak[i].type =
			    rtl865x_pppTable[i].type;
			rtl865x_pppTable_bak[i].refCnt =
			    rtl865x_pppTable[i].refCnt;
			memcpy(rtl865x_pppTable_bak[i].server_mac.octet,
			    ...
			    rtl865x_getVlanFilterDatabaseId(rtl865x_pppTable
							    [i].netif->vid,
							    &fid);
			rtl819x_save_inused_l2_entry(0,
						      (ether_addr_t *) &
						      rtl865x_pppTable
						      [i].server_mac, fid);
...
int rtl819x_restore_hw_ppp_table(void) 
{
			...
			rtl865x_addPpp(entry->netif->name,
