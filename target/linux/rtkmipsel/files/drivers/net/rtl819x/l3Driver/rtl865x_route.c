...
 static rtl865x_route_t *_rtl865x_getRouteEntry(ipaddr_t ipAddr,
						  ipaddr_t ipMask) 
{
...
 static int32 _rtl865x_updateDefaultRoute(rtl865x_route_t * rt,
					    int32 action) 
{
...
 static int32 _rtl865x_arrangeRoute(rtl865x_route_t * start_rt,
				      int32 start_idx) 
{
...
   static int32 _rtl865x_addRoute(ipaddr_t ipAddr, ipaddr_t ipMask,
				      ...
				      ipaddr_t srcIp) 
{
	    ...
	    if (swconfig_vlan_enable) {
		     ...
		     && strncmp(netif->name, RTL_DRV_LAN_NETIF_NAME,
				 MAX_IFNAMESIZE)
		     && strncmp(netif->name, RTL_SWCONFIG_DRV_PPP_NETIF_NAME,
				 MAX_IFNAMESIZE))
	     ...
	     */ 
				...
				if (tmp_rt->valid && tmp_rt->process == RT_ARP
...
 static int32 _rtl865x_delRoute(ipaddr_t ipAddr, ipaddr_t ipMask) 
{
		...
		 if (_rtl865x_usedNetifInRoute(entry->dstNetif->name) ==
		       FAILED)
...
 rtl865x_route_t * _rtl85x_getRouteEntry(ipaddr_t dst) 
{
		...
		if (rt->valid == 1 && rt->ipAddr == (rt->ipMask & dst)
		     && mask <= rt->ipMask) {
    ...
    int32 sw_l3_read(struct seq_file * s, void *v)
{
			      ...
			      idx, entry->ipAddr, entry->ipMask,
			      entry->srcIp, entry->nextHop, entry->process,
...
#endif /*  */
    int32 sw_l3_write(struct file * filp, const char *buff, unsigned long len,
		       void *data)
{
...
int rtl819x_save_inused_route(void) 
{
		  ...
		  sizeof(rtl865x_route_t) * RT_DRV_ENTRY_NUM);
			...
			memcpy(&rtl865x_route_inused_bak[i], rt,
...
 int rtl819x_restore_inused_route(void) 
{
			...
			rtl865x_addRoute(rt->ipAddr, rt->ipMask, rt->nextHop,
