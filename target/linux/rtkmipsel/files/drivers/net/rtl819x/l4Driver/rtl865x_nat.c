...
int32 rtl_nat_expire_interval_update(int proto, int32 interval)
{
	...
	asic_interval =
	    interval >
	    RTL_NAT_AVG_INTERVAL ? interval - RTL_NAT_AVG_INTERVAL : interval -
	    RTL_NAT_MIN_INTERVAL;
	asic_interval =
	    asic_interval <
	    RTL_NAT_MIN_INTERVAL ? RTL_NAT_MIN_INTERVAL : asic_interval;
						      ...
						      *nat_tuple)
{
	...
	hash =
	    rtl8651_naptTcpUdpTableIndex((uint8) nat_tuple->proto,
					 nat_tuple->int_host.ip,
					 nat_tuple->int_host.port,
					 nat_tuple->rem_host.ip,
					 nat_tuple->rem_host.port);
						     ...
						     *nat_tuple)
{
	...
	hash =
	    rtl8651_naptTcpUdpTableIndex((uint8) nat_tuple->proto,
					 nat_tuple->rem_host.ip,
					 nat_tuple->rem_host.port,
					 nat_tuple->ext_host.ip,
					 nat_tuple->ext_host.port);
...
static int _rtl865x_isEntryPreReserved(uint32 index)
{
	...
	if ((natEntry->flags & NAT_PRE_RESERVED)) {
		if (jiffies >= natEntry->reserveTime) {
			...
			/*timer overflow */
			if (((0xFFFFFFFF - natEntry->reserveTime) + jiffies +
			     1) > (RESERVE_EXPIRE_TIME * HZ)) {
				natEntry->flags &= (~(NAT_PRE_RESERVED));
				natEntry->reserveTime = 0;
				return FALSE;
			} else {
				    ...
				    rtl865x_naptHashInfo_t * naptHashInfo)
{
	...
	in = rtl8651_naptTcpUdpTableIndex((uint8) (naptEntry->protocol),
					  naptEntry->remIp, naptEntry->remPort,
					  naptEntry->extIp, naptEntry->extPort);
	out =
	    rtl8651_naptTcpUdpTableIndex((uint8) (naptEntry->protocol),
					 naptEntry->intIp, naptEntry->intPort,
					 naptEntry->remIp, naptEntry->remPort);
...
#if 0
	printk
	    ("%s:%d:%s (%u.%u.%u.%u:%u -> %u.%u.%u.%u:%u -> %u.%u.%u.%u:%u) ,out is %d,in is %d\n",
	     __FUNCTION__, __LINE__, protocol ? "tcp" : "udp", NIPQUAD(intIp),
	     intPort, NIPQUAD(extIp), extPort, NIPQUAD(remIp), remPort, out,
	     in);
#endif
	return SUCCESS;
}
					...
					rtl865x_priority * prio)
{
	...
	very =
	    rtl8651_naptTcpUdpTableIndex(((uint8) naptEntry->protocol) |
					 ...
					 htons((uint16) (naptEntry->remPort)),
					 0, 0);
	in = rtl8651_naptTcpUdpTableIndex((uint8) naptEntry->protocol,
					  htonl(naptEntry->remIp),
					  htons((uint16) (naptEntry->remPort)),
					  htonl(naptEntry->extIp), extPort);
	out =
	    rtl8651_naptTcpUdpTableIndex((uint8) naptEntry->protocol,
					 htonl(naptEntry->intIp),
					 htons((uint16) (naptEntry->intPort)),
					 htonl(naptEntry->remIp),
					 htons((uint16) (naptEntry->remPort)));
	...
	rtlglue_printf
	    ("LR(%s):  %s (%u.%u.%u.%u:%u -> %u.%u.%u.%u:%u) g:(%u.%u.%u.%u:%u)\n",
	     ("add_nat"), ((naptEntry->protocol) ? "tcp" : "udp"),
	     NIPQUAD(naptEntry->intIp), naptEntry->intPort,
	     NIPQUAD(naptEntry->remIp), naptEntry->remPort,
	     NIPQUAD(naptEntry->extIp), naptEntry->extPort);
#endif
	...
	for (i = 0; i < 2; i++) {
		if (rtl865x_getRouteEntry(sip, &rt) == SUCCESS) {
			...
			rule4[i].ruleType_ =
			    (naptEntry->protocol ==
			     RTL865X_PROTOCOL_TCP ? RTL865X_ACL_TCP :
			     RTL865X_ACL_UDP);
			...
			rule4[i].netifIdx_ =
			    _rtl865x_getNetifIdxByNameExt(rt.dstNetif->name);
...

	{
		...
		for (i = 0; i < 2; i++) {
			if (rtl865x_getArpMapping(sip, &arpInfo) == SUCCESS
				...
				memset(rule2[i].srcMacMask_.octet, 0xff,
				       ETHER_ADDR_LEN);
				rule2[i].netifIdx_ =
				    _rtl865x_getNetifIdxByNameExt(rt.
								  dstNetif->name);
		...
		for (i = 0; i < 2; i++) {
			if ((rule2[i].aclIdx == 0)
			    ...
			    && rtl865x_getRouteEntry(naptEntry->remIp,
						     &rt) == SUCCESS) {
				/*      check mac base rule secondly  */
				//memset(&rule2[i], 0, sizeof(rtl865x_AclRule_t));
				rule2[i].ruleType_ = RTL865X_ACL_MAC;
	...
	for (i = 0; i < 2; i++) {
		if (rule4[i].aclIdx && rule2[i].aclIdx) {
			priority[i] =
			    (rule4[i].aclIdx <
			     rule2[i].aclIdx) ? rule4[i].
			    priority_ : rule2[i].priority_;
			upDown[i] =
			    (rule4[i].aclIdx <
			     rule2[i].aclIdx) ? rule4[i].
			    upDown_ : rule2[i].upDown_;
	...
	for (i = 0; i < 2; i++) {
		...
		asic_nat.isStatic = 1;
		asic_nat.isTcp =
		    (naptEntry->protocol == RTL865X_PROTOCOL_TCP) ? 1 : 0;
		asic_nat.isValid = 1;
		asic_nat.offset = ((i == 0) ? offset : (extPort & 0x3f));
		asic_nat.selEIdx = ((i == 0) ? selEidx_out : very & 0x3ff);
		asic_nat.selExtIPIdx =
		    ((i == 0) ? ipidx : ((extPort & 0x3ff) >> 6));
		...
		asic_nat.ageSec =
		    (naptEntry->protocol ==
		     RTL865X_PROTOCOL_TCP) ? nat_tbl.
		    tcp_timeout : nat_tbl.udp_timeout;
#if	defined(CONFIG_RTL_HW_QOS_SUPPORT)
...
static int32 _rtl865x_naptSync(rtl865x_napt_entry * naptEntry, uint32 refresh)
{
	...
	if ((nat_out != NULL) && (nat_in != NULL)) {
		...
		hw_now =
		    (asic_nat_out.ageSec >
		     asic_nat_in.ageSec) ? asic_nat_out.
		    ageSec : asic_nat_in.ageSec;
...
#if 0
				printk
				    ("intIp is 0x%x, intPort is %d, remIp is 0x%x, remPort is %d, extIp is 0x%x, extPort is %d,[%s]:[%d]\n",
				     natEntryPtr->int_ip_,
				     natEntryPtr->int_port_,
				     natEntryPtr->rem_ip_,
				     natEntryPtr->rem_port_,
				     natEntryPtr->ext_ip_,
				     natEntryPtr->ext_port_, __FUNCTION__,
				     __LINE__);
#endif
		...
		if (NAT_INUSE(&nat_tbl.nat_bucket[i])) {
			...
			if (nat_this->flags & NAT_INBOUND) {
				if (nat_this->out != 0xFFFFFFFF) {
					nat_that =
					    &nat_tbl.nat_bucket[nat_this->out];
				} else {
					/*no outbound */
					nat_that = NULL;
				}
				...
				if (nat_this->in != 0xFFFFFFFF) {
					nat_that =
					    &nat_tbl.nat_bucket[nat_this->in];
				} else {
					/*no inbound */
					nat_that = NULL;
				}
			...
			if (nat_this->flags & NAT_PRI_PROCESSED) {
				...
				if (rtl865x_getRouteEntry(sip, &rt) == SUCCESS) {
					...
					if (rule4.netifIdx_ >= 0
						...
						if (rtl865x_qosCheckNaptPriority
						    (&rule4) != SUCCESS) {
							if (nat_this->flags &
							    NAT_INBOUND) {
								rule4.dstIpAddr_
					...
					if (rtl865x_getArpMapping(sip, &arpInfo)
					    == SUCCESS) {
						memset(&rule2, 0,
						       sizeof
						       (rtl865x_AclRule_t));
					...
					if (priority > -1) {
						...
						rtl865x_naptSetAsicWithPriority
						    (nat_this, priority);
			...
			if (natEntryPtr->flags & NAT_OUTBOUND) {
				...
				len +=
				    seq_printf(s,
					       ...
					       natEntryPtr->rem_port_,
					       natEntryPtr->flags,
					       natEntryPtr->out, natEntryPtr->in
#if defined(CONFIG_RTL_EXT_PORT_SUPPORT)
				  ...
				  uint16 dport, uint8 protocol)
{
	...
	nat_tuple = &temp;
	nat_tuple->proto =
	    ((protocol == 6) ? RTL865X_PROTOCOL_TCP : RTL865X_PROTOCOL_UDP);
	...
	if (rtl865x_enableNaptFourWay == TRUE) {
		for (i = 0; i < 4; i++) {
			nat_out = &nat_tbl.nat_bucket[hash];
			if (((sip == nat_out->int_ip_)
			     && (sport == nat_out->int_port_)
			     && (dip == nat_out->rem_ip_)
			     && (dport == nat_out->rem_port_)
	...
	for (i = 0; i < RTL8651_TCPUDPTBL_SIZE; i++) {
		...
		if (NAT_INUSE(tmp)) {
			...
			if ((nat_out->isExtNaptEntry) && (nat_out == nat_in)) {
				...
				if ((nat_in->isExtNaptEntry) && (nat_in != NULL)
				    && (nat_in->flags & NAT_INBOUND)) {
					rtl8651_delAsicNaptTcpUdpTable(inIndex,
								       inIndex);
					memset(nat_in, 0, sizeof(*nat_in));
