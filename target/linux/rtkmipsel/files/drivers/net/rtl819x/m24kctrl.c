...
int stop_perf_counter(int record, unsigned int index)
{
	...
	if (index >=
	    (sizeof(cp3romePerfStat) / sizeof(rtl8651_romeperf_stat_t))) {
	...
	if (get_cpu() == 0) {
		...
		if (1) {
			printk
			    ("<%8lx> CPU %d: [0]%8lx,[1]%8lx,[2]%8lx,[3]%8lx\n",
			     jiffies, get_cpu(), perf_cnt0, perf_cnt1,
			     perf_cnt2, perf_cnt3);
			//pre_jiffies = jiffies;        
		}
#endif
...
void stopCP3Ctrl(int i, unsigned int index)
{
	if (cp3ctrlswitch) {
...
__attribute__ ((unused))
void startCP3Ctrl(unsigned int i)
{
	if (!cp3ctrlswitch) {
		cp3ctrlswitch = 1;
