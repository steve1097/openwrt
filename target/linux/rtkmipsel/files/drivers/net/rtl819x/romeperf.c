...
__IRAM static void CP3_COUNTER0_START(void)
{
	...
	tempVariable32 = /* Counter0 */ ((0x10 | CP3CNT_CYCLES) << 0) |
	    ...
	    /* Counter2 */
	    ((0x10 | CP3CNT_DATA_LOAD_OR_STORE_CACHE_MISS) << 16) |
	    /* Counter3 */
	    ((0x10 | CP3CNT_DATA_LOAD_OR_STORE_MISS_BUSY_CYCLE) << 24);
#elif 1				/* Data (STORE) */
...
");
#else
	void save_counter(uint64 * addr0, uint64 * addr1, uint64 * addr2,
			  uint64 * addr3);
...
int32 rtl8651_romeperfInit()
{
	...
	romePerfStat[ROMEPERF_INDEX_UNTIL_ACLDB].desc =
	    "FwdInput() Until ACLDB";
	romePerfStat[ROMEPERF_INDEX_GET_MTU_AND_SOURCE_MAC].desc =
	    "L3Route_MTU_srcMAC";
	romePerfStat[ROMEPERF_INDEX_PPTPL2TP_1].desc = "L3Route_PPTPL2TP_1";
	romePerfStat[ROMEPERF_INDEX_PPPOE_ARP_CACHE].desc =
	    "L3Route_PPPoE_ArpCache";
	romePerfStat[ROMEPERF_INDEX_PPTPL2TP_SEND].desc =
	    "L3Route_PptpL2tpSend()";
	romePerfStat[ROMEPERF_INDEX_FRAG].desc = "L3Route_Fragment";
	romePerfStat[ROMEPERF_INDEX_EGRESS_ACL].desc = "FwdSend_EgressACL";
	romePerfStat[ROMEPERF_INDEX_PPTPL2TP_ENCAP].desc =
	    "FwdSend_PPTP/L2TP_Encap";
	...
	romePerfStat[ROMEPERF_INDEX_FRAG_2ND_HALF].desc =
	    "FwdSend_Frag_2ndHalf()";
	romePerfStat[ROMEPERF_INDEX_TXPKTPOST].desc =
	    "rtl8651_txPktPostProcessing()";
	...
	romePerfStat[ROMEPERF_INDEX_MDCMDIO].desc =
	    "MDCMDIO PHY Register ACCESS";
	...
	romePerfStat[ROMEPERF_INDEX_rtl_processReceivedInfo].desc =
	    "rtl_processReceivedInfo";
	romePerfStat[ROMEPERF_INDEX_rtl_decideRxDevice].desc =
	    "rtl_decideRxDevice";
	romePerfStat[ROMEPERF_INDEX_rtl_processRxFrame].desc =
	    "rtl_processRxFrame";
	romePerfStat[ROMEPERF_INDEX_rtl_processBridgeShortCut].desc =
	    "rtl_processBridgeShortCut";
	romePerfStat[ROMEPERF_INDEX_rtl_processRxToProtcolStack].desc =
	    "rtl_processRxToProtcolStack";
	romePerfStat[ROMEPERF_INDEX_netif_receive_skb].desc =
	    "netif_receive_skb";
	romePerfStat[ROMEPERF_INDEX_FastPath_Enter].desc = "FastPath_Enter";
	romePerfStat[ROMEPERF_INDEX_checksum_in_fastpath].desc =
	    "checksum_in_fastpath";
	romePerfStat[ROMEPERF_INDEX_enter_fast_path].desc = "enter_fast_path";
	romePerfStat[ROMEPERF_INDEX_ip_finish_output3].desc =
	    "ip_finish_output3";
	romePerfStat[ROMEPERF_INDEX_ndo_start_xmit].desc = "ndo_start_xmit";
	romePerfStat[ROMEPERF_INDEX_rtl_preProcess_xmit].desc =
	    "rtl_preProcess_xmit";
	...
	romePerfStat[ROMEPERF_INDEX_dev_kfree_skb_any].desc =
	    "dev_kfree_skb_any";
...
__IRAM int rtl8651_romeperfExitPoint(int index)
{
	...
	CP3_COUNTER0_STOP();
	CP3_COUNTER0_GET_ALL();
	romePerfStat[index].accCycle[0] +=
	    currCnt[0] - romePerfStat[index].tempCycle[0];
	romePerfStat[index].accCycle[1] +=
	    currCnt[1] - romePerfStat[index].tempCycle[1];
	romePerfStat[index].accCycle[2] +=
	    currCnt[2] - romePerfStat[index].tempCycle[2];
	romePerfStat[index].accCycle[3] +=
	    currCnt[3] - romePerfStat[index].tempCycle[3];
...
int32 rtl8651_romeperfDump(int start, int end)
{
	...
	for (i = start; i <= end; i++) {
		if (romePerfStat[i].executedNum == 0) {
			...
			for (j = 0;
			     j <
			     sizeof(romePerfStat[i].accCycle) /
			     sizeof(romePerfStat[i].accCycle[0]); j++) {
				uint32 *pAccCycle =
				    (uint32 *) & romePerfStat[i].accCycle[j];
				uint32 avrgCycle =
				    /* Hi-word */
				    (pAccCycle[0] *
				     (0xffffffff /
	...
	for (i = start; i <= end; i++) {
		...
		for (j = 0;
			...
			statSnapShot[i].accCycle[j] =
			    romePerfStat[i].accCycle[j];
			statSnapShot[i].tempCycle[j] =
			    romePerfStat[i].tempCycle[j];
		}
	...
	for (i = start; i <= end; i++) {
		if (statSnapShot[i].executedNum == 0) {
			...
			for (j = 0;
			     j <
			     sizeof(statSnapShot[i].accCycle) /
			     sizeof(statSnapShot[i].accCycle[0]); j++) {
				uint32 *pAccCycle =
				    (uint32 *) & statSnapShot[i].accCycle[j];
				uint32 avrgCycle =
				    /* Hi-word */
				    (pAccCycle[0] *
				     (0xffffffff /
...
int rtl865x_perf_proc_read(char *page, char **start, off_t off, int count,
			   int *eof, void *data)
{
	//unsigned long x;
	//spin_lock_irq(x);
