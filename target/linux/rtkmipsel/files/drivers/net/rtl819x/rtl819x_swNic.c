...
TODO:refer to / linux - 3.10 / drivers / net / ethernet / broadcom / bcm63xx_enet.c
    ...
    and follow them to use those functions rtl_open()
{
	/*
		     ...
		     uint32 clusterSize)
{
	...
	if (New_rxDescRing[0] == NULL) {
		...
		for (i = 0; i < NEW_NIC_MAX_RX_DESC_RING; i++) {
			...
			New_rxDescRing[i] = (struct dma_rx_desc *)
			    CACHED_MALLOC((New_rxDescRingCnt[i] +
					   2) * sizeof(struct dma_rx_desc));
			ASSERT_CSP((uint32) New_rxDescRing[i] & 0x0fffffff);
			New_rxDescRing[i] =
			    (struct dma_rx_desc
			     *)(((uint32) New_rxDescRing[i] +
				 (_cpu_dcache_line - 1)) & ~(_cpu_dcache_line -
							     1));
			...
			if (New_rxDescRingCnt[i]) {
				...
				rx_skb[i] = (struct ring_info *)
				    CACHED_MALLOC(sizeof(struct ring_info) *
						  New_rxDescRingCnt[i]);
		...
		for (i = 0; i < NEW_NIC_MAX_TX_DESC_RING; i++) {
			...
			New_txDescRing[i] = (struct dma_tx_desc *)
			    CACHED_MALLOC((New_txDescRingCnt[i] +
					   2) * sizeof(struct dma_tx_desc));
			ASSERT_CSP((uint32) New_txDescRing[i] & 0x0fffffff);
			New_txDescRing[i] =
			    (struct dma_tx_desc
			     *)(((uint32) New_txDescRing[i] +
				 (_cpu_dcache_line - 1)) & ~(_cpu_dcache_line -
							     1));
...
int32 New_swNic_receive(rtl_nicRx_info * info, int retryCount)
{
	...
	for (ring_idx = (NEW_NIC_MAX_RX_DESC_RING - 1);
	     ring_idx >= (info->priority); ring_idx--)
#else
...
get_next:
		...
		hw_idx =
		    (cdp_value -
		     New_rxDescRing_base[ring_idx]) /
		    sizeof(struct dma_rx_desc);
		if (rx_idx == hw_idx) {	// no incoming pkt or Rx desc runout
			// check runout case
			desc =
			    (struct rx_desc *)&New_rxDescRing[ring_idx][rx_idx];
			desc =
			    (struct rx_desc *)(((uint32) desc) | UNCACHE_MASK);
		...
		if (buf) {
			...
			_dma_cache_inv((unsigned long)((struct sk_buff *)
						       skb)->data, 1536);
				 ...
				 struct sk_buff *skb)
{
	...
	if (*((unsigned short *)(skb->data + ETH_ALEN * 2)) !=
		...
		txd->tx_vlantag =
		    RTL_WANPORT_MASK & rtk_get_vlan_tagmask(txd->tx_dvlanid);
#else
	...
	if (*((unsigned short *)(skb->data + ETH_ALEN * 2)) !=
		...
		txd->tx_vlantag =
		    (0x3f &
		     rtk_getTagPortmaskByVid(nicTx->vid, (void *)skb->data));
#else
...
#ifdef _LOCAL_TX_DESC
	txd =
	    (struct tx_desc *)dw_copy((uint32 *) &
				      New_txDescRing[nicTx->txIdx][tx_idx],
				      (uint32 *) txd,
			     ...
			     rtl_nicTx_info * nicTx)
{
	...
	if (first_len) {
		...
		txd->opts1 =
		    (eor |
		     (((skb->len + 4) & TD_PHLEN_MASK) << TD_PHLEN_OFFSET));
		txd->opts1 |= FirstFrag;
		...
		txd->opts5 =
		    ((skb_gso_size << TD_MSS_OFFSET) |
		     (L3_hdr_len << TD_IPV4_HLEN_OFFSET) | (L4_hdr_len));
	...
	for (cur_frag = 0; cur_frag < skb_shinfo(skb)->nr_frags; cur_frag++) {
		...
		if (cur_frag == ((skb_shinfo(skb)->nr_frags) - 1))
			txd->opts2 =
			    (((frag->size +
			       4) & TD_M_LEN_MASK) << TD_M_LEN_OFFSET);
		else
			txd->opts2 =
...
__IRAM_FWD int32 New_swNic_txDone(int idx)
{
	...
	hw_cdp_idx =
	    (hw_cdp_data -
	     New_txDescRing_base[idx]) / sizeof(struct dma_tx_desc);
...
int32 New_check_tx_done_desc_swCore_own(int32 * tx_done_inx)
{
	...
	txd =
	    (struct tx_desc
	     *)(((uint32) (&New_txDescRing[0][New_txPktDoneDescIndex[0]])) |
		UNCACHE_MASK);
...
get_next:
		...
		// if checksum failed, reuse this desc, except the ipv6 ready logo case.
		if ((desc->opts5 & (RD_L3CSOK_MASK | RD_L4CSOK_MASK)) !=
		    (RD_L3CSOK_MASK | RD_L4CSOK_MASK)) {
#ifdef CONFIG_RTL_IPV6READYLOGO
			if (!
			    (((desc->opts5 & (RD_L4CSOK_MASK)) !=
			      RD_L4CSOK_MASK)
			     && ((desc->opts4 & RD_IPV6_MASK) == RD_IPV6_MASK)
			     && ((desc->opts3 & RD_REASON_MASK) == 0x60)))
#endif
			{
#ifdef _DESC_CACHE_ACCESS_RX
				desc =
				    (DMA_RX_DESC *) (((uint32) desc) |
						     UNCACHE_MASK);
#endif
				REUSE_DESC();
				goto get_next;
		...
		// porting from swNic_receive(), need or not ??
#if 0				//defined(CONFIG_RTL_HARDWARE_NAT)
		if (((desc->opts4 & RD_SPA_MASK) >> RD_SPA_OFFSET) >=
		    RTL8651_CPU_PORT) {
#ifdef _DESC_CACHE_ACCESS_RX
			desc = (DMA_RX_DESC *) (((uint32) desc) | UNCACHE_MASK);
#endif
		...
		if (buf) {
			...
			//info->priority = ring_idx; // todo
			info->vid =
			    (desc->opts4 & RD_DVLANID_MASK) >>
			    RD_DVLANID_OFFSET;
			info->pid =
			    (desc->opts4 & RD_SPA_MASK) >> RD_SPA_OFFSET;
			info->rxPri =
			    (desc->opts5 & RD_DPRI_MASK) >> RD_DPRI_OFFSET;
...
static inline void fill_txd_misc(DMA_TX_DESC * txd, rtl_nicTx_info * nicTx,
				 struct sk_buff *skb)
{
	// extract from _swNic_send, purpose: keep a function shortly and easy to read
#if	defined(CONFIG_RTL_HW_QOS_SUPPORT) || defined(CONFIG_RTK_VOIP_QOS)
	txd->opts3 =
	    (txd->opts3 & ~TD_DPRI_MASK) | (nicTx->priority << TD_DPRI_OFFSET);
#endif
	...
	if (*((unsigned short *)(skb->data + ETH_ALEN * 2)) !=
	    htons(ETH_P_8021Q))
		txd->opts2 =
		    (txd->opts2 & ~TD_VLANTAGSET_MASK) | nicTx->tagport;
	else
	...
	if (*((unsigned short *)(skb->data + ETH_ALEN * 2)) !=
	    htons(ETH_P_8021Q))
		txd->opts2 =
		    (txd->opts2 & ~TD_VLANTAGSET_MASK) | (0x3f &
							  rtk_get_vlan_tagmask
							  (nicTx->vid));
	else
		txd->opts2 = (txd->opts2 & ~TD_VLANTAGSET_MASK);
	...
	if (*((unsigned short *)(skb->data + ETH_ALEN * 2)) !=
		...
		txd->opts2 =
		    (txd->opts2 & ~TD_VLANTAGSET_MASK) | (0x3f &
							  rtk_getTagPortmaskByVid
							  (nicTx->vid,
							   (void *)skb->data));
#else
		txd->opts2 =
		    (txd->opts2 & ~TD_VLANTAGSET_MASK) | auto_set_tag_portmask;
#endif
	} else
		txd->opts2 = (txd->opts2 & ~TD_VLANTAGSET_MASK);
#endif
		      ...
		      rtl_nicTx_info * nicTx)
{
	...
	txd->addr = ((uint32) output);
	txd->opts1 =
	    (eor | FirstFrag | LastFrag |
	     ((len & TD_PHLEN_MASK) << TD_PHLEN_OFFSET));
	...
	if ((nicTx->flags & PKTHDR_HWLOOKUP) != 0) {
		txd->opts1 |=
		    ((1 << TD_BRIDGE_OFFSET) | (1 << TD_HWLKUP_OFFSET));
		txd->opts5 = (PKTHDR_EXTPORT_LIST_CPU << TD_EXTSPA_OFFSET);
	}
