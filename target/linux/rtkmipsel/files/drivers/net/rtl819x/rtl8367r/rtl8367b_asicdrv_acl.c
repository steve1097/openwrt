...
void _rtl8367b_aclRuleStSmi2User(rtl8367b_aclrule * pAclUser,
				 rtl8367b_aclrulesmi * pAclSmi);
void _rtl8367b_aclRuleStUser2Smi(rtl8367b_aclrule * pAclUser,
				 rtl8367b_aclrulesmi * pAclSmi);
void _rtl8367b_aclActStSmi2User(rtl8367b_acl_act_t * pAclUser,
				rtl8367b_acl_act_smi_t * pAclSmi);
void _rtl8367b_aclActStUser2Smi(rtl8367b_acl_act_t * pAclUser,
				rtl8367b_acl_act_smi_t * pAclSmi);
...
ret_t rtl8367b_setAsicAclUnmatchedPermit(rtk_uint32 port, rtk_uint32 enabled)
{
	...
	return rtl8367b_setAsicRegBit(RTL8367B_ACL_UNMATCH_PERMIT_REG, port,
				      enabled);
}
...
ret_t rtl8367b_getAsicAclUnmatchedPermit(rtk_uint32 port, rtk_uint32 * pEnabled)
{
	...
	return rtl8367b_getAsicRegBit(RTL8367B_ACL_UNMATCH_PERMIT_REG, port,
				      pEnabled);
}
...
ret_t rtl8367b_setAsicAclRule(rtk_uint32 index, rtl8367b_aclrule * pAclRule)
{
	...
	retVal =
	    rtl8367b_setAsicRegBits(RTL8367B_TABLE_ACCESS_WRDATA_REG
				    (RTL8367B_ACLRULETBLEN), 0x1, 0);
	if (retVal != RT_ERR_OK)
		return retVal;

	regAddr = RTL8367B_TABLE_ACCESS_CTRL_REG;
	regData =
	    RTL8367B_TABLE_ACCESS_REG_DATA(TB_OP_WRITE, TB_TARGET_ACLRULE);
	retVal = rtl8367b_setAsicReg(regAddr, regData);
	if (retVal != RT_ERR_OK)
		return retVal;
	...
	regData =
	    RTL8367B_TABLE_ACCESS_REG_DATA(TB_OP_WRITE, TB_TARGET_ACLRULE);
	retVal =
	    rtl8367b_setAsicRegBits(regAddr,
				    RTL8367B_TABLE_TYPE_MASK |
				    RTL8367B_COMMAND_TYPE_MASK, regData);
	if (retVal != RT_ERR_OK)
		return retVal;
	...
	memcpy(&Rtl8370sVirtualAclRuleTable[index], &aclRuleSmi,
	       sizeof(rtl8367b_aclrulesmi));
#endif
...
ret_t rtl8367b_getAsicAclRule(rtk_uint32 index, rtl8367b_aclrule * pAclRule)
{
	...
	/* Read Valid Bit */
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_TABLE_ACCESS_RDDATA_REG
				(RTL8367B_ACLRULETBLEN), &regData);
	if (retVal != RT_ERR_OK)
		return retVal;
...
#ifdef CONFIG_RTL8367B_ASICDRV_TEST
	memcpy(&aclRuleSmi, &Rtl8370sVirtualAclRuleTable[index],
	       sizeof(rtl8367b_aclrulesmi));
#endif
...
ret_t rtl8367b_setAsicAclNot(rtk_uint32 index, rtk_uint32 not)
{
	...
	return rtl8367b_setAsicRegBit(RTL8367B_ACL_ACTION_CTRL_REG(index),
				      RTL8367B_ACL_OP_NOT_OFFSET(index), not);
}
...
ret_t rtl8367b_getAsicAclNot(rtk_uint32 index, rtk_uint32 * pNot)
{
	...
	return rtl8367b_getAsicRegBit(RTL8367B_ACL_ACTION_CTRL_REG(index),
				      RTL8367B_ACL_OP_NOT_OFFSET(index), pNot);
}
 ...
 */
ret_t rtl8367b_setAsicAclTemplate(rtk_uint32 index,
				  rtl8367b_acltemplate_t * pAclType)
{
	ret_t retVal;
	rtk_uint32 i;
	rtk_uint32 regAddr, regData;
 ...
 *	    None
 */
ret_t rtl8367b_getAsicAclTemplate(rtk_uint32 index,
				  rtl8367b_acltemplate_t * pAclType)
{
	ret_t retVal;
	rtk_uint32 i;
...
ret_t rtl8367b_setAsicAclActCtrl(rtk_uint32 index, rtk_uint32 aclActCtrl)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_ACL_ACTION_CTRL_REG(index),
				       RTL8367B_ACL_OP_ACTION_MASK(index),
				       aclActCtrl);
}
...
ret_t rtl8367b_getAsicAclActCtrl(rtk_uint32 index, rtk_uint32 * pAclActCtrl)
{
	...
	retVal =
	    rtl8367b_getAsicRegBits(RTL8367B_ACL_ACTION_CTRL_REG(index),
				    RTL8367B_ACL_OP_ACTION_MASK(index),
				    &regData);
 ...
 *	    None
 */
ret_t rtl8367b_setAsicAclPortRange(rtk_uint32 index, rtk_uint32 type,
				   rtk_uint32 upperPort, rtk_uint32 lowerPort)
{
	ret_t retVal;
	...
	retVal =
	    rtl8367b_setAsicRegBits(RTL8367B_REG_ACL_SDPORT_RANGE_ENTRY0_CTRL2 +
				    index * 3,
				    RTL8367B_ACL_SDPORT_RANGE_ENTRY0_CTRL2_MASK,
				    type);
	if (retVal != RT_ERR_OK)
		return retVal;

	retVal =
	    rtl8367b_setAsicReg(RTL8367B_REG_ACL_SDPORT_RANGE_ENTRY0_CTRL1 +
				index * 3, upperPort);
	if (retVal != RT_ERR_OK)
		return retVal;

	retVal =
	    rtl8367b_setAsicReg(RTL8367B_REG_ACL_SDPORT_RANGE_ENTRY0_CTRL0 +
				index * 3, lowerPort);
	if (retVal != RT_ERR_OK)
		return retVal;
 ...
 *	    None
 */
ret_t rtl8367b_getAsicAclPortRange(rtk_uint32 index, rtk_uint32 * pType,
				   ...
				   rtk_uint32 * pLowerPort)
{
	...
	retVal =
	    rtl8367b_getAsicRegBits(RTL8367B_REG_ACL_SDPORT_RANGE_ENTRY0_CTRL2 +
				    index * 3,
				    RTL8367B_ACL_SDPORT_RANGE_ENTRY0_CTRL2_MASK,
				    pType);
	if (retVal != RT_ERR_OK)
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_ACL_SDPORT_RANGE_ENTRY0_CTRL1 +
				index * 3, pUpperPort);
	if (retVal != RT_ERR_OK)
		return retVal;

	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_ACL_SDPORT_RANGE_ENTRY0_CTRL0 +
				index * 3, pLowerPort);
	if (retVal != RT_ERR_OK)
		return retVal;
 ...
 *	    None
 */
ret_t rtl8367b_setAsicAclVidRange(rtk_uint32 index, rtk_uint32 type,
				  rtk_uint32 upperVid, rtk_uint32 lowerVid)
{
	ret_t retVal;
	rtk_uint32 regData;
	...
	regData =
	    ((type << RTL8367B_ACL_VID_RANGE_ENTRY0_CTRL1_CHECK0_TYPE_OFFSET) &
	     RTL8367B_ACL_VID_RANGE_ENTRY0_CTRL1_CHECK0_TYPE_MASK) | (upperVid &
								      RTL8367B_ACL_VID_RANGE_ENTRY0_CTRL1_CHECK0_HIGH_MASK);

	retVal =
	    rtl8367b_setAsicReg(RTL8367B_REG_ACL_VID_RANGE_ENTRY0_CTRL1 +
				index * 2, regData);
	if (retVal != RT_ERR_OK)
		return retVal;

	retVal =
	    rtl8367b_setAsicReg(RTL8367B_REG_ACL_VID_RANGE_ENTRY0_CTRL0 +
				index * 2, lowerVid);
	if (retVal != RT_ERR_OK)
		return retVal;
 ...
 *	    None
 */
ret_t rtl8367b_getAsicAclVidRange(rtk_uint32 index, rtk_uint32 * pType,
				  ...
				  rtk_uint32 * pLowerVid)
{
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_ACL_VID_RANGE_ENTRY0_CTRL1 +
				index * 2, &regData);
	if (retVal != RT_ERR_OK)
		return retVal;

	*pType =
	    (regData & RTL8367B_ACL_VID_RANGE_ENTRY0_CTRL1_CHECK0_TYPE_MASK) >>
	    RTL8367B_ACL_VID_RANGE_ENTRY0_CTRL1_CHECK0_TYPE_OFFSET;
	*pUpperVid =
	    regData & RTL8367B_ACL_VID_RANGE_ENTRY0_CTRL1_CHECK0_HIGH_MASK;

	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_ACL_VID_RANGE_ENTRY0_CTRL0 +
				index * 2, pLowerVid);
	if (retVal != RT_ERR_OK)
		return retVal;
 ...
 *	    None
 */
ret_t rtl8367b_setAsicAclIpRange(rtk_uint32 index, rtk_uint32 type,
				 ipaddr_t upperIp, ipaddr_t lowerIp)
{
	...
	retVal =
	    rtl8367b_setAsicRegBits(RTL8367B_REG_ACL_IP_RANGE_ENTRY0_CTRL4 +
				    index * 5,
				    RTL8367B_ACL_IP_RANGE_ENTRY0_CTRL4_MASK,
				    type);
	if (retVal != RT_ERR_OK)
		return retVal;
	...
	retVal =
	    rtl8367b_setAsicReg(RTL8367B_REG_ACL_IP_RANGE_ENTRY0_CTRL2 +
				index * 5, regData);
	if (retVal != RT_ERR_OK)
		return retVal;
	...
	retVal =
	    rtl8367b_setAsicReg(RTL8367B_REG_ACL_IP_RANGE_ENTRY0_CTRL3 +
				index * 5, regData);
	if (retVal != RT_ERR_OK)
		return retVal;
	...
	regData = ipData & 0xFFFF;
	retVal =
	    rtl8367b_setAsicReg(RTL8367B_REG_ACL_IP_RANGE_ENTRY0_CTRL0 +
				index * 5, regData);
	if (retVal != RT_ERR_OK)
		return retVal;
	...
	regData = (ipData >> 16) & 0xFFFF;
	retVal =
	    rtl8367b_setAsicReg(RTL8367B_REG_ACL_IP_RANGE_ENTRY0_CTRL1 +
				index * 5, regData);
	if (retVal != RT_ERR_OK)
		return retVal;
 ...
 *	    None
 */
ret_t rtl8367b_getAsicAclIpRange(rtk_uint32 index, rtk_uint32 * pType,
				 ipaddr_t * pUpperIp, ipaddr_t * pLowerIp)
{
	ret_t retVal;
	rtk_uint32 regData;
	ipaddr_t ipData;
	...
	retVal =
	    rtl8367b_getAsicRegBits(RTL8367B_REG_ACL_IP_RANGE_ENTRY0_CTRL4 +
				    index * 5,
				    RTL8367B_ACL_IP_RANGE_ENTRY0_CTRL4_MASK,
				    pType);
	if (retVal != RT_ERR_OK)
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_ACL_IP_RANGE_ENTRY0_CTRL2 +
				index * 5, &regData);
	if (retVal != RT_ERR_OK)
		return retVal;
	ipData = regData;
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_ACL_IP_RANGE_ENTRY0_CTRL3 +
				index * 5, &regData);
	if (retVal != RT_ERR_OK)
		return retVal;
	...
	*pUpperIp = ipData;
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_ACL_IP_RANGE_ENTRY0_CTRL0 +
				index * 5, &regData);
	if (retVal != RT_ERR_OK)
		return retVal;
	ipData = regData;
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_ACL_IP_RANGE_ENTRY0_CTRL1 +
				index * 5, &regData);
	if (retVal != RT_ERR_OK)
		return retVal;
...
{
	return rtl8367b_setAsicRegBit(RTL8367B_REG_ACL_GPIO_POLARITY,
				      RTL8367B_ACL_GPIO_POLARITY_OFFSET,
				      polarity);
}
...
{
	return rtl8367b_getAsicRegBit(RTL8367B_REG_ACL_GPIO_POLARITY,
				      RTL8367B_ACL_GPIO_POLARITY_OFFSET,
				      pPolarity);
}
