 ...
 *      None
 */
ret_t rtl8367b_setAsicEavPriRemapping(rtk_uint32 srcpriority,
				      rtk_uint32 priority)
{
	...
	return
	    rtl8367b_setAsicRegBits(RTL8367B_EAV_PRIORITY_REMAPPING_REG
				    (srcpriority),
				    RTL8367B_EAV_PRIORITY_REMAPPING_MASK
 ...
 *      None
 */
ret_t rtl8367b_getAsicEavPriRemapping(rtk_uint32 srcpriority,
				      rtk_uint32 * pPriority)
{
	...
	return
	    rtl8367b_getAsicRegBits(RTL8367B_EAV_PRIORITY_REMAPPING_REG
				    (srcpriority),
				    RTL8367B_EAV_PRIORITY_REMAPPING_MASK
...
ret_t rtl8367b_setAsicEavTimeOffsetSeccond(rtk_uint32 second)
{
	...
	retVal =
	    rtl8367b_setAsicReg(RTL8367B_REG_SYS_TIME_OFFSET_L,
				(second & 0xFFFF));
	if (retVal != RT_ERR_OK)
		return retVal;

	return rtl8367b_setAsicReg(RTL8367B_REG_SYS_TIME_OFFSET_H,
				   (second >> 16));
}
...
ret_t rtl8367b_setAsicEavTimeOffset512ns(rtk_uint32 ns)
{
	...
	retVal =
	    rtl8367b_setAsicReg(RTL8367B_REG_SYS_TIME_OFFSET_512NS_L,
				(ns & 0xFFFF));
	if (retVal != RT_ERR_OK)
		return retVal;

	return rtl8367b_setAsicRegBits(RTL8367B_REG_SYS_TIME_OFFSET_512NS_H,
				       RTL8367B_SYS_TIME_OFFSET_512NS_H_SYS_TIME_OFFSET_512NS_MASK,
				       (ns >> 16));
}
...
ret_t rtl8367b_getAsicEavTimeOffset512ns(rtk_uint32 * pNs)
{
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_SYS_TIME_OFFSET_512NS_L, &regData);
	if (retVal != RT_ERR_OK)
		return retVal;

	retVal =
	    rtl8367b_getAsicRegBits(RTL8367B_REG_SYS_TIME_OFFSET_512NS_H,
				    RTL8367B_SYS_TIME_OFFSET_512NS_H_SYS_TIME_OFFSET_512NS_MASK,
				    &regData2);
	if (retVal != RT_ERR_OK)
		return retVal;
...
{
	return rtl8367b_setAsicRegBit(RTL8367B_REG_SYS_TIME_OFFSET_512NS_H,
				      RTL8367B_SYS_TIME_OFFSET_TUNE_OFFSET,
				      enabled);
}
...
{
	return rtl8367b_getAsicReg(RTL8367B_REG_SYS_TIME_SEC_TRANSIT,
				   pTransmit);
}
...
ret_t rtl8367b_getAsicEavSystemTimeSeccond(rtk_uint32 * pSecond)
{
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_SYS_TIME_SEC_HIGH_L, &regData);
	if (retVal != RT_ERR_OK)
		return retVal;

	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_SYS_TIME_SEC_HIGH_H, &regData2);
	if (retVal != RT_ERR_OK)
		return retVal;
...
ret_t rtl8367b_getAsicEavSystemTime512ns(rtk_uint32 * pNs)
{
	...
	retVal =
	    rtl8367b_getAsicRegBits(RTL8367B_REG_SYS_TIME_512NS_H,
				    RTL8367B_SYS_TIME_512NS_H_MASK, &regData2);
	if (retVal != RT_ERR_OK)
		return retVal;
...
ret_t rtl8367b_setAsicEavTimeSyncEn(rtk_uint32 port, rtk_uint32 enabled)
{
	...
	return rtl8367b_setAsicRegBit(RTL8367B_REG_PTP_PORT0_CFG1 +
				      (port * 0x20),
				      RTL8367B_PTP_PORT0_CFG1_TIME_SYNC_EN_OFFSET,
				      enabled);
}
...
ret_t rtl8367b_getAsicEavTimeSyncEn(rtk_uint32 port, rtk_uint32 * pEnabled)
{
	...
	return rtl8367b_getAsicRegBit(RTL8367B_REG_PTP_PORT0_CFG1 +
				      (port * 0x20),
				      RTL8367B_PTP_PORT0_CFG1_TIME_SYNC_EN_OFFSET,
				      pEnabled);
}
...
ret_t rtl8367b_setAsicEavTimeStampFillEn(rtk_uint32 port, rtk_uint32 enabled)
{
	...
	return rtl8367b_setAsicRegBit(RTL8367B_REG_PTP_PORT0_CFG1 +
				      (port * 0x20),
				      RTL8367B_PTP_PORT0_CFG1_TIME_SYNC_TX_TS_FILL_EN_OFFSET,
				      enabled);
}
...
ret_t rtl8367b_getAsicEavTimeSyncValid(rtk_uint32 port, rtk_uint32 * pValid)
{
	...
	return rtl8367b_getAsicRegBit(RTL8367B_REG_PTP_PORT0_CFG1 +
				      (port * 0x20),
				      RTL8367B_PTP_PORT0_CFG1_EGRESS_TS_VALID_OFFSET,
				      pValid);
}
...
ret_t rtl8367b_getAsicEavEgressTimestamp512ns(rtk_uint32 port, rtk_uint32 * pNs)
{
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_PTP_PORT0_CFG0 + (port * 0x20),
				&regData);
	if (retVal != RT_ERR_OK)
		return retVal;

	retVal =
	    rtl8367b_getAsicRegBits(RTL8367B_REG_PTP_PORT0_CFG1 + (port * 0x20),
				    RTL8367B_PTP_PORT0_CFG1_EGRESS_TS_512NS_MASK,
				    &regData2);
	if (retVal != RT_ERR_OK)
		return retVal;
 ...
 *      If this transmit is set/clear which means there is 512NS carry event
 */
ret_t rtl8367b_getAsicEavEgressTimestampSeccond(rtk_uint32 port,
						rtk_uint32 * pSecond)
{
	if (port > RTL8367B_PORTIDMAX)
		return RT_ERR_PORT_ID;

	return rtl8367b_getAsicRegBits(RTL8367B_REG_PTP_PORT0_CFG1 +
				       (port * 0x20),
				       RTL8367B_PTP_PORT0_CFG1_TIME_TS_SECOND_MASK,
				       pSecond);
}
