...
ret_t rtl8367b_getAsicGreenPortPage(rtk_uint32 port, rtk_uint32 * pPage)
{
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_PAGEMETER_PORT_REG(port), &regData);
	if (retVal != RT_ERR_OK)
		return retVal;
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_PAGEMETER_PORT_REG(port) + 1,
				&regData);
	if (retVal != RT_ERR_OK)
		return retVal;
 ...
 *      None
 */
ret_t rtl8367b_setAsicGreenTrafficType(rtk_uint32 priority,
				       rtk_uint32 traffictype)
{
	...
	return rtl8367b_setAsicRegBit(RTL8367B_REG_HIGHPRI_CFG, priority,
				      (traffictype ? 1 : 0));
}
 ...
 *      None
 */
ret_t rtl8367b_getAsicGreenTrafficType(rtk_uint32 priority,
				       rtk_uint32 * pTraffictype)
{
	if (priority > RTL8367B_PRIMAX)
		return RT_ERR_QOS_INT_PRIORITY;

	return rtl8367b_getAsicRegBit(RTL8367B_REG_HIGHPRI_CFG, priority,
				      pTraffictype);
}
 ...
 *      None
 */
ret_t rtl8367b_getAsicGreenHighPriorityTraffic(rtk_uint32 port,
					       rtk_uint32 * pIndicator)
{
	if (port > RTL8367B_PORTIDMAX)
		return RT_ERR_PORT_ID;
...
ret_t rtl8367b_setAsicGreenEthernet(rtk_uint32 green)
{
	...
	if ((data == 0x0276) || (data == 0x0597) || (data == 0x6367)) {
		for (port = 0; port <= RTL8367B_PHY_INTERNALNOMAX; port++) {
			/* 0xa420[2:0] */
			if ((retVal =
			     rtl8367b_getAsicPHYOCPReg(port, 0xA420,
						       &regData)) != RT_ERR_OK)
				return retVal;
			...
			if (phy_status == 3) {
				/* 0xb820[4] = 1 */
				if ((retVal =
				     rtl8367b_getAsicPHYOCPReg(port, 0xB820,
							       &regData)) !=
				    RT_ERR_OK)
					return retVal;
				...
				if ((retVal =
				     rtl8367b_setAsicPHYOCPReg(port, 0xB820,
							       regData)) !=
				    RT_ERR_OK)
					return retVal;
				...
				while (checkCounter) {
					retVal =
					    rtl8367b_getAsicPHYOCPReg(port,
								      0xB800,
								      &regData);
					if ((retVal != RT_ERR_OK)
					    || ((regData & 0x0040) != 0x0040)) {
						checkCounter--;
						if (0 == checkCounter) {
							return
			...
			if ((retVal =
			     rtl8367b_setAsicPHYOCPReg(port, 0xA438,
						       regData)) != RT_ERR_OK)
				return retVal;
	...
	if ((retVal =
	     rtl8367b_setAsicRegBit(RTL8367B_REG_PHY_AD,
				    RTL8367B_EN_PHY_GREEN_OFFSET,
				    green)) != RT_ERR_OK)
		return retVal;
...
{
	return rtl8367b_getAsicRegBit(RTL8367B_REG_PHY_AD,
				      RTL8367B_EN_PHY_GREEN_OFFSET, green);
}
...
ret_t rtl8367b_setAsicPowerSaving(rtk_uint32 phy, rtk_uint32 enable)
{
	...
	if ((data == 0x0276) || (data == 0x0597) || (data == 0x6367)) {
		...
		/* 0xa420[2:0] */
		if ((retVal =
		     rtl8367b_getAsicPHYOCPReg(phy, 0xA420,
					       &regData)) != RT_ERR_OK)
			return retVal;
		...
		if (phy_status == 3) {
			/* 0xb820[4] = 1 */
			if ((retVal =
			     rtl8367b_getAsicPHYOCPReg(phy, 0xB820,
						       &regData)) != RT_ERR_OK)
				return retVal;
			...
			if ((retVal =
			     rtl8367b_setAsicPHYOCPReg(phy, 0xB820,
						       regData)) != RT_ERR_OK)
				return retVal;
			...
			while (checkCounter) {
				retVal =
				    rtl8367b_getAsicPHYOCPReg(phy, 0xB800,
							      &regData);
				if ((retVal != RT_ERR_OK)
				    || ((regData & 0x0040) != 0x0040)) {
					checkCounter--;
		...
		if ((retVal =
		     rtl8367b_getAsicPHYReg(phy, phyReg,
					    &phyData)) != RT_ERR_OK)
			return retVal;
		...
		if ((retVal =
		     rtl8367b_setAsicPHYReg(phy, phyReg, phyData)) != RT_ERR_OK)
			return retVal;
		...
		if ((retVal =
		     rtl8367b_setAsicPHYReg(phy, RTL8367B_PHY_PAGE_ADDRESS,
					    0)) != RT_ERR_OK)
			return retVal;
		...
		phyData =
		    (phyData & (~PHY_POWERSAVING_MASK)) | (enable <<
							   PHY_POWERSAVING_OFFSET);
