 ...
 */
ret_t rtl8367b_setAsicFieldSelector(rtk_uint32 index, rtk_uint32 format,
				    rtk_uint32 offset)
{
	rtk_uint32 regData;
	...
	regData =
	    (((format << RTL8367B_FIELD_SELECTOR_FORMAT_OFFSET) &
	      RTL8367B_FIELD_SELECTOR_FORMAT_MASK) | ((offset <<
						       RTL8367B_FIELD_SELECTOR_OFFSET_OFFSET)
						      &
						      RTL8367B_FIELD_SELECTOR_OFFSET_MASK));
 ...
 *      None
 */
ret_t rtl8367b_getAsicFieldSelector(rtk_uint32 index, rtk_uint32 * pFormat,
				    rtk_uint32 * pOffset)
{
	ret_t retVal;
	rtk_uint32 regData;
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_FIELD_SELECTOR_REG(index), &regData);
	if (retVal != RT_ERR_OK)
		return retVal;

	*pFormat =
	    ((regData & RTL8367B_FIELD_SELECTOR_FORMAT_MASK) >>
	     RTL8367B_FIELD_SELECTOR_FORMAT_OFFSET);
	*pOffset =
	    ((regData & RTL8367B_FIELD_SELECTOR_OFFSET_MASK) >>
	     RTL8367B_FIELD_SELECTOR_OFFSET_OFFSET);
