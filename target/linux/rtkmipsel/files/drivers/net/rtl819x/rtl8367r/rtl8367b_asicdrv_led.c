...
{
	ret_t retVal;
	CONST rtk_uint16 bits[RTL8367B_LEDGROUPNO] =
	    { RTL8367B_LED0_CFG_MASK, RTL8367B_LED1_CFG_MASK,
		RTL8367B_LED2_CFG_MASK
	};
	...
	retVal =
	    rtl8367b_setAsicRegBit(RTL8367B_REG_LED_CONFIGURATION,
				   RTL8367B_LED_CONFIG_SEL_OFFSET, 0);
	if (retVal != RT_ERR_OK)
		return retVal;

	return rtl8367b_setAsicRegBits(RTL8367B_REG_LED_CONFIGURATION,
				       bits[ledno], config);
}
 ...
 *		None
 */
ret_t rtl8367b_getAsicLedIndicateInfoConfig(rtk_uint32 ledno,
					    rtk_uint32 * pConfig)
{
	CONST rtk_uint16 bits[RTL8367B_LEDGROUPNO] =
	    { RTL8367B_LED0_CFG_MASK, RTL8367B_LED1_CFG_MASK,
...
ret_t rtl8367b_setAsicLedGroupMode(rtk_uint32 mode)
{
	...
	retVal =
	    rtl8367b_setAsicRegBit(RTL8367B_REG_LED_CONFIGURATION,
				   RTL8367B_LED_CONFIG_SEL_OFFSET, 1);
	if (retVal != RT_ERR_OK)
		return retVal;

	return rtl8367b_setAsicRegBits(RTL8367B_REG_LED_CONFIGURATION,
				       RTL8367B_DATA_LED_MASK, mode);
}
...
ret_t rtl8367b_getAsicLedGroupMode(rtk_uint32 * pMode)
{
	...
	retVal =
	    rtl8367b_getAsicRegBit(RTL8367B_REG_LED_CONFIGURATION,
				   RTL8367B_LED_CONFIG_SEL_OFFSET, &regData);
	if (retVal != RT_ERR_OK)
		return retVal;
 ...
 *		None
 */
ret_t rtl8367b_setAsicForceLed(rtk_uint32 port, rtk_uint32 group,
			       rtk_uint32 mode)
{
	rtk_uint16 regAddr;
 ...
 *		None
 */
ret_t rtl8367b_getAsicForceLed(rtk_uint32 port, rtk_uint32 group,
			       rtk_uint32 * pMode)
{
	rtk_uint16 regAddr;
...
ret_t rtl8367b_setAsicForceGroupLed(rtk_uint32 groupmask, rtk_uint32 mode)
{
	...
	retVal =
	    rtl8367b_setAsicRegBits(RTL8367B_LED_FORCE_CTRL,
				    RTL8367B_LED_FORCE_MODE_MASK, bitmask);

	retVal =
	    rtl8367b_setAsicRegBits(RTL8367B_LED_FORCE_CTRL,
				    RTL8367B_FORCE_MODE_MASK, mode);

	if (LEDFORCEMODE_NORMAL == mode)
		retVal =
		    rtl8367b_setAsicRegBits(RTL8367B_LED_FORCE_CTRL,
					    RTL8367B_LED_FORCE_MODE_MASK, 0);
...
ret_t rtl8367b_getAsicForceGroupLed(rtk_uint32 * groupmask, rtk_uint32 * pMode)
{
	...
	/* Get Related Registers */
	retVal =
	    rtl8367b_getAsicRegBits(RTL8367B_LED_FORCE_CTRL,
				    RTL8367B_LED_FORCE_MODE_MASK, &regData);
	...
	return rtl8367b_getAsicRegBits(RTL8367B_LED_FORCE_CTRL,
				       RTL8367B_FORCE_MODE_MASK, pMode);
}
...
ret_t rtl8367b_setAsicLedBlinkRate(rtk_uint32 blinkRate)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_REG_LED_MODE,
				       RTL8367B_SEL_LEDRATE_MASK, blinkRate);
}
...
{
	return rtl8367b_getAsicRegBits(RTL8367B_REG_LED_MODE,
				       RTL8367B_SEL_LEDRATE_MASK, pBlinkRate);
}
...
ret_t rtl8367b_setAsicLedForceBlinkRate(rtk_uint32 blinkRate)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_REG_LED_MODE,
				       RTL8367B_FORCE_RATE_MASK, blinkRate);
}
...
{
	return rtl8367b_getAsicRegBits(RTL8367B_REG_LED_MODE,
				       RTL8367B_FORCE_RATE_MASK, pBlinkRate);
}
...
ret_t rtl8367b_setAsicLedGroupEnable(rtk_uint32 group, rtk_uint32 portmask)
{
	...
	if ((retVal =
	     rtl8367b_setAsicRegBits(regAddr, regDataMask,
				     portmask)) != RT_ERR_OK)
		return retVal;
...
ret_t rtl8367b_getAsicLedGroupEnable(rtk_uint32 group, rtk_uint32 * portmask)
{
	...
	if ((retVal =
	     rtl8367b_getAsicRegBits(regAddr, regDataMask,
				     portmask)) != RT_ERR_OK)
		return retVal;
...
ret_t rtl8367b_getAsicLedOperationMode(rtk_uint32 * mode)
{
	...
	if ((retVal =
	     rtl8367b_getAsicRegBit(RTL8367B_REG_LED_SYS_CONFIG,
				    RTL8367B_LED_SELECT_OFFSET,
				    &regData)) != RT_ERR_OK)
		return retVal;
...
@comm
    The API can turn on/off led serial mode and set signal to active high/low.
 */
ret_t rtl8367b_setAsicLedSerialModeConfig(rtk_uint32 active,
					  rtk_uint32 serimode)
{
	ret_t retVal;
	...
	if ((retVal =
	     rtl8367b_setAsicRegBit(RTL8367B_REG_LED_SYS_CONFIG,
				    RTL8367B_SERI_LED_ACT_LOW_OFFSET,
				    active)) != RT_ERR_OK)
		return retVal;

	/*set to 8G mode (not 16G mode) */
	if ((retVal =
	     rtl8367b_setAsicRegBit(RTL8367B_REG_LED_MODE,
				    RTL8367B_DLINK_TIME_OFFSET,
				    serimode)) != RT_ERR_OK)
		return retVal;
...
@comm
    The API can get LED serial mode setup and get signal active high/low.
 */
ret_t rtl8367b_getAsicLedSerialModeConfig(rtk_uint32 * active,
					  rtk_uint32 * serimode)
{
	ret_t retVal;
	...
	if ((retVal =
	     rtl8367b_getAsicRegBit(RTL8367B_REG_LED_MODE,
				    RTL8367B_DLINK_TIME_OFFSET,
				    serimode)) != RT_ERR_OK)
		return retVal;
