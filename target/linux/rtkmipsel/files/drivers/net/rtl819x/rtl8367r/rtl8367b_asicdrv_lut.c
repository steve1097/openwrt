...
{
	return rtl8367b_setAsicRegBit(RTL8367B_REG_LUT_CFG,
				      RTL8367B_LUT_IPMC_HASH_OFFSET, enabled);
}
...
{
	return rtl8367b_getAsicRegBit(RTL8367B_REG_LUT_CFG,
				      RTL8367B_LUT_IPMC_HASH_OFFSET, pEnabled);
}
...
{
	return rtl8367b_setAsicRegBit(RTL8367B_REG_LUT_CFG,
				      RTL8367B_LUT_IPMC_LOOKUP_OP_OFFSET, type);
}
...
{
	return rtl8367b_getAsicRegBit(RTL8367B_REG_LUT_CFG,
				      RTL8367B_LUT_IPMC_LOOKUP_OP_OFFSET,
				      pType);
}
...
ret_t rtl8367b_setAsicLutAgeTimerSpeed(rtk_uint32 timer, rtk_uint32 speed)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_REG_LUT_CFG,
				       RTL8367B_AGE_TIMER_MASK |
				       RTL8367B_AGE_SPEED_MASK,
				       (timer << RTL8367B_AGE_TIMER_OFFSET) |
				       (speed << RTL8367B_AGE_SPEED_OFFSET));
}
...
ret_t rtl8367b_getAsicLutAgeTimerSpeed(rtk_uint32 * pTimer, rtk_uint32 * pSpeed)
{
	...
	*pTimer =
	    (regData & RTL8367B_AGE_TIMER_MASK) >> RTL8367B_AGE_TIMER_OFFSET;

	*pSpeed =
	    (regData & RTL8367B_AGE_SPEED_MASK) >> RTL8367B_AGE_SPEED_OFFSET;
...
ret_t rtl8367b_setAsicLutCamTbUsage(rtk_uint32 enabled)
{
	...
	retVal =
	    rtl8367b_setAsicRegBit(RTL8367B_REG_LUT_CFG,
				   RTL8367B_BCAM_DISABLE_OFFSET,
				   enabled ? 0 : 1);
...
ret_t rtl8367b_getAsicLutCamTbUsage(rtk_uint32 * pEnabled)
{
	...
	if ((retVal =
	     rtl8367b_getAsicRegBit(RTL8367B_REG_LUT_CFG,
				    RTL8367B_BCAM_DISABLE_OFFSET,
				    &regData)) != RT_ERR_OK)
		return retVal;
...
ret_t rtl8367b_setAsicLutLearnLimitNo(rtk_uint32 port, rtk_uint32 number)
{
	...
	return rtl8367b_setAsicReg(RTL8367B_LUT_PORT_LEARN_LIMITNO_REG(port),
				   number);
}
...
ret_t rtl8367b_getAsicLutLearnLimitNo(rtk_uint32 port, rtk_uint32 * pNumber)
{
	...
	return rtl8367b_getAsicReg(RTL8367B_LUT_PORT_LEARN_LIMITNO_REG(port),
				   pNumber);
}
...
ret_t rtl8367b_setAsicLutLearnOverAct(rtk_uint32 action)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_REG_PORT_SECURITY_CTRL,
				       RTL8367B_LUT_LEARN_OVER_ACT_MASK,
				       action);
}
...
{
	return rtl8367b_getAsicRegBits(RTL8367B_REG_PORT_SECURITY_CTRL,
				       RTL8367B_LUT_LEARN_OVER_ACT_MASK,
				       pAction);
}
...
void _rtl8367b_fdbStSmi2User(rtl8367b_luttb * pLutSt, rtl8367b_fdbtb * pFdbSmi)
{
	...
	if (pFdbSmi->smi_ipmul.l3lookup) {
		...
		pLutSt->dip =
		    (pLutSt->dip << 8) | pFdbSmi->smi_ipmul.dip3 | 0xE0;
#else
...
ret_t rtl8367b_setAsicL2LookupTb(rtl8367b_luttb * pL2Table)
{
	...
	for (i = 0; i < RTL8367B_LUT_ENTRY_SIZE; i++) {
		retVal =
		    rtl8367b_setAsicReg(RTL8367B_TABLE_ACCESS_WRDATA_BASE + i,
					regData);
		if (retVal != RT_ERR_OK)
			return retVal;
	...
	tblCmd =
	    (RTL8367B_TABLE_ACCESS_REG_DATA(TB_OP_WRITE, TB_TARGET_L2)) &
	...
	retVal =
	    rtl8367b_getAsicRegBit(RTL8367B_TABLE_ACCESS_STATUS_REG,
				   RTL8367B_HIT_STATUS_OFFSET, &regData);
	if (retVal != RT_ERR_OK)
		return retVal;
	...
	/*Read access address */
	retVal =
	    rtl8367b_getAsicRegBits(RTL8367B_TABLE_ACCESS_STATUS_REG,
				    RTL8367B_TYPE_MASK |
				    RTL8367B_TABLE_LUT_ADDR_ADDRESS_MASK,
				    &regData);
	if (retVal != RT_ERR_OK)
		return retVal;
...
ret_t rtl8367b_getAsicL2LookupTb(rtk_uint32 method, rtl8367b_luttb * pL2Table)
{
	...
	tblCmd =
	    (method << RTL8367B_ACCESS_METHOD_OFFSET) &
	    RTL8367B_ACCESS_METHOD_MASK;
	...
	case LUTREADMETHOD_NEXT_L2L3MC:
		retVal =
		    rtl8367b_setAsicReg(RTL8367B_TABLE_ACCESS_ADDR_REG,
					pL2Table->address);
		if (retVal != RT_ERR_OK)
			return retVal;
		break;
	case LUTREADMETHOD_MAC:
		...
		for (i = 0; i < RTL8367B_LUT_ENTRY_SIZE; i++) {
			retVal =
			    rtl8367b_setAsicReg
			    (RTL8367B_TABLE_ACCESS_WRDATA_BASE + i, regData);
			if (retVal != RT_ERR_OK)
				return retVal;
	...
	case LUTREADMETHOD_NEXT_L2UCSPA:
		...
		tblCmd =
		    tblCmd | ((pL2Table->spa << RTL8367B_SPA_OFFSET) &
			      RTL8367B_SPA_MASK);
	...
	tblCmd =
	    tblCmd | ((RTL8367B_TABLE_ACCESS_REG_DATA(TB_OP_READ, TB_TARGET_L2))
	...
	if (!pL2Table->lookup_hit) {
		...
		if (pL2Table->address >= 0x800) {
			retVal =
			    rtl8367b_getAsicReg
			    (RTL8367B_TABLE_ACCESS_RDDATA_BASE + 5, &regData);
			if (retVal != RT_ERR_OK)
				return retVal;
	...
	for (i = 0; i < RTL8367B_LUT_ENTRY_SIZE; i++) {
		retVal =
		    rtl8367b_getAsicReg(RTL8367B_TABLE_ACCESS_RDDATA_BASE + i,
					&regData);
		if (retVal != RT_ERR_OK)
			return retVal;
...
ret_t rtl8367b_getAsicLutLearnNo(rtk_uint32 port, rtk_uint32 * pNumber)
{
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_L2_LRN_CNT_REG(port), pNumber);
...
ret_t rtl8367b_setAsicLutForceFlush(rtk_uint32 portmask)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_FORCE_FLUSH_REG,
				       RTL8367B_FORCE_FLUSH_PORTMASK_MASK,
				       portmask);
}
...
{
	return rtl8367b_getAsicRegBits(RTL8367B_FORCE_FLUSH_REG,
				       RTL8367B_BUSY_STATUS_MASK, pPortmask);
}
...
ret_t rtl8367b_setAsicLutFlushMode(rtk_uint32 mode)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_REG_L2_FLUSH_CTRL2,
				       RTL8367B_LUT_FLUSH_MODE_MASK, mode);
}
...
{
	return rtl8367b_getAsicRegBits(RTL8367B_REG_L2_FLUSH_CTRL2,
				       RTL8367B_LUT_FLUSH_MODE_MASK, pMode);
}
...
{
	return rtl8367b_setAsicRegBit(RTL8367B_REG_L2_FLUSH_CTRL2,
				      RTL8367B_LUT_FLUSH_TYPE_OFFSET, type);
}
...
{
	return rtl8367b_getAsicRegBit(RTL8367B_REG_L2_FLUSH_CTRL2,
				      RTL8367B_LUT_FLUSH_TYPE_OFFSET, pType);
}
...
ret_t rtl8367b_setAsicLutFlushVid(rtk_uint32 vid)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_REG_L2_FLUSH_CTRL1,
				       RTL8367B_LUT_FLUSH_VID_MASK, vid);
}
...
{
	return rtl8367b_getAsicRegBits(RTL8367B_REG_L2_FLUSH_CTRL1,
				       RTL8367B_LUT_FLUSH_VID_MASK, pVid);
}
...
ret_t rtl8367b_setAsicLutFlushFid(rtk_uint32 fid)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_REG_L2_FLUSH_CTRL1,
				       RTL8367B_LUT_FLUSH_FID_MASK, fid);
}
...
{
	return rtl8367b_getAsicRegBits(RTL8367B_REG_L2_FLUSH_CTRL1,
				       RTL8367B_LUT_FLUSH_FID_MASK, pFid);
}
...
ret_t rtl8367b_setAsicLutDisableAging(rtk_uint32 port, rtk_uint32 disabled)
{
	...
	return rtl8367b_setAsicRegBit(RTL8367B_LUT_AGEOUT_CTRL_REG, port,
				      disabled);
}
...
ret_t rtl8367b_getAsicLutDisableAging(rtk_uint32 port, rtk_uint32 * pDisabled)
{
	...
	return rtl8367b_getAsicRegBit(RTL8367B_LUT_AGEOUT_CTRL_REG, port,
				      pDisabled);
}
...
ret_t rtl8367b_setAsicLutLinkDownForceAging(rtk_uint32 enable)
{
	...
	return rtl8367b_setAsicRegBit(RTL8367B_REG_LUT_CFG,
				      RTL8367B_LINKDOWN_AGEOUT_OFFSET,
				      enable ? 0 : 1);
}
