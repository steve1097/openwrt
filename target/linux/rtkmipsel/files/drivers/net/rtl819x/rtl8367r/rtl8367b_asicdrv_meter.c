 ...
 *      None
 */
ret_t rtl8367b_setAsicShareMeter(rtk_uint32 index, rtk_uint32 rate,
				 rtk_uint32 ifg)
{
	ret_t retVal;
	...
	/*17-bits Rate */
	retVal =
	    rtl8367b_setAsicReg(RTL8367B_METER_RATE_REG(index), rate & 0xFFFF);
	if (retVal != RT_ERR_OK)
		return retVal;

	retVal =
	    rtl8367b_setAsicReg(RTL8367B_METER_RATE_REG(index) + 1,
				(rate & 0x10000) >> 16);
	if (retVal != RT_ERR_OK)
		return retVal;
	 ...
	 /*IFG*/
	    retVal =
	    rtl8367b_setAsicRegBit(RTL8367B_METER_IFG_CTRL_REG(index),
				   RTL8367B_METER_IFG_OFFSET(index), ifg);
 ...
 *      None
 */
ret_t rtl8367b_getAsicShareMeter(rtk_uint32 index, rtk_uint32 * pRate,
				 rtk_uint32 * pIfg)
{
	rtk_uint32 regData;
	rtk_uint32 regData2;
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_METER_RATE_REG(index) + 1, &regData2);
	if (retVal != RT_ERR_OK)
		return retVal;
	...
	*pRate = ((regData2 << 16) & 0x10000) | regData;
	 /*IFG*/
	    retVal =
	    rtl8367b_getAsicRegBit(RTL8367B_METER_IFG_CTRL_REG(index),
				   RTL8367B_METER_IFG_OFFSET(index), pIfg);
 ...
 *      None
 */
ret_t rtl8367b_setAsicShareMeterBucketSize(rtk_uint32 index,
					   rtk_uint32 lbthreshold)
{
	if (index > RTL8367B_METERMAX)
		return RT_ERR_FILTER_METER_ID;

	return rtl8367b_setAsicReg(RTL8367B_METER_BUCKET_SIZE_REG(index),
				   lbthreshold);
}
 ...
 *      None
 */
ret_t rtl8367b_getAsicShareMeterBucketSize(rtk_uint32 index,
					   rtk_uint32 * pLbthreshold)
{
	if (index > RTL8367B_METERMAX)
		return RT_ERR_FILTER_METER_ID;

	return rtl8367b_getAsicReg(RTL8367B_METER_BUCKET_SIZE_REG(index),
				   pLbthreshold);
}
...
ret_t rtl8367b_setAsicMeterExceedStatus(rtk_uint32 index)
{
	...
	return
	    rtl8367b_setAsicRegBit(RTL8367B_METER_OVERRATE_INDICATOR_REG(index),
				   RTL8367B_METER_EXCEED_OFFSET(index), 1);
}
...
ret_t rtl8367b_getAsicMeterExceedStatus(rtk_uint32 index, rtk_uint32 * pStatus)
{
	...
	return
	    rtl8367b_getAsicRegBit(RTL8367B_METER_OVERRATE_INDICATOR_REG(index),
				   RTL8367B_METER_EXCEED_OFFSET(index),
				   pStatus);
}
