...
ret_t rtl8367b_setAsicMacAddress(ether_addr_t mac)
{
	...
	for (i = 0; i <= 2; i++) {
		retVal =
		    rtl8367b_setAsicReg(RTL8367B_REG_SWITCH_MAC2 - i, regData);
		if (retVal != RT_ERR_OK)
			return retVal;
...
ret_t rtl8367b_getAsicMacAddress(ether_addr_t * pMac)
{
	...
	for (i = 0; i <= 2; i++) {
		retVal =
		    rtl8367b_getAsicReg(RTL8367B_REG_SWITCH_MAC2 - i, &regData);
		if (retVal != RT_ERR_OK)
			return retVal;
...
ret_t rtl8367b_getAsicDebugInfo(rtk_uint32 port, rtk_uint32 * pDebugifo)
{
	...
	return rtl8367b_getAsicRegBits(RTL8367B_DEBUG_INFO_REG(port),
				       RTL8367B_DEBUG_INFO_MASK(port),
				       pDebugifo);
}
...
{
	return rtl8367b_setAsicRegBit(RTL8367B_REG_CFG_BACKPRESSURE,
				      RTL8367B_LONGTXE_OFFSET, mode);
}
...
{
	return rtl8367b_getAsicRegBit(RTL8367B_REG_CFG_BACKPRESSURE,
				      RTL8367B_LONGTXE_OFFSET, pMode);
}
...
{
	return rtl8367b_setAsicRegBits(RTL8367B_REG_MAX_LENGTH_LIMINT_IPG,
				       RTL8367B_MAX_LENTH_CTRL_MASK, maxLength);
}
...
{
	return rtl8367b_getAsicRegBits(RTL8367B_REG_MAX_LENGTH_LIMINT_IPG,
				       RTL8367B_MAX_LENTH_CTRL_MASK,
				       pMaxLength);
}
 ...
 *      None
 */
ret_t rtl8367b_setAsicMaxLengthAltTxRx(rtk_uint32 maxLength,
				       rtk_uint32 pmskGiga,
				       rtk_uint32 pmask100M)
{
	ret_t retVal;

	retVal =
	    rtl8367b_setAsicReg(RTL8367B_REG_MAX_LENGTH_CFG,
				((pmskGiga << RTL8367B_MAX_LENGTH_GIGA_OFFSET) &
				 RTL8367B_MAX_LENGTH_GIGA_MASK) | (pmask100M &
								   RTL8367B_MAX_LENGTH_10_100M_MASK));
	if (retVal != RT_ERR_OK)
		return retVal;

	return rtl8367b_setAsicRegBits(RTL8367B_REG_MAX_LEN_RX_TX,
				       RTL8367B_MAX_LEN_RX_TX_MASK, maxLength);
}
 ...
 *      None
 */
ret_t rtl8367b_getAsicMaxLengthAltTxRx(rtk_uint32 * pMaxLength,
				       rtk_uint32 * pPmskGiga,
				       rtk_uint32 * pPmask100M)
{
	ret_t retVal;
	rtk_uint32 regData;
	...
	*pPmskGiga =
	    (regData & RTL8367B_MAX_LENGTH_GIGA_MASK) >>
	    RTL8367B_MAX_LENGTH_GIGA_OFFSET;
	*pPmask100M = regData & RTL8367B_MAX_LENGTH_10_100M_MASK;

	return rtl8367b_getAsicRegBits(RTL8367B_REG_MAX_LEN_RX_TX,
				       RTL8367B_MAX_LEN_RX_TX_MASK, pMaxLength);
}

#include <rtk_api.h>

rtk_api_ret_t rtk_port_phyMdx_set(rtk_port_t port,
				  rtk_port_phy_mdix_mode_t mode)
{
	rtk_uint32 data, index, reg;
	rtk_api_ret_t retVal;
	rtk_uint32 regData;
	CONST_T rtk_uint16 regData1[][2] = { {0x201F, 0x0007}
	, {0x201E, 0x002D}
	, {0x2018, 0xF010}
	, {0x201F, 0x0000}
	,
	...
	CONST_T rtk_uint16 regData2[][2] = { {0x201F, 0x0007}
	, {0x201E, 0x002D}
	, {0x2018, 0xF020}
	, {0x201F, 0x0000}
	,
	...
	CONST_T rtk_uint16 regData3[][2] = { {0x201F, 0x0007}
	, {0x201E, 0x002D}
	, {0x2018, 0xF020}
	, {0x201F, 0x0000}
	,
	...
	if ((data == 0x0276) || (data == 0x0597) || (data == 0x6367)) {
		...
		case PHY_AUTO_CROSSOVER_MODE:
			if ((retVal =
			     rtl8367b_getAsicPHYReg(port, 24,
						    &regData)) != RT_ERR_OK)
				return retVal;
			...
			if ((retVal =
			     rtl8367b_setAsicPHYReg(port, 24,
						    regData)) != RT_ERR_OK)
				return retVal;
			break;
		...
		case PHY_AUTO_CROSSOVER_MODE:
			for (index = 0;
			     index <
			     (sizeof(regData1) / ((sizeof(rtk_uint16)) * 2));
			     index++) {
				reg = (regData1[index][0] - 0x2000) % 0x0020;
				if ((retVal =
				     rtl8367b_setAsicPHYReg(port, reg,
							    regData1[index][1]))
				    != RT_ERR_OK)
					return retVal;
			}
		...
		case PHY_FORCE_MDI_MODE:
			for (index = 0;
			     index <
			     (sizeof(regData2) / ((sizeof(rtk_uint16)) * 2));
			     index++) {
		...
		case PHY_FORCE_MDIX_MODE:
			for (index = 0;
			     index <
			     (sizeof(regData3) / ((sizeof(rtk_uint16)) * 2));
			     index++) {
...
rtk_api_ret_t rtk_port_phyMdx_get(rtk_port_t port,
				  rtk_port_phy_mdix_mode_t * pMode)
{
	rtk_uint32 data;
	rtk_api_ret_t retVal;
	rtk_uint32 regData;
	...
	if ((data == 0x0276) || (data == 0x0597) || (data == 0x6367)) {
		if ((retVal =
		     rtl8367b_getAsicPHYReg(port, 24, &regData)) != RT_ERR_OK)
			return retVal;
		...
		if ((retVal =
		     rtl8367b_setAsicPHYReg(port, 0x1F, 0x7)) != RT_ERR_OK)
			return retVal;

		if ((retVal =
		     rtl8367b_setAsicPHYReg(port, 0x1E, 0x2D)) != RT_ERR_OK)
			return retVal;

		if ((retVal =
		     rtl8367b_getAsicPHYReg(port, 0x18, &data)) != RT_ERR_OK)
			return retVal;

		if ((retVal =
		     rtl8367b_setAsicPHYReg(port, 0x1F, 0)) != RT_ERR_OK)
			return retVal;

		if (data & 0x0020) {
			if ((retVal =
			     rtl8367b_getAsicPHYReg(port, 0x10,
						    &data)) != RT_ERR_OK)
				return retVal;
