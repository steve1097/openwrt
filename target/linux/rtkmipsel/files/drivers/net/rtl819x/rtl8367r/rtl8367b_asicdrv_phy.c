 ...
 *      None
 */
ret_t rtl8367b_setAsicPHYReg(rtk_uint32 phyNo, rtk_uint32 phyAddr,
			     rtk_uint32 value)
{
	ret_t retVal;
	rtk_uint32 regAddr;
	...
	/* Default OCP Address */
	if ((retVal =
	     rtl8367b_setAsicRegBits(RTL8367B_REG_GPHY_OCP_MSB_0,
				     RTL8367B_CFG_CPU_OCPADR_MSB_MASK,
				     0x29)) != RT_ERR_OK)
		return retVal;
 ...
 *      None
 */
ret_t rtl8367b_getAsicPHYReg(rtk_uint32 phyNo, rtk_uint32 phyAddr,
			     rtk_uint32 * value)
{
	ret_t retVal;
	rtk_uint32 regAddr;
 ...
 *      None
 */
ret_t rtl8367b_setAsicPHYOCPReg(rtk_uint32 phyNo, rtk_uint32 ocpAddr,
				rtk_uint32 ocpData)
{
	ret_t retVal;
	rtk_uint32 regAddr;
	...
	regAddr =
	    RTL8367B_PHY_BASE | (ocpAddr9_6 << 8) | (phyNo <<
						     RTL8367B_PHY_OFFSET) |
	    ocpAddr5_1;
 ...
 *      None
 */
ret_t rtl8367b_getAsicPHYOCPReg(rtk_uint32 phyNo, rtk_uint32 ocpAddr,
				rtk_uint32 * pRegData)
{
	ret_t retVal;
	rtk_uint32 regAddr;
			     ...
			     rtk_uint32 phyData)
{
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_INDRECT_ACCESS_STATUS, &busyFlag);
	if (retVal != RT_ERR_OK)
		return retVal;
	...
	/*prepare access data */
	retVal =
	    rtl8367b_setAsicReg(RTL8367B_REG_INDRECT_ACCESS_WRITE_DATA,
				phyData);
	if (retVal != RT_ERR_OK)
		return retVal;
	...
	retVal =
	    rtl8367b_setAsicReg(RTL8367B_REG_INDRECT_ACCESS_ADDRESS, regData);
	if (retVal != RT_ERR_OK)
		return retVal;

	/*Set WRITE Command */
	retVal =
	    rtl8367b_setAsicReg(RTL8367B_REG_INDRECT_ACCESS_CTRL,
				RTL8367B_CMD_MASK | RTL8367B_RW_MASK);
	...
	while (checkCounter) {
		retVal =
		    rtl8367b_getAsicReg(RTL8367B_REG_INDRECT_ACCESS_STATUS,
					&busyFlag);
			     ...
			     rtk_uint32 * pRegData)
{
	...
	/*get PHY register */
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_INDRECT_ACCESS_READ_DATA,
				&regData);
	if (retVal != RT_ERR_OK)
		return retVal;
