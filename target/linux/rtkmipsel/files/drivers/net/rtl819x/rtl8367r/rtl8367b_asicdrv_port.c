...
ret_t rtl8367b_setAsicPortUnknownDaBehavior(rtk_uint32 behavior)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_PORT_SECURIT_CTRL_REG,
				       RTL8367B_UNKNOWN_UNICAST_DA_BEHAVE_MASK,
				       behavior);
}
...
{
	return rtl8367b_getAsicRegBits(RTL8367B_PORT_SECURIT_CTRL_REG,
				       RTL8367B_UNKNOWN_UNICAST_DA_BEHAVE_MASK,
				       pBehavior);
}
...
ret_t rtl8367b_setAsicPortUnknownSaBehavior(rtk_uint32 behavior)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_PORT_SECURIT_CTRL_REG,
				       RTL8367B_UNKNOWN_SA_BEHAVE_MASK,
				       behavior);
}
...
ret_t rtl8367b_setAsicPortUnmatchedSaBehavior(rtk_uint32 behavior)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_PORT_SECURIT_CTRL_REG,
				       RTL8367B_UNMATCHED_SA_BEHAVE_MASK,
				       behavior);
}
...
ret_t rtl8367b_setAsicPortUnknownDaFloodingPortmask(rtk_uint32 portmask)
{
	...
	return rtl8367b_setAsicReg(RTL8367B_UNUCAST_FLOADING_PMSK_REG,
				   portmask);
}
...
{
	return rtl8367b_getAsicReg(RTL8367B_UNUCAST_FLOADING_PMSK_REG,
				   pPortmask);
}
...
ret_t rtl8367b_setAsicPortUnknownMulticastFloodingPortmask(rtk_uint32 portmask)
{
	...
	return rtl8367b_setAsicReg(RTL8367B_UNMCAST_FLOADING_PMSK_REG,
				   portmask);
}
...
ret_t rtl8367b_getAsicPortUnknownMulticastFloodingPortmask(rtk_uint32 *
							   pPortmask)
{
	return rtl8367b_getAsicReg(RTL8367B_UNMCAST_FLOADING_PMSK_REG,
				   pPortmask);
}
...
ret_t rtl8367b_setAsicPortBlockSpa(rtk_uint32 port, rtk_uint32 permit)
{
	...
	return rtl8367b_setAsicRegBit(RTL8367B_SOURCE_PORT_BLOCK_REG, port,
				      permit);
}
...
ret_t rtl8367b_getAsicPortBlockSpa(rtk_uint32 port, rtk_uint32 * pPermit)
{
	...
	return rtl8367b_getAsicRegBit(RTL8367B_SOURCE_PORT_BLOCK_REG, port,
				      pPermit);
}
...
ret_t rtl8367b_setAsicPortDos(rtk_uint32 type, rtk_uint32 drop)
{
	...
	return rtl8367b_setAsicRegBit(RTL8367B_REG_DOS_CFG,
				      RTL8367B_DROP_DAEQSA_OFFSET + type, drop);
}
...
ret_t rtl8367b_getAsicPortDos(rtk_uint32 type, rtk_uint32 * pDrop)
{
	...
	return rtl8367b_getAsicRegBit(RTL8367B_REG_DOS_CFG,
				      RTL8367B_DROP_DAEQSA_OFFSET + type,
				      pDrop);
}
 ...
 *      None
 */
ret_t rtl8367b_setAsicPortForceLink(rtk_uint32 port,
				    rtl8367b_port_ability_t * pPortAbility)
{
	rtk_uint32 regData;
	...
	regData = *accessPtr;
	return rtl8367b_setAsicReg(RTL8367B_REG_MAC0_FORCE_SELECT + port,
				   regData);
}
 ...
 *      None
 */
ret_t rtl8367b_getAsicPortForceLink(rtk_uint32 port,
				    rtl8367b_port_ability_t * pPortAbility)
{
	ret_t retVal;
	rtk_uint32 regData;
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_MAC0_FORCE_SELECT + port,
				&regData);
	if (retVal != RT_ERR_OK)
		return retVal;
 ...
 *      None
 */
ret_t rtl8367b_getAsicPortStatus(rtk_uint32 port,
				 rtl8367b_port_status_t * pPortStatus)
{
	ret_t retVal;
	rtk_uint32 regData;
	rtk_uint16 *accessPtr;
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_REG_PORT0_STATUS + port, &regData);
	if (retVal != RT_ERR_OK)
		return retVal;
 ...
 *      None
 */
ret_t rtl8367b_setAsicPortForceLinkExt(rtk_uint32 id,
				       rtl8367b_port_ability_t * pPortAbility)
{
	rtk_uint32 reg_data;
	...
	if (0 == id || 1 == id)
		return rtl8367b_setAsicReg(RTL8367B_REG_DIGITAL_INTERFACE0_FORCE
					   + id, reg_data);
	else
		return
		    rtl8367b_setAsicReg(RTL8367B_REG_DIGITAL_INTERFACE2_FORCE,
					reg_data);
}
 ...
 *      None
 */
ret_t rtl8367b_getAsicPortForceLinkExt(rtk_uint32 id,
				       rtl8367b_port_ability_t * pPortAbility)
{
	rtk_uint32 reg_data;
	...
	if (0 == id || 1 == id)
		retVal =
		    rtl8367b_getAsicReg(RTL8367B_REG_DIGITAL_INTERFACE0_FORCE +
					id, &reg_data);
	else
		retVal =
		    rtl8367b_getAsicReg(RTL8367B_REG_DIGITAL_INTERFACE2_FORCE,
					&reg_data);
...
ret_t rtl8367b_setAsicPortExtMode(rtk_uint32 id, rtk_uint32 mode)
{
	...
	if (mode == EXT_GMII) {
		if ((retVal =
		     rtl8367b_setAsicRegBit(RTL8367B_REG_EXT0_RGMXF,
					    RTL8367B_EXT0_RGTX_INV_OFFSET,
					    1)) != RT_ERR_OK)
			return retVal;

		if ((retVal =
		     rtl8367b_setAsicRegBit(RTL8367B_REG_EXT1_RGMXF,
					    RTL8367B_EXT1_RGTX_INV_OFFSET,
					    1)) != RT_ERR_OK)
			return retVal;

		if ((retVal =
		     rtl8367b_setAsicRegBits(RTL8367B_REG_EXT_TXC_DLY,
					     RTL8367B_EXT1_GMII_TX_DELAY_MASK,
					     5)) != RT_ERR_OK)
			return retVal;

		if ((retVal =
		     rtl8367b_setAsicRegBits(RTL8367B_REG_EXT_TXC_DLY,
					     RTL8367B_EXT0_GMII_TX_DELAY_MASK,
					     6)) != RT_ERR_OK)
			return retVal;
	}
	...
	if ((mode == EXT_TMII_MAC) || (mode == EXT_TMII_PHY)) {
		if ((retVal =
		     rtl8367b_setAsicRegBit(RTL8367B_REG_BYPASS_LINE_RATE, id,
					    1)) != RT_ERR_OK)
			return retVal;
	} else {
		if ((retVal =
	...
	if (0 == id || 1 == id)
		return
		    ...
		    (RTL8367B_REG_DIGITAL_INTERFACE_SELECT,
		     RTL8367B_SELECT_GMII_0_MASK << (id *
						     RTL8367B_SELECT_GMII_1_OFFSET),
		     mode);
	else
		return
		    rtl8367b_setAsicRegBits
		    (RTL8367B_REG_DIGITAL_INTERFACE_SELECT_1,
		     RTL8367B_SELECT_RGMII_2_MASK, mode);
}
...
ret_t rtl8367b_getAsicPortExtMode(rtk_uint32 id, rtk_uint32 * pMode)
{
	...
	if (0 == id || 1 == id)
		...
		return
		    rtl8367b_getAsicRegBits
		    (RTL8367B_REG_DIGITAL_INTERFACE_SELECT_1,
		     RTL8367B_SELECT_RGMII_2_MASK, pMode);
}
...
ret_t rtl8367b_setAsicPortEnableAll(rtk_uint32 enable)
{
	...
	return rtl8367b_setAsicRegBit(RTL8367B_REG_PHY_AD,
				      RTL8367B_PDNPHY_OFFSET, !enable);
}
...
ret_t rtl8367b_getAsicPortEnableAll(rtk_uint32 * pEnable)
{
	...
	retVal =
	    rtl8367b_getAsicRegBit(RTL8367B_REG_PHY_AD, RTL8367B_PDNPHY_OFFSET,
				   &regData);
	if (retVal != RT_ERR_OK)
		return retVal;
...
ret_t rtl8367b_setAsicPortSmallIpg(rtk_uint32 port, rtk_uint32 enable)
{
	...
	return rtl8367b_setAsicRegBit(RTL8367B_PORT_SMALL_IPG_REG(port),
				      RTL8367B_PORT0_MISC_CFG_SMALL_TAG_IPG_OFFSET,
				      enable);
}
...
ret_t rtl8367b_getAsicPortSmallIpg(rtk_uint32 port, rtk_uint32 * pEnable)
{
	...
	return rtl8367b_getAsicRegBit(RTL8367B_PORT_SMALL_IPG_REG(port),
				      RTL8367B_PORT0_MISC_CFG_SMALL_TAG_IPG_OFFSET,
				      pEnable);
}
...
ret_t rtl8367b_setAsicPortLoopback(rtk_uint32 port, rtk_uint32 enable)
{
	...
	return rtl8367b_setAsicRegBit(RTL8367B_PORT_MISC_CFG_REG(port),
				      RTL8367B_PORT0_MISC_CFG_MAC_LOOPBACK_OFFSET,
				      enable);
}
...
ret_t rtl8367b_getAsicPortLoopback(rtk_uint32 port, rtk_uint32 * pEnable)
{
	...
	return rtl8367b_getAsicRegBit(RTL8367B_PORT_MISC_CFG_REG(port),
				      RTL8367B_PORT0_MISC_CFG_MAC_LOOPBACK_OFFSET,
				      pEnable);
}
	...
	if (regData == 0x6367) {
		for (port = 0; port < RTL8367B_PHYNO; port++) {
			if (portmask & (0x0001 << port)) {
				if ((retVal =
				     rtl8367b_setAsicPHYOCPReg(port, 0xa422,
							       0x00f1)) !=
				    RT_ERR_OK)
					return retVal;
			}
		...
		if ((retVal =
		     rtl8367b_setAsicRegBits(RTL8367B_REG_RTCT_ENABLE,
					     RTL8367B_RTCT_ENABLE_PORT_MASK_MASK,
					     portmask)) != RT_ERR_OK)
			return retVal;

		if ((retVal =
		     rtl8367b_setAsicRegBit(RTL8367B_REG_SEL_RTCT_PARA,
					    RTL8367B_EN_RTCT_TIMOUT_OFFSET,
					    1)) != RT_ERR_OK)
			return retVal;

		if ((retVal =
		     rtl8367b_setAsicRegBit(RTL8367B_REG_SEL_RTCT_PARA,
					    RTL8367B_EN_ALL_RTCT_OFFSET,
					    0)) != RT_ERR_OK)
			return retVal;

		if ((retVal =
		     rtl8367b_setAsicRegBit(RTL8367B_REG_SEL_RTCT_PARA,
					    RTL8367B_DO_RTCT_COMMAND_OFFSET,
					    0)) != RT_ERR_OK)
			return retVal;
 ...
 */
ret_t rtl8367b_getAsicPortRTCTResult(rtk_uint32 port,
				     rtl8367b_port_rtct_result_t * pResult)
{
	ret_t retVal;
	...
	if ((regData == 0x6367)) {
		if ((retVal =
		     rtl8367b_getAsicPHYOCPReg(port, 0xa422,
					       &regData)) != RT_ERR_OK)
			return retVal;
		...
		if ((regData & 0x8000) == 0x8000) {
			/* Channel A */
			if ((retVal =
			     rtl8367b_setAsicPHYOCPReg(port, 0xa436,
						       0x802a)) != RT_ERR_OK)
				return retVal;

			if ((retVal =
			     rtl8367b_getAsicPHYOCPReg(port, 0xa438,
						       &regData)) != RT_ERR_OK)
				return retVal;

			pResult->channelAOpen = (regData == 0x0048) ? 1 : 0;
			pResult->channelAShort = (regData == 0x0050) ? 1 : 0;
			pResult->channelAMismatch = ((regData == 0x0042)
						     || (regData ==
							 0x0044)) ? 1 : 0;
			pResult->channelALinedriver =
			    (regData == 0x0041) ? 1 : 0;
			...
			pResult->channelBMismatch = ((regData == 0x0042)
						     || (regData ==
							 0x0044)) ? 1 : 0;
			pResult->channelBLinedriver =
			    (regData == 0x0041) ? 1 : 0;
			...
			pResult->channelCMismatch = ((regData == 0x0042)
						     || (regData ==
							 0x0044)) ? 1 : 0;
			pResult->channelCLinedriver =
			    (regData == 0x0041) ? 1 : 0;
			...
			pResult->channelDMismatch = ((regData == 0x0042)
						     || (regData ==
							 0x0044)) ? 1 : 0;
			pResult->channelDLinedriver =
			    (regData == 0x0041) ? 1 : 0;
		...
		if ((retVal =
		     rtl8367b_setAsicPHYReg(port, RTL8367B_PHY_PAGE_ADDRESS,
					    RTL8367B_RTCT_PAGE)) != RT_ERR_OK)
			return retVal;

		if ((retVal =
		     rtl8367b_getAsicPHYReg(port, RTL8367B_RTCT_RESULT_A_REG,
					    &regData)) != RT_ERR_OK)
			return retVal;
		...
		if ((regData & 0x4000) == 0x4000) {
			...
			if ((retVal =
			     rtl8367b_getAsicPHYReg(port,
						    RTL8367B_RTCT_RESULT_B_REG,
						    &regData)) != RT_ERR_OK)
				return retVal;
