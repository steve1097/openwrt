 ...
 *      None
 */
ret_t rtl8367b_setAsicPriorityDot1qRemapping(rtk_uint32 srcpriority,
					     rtk_uint32 priority)
{
	...
	return
	    rtl8367b_setAsicRegBits(RTL8367B_QOS_1Q_PRIORITY_REMAPPING_REG
				    (srcpriority),
				    RTL8367B_QOS_1Q_PRIORITY_REMAPPING_MASK
 ...
 *      None
 */
ret_t rtl8367b_getAsicPriorityDot1qRemapping(rtk_uint32 srcpriority,
					     rtk_uint32 * pPriority)
{
	if (srcpriority > RTL8367B_PRIMAX)
		return RT_ERR_QOS_INT_PRIORITY;

	return
	    rtl8367b_getAsicRegBits(RTL8367B_QOS_1Q_PRIORITY_REMAPPING_REG
				    (srcpriority),
				    RTL8367B_QOS_1Q_PRIORITY_REMAPPING_MASK
				    (srcpriority), pPriority);
}
...
ret_t rtl8367b_setAsicPriorityPortBased(rtk_uint32 port, rtk_uint32 priority)
{
	...
	return
	    rtl8367b_setAsicRegBits(RTL8367B_QOS_PORTBASED_PRIORITY_REG(port),
				    RTL8367B_QOS_PORTBASED_PRIORITY_MASK(port),
				    priority);
}
...
ret_t rtl8367b_getAsicPriorityPortBased(rtk_uint32 port, rtk_uint32 * pPriority)
{
	...
	return
	    rtl8367b_getAsicRegBits(RTL8367B_QOS_PORTBASED_PRIORITY_REG(port),
				    RTL8367B_QOS_PORTBASED_PRIORITY_MASK(port),
				    pPriority);
}
...
ret_t rtl8367b_setAsicPriorityDscpBased(rtk_uint32 dscp, rtk_uint32 priority)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_QOS_DSCP_TO_PRIORITY_REG(dscp),
				       RTL8367B_QOS_DSCP_TO_PRIORITY_MASK(dscp),
				       priority);
}
...
ret_t rtl8367b_getAsicPriorityDscpBased(rtk_uint32 dscp, rtk_uint32 * pPriority)
{
	...
	return rtl8367b_getAsicRegBits(RTL8367B_QOS_DSCP_TO_PRIORITY_REG(dscp),
				       RTL8367B_QOS_DSCP_TO_PRIORITY_MASK(dscp),
				       pPriority);
}
 ...
 *      None
 */
ret_t rtl8367b_setAsicPriorityDecision(rtk_uint32 prisrc,
				       rtk_uint32 decisionPri)
{
	...
	return
	    rtl8367b_setAsicRegBits(RTL8367B_QOS_INTERNAL_PRIORITY_DECISION_REG
				    (prisrc),
				    RTL8367B_QOS_INTERNAL_PRIORITY_DECISION_MASK
				    (prisrc), decisionPri);
}
 ...
 *      None
 */
ret_t rtl8367b_getAsicPriorityDecision(rtk_uint32 prisrc,
				       rtk_uint32 * pDecisionPri)
{
	if (prisrc >= PRIDEC_END)
		return RT_ERR_QOS_SEL_PRI_SOURCE;

	return
	    rtl8367b_getAsicRegBits(RTL8367B_QOS_INTERNAL_PRIORITY_DECISION_REG
				    (prisrc),
				    RTL8367B_QOS_INTERNAL_PRIORITY_DECISION_MASK
				    (prisrc), pDecisionPri);
}
...
ret_t rtl8367b_setAsicOutputQueueMappingIndex(rtk_uint32 port, rtk_uint32 index)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_QOS_PORT_QUEUE_NUMBER_REG(port),
				       RTL8367B_QOS_PORT_QUEUE_NUMBER_MASK
				       (port), index);
}
 ...
 *      None
 */
ret_t rtl8367b_getAsicOutputQueueMappingIndex(rtk_uint32 port,
					      rtk_uint32 * pIndex)
{
	if (port > RTL8367B_PORTIDMAX)
		return RT_ERR_PORT_ID;

	return rtl8367b_getAsicRegBits(RTL8367B_QOS_PORT_QUEUE_NUMBER_REG(port),
				       RTL8367B_QOS_PORT_QUEUE_NUMBER_MASK
				       (port), pIndex);
}
 ...
 *      None
 */
ret_t rtl8367b_setAsicPriorityToQIDMappingTable(rtk_uint32 index,
						rtk_uint32 priority,
						rtk_uint32 qid)
{
	if (index >= RTL8367B_QUEUENO)
	...
	return
	    rtl8367b_setAsicRegBits(RTL8367B_QOS_1Q_PRIORITY_TO_QID_REG
				    (index, priority),
				    RTL8367B_QOS_1Q_PRIORITY_TO_QID_MASK
				    (priority), qid);
}
 ...
 *      None
 */
ret_t rtl8367b_getAsicPriorityToQIDMappingTable(rtk_uint32 index,
						rtk_uint32 priority,
						rtk_uint32 * pQid)
{
	if (index >= RTL8367B_QUEUENO)
	...
	return
	    rtl8367b_getAsicRegBits(RTL8367B_QOS_1Q_PRIORITY_TO_QID_REG
				    (index, priority),
				    RTL8367B_QOS_1Q_PRIORITY_TO_QID_MASK
				    (priority), pQid);
}
...
ret_t rtl8367b_setAsicRemarkingDot1pAbility(rtk_uint32 port, rtk_uint32 enabled)
{
	return rtl8367b_setAsicRegBit(RTL8367B_PORT_MISC_CFG_REG(port),
				      RTL8367B_1QREMARK_ENABLE_OFFSET, enabled);
}
 ...
 *      None
 */
ret_t rtl8367b_getAsicRemarkingDot1pAbility(rtk_uint32 port,
					    rtk_uint32 * pEnabled)
{
	return rtl8367b_getAsicRegBit(RTL8367B_PORT_MISC_CFG_REG(port),
				      RTL8367B_1QREMARK_ENABLE_OFFSET,
				      pEnabled);
}
 ...
 *      None
 */
ret_t rtl8367b_setAsicRemarkingDot1pParameter(rtk_uint32 priority,
					      rtk_uint32 newPriority)
{
	if (priority > RTL8367B_PRIMAX || newPriority > RTL8367B_PRIMAX)
		return RT_ERR_QOS_INT_PRIORITY;

	return rtl8367b_setAsicRegBits(RTL8367B_QOS_1Q_REMARK_REG(priority),
				       RTL8367B_QOS_1Q_REMARK_MASK(priority),
				       newPriority);
}
 ...
 *      None
 */
ret_t rtl8367b_getAsicRemarkingDot1pParameter(rtk_uint32 priority,
					      rtk_uint32 * pNewPriority)
{
	if (priority > RTL8367B_PRIMAX)
		return RT_ERR_QOS_INT_PRIORITY;

	return rtl8367b_getAsicRegBits(RTL8367B_QOS_1Q_REMARK_REG(priority),
				       RTL8367B_QOS_1Q_REMARK_MASK(priority),
				       pNewPriority);
}
...
{
	return rtl8367b_setAsicRegBit(RTL8367B_REMARKING_CTRL_REG,
				      RTL8367B_REMARKING_DSCP_ENABLE_OFFSET,
				      enabled);
}
...
{
	return rtl8367b_getAsicRegBit(RTL8367B_REMARKING_CTRL_REG,
				      RTL8367B_REMARKING_DSCP_ENABLE_OFFSET,
				      pEnabled);
}
 ...
 *      None
 */
ret_t rtl8367b_setAsicRemarkingDscpParameter(rtk_uint32 priority,
					     rtk_uint32 newDscp)
{
	if (priority > RTL8367B_PRIMAX)
		return RT_ERR_QOS_INT_PRIORITY;
	...
	return rtl8367b_setAsicRegBits(RTL8367B_QOS_DSCP_REMARK_REG(priority),
				       RTL8367B_QOS_DSCP_REMARK_MASK(priority),
				       newDscp);
}
 ...
 *      None
 */
ret_t rtl8367b_getAsicRemarkingDscpParameter(rtk_uint32 priority,
					     rtk_uint32 * pNewDscp)
{
	if (priority > RTL8367B_PRIMAX)
		return RT_ERR_QOS_INT_PRIORITY;

	return rtl8367b_getAsicRegBits(RTL8367B_QOS_DSCP_REMARK_REG(priority),
				       RTL8367B_QOS_DSCP_REMARK_MASK(priority),
				       pNewDscp);
}
