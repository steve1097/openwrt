...
ret_t rtl8367b_setAsicRma(rtk_uint32 index, rtl8367b_rma_t * pRmacfg)
{
	...
	if ((index >= 0x4 && index <= 0x7) || (index >= 0x9 && index <= 0x0C)
	    || (0x0F == index))
		index = 0x04;
	else if ((index >= 0x13 && index <= 0x17) || (0x19 == index)
		 || (index >= 0x1B && index <= 0x1f))
		index = 0x13;
	else if (index >= 0x22 && index <= 0x2F)
		index = 0x22;

	retVal =
	    rtl8367b_setAsicRegBits(RTL8367B_REG_RMA_CTRL00,
				    RTL8367B_TRAP_PRIORITY_MASK,
				    pRmacfg->trap_priority);
	if (retVal != RT_ERR_OK)
		return retVal;
...
ret_t rtl8367b_getAsicRma(rtk_uint32 index, rtl8367b_rma_t * pRmacfg)
{
	...
	*pRmacfg = *(rtl8367b_rma_t *) (&regData);
	...
	retVal =
	    rtl8367b_getAsicRegBits(RTL8367B_REG_RMA_CTRL00,
				    RTL8367B_TRAP_PRIORITY_MASK, &regData);
	if (retVal != RT_ERR_OK)
		return retVal;
