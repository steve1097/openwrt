...
ret_t rtl8367b_setAsicLeakyBucketParameter(rtk_uint32 tick, rtk_uint32 token)
{
	...
	retVal =
	    rtl8367b_setAsicRegBits(RTL8367B_LEAKY_BUCKET_TICK_REG,
				    RTL8367B_LEAKY_BUCKET_TICK_MASK, tick);

	if (retVal != RT_ERR_OK)
		return retVal;

	retVal =
	    rtl8367b_setAsicRegBits(RTL8367B_LEAKY_BUCKET_TOKEN_REG,
				    RTL8367B_LEAKY_BUCKET_TOKEN_MASK, token);
 ...
 *      None
 */
ret_t rtl8367b_getAsicLeakyBucketParameter(rtk_uint32 * tick,
					   rtk_uint32 * token)
{
	ret_t retVal;

	retVal =
	    rtl8367b_getAsicRegBits(RTL8367B_LEAKY_BUCKET_TICK_REG,
				    RTL8367B_LEAKY_BUCKET_TICK_MASK, tick);
	...
	retVal =
	    rtl8367b_getAsicRegBits(RTL8367B_LEAKY_BUCKET_TOKEN_REG,
				    RTL8367B_LEAKY_BUCKET_TOKEN_MASK, token);
 ...
 *      None
 */
ret_t rtl8367b_setAsicAprMeter(rtk_uint32 port, rtk_uint32 qid,
			       rtk_uint32 apridx)
{
	ret_t retVal;
	...
	retVal =
	    rtl8367b_setAsicRegBits(RTL8367B_SCHEDULE_PORT_APR_METER_REG
				    (port, qid),
				    RTL8367B_SCHEDULE_PORT_APR_METER_MASK(qid),
				    apridx);
 ...
 *      None
 */
ret_t rtl8367b_getAsicAprMeter(rtk_uint32 port, rtk_uint32 qid,
			       rtk_uint32 * apridx)
{
	ret_t retVal;
	...
	retVal =
	    rtl8367b_getAsicRegBits(RTL8367B_SCHEDULE_PORT_APR_METER_REG
				    (port, qid),
				    RTL8367B_SCHEDULE_PORT_APR_METER_MASK(qid),
				    apridx);
...
ret_t rtl8367b_setAsicAprEnable(rtk_uint32 port, rtk_uint32 aprEnable)
{
	...
	retVal =
	    rtl8367b_setAsicRegBit(RTL8367B_SCHEDULE_APR_CTRL_REG,
				   RTL8367B_SCHEDULE_APR_CTRL_OFFSET(port),
				   aprEnable);
...
ret_t rtl8367b_getAsicAprEnable(rtk_uint32 port, rtk_uint32 * aprEnable)
{
	...
	retVal =
	    rtl8367b_getAsicRegBit(RTL8367B_SCHEDULE_APR_CTRL_REG,
				   RTL8367B_SCHEDULE_APR_CTRL_OFFSET(port),
				   aprEnable);
 ...
 *      None
 */
ret_t rtl8367b_setAsicWFQWeight(rtk_uint32 port, rtk_uint32 qid,
				rtk_uint32 qWeight)
{
	ret_t retVal;
	...
	retVal =
	    rtl8367b_setAsicReg(RTL8367B_SCHEDULE_PORT_QUEUE_WFQ_WEIGHT_REG
				(port, qid), qWeight);
 ...
 *      None
 */
ret_t rtl8367b_getAsicWFQWeight(rtk_uint32 port, rtk_uint32 qid,
				rtk_uint32 * qWeight)
{
	ret_t retVal;
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_SCHEDULE_PORT_QUEUE_WFQ_WEIGHT_REG
				(port, qid), qWeight);
...
ret_t rtl8367b_setAsicWFQBurstSize(rtk_uint32 burstsize)
{
	...
	retVal =
	    rtl8367b_setAsicReg(RTL8367B_SCHEDULE_WFQ_BURST_SIZE_REG,
				burstsize);
...
ret_t rtl8367b_getAsicWFQBurstSize(rtk_uint32 * burstsize)
{
	...
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_SCHEDULE_WFQ_BURST_SIZE_REG,
				burstsize);
 ...
 *      None
 */
ret_t rtl8367b_setAsicQueueType(rtk_uint32 port, rtk_uint32 qid,
				rtk_uint32 queueType)
{
	...
	retVal =
	    rtl8367b_setAsicRegBit(RTL8367B_SCHEDULE_QUEUE_TYPE_REG(port),
				   RTL8367B_SCHEDULE_QUEUE_TYPE_OFFSET(port,
								       qid),
				   queueType);
 ...
 *      None
 */
ret_t rtl8367b_getAsicQueueType(rtk_uint32 port, rtk_uint32 qid,
				rtk_uint32 * queueType)
{
	ret_t retVal;
	...
	retVal =
	    rtl8367b_getAsicRegBit(RTL8367B_SCHEDULE_QUEUE_TYPE_REG(port),
				   RTL8367B_SCHEDULE_QUEUE_TYPE_OFFSET(port,
								       qid),
				   queueType);
...
ret_t rtl8367b_setAsicPortEgressRate(rtk_uint32 port, rtk_uint32 rate)
{
	...
	regData =
	    (RTL8367B_QOS_GRANULARTY_MSB_MASK & rate) >>
	    RTL8367B_QOS_GRANULARTY_MSB_OFFSET;

	retVal =
	    rtl8367b_setAsicRegBit(regAddr,
				   RTL8367B_PORT0_EGRESSBW_CTRL1_OFFSET,
				   regData);
...
ret_t rtl8367b_getAsicPortEgressRate(rtk_uint32 port, rtk_uint32 * rate)
{
	...
	regAddr = RTL8367B_PORT_EGRESSBW_MSB_REG(port);
	retVal =
	    rtl8367b_getAsicRegBit(regAddr,
				   RTL8367B_PORT0_EGRESSBW_CTRL1_OFFSET,
				   &regData2);
	if (retVal != RT_ERR_OK)
		return retVal;
...
ret_t rtl8367b_setAsicPortEgressRateIfg(rtk_uint32 ifg)
{
	...
	retVal =
	    rtl8367b_setAsicRegBit(RTL8367B_REG_SCHEDULE_WFQ_CTRL,
				   RTL8367B_SCHEDULE_WFQ_CTRL_OFFSET, ifg);
...
ret_t rtl8367b_getAsicPortEgressRateIfg(rtk_uint32 * ifg)
{
	...
	retVal =
	    rtl8367b_getAsicRegBit(RTL8367B_REG_SCHEDULE_WFQ_CTRL,
				   RTL8367B_SCHEDULE_WFQ_CTRL_OFFSET, ifg);
