...
void _rtl8367b_svlanConfStUser2Smi(rtl8367b_svlan_memconf_t * pUserSt,
				   rtl8367b_svlan_memconf_smi_t * pSmiSt);
void _rtl8367b_svlanConfStSmi2User(rtl8367b_svlan_memconf_t * pUserSt,
				   rtl8367b_svlan_memconf_smi_t * pSmiSt);
void _rtl8367b_svlanMc2sStUser2Smi(rtl8367b_svlan_mc2s_t * pUserSt,
				   rtl8367b_svlan_mc2s_smi_t * pSmiSt);
void _rtl8367b_svlanMc2sStSmi2User(rtl8367b_svlan_mc2s_t * pUserSt,
				   rtl8367b_svlan_mc2s_smi_t * pSmiSt);
void _rtl8367b_svlanSp2cStUser2Smi(rtl8367b_svlan_s2c_t * pUserSt,
				   rtl8367b_svlan_s2c_smi_t * pSmiSt);
void _rtl8367b_svlanSp2cStSmi2User(rtl8367b_svlan_s2c_t * pUserSt,
				   rtl8367b_svlan_s2c_smi_t * pSmiSt);
/* Function Name:
...
{
	return rtl8367b_setAsicReg(RTL8367B_REG_SVLAN_UPLINK_PORTMASK,
				   portMask);
}
...
{
	return rtl8367b_getAsicReg(RTL8367B_REG_SVLAN_UPLINK_PORTMASK,
				   pPortmask);
}
...
ret_t rtl8367b_setAsicSvlanPrioritySel(rtk_uint32 priSel)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_REG_SVLAN_CFG,
				       RTL8367B_VS_SPRISEL_MASK, priSel);
}
...
{
	return rtl8367b_getAsicRegBits(RTL8367B_REG_SVLAN_CFG,
				       RTL8367B_VS_SPRISEL_MASK, pPriSel);
}
...
ret_t rtl8367b_setAsicSvlanTrapPriority(rtk_uint32 priority)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_REG_QOS_TRAP_PRIORITY0,
				       RTL8367B_SVLAN_PRIOIRTY_MASK, priority);
}
...
{
	return rtl8367b_getAsicRegBits(RTL8367B_REG_QOS_TRAP_PRIORITY0,
				       RTL8367B_SVLAN_PRIOIRTY_MASK, pPriority);
}
...
ret_t rtl8367b_setAsicSvlanDefaultVlan(rtk_uint32 port, rtk_uint32 index)
{
	...
	if (port & 1)
		retVal =
		    ...
		    (RTL8367B_REG_SVLAN_PORTBASED_SVIDX_CTRL0 + (port >> 1),
		     RTL8367B_VS_PORT1_SVIDX_MASK, index);
	else
		retVal =
		    rtl8367b_setAsicRegBits
		    (RTL8367B_REG_SVLAN_PORTBASED_SVIDX_CTRL0 + (port >> 1),
		     RTL8367B_VS_PORT0_SVIDX_MASK, index);
...
ret_t rtl8367b_getAsicSvlanDefaultVlan(rtk_uint32 port, rtk_uint32 * pIndex)
{
	...
	if (port & 1)
		retVal =
		    ...
		    (RTL8367B_REG_SVLAN_PORTBASED_SVIDX_CTRL0 + (port >> 1),
		     RTL8367B_VS_PORT1_SVIDX_MASK, pIndex);
	else
		retVal =
		    rtl8367b_getAsicRegBits
		    (RTL8367B_REG_SVLAN_PORTBASED_SVIDX_CTRL0 + (port >> 1),
		     RTL8367B_VS_PORT0_SVIDX_MASK, pIndex);
...
{
	return rtl8367b_setAsicRegBit(RTL8367B_REG_SVLAN_CFG,
				      RTL8367B_VS_UIFSEG_OFFSET, enabled);
}
...
{
	return rtl8367b_getAsicRegBit(RTL8367B_REG_SVLAN_CFG,
				      RTL8367B_VS_UIFSEG_OFFSET, pEnabled);
}
 ...
 *      None
 */
ret_t rtl8367b_setAsicSvlanMemberConfiguration(rtk_uint32 index,
					       ...
					       pSvlanMemCfg)
{
	...
	for (i = 0; i < 3; i++) {
		retVal =
		    rtl8367b_setAsicReg(RTL8367B_SVLAN_MEMBERCFG_BASE_REG(index)
					+ i, regData);
		if (retVal != RT_ERR_OK)
			return retVal;
 ...
 *      None
 */
ret_t rtl8367b_getAsicSvlanMemberConfiguration(rtk_uint32 index,
					       rtl8367b_svlan_memconf_t *
					       pSvlanMemCfg)
{
	ret_t retVal;
	...
	for (i = 0; i < 3; i++) {
		retVal =
		    rtl8367b_getAsicReg(RTL8367B_SVLAN_MEMBERCFG_BASE_REG(index)
					+ i, &regData);
		if (retVal != RT_ERR_OK)
			return retVal;
 ...
 */
ret_t rtl8367b_setAsicSvlanC2SConf(rtk_uint32 index, rtk_uint32 evid,
				   rtk_uint32 portmask, rtk_uint32 svidx)
{
	ret_t retVal;
	rtk_uint32 regData;
	rtk_uint16 *accessPtr;
	...
	for (i = 0; i < 3; i++) {
		retVal =
		    rtl8367b_setAsicReg(RTL8367B_SVLAN_C2SCFG_BASE_REG(index) +
					i, regData);
		if (retVal != RT_ERR_OK)
			return retVal;
 ...
 *      None
 */
ret_t rtl8367b_getAsicSvlanC2SConf(rtk_uint32 index, rtk_uint32 * pEvid,
				   rtk_uint32 * pPortmask, rtk_uint32 * pSvidx)
{
	ret_t retVal;
	rtk_uint32 regData;
	rtk_uint16 *accessPtr;
	...
	for (i = 0; i < 3; i++) {
		retVal =
		    rtl8367b_getAsicReg(RTL8367B_SVLAN_C2SCFG_BASE_REG(index) +
					i, &regData);
		if (retVal != RT_ERR_OK)
			return retVal;
				   ...
				   rtl8367b_svlan_mc2s_smi_t * pSmiSt)
{
	...
	pUserSt->smask =
	    (pSmiSt->mask3 << 24) | (pSmiSt->
				     mask2 << 16) | (pSmiSt->mask1 << 8) |
	    pSmiSt->mask0;

	pUserSt->sdata =
	    (pSmiSt->data3 << 24) | (pSmiSt->
				     data2 << 16) | (pSmiSt->data1 << 8) |
	    pSmiSt->data0;
 ...
 */
ret_t rtl8367b_setAsicSvlanMC2SConf(rtk_uint32 index,
				    rtl8367b_svlan_mc2s_t * pSvlanMc2sCfg)
{
	ret_t retVal;
	rtk_uint32 regData;
	rtk_uint16 *accessPtr;
	...
	for (i = 0; i < 5; i++) {
		retVal =
		    rtl8367b_setAsicReg(RTL8367BG_SVLAN_MCAST2S_ENTRY_BASE_REG
					(index) + i, regData);
		if (retVal != RT_ERR_OK)
			return retVal;
 ...
 *      None
 */
ret_t rtl8367b_getAsicSvlanMC2SConf(rtk_uint32 index,
				    rtl8367b_svlan_mc2s_t * pSvlanMc2sCfg)
{
	ret_t retVal;
	rtk_uint32 regData;
	...
	for (i = 0; i < 5; i++) {
		retVal =
		    rtl8367b_getAsicReg(RTL8367BG_SVLAN_MCAST2S_ENTRY_BASE_REG
					(index) + i, &regData);
		if (retVal != RT_ERR_OK)
			return retVal;
 ...
 *      None
 */
ret_t rtl8367b_setAsicSvlanSP2CConf(rtk_uint32 index,
				    rtl8367b_svlan_s2c_t * pSvlanSp2cCfg)
{
	ret_t retVal;
	rtk_uint32 regData;
	rtk_uint16 *accessPtr;
	...
	for (i = 0; i < 2; i++) {
		retVal =
		    rtl8367b_setAsicReg(RTL8367B_SVLAN_S2C_ENTRY_BASE_REG(index)
					+ i, regData);
		if (retVal != RT_ERR_OK)
			return retVal;
 ...
 *      None
 */
ret_t rtl8367b_getAsicSvlanSP2CConf(rtk_uint32 index,
				    rtl8367b_svlan_s2c_t * pSvlanSp2cCfg)
{
	ret_t retVal;
	rtk_uint32 regData;
	...
	for (i = 0; i < 2; i++) {
		retVal =
		    rtl8367b_getAsicReg(RTL8367B_SVLAN_S2C_ENTRY_BASE_REG(index)
					+ i, &regData);
		if (retVal != RT_ERR_OK)
			return retVal;
...
ret_t rtl8367b_setAsicSvlanDmacCvidSel(rtk_uint32 port, rtk_uint32 enabled)
{
	...
	return rtl8367b_setAsicRegBit(RTL8367B_REG_SVLAN_CFG,
				      RTL8367B_VS_PORT0_DMACVIDSEL_OFFSET +
				      port, enabled);
}
...
ret_t rtl8367b_setAsicSvlanUntagVlan(rtk_uint32 index)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_REG_SVLAN_UNTAG_UNMAT_CFG,
				       RTL8367B_VS_UNTAG_SVIDX_MASK, index);
}
...
{
	return rtl8367b_getAsicRegBits(RTL8367B_REG_SVLAN_UNTAG_UNMAT_CFG,
				       RTL8367B_VS_UNTAG_SVIDX_MASK, pIndex);
}
...
ret_t rtl8367b_setAsicSvlanUnmatchVlan(rtk_uint32 index)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_REG_SVLAN_UNTAG_UNMAT_CFG,
				       RTL8367B_VS_UNMAT_SVIDX_MASK, index);
}
...
{
	return rtl8367b_getAsicRegBits(RTL8367B_REG_SVLAN_UNTAG_UNMAT_CFG,
				       RTL8367B_VS_UNMAT_SVIDX_MASK, pIndex);
}
