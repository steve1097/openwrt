...
void _rtl8367b_VlanMCStUser2Smi(rtl8367b_vlanconfiguser * pVlanCg,
				rtl8367b_vlanconfigsmi * pSmiVlanCfg)
{
...
void _rtl8367b_VlanMCStSmi2User(rtl8367b_vlanconfigsmi * pSmiVlanCfg,
				rtl8367b_vlanconfiguser * pVlanCg)
{
...
void _rtl8367b_Vlan4kStUser2Smi(rtl8367b_user_vlan4kentry * pUserVlan4kEntry,
				rtl8367b_vlan4kentrysmi * pSmiVlan4kEntry)
{
...
void _rtl8367b_Vlan4kStSmi2User(rtl8367b_vlan4kentrysmi * pSmiVlan4kEntry,
				rtl8367b_user_vlan4kentry * pUserVlan4kEntry)
{
 ...
 *      None
 */
ret_t rtl8367b_setAsicVlanMemberConfig(rtk_uint32 index,
				       rtl8367b_vlanconfiguser * pVlanCg)
{
	ret_t retVal;
	rtk_uint32 regAddr;
	...
	for (page_idx = 0; page_idx < 4; page_idx++) {	/* 4 pages per VLAN Member Config */
		regAddr =
		    RTL8367B_VLAN_MEMBER_CONFIGURATION_BASE + (index * 4) +
		    page_idx;
		regData = *tableAddr;
 ...
 *      None
 */
ret_t rtl8367b_getAsicVlanMemberConfig(rtk_uint32 index,
				       rtl8367b_vlanconfiguser * pVlanCg)
{
	ret_t retVal;
	rtk_uint32 page_idx;
...
ret_t rtl8367b_setAsicVlan4kEntry(rtl8367b_user_vlan4kentry * pVlan4kEntry)
{
	...
	for (page_idx = 0; page_idx < (sizeof(rtl8367b_vlan4kentrysmi) / 2);
	     page_idx++) {
		regData = *tableAddr;
		retVal =
		    rtl8367b_setAsicReg(RTL8367B_TABLE_ACCESS_WRDATA_BASE +
					page_idx, regData);
		if (retVal != RT_ERR_OK)
			return retVal;
	...
	/* Write Command */
	retVal =
	    rtl8367b_setAsicRegBits(RTL8367B_TABLE_ACCESS_CTRL_REG,
				    RTL8367B_TABLE_TYPE_MASK |
				    RTL8367B_COMMAND_TYPE_MASK,
				    RTL8367B_TABLE_ACCESS_REG_DATA(TB_OP_WRITE,
								   TB_TARGET_CVLAN));
	if (retVal != RT_ERR_OK)
		return retVal;

#if defined(CONFIG_RTL8367B_ASICDRV_TEST)
	memcpy(&Rtl8370sVirtualVlanTable[pVlan4kEntry->vid], &vlan_4k_entry,
	       sizeof(rtl8367b_vlan4kentrysmi));
#endif

#if !defined(DISABLE_VLAN_SHADOW)
	memcpy(&user_4kvlan[pVlan4kEntry->vid], pVlan4kEntry,
	       sizeof(rtl8367b_user_vlan4kentry));
#endif
...
ret_t rtl8367b_getAsicVlan4kEntry(rtl8367b_user_vlan4kentry * pVlan4kEntry)
{
	...
	retVal =
	    rtl8367b_setAsicRegBits(RTL8367B_TABLE_ACCESS_CTRL_REG,
				    RTL8367B_TABLE_TYPE_MASK |
				    RTL8367B_COMMAND_TYPE_MASK,
				    RTL8367B_TABLE_ACCESS_REG_DATA(TB_OP_READ,
								   TB_TARGET_CVLAN));
	if (retVal != RT_ERR_OK)
		return retVal;
	...
	/* Check ASIC Command */
	retVal =
	    rtl8367b_getAsicRegBit(RTL8367B_TABLE_ACCESS_STATUS_REG,
				   RTL8367B_TABLE_LUT_ADDR_BUSY_FLAG_OFFSET,
				   &regData);
	if (retVal != RT_ERR_OK)
	...
	for (page_idx = 0; page_idx < (sizeof(rtl8367b_vlan4kentrysmi) / 2);
	     page_idx++) {
		retVal =
		    rtl8367b_getAsicReg(RTL8367B_TABLE_ACCESS_RDDATA_BASE +
					page_idx, &regData);
		if (retVal != RT_ERR_OK)
			return retVal;
	...
	memcpy(pVlan4kEntry, &user_4kvlan[pVlan4kEntry->vid],
	       sizeof(rtl8367b_user_vlan4kentry));
...
#if defined(CONFIG_RTL8367B_ASICDRV_TEST)
	_rtl8367b_Vlan4kStSmi2User(&Rtl8370sVirtualVlanTable[pVlan4kEntry->vid],
				   pVlan4kEntry);
#endif
 ...
 *      None
 */
ret_t rtl8367b_setAsicVlanAccpetFrameType(rtk_uint32 port,
					  rtl8367b_accframetype frameType)
{
	...
	return
	    rtl8367b_setAsicRegBits(RTL8367B_VLAN_ACCEPT_FRAME_TYPE_REG(port),
				    RTL8367B_VLAN_ACCEPT_FRAME_TYPE_MASK(port),
				    frameType);
}
 ...
 *      None
 */
ret_t rtl8367b_getAsicVlanAccpetFrameType(rtk_uint32 port,
					  rtl8367b_accframetype * pFrameType)
{
	if (port > RTL8367B_PORTIDMAX)
		return RT_ERR_PORT_ID;

	return
	    rtl8367b_getAsicRegBits(RTL8367B_VLAN_ACCEPT_FRAME_TYPE_REG(port),
				    RTL8367B_VLAN_ACCEPT_FRAME_TYPE_MASK(port),
				    pFrameType);
}
...
ret_t rtl8367b_setAsicVlanIngressFilter(rtk_uint32 port, rtk_uint32 enabled)
{
	...
	return rtl8367b_setAsicRegBit(RTL8367BG_VLAN_INGRESS_REG, port,
				      enabled);
}
...
ret_t rtl8367b_getAsicVlanIngressFilter(rtk_uint32 port, rtk_uint32 * pEnable)
{
	...
	return rtl8367b_getAsicRegBit(RTL8367BG_VLAN_INGRESS_REG, port,
				      pEnable);
}
 ...
 *      None
 */
ret_t rtl8367b_setAsicVlanEgressTagMode(rtk_uint32 port,
					rtl8367b_egtagmode tagMode)
{
	if (port > RTL8367B_PORTIDMAX)
		return RT_ERR_PORT_ID;
	...
	return rtl8367b_setAsicRegBits(RTL8367B_PORT_MISC_CFG_REG(port),
				       RTL8367B_VLAN_EGRESS_MDOE_MASK, tagMode);
}
 ...
 *      None
 */
ret_t rtl8367b_getAsicVlanEgressTagMode(rtk_uint32 port,
					rtl8367b_egtagmode * pTagMode)
{
	if (port > RTL8367B_PORTIDMAX)
		return RT_ERR_PORT_ID;

	return rtl8367b_getAsicRegBits(RTL8367B_PORT_MISC_CFG_REG(port),
				       RTL8367B_VLAN_EGRESS_MDOE_MASK,
				       pTagMode);
}
 ...
 *      None
 */
ret_t rtl8367b_setAsicVlanPortBasedVID(rtk_uint32 port, rtk_uint32 index,
				       rtk_uint32 pri)
{
 ...
 *      None
 */
ret_t rtl8367b_getAsicVlanPortBasedVID(rtk_uint32 port, rtk_uint32 * pIndex,
				       rtk_uint32 * pPri)
{
	rtk_uint32 regAddr, bit_mask;
	ret_t retVal;
 ...
 *      None
 */
ret_t rtl8367b_setAsicVlanProtocolBasedGroupData(rtk_uint32 index,
						 ...
						 pPbCfg)
{
	...
	/* Frame type */
	retVal =
	    rtl8367b_setAsicRegBits(RTL8367BG_VLAN_PPB_FRAMETYPE_REG(index),
				    RTL8367BG_VLAN_PPB_FRAMETYPE_MASK,
				    frameType);
	if (retVal != RT_ERR_OK)
		return retVal;
	...
	/* Ether type */
	retVal =
	    rtl8367b_setAsicReg(RTL8367B_VLAN_PPB_ETHERTYPR_REG(index),
				etherType);
	if (retVal != RT_ERR_OK)
		return retVal;
 ...
 *      None
 */
ret_t rtl8367b_getAsicVlanProtocolBasedGroupData(rtk_uint32 index,
						 rtl8367b_protocolgdatacfg *
						 pPbCfg)
{
	rtk_uint32 frameType;
	...
	/* Read Frame type */
	retVal =
	    rtl8367b_getAsicRegBits(RTL8367BG_VLAN_PPB_FRAMETYPE_REG(index),
				    RTL8367BG_VLAN_PPB_FRAMETYPE_MASK,
				    &frameType);
	if (retVal != RT_ERR_OK)
		return retVal;
	...
	/* Read Ether type */
	retVal =
	    rtl8367b_getAsicReg(RTL8367B_VLAN_PPB_ETHERTYPR_REG(index),
				&etherType);
	if (retVal != RT_ERR_OK)
		return retVal;
 ...
 *      None
 */
ret_t rtl8367b_setAsicVlanPortAndProtocolBased(rtk_uint32 port,
					       rtk_uint32 index,
 ...
 *      None
 */
ret_t rtl8367b_getAsicVlanPortAndProtocolBased(rtk_uint32 port,
					       rtk_uint32 index,
					       rtl8367b_protocolvlancfg *
					       pPpbCfg)
{
...
{
	return rtl8367b_setAsicRegBit(RTL8367B_REG_VLAN_CTRL,
				      RTL8367B_VLAN_CTRL_OFFSET, enabled);
}
...
{
	return rtl8367b_getAsicRegBit(RTL8367B_REG_VLAN_CTRL,
				      RTL8367B_VLAN_CTRL_OFFSET, pEnabled);
}
...
{
	return rtl8367b_setAsicRegBit(RTL8367B_REG_UNTAG_DSCP_PRI_CFG,
				      RTL8367B_UNTAG_DSCP_PRI_CFG_OFFSET,
				      enabled);
}
...
{
	return rtl8367b_getAsicRegBit(RTL8367B_REG_UNTAG_DSCP_PRI_CFG,
				      RTL8367B_UNTAG_DSCP_PRI_CFG_OFFSET,
				      enabled);
}
...
ret_t rtl8367b_getAsicPortBasedFidEn(rtk_uint32 port, rtk_uint32 * pEnabled)
{
	...
	return rtl8367b_getAsicRegBit(RTL8367B_REG_PORT_PBFIDEN, port,
				      pEnabled);
}
 ...
 *      None
 */
ret_t rtl8367b_setAsicSpanningTreeStatus(rtk_uint32 port, rtk_uint32 msti,
					 rtk_uint32 state)
{
	if (port > RTL8367B_PORTIDMAX)
		return RT_ERR_PORT_ID;
	...
	return rtl8367b_setAsicRegBits(RTL8367B_VLAN_MSTI_REG(msti, port),
				       RTL8367B_VLAN_MSTI_MASK(port), state);
}
 ...
 *      None
 */
ret_t rtl8367b_getAsicSpanningTreeStatus(rtk_uint32 port, rtk_uint32 msti,
					 rtk_uint32 * pState)
{
	if (port > RTL8367B_PORTIDMAX)
		return RT_ERR_PORT_ID;
	...
	return rtl8367b_getAsicRegBits(RTL8367B_VLAN_MSTI_REG(msti, port),
				       RTL8367B_VLAN_MSTI_MASK(port), pState);
}
...
{
	return rtl8367b_setAsicRegBit(RTL8367B_REG_VLAN_TRANSPARENT_EN_CFG,
				      RTL8367B_VLAN_TRANSPARENT_EN_CFG_OFFSET,
				      enabled);
}
...
{
	return rtl8367b_getAsicRegBit(RTL8367B_REG_VLAN_TRANSPARENT_EN_CFG,
				      RTL8367B_VLAN_TRANSPARENT_EN_CFG_OFFSET,
				      pEnabled);
}
...
ret_t rtl8367b_setAsicVlanEgressKeep(rtk_uint32 port, rtk_uint32 portmask)
{
	...
	return rtl8367b_setAsicRegBits(RTL8367B_REG_VLAN_EGRESS_KEEP_CTRL0 +
				       (port >> 1),
				       RTL8367B_PORT0_VLAN_KEEP_MASK_MASK <<
				       ((port & 1) * 8), portmask);
}
...
ret_t rtl8367b_getAsicVlanEgressKeep(rtk_uint32 port, rtk_uint32 * pPortmask)
{
	...
	return rtl8367b_getAsicRegBits(RTL8367B_REG_VLAN_EGRESS_KEEP_CTRL0 +
				       (port >> 1),
				       RTL8367B_PORT0_VLAN_KEEP_MASK_MASK <<
				       ((port & 1) * 8), pPortmask);
}
