...
extern int rtk_vlan_get(uint32 vid, uint32 * pMbrmsk, uint32 * pUntagmsk,
			uint32 * pFid);
extern int rtk_vlan_portPvid_get(uint32 port, uint32 * pPvid,
				 uint32 * pPriority);
#endif
...
*sw_napt_entry,
#endif
*port_bandwidth_entry, *queue_bandwidth_entry, *priority_decision_entry,
...
void memDump(void *start, uint32 size, int8 * strHeader)
{
	...
	for (index = 0; index < row; index++, line += 16) {
		...
		for (index2 = 0; index2 < max; index2++) {
			...
			ascii[index2] =
			    ((uint8) buf[index2] < 32) ? empty : buf[index2];
			if ((index2 + 1) % 4 == 0)
				rtlglue_printf("  ");
		}
...
int vlan_show(void)
{
	...
	for (i = 0; i < RTL865XC_VLAN_NUMBER; i++) {
		...
		for (j = 0; j < RTL8651_PORT_NUMBER + rtl8651_totalExtPortNum;
		     j++) {
...
static int32 vlan_single_show(struct seq_file *s, void *v)
{
...

	{
		...
		for (i = 0; i < RTL865XC_VLANTBL_SIZE; i++) {
			...
			for (j = 0;
			     j < RTL8651_PORT_NUMBER + rtl8651_totalExtPortNum;
			     j++) {
				if (vlan.memberPortMask & (1 << j))
					seq_printf(s, "%d ", j);
			}
		...
		if (tmpbuf[0] == '1') {
			...
			if (!memcmp(cmd_addr, "dump", 4)) {
				...
				rtlglue_printf("Vlan info:\n");
				{
					...
					if (rtl8651_getAsicVlan(vid, &vlan) ==
					    FAILED)
						return len;
					...
					for (j = 0;
					     j <
					     RTL8651_PORT_NUMBER +
					     rtl8651_totalExtPortNum; j++) {
						if (vlan.memberPortMask &
						    (1 << j))
							rtlglue_printf("%d ",
								       j);
					}
				...
				extern int32 rtl865x_addVlanPortMember2(uint16
									vid,
									uint32
									portMask,
									uint32
				...
				extern int32 rtl865x_addVlanPortMember2(uint16
									...
									uint32
									untagPortMask);
				extern int update_vlanconfig(uint32 vid,
							     uint32 portMask,
							     uint32
							     untagPortMask);
...
errout:
			rtlglue_printf
			    ("vlan operation only support \"dump\" as the first parameter\n");
		...
		if (intf.valid) {
			...
			for (j = 0;
				...
				if ((vlan.memberPortMask & (1 << j)) == 0)
					continue;
				if (((READ_MEM32(PCRP0 + j * 4) &
				      STP_PortST_MASK) >> STP_PortST_OFFSET) >
				    4)
					seq_printf(s, "--- ");
				else
					seq_printf(s, "%d:%s ", j,
			...
			int *eof, void *data)
{
	...
	for (i = 0; i < RTL865XC_NETIFTBL_SIZE; i++) {
		...
		if (intf.valid) {
			...
			len +=
			    sprintf(page + len,
				    "[%d]  VID[%d] %x:%x:%x:%x:%x:%x", i,
				    intf.vid, mac[0], mac[1], mac[2], mac[3],
				    mac[4], mac[5]);
			len +=
...
again:
		...
		for (; acl_start <= acl_end; acl_start++) {
			if (_rtl865x_getAclFromAsic(acl_start, &asic_acl) ==
			    FAILED)
				rtlglue_printf
			...
			case RTL865X_ACL_IP:
				...
				len +=
				    sprintf(page + len,
					    ...
					    ((asic_acl.dstIpAddr_ & 0x00ff0000)
					     >> 16),
					    ((asic_acl.dstIpAddr_ & 0x0000ff00)
					     >> 8),
				...
				len +=
				    sprintf(page + len,
					    ...
					    ((asic_acl.srcIpAddr_ & 0x00ff0000)
					     >> 16),
					    ((asic_acl.srcIpAddr_ & 0x0000ff00)
					     >> 8),
			...
			case RTL865X_ACL_TCP:
				...
				len +=
				    sprintf(page + len,
					    ...
					    asic_acl.tcpFlag_,
					    asic_acl.tcpFlagMask_,
					    asic_acl.tcpURG_, asic_acl.tcpACK_,
					    asic_acl.tcpPSH_, asic_acl.tcpRST_,
					    asic_acl.tcpSYN_, asic_acl.tcpFIN_);
				break;
			...
			case RTL865X_ACL_PRIORITY:
				len +=
				    sprintf(page + len, "\tprioirty: %d   ",
					    asic_acl.priority_);
				break;

			case RTL865X_ACL_DEFAULT_REDIRECT:
				len +=
				    sprintf(page + len, "\tnextHop:%d  ",
			...
			case RTL865X_ACL_LOG_RATE_EXCEED_BPS:
				len +=
				    sprintf(page + len, "\tratelimitIdx: %d  ",
			...
			len +=
			    sprintf(page + len, "pktOpApp: %d\n",
				    asic_acl.pktOpApp_);

		}
#else
		for (; acl_start <= acl_end; acl_start++) {
			if (rtl8651_getAsicAclRule(acl_start, &asic_acl) ==
			    FAILED)
				rtlglue_printf
				    ("=============%s(%d): get asic acl rule error!\n",
				     __FUNCTION__, __LINE__);
		...
		for (; acl_start <= acl_end; acl_start++) {
			...
			case RTL865X_ACL_IPV6_RANGE:
				...
				if (acl_temp <= acl_end) {	/* it should be less than or equal to acl_end. */
					...
					if (_rtl865x_getAclFromAsic
					    (acl_temp, &asic_acl2) == FAILED)
						seq_printf(s,
							   "=============%s(%d): get asic acl rule error!\n",
							   __FUNCTION__,
							   __LINE__);
					if ((!asic_acl2.ipv6EntryType_)
						...
						asic_acl.
						    dstIpV6Addr_.v6_addr32[3] =
						    asic_acl2.
						    dstIpV6Addr_.v6_addr32[3];
						asic_acl.
						    dstIpV6Addr_.v6_addr32[2] =
						    asic_acl2.
						    dstIpV6Addr_.v6_addr32[2];
						asic_acl.
						    dstIpV6Addr_.v6_addr32[1] =
						    asic_acl2.
						    dstIpV6Addr_.v6_addr32[1];
						asic_acl.
						    dstIpV6Addr_.v6_addr32[0] =
						    asic_acl2.
						    dstIpV6Addr_.v6_addr32[0];

						asic_acl.
						    dstIpV6AddrMask_.v6_addr32
						    [3] =
						    asic_acl2.
						    dstIpV6AddrMask_.v6_addr32
						    [3];
						...
						if (RTL865X_ACL_IPV6 ==
						    asic_acl.ruleType_) {
							memset((void *)
							       &start_ip.v6_addr32[0], 0x00, sizeof(start_ip));
							memset((void *)
							       &end_ip.v6_addr32
							       [0], 0x00,
							       sizeof(end_ip));
							ntohl_array((uint32 *) &
								    asic_acl.srcIpV6Addr_.v6_addr32
								    [0],
								    (void *)
								    &start_ip.v6_addr32[0], 4);
							ntohl_array((uint32 *) &
								    asic_acl.srcIpV6AddrMask_.v6_addr32
								    [0],
								    (void *)
								    &end_ip.v6_addr32[0], 4);
							seq_printf(s,
							...
							memset((void *)
							       ...
							       sizeof(end_ip));
							ntohl_array((uint32 *) &
								    asic_acl.dstIpV6Addr_.v6_addr32
								    [0],
								    (void *)
								    &start_ip.v6_addr32[0], 4);
							ntohl_array((uint32 *) &
								    asic_acl.dstIpV6AddrMask_.v6_addr32
								    [0],
								    (void *)
								    &end_ip.v6_addr32[0], 4);
							seq_printf(s,
							...
							memset((void *)
							       ...
							       sizeof(end_ip));
							ntohl_array((uint32 *) &
								    asic_acl.srcIpV6AddrLB_.v6_addr32
								    [0],
								    (void *)
								    &start_ip.v6_addr32[0], 4);
							ntohl_array((uint32 *) &
								    asic_acl.srcIpV6AddrUB_.v6_addr32
								    [0],
								    (void *)
								    &end_ip.v6_addr32[0], 4);
							seq_printf(s,
							...
							memset((void *)
							       ...
							       sizeof(end_ip));
							ntohl_array((uint32 *) &
								    asic_acl.dstIpV6AddrLB_.v6_addr32
								    [0],
								    (void *)
								    &start_ip.v6_addr32[0], 4);
							ntohl_array((uint32 *) &
								    asic_acl.dstIpV6AddrUB_.v6_addr32
								    [0],
								    (void *)
								    &end_ip.v6_addr32[0], 4);
							seq_printf(s,
			...
			if (!((asic_acl.ruleType_ == RTL865X_ACL_IPV6)
			      || (asic_acl.ruleType_ == RTL865X_ACL_IPV6_RANGE)
#if defined(CONFIG_RTL_8197F)
...
static int32 hs_read(struct seq_file *s, void *v)
{
	...
	virtualMacGetHsb(hsb_r);
	{
		...
		seq_printf(s, "\tda:%02x-%02x-%02x-%02x-%02x-%02x",
			   hsb_r->da[0], hsb_r->da[1], hsb_r->da[2],
			   hsb_r->da[3], hsb_r->da[4], hsb_r->da[5]);
		seq_printf(s, "\tsa:%02x-%02x-%02x-%02x-%02x-%02x\n",
			   hsb_r->sa[0], hsb_r->sa[1], hsb_r->sa[2],
			   hsb_r->sa[3], hsb_r->sa[4], hsb_r->sa[5]);
		...
		seq_printf(s,
			   ...
			   hsb_r->sipv6_127_96 >> 16,
			   hsb_r->sipv6_127_96 & 0xFFFF,
			   hsb_r->sipv6_95_64 >> 16,
			   hsb_r->sipv6_95_64 & 0xFFFF,
			   hsb_r->sipv6_63_32 >> 16,
			   hsb_r->sipv6_63_32 & 0xFFFF, hsb_r->sipv6_31_0 >> 16,
			   hsb_r->sipv6_31_0 & 0xFFFF);

		seq_printf(s,
			   "\tdip6:\t%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n",
			   hsb_r->dipv6_127_96 >> 16,
			   hsb_r->dipv6_127_96 & 0xFFFF,
			   hsb_r->dipv6_95_64 >> 16,
			   hsb_r->dipv6_95_64 & 0xFFFF,
			   hsb_r->dipv6_63_32 >> 16,
			   hsb_r->dipv6_63_32 & 0xFFFF, hsb_r->dipv6_31_0 >> 16,
			   hsb_r->dipv6_31_0 & 0xFFFF);
...
*/
		...
		seq_printf(s,
			   "\tsip6:\t%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n",
			   hsa_r->sipv6_127_96 >> 16,
			   hsa_r->sipv6_127_96 & 0xFFFF,
			   hsa_r->sipv6_95_64 >> 16,
			   hsa_r->sipv6_95_64 & 0xFFFF,
			   hsa_r->sipv6_63_32 >> 16,
			   hsa_r->sipv6_63_32 & 0xFFFF, hsa_r->sipv6_31_0 >> 16,
			   hsa_r->sipv6_31_0 & 0xFFFF);

		seq_printf(s,
			   "\tdip6:\t%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n",
			   hsa_r->dipv6_127_96 >> 16,
			   hsa_r->dipv6_127_96 & 0xFFFF,
			   hsa_r->dipv6_95_64 >> 16,
			   hsa_r->dipv6_95_64 & 0xFFFF,
			   hsa_r->dipv6_63_32 >> 16,
			   hsa_r->dipv6_63_32 & 0xFFFF, hsa_r->dipv6_31_0 >> 16,
			   hsa_r->dipv6_31_0 & 0xFFFF);
		     ...
		     void *data)
{
	...
	virtualMacGetHsb(hsb_r);
	{
		...
		/* Protocol contents */
		len +=
		    sprintf(page + len, "\ttagif:%d\tpppoeId:%d", hsb_r->tagif,
			    hsb_r->pppoeid);
		len +=
		    sprintf(page + len, "\tethrtype:0x%04x\n", hsb_r->ethtype);
		len +=
		    sprintf(page + len, "\tllc_other:%d\tsnap:%d\n",
			    hsb_r->llcothr, hsb_r->snap);
		len +=
		    sprintf(page + len, "\tda:%02x-%02x-%02x-%02x-%02x-%02x",
			    hsb_r->da[0], hsb_r->da[1], hsb_r->da[2],
			    hsb_r->da[3], hsb_r->da[4], hsb_r->da[5]);
		len +=
		    sprintf(page + len, "\tsa:%02x-%02x-%02x-%02x-%02x-%02x\n",
			    hsb_r->sa[0], hsb_r->sa[1], hsb_r->sa[2],
			    hsb_r->sa[3], hsb_r->sa[4], hsb_r->sa[5]);
		...
		inet_ntoa_r(addr, addr_s);
		len +=
		    sprintf(page + len, "\tsip:%s(hex:%08x)   ", addr_s,
			    hsb_r->sip);
		len +=
		    sprintf(page + len, "\tsprt:%d (hex:%x)\n ",
			    (int)hsb_r->sprt, hsb_r->sprt);
		addr = ntohl(hsb_r->dip);
		inet_ntoa_r(addr, addr_s);
		len +=
		    sprintf(page + len, "\tdip:%s(hex:%08x) ", addr_s,
			    hsb_r->dip);;
		len +=
		    sprintf(page + len, "\tdprt:%d(hex:%08x)\n", hsb_r->dprt,
			    hsb_r->dprt);
		...
		len +=
		    sprintf(page + len, "\tl3csok:%d\tl4csok:%d\tipfragif:%d\n",
			    hsb_r->l3csok, hsb_r->l4csok, hsb_r->ipfo0_n);
		...
		len +=
		    sprintf(page + len,
			    ...
			    hsb_r->dipv6_95_64 & 0xFFFF,
			    hsb_r->dipv6_63_32 >> 16,
			    hsb_r->dipv6_63_32 & 0xFFFF,
			    hsb_r->dipv6_31_0 >> 16,
			    hsb_r->dipv6_31_0 & 0xFFFF);

		len +=
		    sprintf(page + len, "\thop_limit:0x%x", hsb_r->hop_limit);
	...
	virtualMacGetHsa(hsa_r);
	{
		...
		len +=
		    sprintf(page + len, "\tmac:%02x-%02x-%02x-%02x-%02x-%02x\n",
			    hsa_r->nhmac[0], hsa_r->nhmac[1], hsa_r->nhmac[2],
			    hsa_r->nhmac[3], hsa_r->nhmac[4], hsa_r->nhmac[5]);
		...
		inet_ntoa_r(addr, addr_s);
		len +=
		    sprintf(page + len, "\ttrip:%s(hex:%08x)", addr_s,
			    hsa_r->trip);
		len +=
		    sprintf(page + len, "\tprt:%d\tipmcast:%d\n", hsa_r->port,
			    hsa_r->ipmcastr);
		...
		len +=
		    sprintf(page + len, "\twhy2cpu 0x%x (%d)\n", hsa_r->why2cpu,
			    hsa_r->why2cpu);
...
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
		len +=
		    sprintf(page + len, "\tleno:%d(0x%x) \tmirrort:%d\n",
			    hsa_r->leno, hsa_r->leno, hsa_r->mirrort);
#else
		len +=
		    sprintf(page + len, "\tleno:%d(0x%x)\n", hsa_r->leno,
			    hsa_r->leno);
#endif
		...
		len +=
		    sprintf(page + len, "\tdvid:%d(0x%x)", hsa_r->dvid,
			    hsa_r->dvid);
		len +=
		    sprintf(page + len, "\tdestination interface :%d\n",
			    hsa_r->difid);
		len += sprintf(page + len, "\trxtag:%d", hsa_r->rxtag);
		len += sprintf(page + len, "\t\tdvtag:0x%x", hsa_r->dvtag);
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
		len +=
		    sprintf(page + len, "\tspa:%d \tipv4_1st:%d", hsa_r->spao,
			    hsa_r->ipv4_1st);
#else
		len += sprintf(page + len, "\tspa:%d", hsa_r->spao);
#endif
		len +=
		    sprintf(page + len, "\tdpext:0x%x\thwfwrd:%d\n",
		...
		len +=
		    sprintf(page + len, "\tptp_type:0x%x\n", hsa_r->ptp_type);
...
*/
		...
		len +=
		    sprintf(page + len, "\tmltcst_v6:0x%x", hsa_r->mltcst_v6);
		len +=
		    sprintf(page + len, "\taddip_pri:0x%x\n", hsa_r->addip_pri);
		len += sprintf(page + len, ")\n");
#else
		len += sprintf(page + len, ")\n");
#endif
			      ...
			      unsigned long len, void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "mirror", 6)) {
			...
			rtlglue_printf
			    ("mirror rx port mask(0x%x) tx port mask(0x%x), mirror port mask(0x%x)\n",
...
errout:
		rtlglue_printf
		    ("Mirror port configuration only support \"mirror\"as the first parameter\n");
		rtlglue_printf
		    ("mirror: \"mirror rx_portmask tx_portmask mirror_portmask\"\n");
	}
...
static int32 l2_read(struct seq_file *s, void *v)
{
	...
	for (row = 0x0; row < RTL8651_L2TBL_ROW; row++) {
		for (col = 0; col < RTL8651_L2TBL_COLUMN; col++) {
			...
			if (rtl8651_getAsicL2Table(row, col, &asic_l2) ==
			    FAILED) {
				continue;
			}
		     ...
		     void *data)
{
	...
	for (row = 0x0; row < RTL8651_L2TBL_ROW; row++) {
		for (col = 0; col < RTL8651_L2TBL_COLUMN; col++) {
			...
			len += sprintf(page + len, ")");
			len +=
			    sprintf(page + len, "%s %s %s %s age:%d ",
				    asic_l2.cpu ? "CPU" : "FWD",
				    asic_l2.isStatic ? "STA" : "DYN",
				    asic_l2.srcBlk ? "BLK" : "",
				    asic_l2.nhFlag ? "NH" : "", asic_l2.ageSec);
...
static int32 rtl865x_proc_hw_mcast_read(struct seq_file *s, void *v)
{
	...
	for (entry = 0; entry < RTL8651_MULTICASTTBL_SIZE; entry++) {
		if (rtl8651_getAsicIpMulticastTable(entry, &asic) != SUCCESS) {
			...
			seq_printf(s,
				   ...
				   entry, ucdip[0], ucdip[1], ucdip[2],
				   ucdip[3], ucsip[0], ucsip[1], ucsip[2],
				   ucsip[3], asic.mbr);
			seq_printf(s,
				   "\t       svid:%d, spa:%d, extIP:%d, age:%d, cpu:%d\n",
				   asic.svid, asic.port, asic.extIdx, asic.age,
	...
	for (entry = 0; entry < RTL8651_MULTICASTTBL_SIZE; entry++) {
		...
		rtl_periodic_watchdog_kick(entry, WATCHDOG_NUM_OF_TIMES);
#endif
		if (rtl8198C_getAsicIpMulticastv6Table(entry, &asic6) !=
		    SUCCESS) {
#if 0
			seq_printf(s,
				   "\t[%d]  (INVALID)dip(%d.%d.%d.%d) sip(%d.%d.%d.%d) mbr(0x%x)\n",
			...
			seq_printf(s, "\t[%d]  (OK)dip:%08x:%08x:%08x:%08x\n",
				   entry,
				   asic6.dip.v6_addr32[0],
				   asic6.dip.v6_addr32[1],
				   asic6.dip.v6_addr32[2],
				   asic6.dip.v6_addr32[3]);
			seq_printf(s, "\t          sip:%08x:%08x:%08x:%08x\n",
				   asic6.sip.v6_addr32[0],
				   asic6.sip.v6_addr32[1],
				   asic6.sip.v6_addr32[2],
				   asic6.sip.v6_addr32[3]);
	...
	seq_printf(s,
		   ...
		   _rtl8198C_getAddMcastv6OpCnt(),
		   _rtl8198C_getDelMcastv6OpCnt(),
					...
					int count, int *eof, void *data)
{
	...
	for (entry = 0; entry < RTL8651_MULTICASTTBL_SIZE; entry++) {
		if (rtl8651_getAsicIpMulticastTable(entry, &asic) != SUCCESS) {
			len +=
			    sprintf(page + len, "\t[%d]  (Invalid Entry)\n",
				    entry);
			continue;
		}
		len +=
	...
	for (i = 0; i < RTL8651_ARPTBL_SIZE; i++) {
		...
		for (j = 0; j < RTL8651_ROUTINGTBL_SIZE; j++) {
			...
			if (asic_l3.arpStart <= (i >> 3)
				...
				ipAddr =
				    (asic_l3.ipAddr & asic_l3.ipMask) + (i -
									 (asic_l3.arpStart
									  <<
									  3));
				...
				if (rtl8651_getAsicL2Table_Patch
				    (asic_arp.nextHopRow,
					...
					for (port = 0;
					     port <
					     RTL8651_PORT_NUMBER +
					     rtl8651_totalExtPortNum; port++) {
						if (asic_l2.memberPortMask &
						    (1 << port))
							seq_printf(s, "%d ",
								   port);
						else
		      ...
		      void *data)
{
	...
	for (i = 0; i < RTL8651_ARPTBL_SIZE; i++) {
		...
		len +=
		    sprintf(page + len, " ARP:%3d  L2:%3d,%d,aging:%d\n", i,
			    asic_arp.nextHopRow, asic_arp.nextHopColumn,
			    asic_arp.aging);
...
//                      mask, str[asic_l3.process],strNetType[netIdx],asic_l3.DSLEG,asic_l3.DSL_IDX);
		...
		case 0x05:	/* NAPT Next Hop */
			seq_printf(s,
				   ...
				   asic_l3.nhStart, asic_l3.nhNum,
				   asic_l3.nhNxt, asic_l3.nhAlgo,
				   asic_l3.ipDomain);
			break;
			...
			int *eof, void *data)
{
	...
	for (i = 0; i < RTL8651_PPPOETBL_SIZE; i++) {
		if (rtl8651_getAsicPppoe(i, &asic_pppoe) == FAILED)
			continue;
		len +=
		    sprintf(page + len, "\t[%d]  sessionID(%d)  ageSec(%d)\n",
...
static ssize_t sw_napt_proc_write(struct file *file,
...
static int32 port_bandwidth_read(struct seq_file *s, void *v)
{
	...
	for (port = 0; port <= CPU; port++) {
		...
		if (port & 1) {
			data0 =
			    (regData & IBWC_ODDPORT_MASK) >>
			    IBWC_ODDPORT_OFFSET;
		} else {
			data0 =
			    (regData & IBWC_EVENPORT_MASK) >>
		...
		pBandwidth_high =
		    (regData2 & (IBCR3_PORT_MASK << IBCR3_PORT_OFFSET(port))) >>
	...
	for (port = 0; port <= CPU; port++) {
		...
		if (data0) {
			len +=
			    sprintf(page + len,
				    "Port%d Ingress:[%d.%03dMbps]	",
				    port, (data0 << 10) / 1000000,
				    ((data0 << 10) % 1000000) / 1000);
		} else {
			len +=
			    sprintf(page + len, "Port%d Ingress:FullSpeed	",
				    port);
		}
		...
		if (data1 != (APR_MASK >> APR_OFFSET)) {
			data1++;
			len +=
			    sprintf(page + len, "Engress:[%d.%03dMbps]\n",
				    (data1 << 10) / 1000000,
				    ((data1 << 10) % 1000000) / 1000);
		} else {
		...
		if (data1 != (APR_MASK >> APR_OFFSET)) {
			...
			if (data1 < 16)
				len +=
				    sprintf(page + len, "Engress:[%dKbps]\n",
					    (data1 << 16) / 1000);
			else
				len +=
				    sprintf(page + len,
...
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
		len +=
		    sprintf(page + len, "Port%d Queue number:	%d\n",
			    port, data0 + 1);
#else
		len +=
		    sprintf(page + len, "Port%d Queue number:	%d\n",
		...
		if (rtl8651_getAsicRateLimitTable(entry, &asic_rl) == SUCCESS) {
			len +=
			    sprintf(page + len,
				    " [%d]  Token(%u)  MaxToken(%u)  remainTime Unit(%u)  \n\trefillTimeUnit(%u)  refillToken(%u)\n",
				    entry, asic_rl.token, asic_rl.maxToken,
				    asic_rl.t_remainUnit,
				    asic_rl.t_intervalUnit,
				    asic_rl.refill_number);
		} else
			len +=
			    sprintf(page + len, " [%d]  Invalid entry\n",
			...
			int *eof, void *data)
{
	...
	for (port = 0; port < RTL8651_PORT_NUMBER + totalExtPortNum; port++) {
		regData = READ_MEM32(PCRP0 + port * 4);
		len +=
		    sprintf(page + len, "port%d, %s BCSC, %s BC, %s MC\n", port,
			    regData & ENBCSC ? "enable" : "disable",
			    regData & BCSC_ENBROADCAST ? "enable" : "disable",
			    regData & BCSC_ENMULTICAST ? "enable" : "disable");
	}
...
#ifdef CONFIG_RTL_ADAPTABLE_TSO
	seq_printf(s, "    TSO:		%s\n",
		   (tso_feature & (NETIF_F_TSO | NETIF_F_TSO_ECN)) ? "ON"
		   : "OFF");
#endif
				    ...
				    int count, int *eof, void *data)
{
...
#endif
	len +=
	    sprintf(page + len, "    rx_noBuffer_cnt:	%u\n", rx_noBuffer_cnt);
	len +=
	    sprintf(page + len, "    tx_ringFull_cnt:	%u\n", tx_ringFull_cnt);
	len += sprintf(page + len, "    tx_drop_cnt:	%u\n", tx_drop_cnt);

	len +=
	    sprintf(page + len, "    statistic_total:	%d\n", statistic_total);
	len += sprintf(page + len, "    statistic_ps:	%d\n", statistic_ps);
	len +=
	    sprintf(page + len, "    statistic_ipv6_fp:	%d\n",
		    statistic_ipv6_fp);
#if defined(CONFIG_OPENWRT_SDK)
	len +=
	    sprintf(page + len, "    statistic_fastpath:	%d\n",
		    statistic_fastpath);
#else
	...
	len +=
	    sprintf(page + len, "	  cnt_swcore_tx:	%d\n",
		    cnt_swcore_tx);
	len +=
	    sprintf(page + len, "	  cnt_swcore_rx:	%d\n",
		    cnt_swcore_rx);
	len +=
	    sprintf(page + len, "	  cnt_swcore_link:	%d\n",
		    cnt_swcore_link);
	len +=
	    sprintf(page + len, "	  cnt_swcore_err:	%d\n",
		    cnt_swcore_err);
#endif
	...
	if (buffer && !copy_from_user(tmpBuf, buffer, len)) {
		...
		if (strncmp(cmd_addr, "clear", 5) == 0) {
			...
			for (i = start; i <= end; i++) {
				...
				printk
				    ("index[%d] accCycle[0]=%llu executedNum=%llu average=%llu\n",
				     i, cp3romePerfStat[i].accCycle[0],
				     cp3romePerfStat[i].executedNum,
				     avrgCycle[i]);
				...
				printk
				    ("index[%d] accCycle[1]=%llu executedNum=%llu average=%llu\n",
				     i, cp3romePerfStat[i].accCycle[1],
				     cp3romePerfStat[i].executedNum,
				     avrgCycle[i]);
			}
					   ...
					   int count, int *eof, void *data)
{
	...
	len =
	    sprintf(page, "original eth private buf total(%d)\n",
		    MAX_ETH_SKB_NUM);
...
static int32 diagnostic_read(struct seq_file *s, void *v)
{
	...
	for (port = 0; port <= CPU; port++) {
		...
		for (regIdx = 0; regIdx < 3; regIdx++) {
			regData =
			    READ_MEM32(P0_DCR0 + (port << 4) + (regIdx << 2));
			seq_printf(s, "%d %d ",
...
errout:
		rtlglue_printf
		    ("Memory operation only support \"read\" and \"write\" as the first parameter\n");
...
static int32 port_status_read(struct seq_file *s, void *v)
{
	...
	for (port = PHY0; port <= PHY4; port++) {
		...
		if ((rtk_port_macStatus_get(port, &sts)) == 0) {
			...
			seq_printf(s, "Speed %s\n\n",
				   (sts.speed) ==
				   PortStatusLinkSpeed100M ? "100M"
				   : ((sts.speed) ==
				      PortStatusLinkSpeed1000M ? "1G"
				      : ((sts.speed) ==
...
#else
		data0 =
		    (regData & (PortEEEStatus_MASK)) >> PortEEEStatus_OFFSET;
	...
	for (port = PHY0; port < PHY4; port++) {
		...
		if ((rtk_port_phyStatus_get(port, &status, &speed, &duplex)) ==
		    0) {
			seq_printf(s, "Port%d ", port);
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "port", 4)) {
			...
			if (type == DUPLEX_10M) {
				for (port = 0; port < CPU; port++) {
					if ((1 << port) & port_mask) {
						rtl8651_setAsicEthernetPHYAutoNeg
						    (port, TRUE);
				...
				for (port = 0; port < CPU; port++) {
					...
					if ((1 << port) & port_mask) {
						if (type == PORT_DOWN) {
							rtl8651_setAsicEthernetPHYPowerDown
							    (port, 1);
							...
							/*Set PHY Register */
							rtl8651_setAsicEthernetPHYSpeed
							    (port,
							     forceLinkSpeed);
	...
	defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
		...
		seq_printf(s,
			   ...
			   (OFFSET_DOT3STATSMULTIPLECOLLISIONFRAMES_P0 +
			    addrOffset_fromP0),
			   rtl8651_returnAsicCounter
			   (OFFSET_DOT3OUTPAUSEFRAMES_P0 + addrOffset_fromP0));
...
void display_8367r_port_stat(uint32 port, rtk_stat_port_cntr_t * pPort_cntrs)
{
	...
	rtlglue_printf
	    ("   Len= 64: %u pkts, 65 - 127: %u pkts, 128 - 255: %u pkts\n",
...
void display_8370_port_stat(uint32 port, rtk_stat_port_cntr_t * p)
{
	...
	rtlglue_printf
	    ("   Rcv %llu bytes, Unicast %u pkts, Multicast %u pkts, Broadcast %u pkts\n",
	...
	rtlglue_printf("   64: %u pkts, 65 -127: %u pkts, 128 -255: %u pkts\n",
		       p->etherStatsPkts64Octets,
		       p->etherStatsPkts65to127Octets,
		       p->etherStatsPkts128to255Octets);
	rtlglue_printf
	    ("   256 - 511: %u pkts, 512 - 1023: %u pkts, 1024 - max: %u pkts\n",
	     p->etherStatsPkts256to511Octets, p->etherStatsPkts512to1023Octets,
	     p->etherStatsPkts1024toMaxOctets);
	rtlglue_printf
	    ("   Drop: %u, FCSError: %u, Fragment: %u, JabberErr: %u\n",
	     p->etherStatsDropEvents, p->dot3StatsFCSErrors,
	     p->etherStatsFragments, p->etherStatsJabbers);
	rtlglue_printf
	    ("   OverSize: %u, UnderSize: %u, Unknown: %u, inOampdu: %u\n",
					...
					int count, int *eof, void *data)
{
	...
	for (port = 0; port < 4; port++) {
		rtl_get_portRate(port, &rxRate, &txRate);
		len +=
		    sprintf(page + len, "port%d rx:%ld tx:%ld(kbps)\n", port,
			    rxRate, txRate);
	}
			    ...
			    unsigned long len, void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "read", 4)) {
			...
			if (ret == SUCCESS) {
				rtlglue_printf
				    ("read phyId(%d), devId(%d), regId(%d), regData:0x%x\n",
			...
			rtl8651_setAsicEthernetPHYReg(phyId, 13,
						      (devId | 0x4000));
			ret =
			    rtl8651_getAsicEthernetPHYReg(phyId, 14,
							  &dataReadBack);

			if (ret == SUCCESS) {
				rtlglue_printf
				    ("extWrite phyId(%d), devId(%d), regId(%d), regData:0x%x, regData(read back):0x%x\n",
				     phyId, devId, regId, regData,
				     dataReadBack);
			} else {
				rtlglue_printf("error input!\n");
			       ...
			       unsigned long len, void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "read", 4)) {
			...
			ret =
			    rtl8651_setAsicEthernetPHYReg(phyId, regId,
							  regData);
			if (ret == SUCCESS) {
				rtlglue_printf
				    ("Write phyId(%d), regId(%d), regData:0x%x\n",
				     phyId, regId, regData);
			} else {
				rtlglue_printf("error input!\n");
			}
			...
			if (ret == 0) {
				rtlglue_printf
				    ("rtl8370_getAsicReg: reg= %x, data= %x\n",
				     regId, regData);
			} else {
			...
			if (ret == 0) {
				rtlglue_printf
				    ("rtl8370_setAsicReg: reg= %x, data= %x\n",
				     regId, regData);
			} else {
			...
			if (ret == 0) {
				rtlglue_printf
				    ("rtl8367b_getAsicReg: reg= %x, data= %x\n",
				     regId, regData);
			} else {
				rtlglue_printf("get fail %d\n", ret);
			}
			...
			if (ret == 0) {
				rtlglue_printf
				    ("rtl8367b_setAsicReg: reg= %x, data= %x\n",
				     regId, regData);
			} else {
				rtlglue_printf("set fail %d\n", ret);
			}
			...
			if (ret == SUCCESS) {
				rtlglue_printf
				    ("read 8367R/RB phyId(%d), regId(%d), regData:0x%x\n",
				     phyId, regId, regData);
			} else {
				rtlglue_printf("error input!\n");
			}
			...
			if (ret == SUCCESS) {
				rtlglue_printf
				    ("Write 8367R/RB phyId(%d), regId(%d), regData:0x%x\n",
				     phyId, regId, regData);
			} else {
				rtlglue_printf("error input!\n");
			}
			...
			if (tokptr[0] == 'm') {
				...
				if (ret == SUCCESS)
					rtlglue_printf
					    ("set 8367RB phyId(%d) to mdi mode: %d\n",
					     phyId, regId);
				else
					rtlglue_printf
					    ("rtk_port_phyMdx_set return error\n");
			} else {
				regId = simple_strtol(tokptr, NULL, 0);
				...
				if (ret == SUCCESS)
					rtlglue_printf
					    ("set  8367RB phyId(%d) to mode: %d\n",
					     phyId, regId);
				else
					rtlglue_printf
					    ("rtk_port_phyTestMode_set return error\n");
			}
			...
			if (ret == SUCCESS) {
				rtlglue_printf
				    ("read 8370 phyId(%d), regId(%d), regData:0x%x\n",
				     phyId, regId, regData);
			} else {
				rtlglue_printf("error input!\n");
			}
			...
			if (ret == SUCCESS) {
				rtlglue_printf
				    ("Write 8370 phyId(%d), regId(%d), regData:0x%x\n",
				     phyId, regId, regData);
			} else {
				rtlglue_printf("error input!\n");
			}
			...
			if (ret == SUCCESS) {
				rtlglue_printf
				    ("read 8325 phyId(%d), regId(%d), regData:0x%x\n",
				     phyId, regId, data);
			} else {
				rtlglue_printf("error input!\n");
			}
			...
			if (ret == SUCCESS) {
				rtlglue_printf
				    ("Write 8325 phyId(%d), regId(%d), regData:0x%x\n",
				     phyId, regId, regData);
			} else {
				rtlglue_printf("error input!\n");
			}
			...
			if (ret == SUCCESS) {
				rtlglue_printf
				    ("extRead phyId(%d), pageId(%d), regId(%d), regData:0x%x\n",
			...
			if (ret == SUCCESS) {
				rtlglue_printf
				    ("extWrite phyId(%d), pageId(%d), regId(%d), regData:0x%x\n",
				     phyId, pageId, regId, regData);
			} else {
				rtlglue_printf("error input!\n");
			}
				...
				for (page = 0; page < 4; page++) {
					...
					for (reg = 0; reg < 32; reg++) {
						rtl8651_getAsicEthernetPHYReg
						    (port, reg, &regData);
						rtlglue_printf
						    ("port:%d,page:%d,regId:%d,regData:0x%x\n",
						     port, page, reg, regData);
					}
					if (page != 3) {
						rtlglue_printf
...
static int32 fc_threshold_read(struct seq_file *s, void *v)
{
	...
	for (port = PHY0; port <= CPU; port++) {
		...
		data0 =
		    (regData & P_MaxDSC_FCOFF_MASK) >> P_MaxDSC_FCOFF_OFFSET;
		...
		//if(port<CPU)
		{
			for (group = GR0; group <= GR2; group++) {
				...
				data0 =
				    (regData & QG_DSC_FCOFF_MASK) >>
				    QG_DSC_FCOFF_OFFSET;
				seq_printf(s, "   QG_DSC_FCOFF:%d, ", data0);
				data0 =
				    (regData & QG_DSC_FCON_MASK) >>
				    QG_DSC_FCON_OFFSET;
			       ...
			       int *eof, void *data)
{
	...
	regData = READ_MEM32(QNUMCR);
	len +=
	    sprintf(page + len, "The number of output queue for port(0~6) :\n");
	...
	for (port = PHY0; port <= CPU; port++) {
		...
		//if(port<CPU)
		{
			for (group = GR0; group <= GR2; group++) {
				len +=
				    sprintf(page + len, "   Port%dGroup%d\n",
					    port, group);
				/* QDBFCRP0G0,QDBFCRP0G1,QDBFCRP0G2
				 * QDBFCRP1G0,QDBFCRP1G1,QDBFCRP1G2
				 * QDBFCRP2G0,QDBFCRP2G1,QDBFCRP2G2
				...
				data0 =
				    ...
				    QG_DSC_FCOFF_OFFSET;
				len +=
				    sprintf(page + len, "   QG_DSC_FCOFF:%d, ",
					    data0);
				data0 =
				    (regData & QG_DSC_FCON_MASK) >>
				    QG_DSC_FCON_OFFSET;
				len +=
				    sprintf(page + len, "QG_DSC_FCON:%d, ",
					    data0);
				...
				data0 =
				    ...
				    QG_QLEN_FCOFF_OFFSET;
				len +=
				    sprintf(page + len, "QG_QLEN_FCOFF:%d, ",
					    data0);
				data0 =
				    (regData & QG_QLEN_FCON_MASK) >>
				    QG_QLEN_FCON_OFFSET;
				len +=
				    sprintf(page + len, "QG_QLEN_FCON:%d\n",
					    data0);
...
static int32 l3v6_read(struct seq_file *s, void *v)
{
	...
	for (idx = 0; idx < RTL8198C_ROUTINGV6TBL_SIZE; idx++) {
		...
		seq_printf(s,
			   ...
			   htons(tmp_ip.v6_addr16[1]),
			   htons(tmp_ip.v6_addr16[2]),
			   htons(tmp_ip.v6_addr16[3]),
			   htons(tmp_ip.v6_addr16[4]),
			   htons(tmp_ip.v6_addr16[5]),
			   htons(tmp_ip.v6_addr16[6]),
		       ...
		       void *data)
{
	...
	for (idx = 0; idx < RTL8198C_ROUTINGV6TBL_SIZE; idx++) {
		...
		len +=
		    sprintf(page + len, "\tsix_rd_eg :0x%x\tsix_rd_idx:0x%x\n",
			    asic_l3v6.six_rd_eg, asic_l3v6.six_rd_idx);
	}
	...
	for (i = 0; i < RTL8198C_ARPV6TBL_SIZE; i++) {
		...
		len +=
		    sprintf(page + len,
			    ...
			    i, asic_arpv6.nextHopRow, asic_arpv6.nextHopColumn,
			    asic_arpv6.subnet_id,
			    asic_arpv6.hostid.v6_addr16[4],
...
static int32 nexthop6_read(struct seq_file *s, void *v)
{
	...
	for (idx = 0; idx < RTL8198C_NEXTHOPV6TBL_SIZE; idx++) {
		if (rtl8198C_getAsicNextHopTableV6(idx, &asic_nxthopv6) ==
		    FAILED)
			continue;
		...
		seq_printf(s,
			   ...
			   idx,
			   (asic_nxthopv6.isPppoe ==
			    TRUE ? "pppoe" : "ethernet"), asic_nxthopv6.dvid,
			   asic_nxthopv6.pppoeIdx,
	...
	for (entry = 0; entry < RTL8198C_MULTICASTV6TBL_SIZE; entry++) {
		...
		if (rtl8198C_getAsicIpMulticastv6Table(entry, &asic) != SUCCESS) {
			...
			seq_printf(s,
				   ...
				   asic.port, asic.age, asic.cpu,
				   asic.six_rd_eg, asic.six_rd_idx);
		}
	}
		...
		if (rtl8198C_getAsicIpMulticastv6Table(entry, &asic) != SUCCESS) {
			...
			len +=
			    sprintf(page + len,
				    "\t[%d]  (OK)dip(%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x) sip(%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x) mbr(0x%x)\n",
				    entry, asic.dip.v6_addr16[0],
				    asic.dip.v6_addr16[1],
				    asic.dip.v6_addr16[2],
				    asic.dip.v6_addr16[3],
				    asic.dip.v6_addr16[4],
				    asic.dip.v6_addr16[5],
				    asic.dip.v6_addr16[6],
				    asic.dip.v6_addr16[7],
				    asic.sip.v6_addr16[0],
				    asic.sip.v6_addr16[1],
				    asic.sip.v6_addr16[2],
				    asic.sip.v6_addr16[3],
				    asic.sip.v6_addr16[4],
				    asic.sip.v6_addr16[5],
				    asic.sip.v6_addr16[6],
				    asic.sip.v6_addr16[7], asic.mbr);
			len +=
			    sprintf(page + len,
				    "\t       spa:%d, age:%d, cpu:%d, 6rd_eg_en:%d, 6rd_eg_idx:%d\n",
...
static int32 ip6rd_read(struct seq_file *s, void *v)
{
	...
	for (idx = 0; idx < RTL8198C_6RDTBL_SIZE; idx++) {
		...
		seq_printf(s,
			   ...
			   htons(tmp_ip.v6_addr16[0]),
			   htons(tmp_ip.v6_addr16[1]),
			   htons(tmp_ip.v6_addr16[2]),
			   htons(tmp_ip.v6_addr16[3]), asic.six_rd_prefix_len);
		seq_printf(s,
			   "        brip(%d.%d.%d.%d)/bripm:(%d) 6RD MTU:%d(0x%x) valid:%d\n",
			   (asic.br_ip_addr >> 24),
			   ((asic.br_ip_addr & 0x00ff0000) >> 16),
			   ((asic.br_ip_addr & 0x0000ff00) >> 8),
			   (asic.br_ip_addr & 0xff), asic.br_ip_mask_len,
			   asic.mtu, asic.mtu, asic.valid);
	}
#endif
	return 0;
}
...
static int32 dslite_read(struct seq_file *s, void *v)
{
	...
	for (idx = 0; idx < RTL8198C_DSLITETBL_SIZE; idx++) {
		...
		seq_printf(s,
			   ...
			   htons(tmp_ip.v6_addr16[5]),
			   htons(tmp_ip.v6_addr16[6]),
			   htons(tmp_ip.v6_addr16[7]), asic.aftr_ipv6_mask,
			   asic.mtu, asic.mtu, asic.valid);
	}
#endif
	return 0;
...
static int32 sw_dslite_read(struct seq_file *s, void *v)
{
	...
	for (idx = 0; idx < RTL8198C_DSLITETBL_SIZE; idx++) {
		...
		seq_printf(s,
			   ...
			   entry.aftr_ipv6_addr.v6_addr16[7],
			   entry.aftr_ipv6_mask, entry.mtu, entry.mtu,
			   entry.valid);
	}
	return 0;
}
...
static ssize_t stats_single_write(struct file *file,
				  const char __user * userbuf, size_t count,
				  loff_t * off)
{
...
static ssize_t netif_single_write(struct file *file,
				  const char __user * userbuf, size_t count,
				  loff_t * off)
{
...
static ssize_t advRt_single_write(struct file *file,
				  const char __user * userbuf, size_t count,
				  loff_t * off)
{
...
static ssize_t sw_l3_single_write(struct file *file,
				  const char __user * userbuf, size_t count,
				  loff_t * off)
{
...
static ssize_t pppoe_single_write(struct file *file,
				  const char __user * userbuf, size_t count,
				  loff_t * off)
{
...
static ssize_t sw_l2_single_write(struct file *file,
				  const char __user * userbuf, size_t count,
				  loff_t * off)
{
...
static ssize_t sixrd_single_write(struct file *file,
				  const char __user * userbuf, size_t count,
				  loff_t * off)
{
...
static int32 vlan_8367_single_show(struct seq_file *s, void *v)
{
	...
	rtlglue_printf("%s\n", "ASIC VLAN Table:");
	{
		...
		for (i = 0; i < 4095; i++) {
			...
			if (rtk_vlan_get(i, &pMbrmsk, &pUntagmsk, &pFid) ==
			    FAILED)
				continue;
	...
	if (rtl865x_proc_dir) {
		...
		/*stats */
		{
			...
			if (stats_debug_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for stats");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*vlan */
		{
			...
			if (vlan_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for vlan");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*netif */
		{
			...
			if (netif_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for netif");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*swnetif */
		{
...
#else
			sw_netif_entry =
			    create_proc_entry("sw_netif", 0, rtl865x_proc_dir);
			if (sw_netif_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for sw netif");
				retval = FAILED;
				goto out;
			}
#endif
...
#if defined(CONFIG_RTL_8367R_SUPPORT) && defined(CONFIG_OPENWRT_SDK)	//mark_bb
		{
			proc_create_data("8367_vlan", 0, rtl865x_proc_dir,
					 &vlan_8367_single_seq_file_operations,
					 NULL);
		...
		/*acl */
		{
			...
			if (acl_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for acl");
				retval = FAILED;
				goto out;
			}
#endif
...
#if defined(CONFIG_RTL_MULTIPLE_WAN)
		{
			...
			if (advRt_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for advRt");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*storm control */
		{
			...
			if (storm_control != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for storm control");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*soft acl chains */
		{
			...
			if (acl_chains_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for aclChains");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*hs */
		{
			...
			if (hs_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for hs");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*nic mbuf */
		{
...
#else
			nic_skb_buff =
			    create_proc_entry("nic_mbuf", 0, rtl865x_proc_dir);
			if (nic_skb_buff != NULL) {
				...
				rtlglue_printf
				    ("can't create nic_mbuf entry for hs");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*rx ring */
		{
...
#else
			rxRing_entry =
			    create_proc_entry("rxRing", 0, rtl865x_proc_dir);
			if (rxRing_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create rxRing entry for hs");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*tx ring */
		{
...
#else
			txRing_entry =
			    create_proc_entry("txRing", 0, rtl865x_proc_dir);
			if (txRing_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create txRing entry for hs");
				retval = FAILED;
				goto out;
			}
#endif
...
#else
			mbuf_entry =
			    create_proc_entry("mbufRing", 0, rtl865x_proc_dir);
			if (mbuf_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create mbuf entry for mbufRing");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*pvid */
		{
			...
			if (pvid_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for pvid");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*mirrorPort */
		{
			...
			if (mirrorPort_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for mirrorPort");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*l2 */
		{
			...
			if (l2_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for l2");
				retval = FAILED;
				goto out;
			}
#endif
...
#else
			hwMCast_entry =
			    create_proc_entry("hwMCast", 0, rtl865x_proc_dir);
...
#else
			swMCast_entry =
			    create_proc_entry("swMCast", 0, rtl865x_proc_dir);
		...
		/*arp */
		{
			...
			if (arp_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for arp");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*nextHop */
		{
...
#else
			nexthop_entry =
			    create_proc_entry("nexthop", 0, rtl865x_proc_dir);
			if (nexthop_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for nexthop");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*l3 */
		{
			...
			if (l3_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for l3");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*ip */
		{
			...
			if (ip_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for ip");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*pppoe */
		{
			...
			if (pppoe_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for pppoe");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*napt */
		{
			...
			if (napt_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for napt");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*software napt */
		{
...
#else
			sw_napt_entry =
			    create_proc_entry("sw_napt", 0, rtl865x_proc_dir);
			if (sw_napt_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for sw_napt");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*software l2 */
		{
			...
			if (sw_l2_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for sw_l2");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*port_bandwidth */
		{
			...
			if (port_bandwidth_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for port_bandwidth");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*queue_bandwidth */
		{
			...
			if (queue_bandwidth_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for queue_bandwidth");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*priority_decision */
		{
			...
			proc_create_data("priority_decision", 0,
					 rtl865x_proc_dir,
					 &priority_decision_proc_fops, NULL);
#else
			...
			if (priority_decision_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for priority_decision");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/* rate limit table */
		{
...
#else
			rateLimit_entry =
			    create_proc_entry("rateLimit", 0, rtl865x_proc_dir);
			if (rateLimit_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for rate limit table");
				retval = FAILED;
				goto out;
			}
#endif
...
#else
			eventMgr_entry =
			    create_proc_entry("eventMgr", 0, rtl865x_proc_dir);
...
#else
			perf_dump =
			    create_proc_entry("perf_dump", 0, rtl865x_proc_dir);
			if (perf_dump != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for dump_perf");
				retval = FAILED;
				goto out;
			}
#endif
...
#else
			mem_entry =
			    create_proc_entry("memory", 0, rtl865x_proc_dir);
			if (mem_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for memory");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*diagnostic */
		{
			...
			if (diagnostic_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for diagnostic");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*port_status */
		{
			...
			if (port_status_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for port_status");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*phy */
		{
...
#else
			phyReg_entry =
			    create_proc_entry("phyReg", 0, rtl865x_proc_dir);
			if (phyReg_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for phyTeg");
				retval = FAILED;
				goto out;
			}
#endif
...
#else
			portRate_entry =
			    create_proc_entry("portRate", 0, rtl865x_proc_dir);
		...
		/* indirect access for special phy register */
		{
			...
			if (mmd_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for mmd");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*maccr */
		{
			...
			if (mac_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for mac");
				retval = FAILED;
				goto out;
			}
		...
		/*FC threshold */
		{
			...
			if (fc_threshold_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for fc_threshold");
				retval = FAILED;
				goto out;
			}
		...
		/*l3v6 */
		{
			...
			if (l3v6_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for l3v6");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*arp6 */
		{
			...
			if (arp6_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for arp6");
				retval = FAILED;
				goto out;
			}
#endif
...
/*sw-arp6*/
...
#else
			sw_arp6_entry =
			    create_proc_entry("sw_arp6", 0, rtl865x_proc_dir);
		...
		/*nexthop6 */
		{
...
#else
			nexthop6_entry =
			    create_proc_entry("nexthop6", 0, rtl865x_proc_dir);
			if (nexthop6_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for nexthop6");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*mcast6 */
		{
...
#else
			mcast6_entry =
			    create_proc_entry("mcast6", 0, rtl865x_proc_dir);
			if (mcast6_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for mcast6");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*6rd */
		{
			...
			if (ip6rd_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for 6rd");
				retval = FAILED;
				goto out;
			}
#endif
		...
		/*dslite */
		{
...
#else
			dslite_entry =
			    create_proc_entry("dslite", 0, rtl865x_proc_dir);
			if (dslite_entry != NULL) {
				...
				rtlglue_printf
				    ("can't create proc entry for dslite");
				retval = FAILED;
				goto out;
			}
#endif
...
#else
			sw_dslite_entry =
			    create_proc_entry("sw_dslite", 0, rtl865x_proc_dir);
