	...
	if (srcPortNum < RTL8651_CPU_PORT) {
		...
		if (extPortEnabled) {
			if ((m_pkthdr->ph_extPortList & PKTHDR_EXTPORTMASK_CPU)
				...
				if (m_pkthdr->ph_extPortList &&
				    (5 !=
				     extPortMaskToPortNum
				     [m_pkthdr->ph_extPortList])) {
#if defined(CONFIG_RTL_HARDWARE_NAT)&&(defined(CONFIG_RTL8192SE)||defined(CONFIG_RTL8192CD))
...
inline int return_to_rxing_check(int ringIdx)
{
	...
	ret =
	    ((rxDescReadyForHwIndex[ringIdx] != currRxPkthdrDescIndex[ringIdx])
...
static inline int buffer_reuse(int ringIdx)
{
	...
	gap =
	    (index2 >
	     index1) ? (index2 - index1) : (index2 + rxPkthdrRingCnt[ringIdx] -
					    index1);

	if ((rxPkthdrRingCnt[ringIdx] - gap) <
	    (rxPkthdrRefillThreshold[ringIdx])) {
#if !defined(CONFIG_SMP)
    ...
    static void increase_rx_idx_release_pkthdr(struct sk_buff *skb, int idx)
{
	...
	pReadyForHw =
	    (struct rtl_pktHdr *)(rxPkthdrRing_base[idx] +
				  (sizeof(struct rtl_pktHdr) *
				   currRxPkthdrDescIndex[idx]));
#else
	pReadyForHw =
	...
	mbufIndex =
	    ((uint32) (pReadyForHw->ph_mbuf) -
	     rxMbufRing_base) / (sizeof(struct rtl_mBuf));
#else
	mbufIndex =
	    ((uint32) (pReadyForHw->ph_mbuf) -
	     (rxMbufRing[0] & ~(DESC_OWNED_BIT | DESC_WRAP))) /
    ...
    static int __swNic_geRxRingIdx(uint32 rxRingIdx, uint32 * currRxPktDescIdx)
{
	...
	if ((rxDescCrossBoundFlag[rxRingIdx] == 0
	     && (currRxPkthdrDescIndex[rxRingIdx] >=
		 rxDescReadyForHwIndex[rxRingIdx]))
	...
	for (i = RTL865X_SWNIC_TXRING_HW_PKTDESC - 1; i >= 0; i--) {
		...
		for (j = 0; j < txPkthdrRingCnt[i]; j++) {
			...
			if (pPkthdr->ph_mbuf->skb) {
				if (is_rtl865x_eth_priv_buf
				    (((struct sk_buff *)pPkthdr->ph_mbuf->
				      skb)->head))
					txCnt++;
			}
		}
...
int32 swNic_flushRxRingByPriority(int priority)
{
	...
	for (i = priority - 1; i >= 0; i--) {
		...
		while ((rxPkthdrRing[i][currRxPkthdrDescIndex[i]] &
			...
			pPkthdr =
			    (struct rtl_pktHdr
			     *)(rxPkthdrRing[i][currRxPkthdrDescIndex[i]] &
				~(DESC_OWNED_BIT | DESC_WRAP));
...
get_next:
...
#if defined(RTL_MULTIPLE_RX_TX_RING)
	if (swNic_getRxringIdx(&rxRingIdx, &currRxPktDescIdx, info->priority) ==
	    SUCCESS)
#else
	rxRingIdx = 0;
...
#endif
	{
		...
		pPkthdr =
		    (struct rtl_pktHdr
		     *)(rxPkthdrRing[rxRingIdx][currRxPktDescIdx] &
			~(DESC_OWNED_BIT | DESC_WRAP));
#endif
		/* Increment counter */
		//rxPktCounter++;
		...
		if ((pPkthdr->ph_flags & (CSUM_TCPUDP_OK | CSUM_IP_OK)) !=
			...
			if (!
			    (((pPkthdr->ph_flags & (CSUM_TCPUDP_OK)) !=
			      CSUM_TCPUDP_OK)
		...
		if (buf) {
			...
			if (retryCount > RTL_NIC_RX_RETRY_MAX) {
				/* drop pkt */
				increase_rx_idx_release_pkthdr((void *)
							       pPkthdr->ph_mbuf->skb, rxRingIdx);
			}
#if defined(CONFIG_SMP)
			SMP_UNLOCK_ETH_RECV(flags);
#endif
...
int32 swNic_freeRxRing(void)
{
...
#endif
	{
		...
		_dma_cache_inv((unsigned long)(pPkthdr->ph_mbuf),
			       sizeof(struct rtl_mBuf));
#endif
		_dma_cache_inv((unsigned
				long)(((struct sk_buff *)pPkthdr->
				       ph_mbuf->skb)->data),
			       pPkthdr->ph_len - 4);
		RTL_ETH_NIC_DROP_RX_PKT_RESTART;
...
*************************************************************************/
__MIPS16
    __IRAM_FWD inline int32 _swNic_send(void *skb, void *output, uint32 len,
					rtl_nicTx_info * nicTx)
{
...
#if defined(CONFIG_RTL_ENHANCE_RELIABILITY) && (defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F))
	pPkthdr =
	    (struct rtl_pktHdr *)((int32) txPkthdrRing_base[nicTx->txIdx] +
				  (sizeof(struct rtl_pktHdr) *
				   currTxPkthdrDescIndex[nicTx->txIdx]));

#else
	/* Fetch packet header from Tx ring */
	pPkthdr = (struct rtl_pktHdr *)((int32)
					txPkthdrRing[nicTx->txIdx]
					[currTxPkthdrDescIndex[nicTx->txIdx]]
					& ~(DESC_OWNED_BIT | DESC_WRAP));
	...
	if (*((unsigned short *)((unsigned char *)output + ETH_ALEN * 2)) !=
		...
		pPkthdr->ph_txCVlanTagAutoAdd =
		    RTL_WANPORT_MASK & rtk_get_vlan_tagmask(pPkthdr->ph_vlanId);
#else
		pPkthdr->ph_txCVlanTagAutoAdd =
...
#endif
	{
		if (pPkthdr->ph_portlist == RTL_WANPORT_MASK) {
			if (((struct sk_buff *)skb)->srcVlanPriority > 0
				...
				if (*(unsigned short *)
				    ((unsigned char *)output + ETH_ALEN * 2) ==
				    __constant_htons(ETH_P_8021Q)) {
					pPkthdr->ph_vlanTagged = 1;
					pPkthdr->ph_txCVlanTagAutoAdd =
					    RTL_WANPORT_MASK;
...
#if defined(CONFIG_RTL_ENHANCE_RELIABILITY) && !defined(CONFIG_RTL_8198C) &&  !defined(CONFIG_RTL_8197F)
	{
		...
		if ((pkthdr2 & DESC_OWNED_BIT) == 0)
			rtlglue_printf
			    ("_swNic_send: idx= %d, read back pkthdr= 0x%x.\n",
			     ret, pkthdr2);
	}
#endif
	...
	printk("index %d address 0x%p, 0x%x 0x%p.\n", ret,
	       ...
	       (*(volatile uint32 *)&txPkthdrRing[nicTx->txIdx][ret]), pPkthdr);
	printk
	    ("Flags 0x%x proto 0x%x portlist 0x%x vid %d extPort %d srcExtPort %d len %d.\n",
		...
		while (txPktDoneDescIndex[idx] != currTxPkthdrDescIndex[idx]) {
			if ((*(volatile uint32 *)
				...
				if (rtl_chip_version == RTL8196C_REVISION_A)
					txPkthdrRing[idx][txPktDoneDescIndex
							  [idx]] =
					    txPkthdrRing_backup[idx]
					    [txPktDoneDescIndex[idx]];
#endif
...
void swNic_freeRxBuf(void)
{
	...
	for (i = RTL865X_SWNIC_RXRING_HW_PKTDESC - 1; i >= 0; i--) {
		...
		for (idx = 0; idx < rxPkthdrRingCnt[i]; idx++) {
			if (!
			    ((rxPkthdrRing[i][idx] & DESC_OWNED_BIT) ==
			     DESC_RISC_OWNED)) {
				pPkthdr =
...
void swNic_freeTxRing(void)
{
	...
	for (idx = 0; idx < RTL865X_SWNIC_TXRING_HW_PKTDESC; idx++) {
		while (txPktDoneDescIndex[idx] != currTxPkthdrDescIndex[idx]) {
			...
			if (rtl_chip_version == RTL8196C_REVISION_A)
				txPkthdrRing[idx][txPktDoneDescIndex[idx]] =
				    txPkthdrRing_backup[idx][txPktDoneDescIndex
							     [idx]];
#endif
		 ...
		 uint32 clusterSize)
{
	...
	if (rxMbufRing == NULL) {
...
#ifdef CONFIG_RTL8196C_REVISION_B
			if (rtl_chip_version == RTL8196C_REVISION_A)
				txPkthdrRing_backup[i] = (uint32 *)
				    UNCACHED_MALLOC(txPkthdrRingCnt[i] *
						    sizeof(uint32));
#endif
		...
		rxMbufRing = (uint32 *)
		    UNCACHED_MALLOC((rxMbufRingCnt +
				     RESERVERD_MBUF_RING_NUM) *
				    sizeof(uint32 *));
		...
		pPkthdrList_start = (struct rtl_pktHdr *)
		    kmalloc((totalRxPkthdrRingCnt +
			     totalTxPkthdrRingCnt) * sizeof(struct rtl_pktHdr),
			    GFP_ATOMIC);
		ASSERT_CSP((uint32) pPkthdrList_start & 0x0fffffff);

		/* Allocate mbufs */
		pMbufList_start = (struct rtl_mBuf *)kmalloc((rxMbufRingCnt +
							      RESERVERD_MBUF_RING_NUM
							      +
...
#else
		pPkthdrList_start = (struct rtl_pktHdr *)
		    kmalloc((totalRxPkthdrRingCnt + totalTxPkthdrRingCnt +
			     1) * sizeof(struct rtl_pktHdr), GFP_ATOMIC);
		ASSERT_CSP((uint32) pPkthdrList_start & 0x0fffffff);

		pPkthdrList_start =
		    (struct rtl_pktHdr
		     *)(((uint32) pPkthdrList_start +
			 (_cpu_dcache_line - 1)) & ~(_cpu_dcache_line - 1));
		...
		pMbufList_start =
		    (struct rtl_mBuf
		     *)(((uint32) pMbufList_start +
			 (_cpu_dcache_line - 1)) & ~(_cpu_dcache_line - 1));
#ifdef CONFIG_RTL_ETH_TX_SG
		memset(pMbufList_start, 0,
		       (rxMbufRingCnt + RESERVERD_MBUF_RING_NUM +
			(totalTxPkthdrRingCnt) + SG_MBUF_NUM +
			2) * sizeof(struct rtl_mBuf));
#else
		memset(pMbufList_start, 0,
		       (rxMbufRingCnt + RESERVERD_MBUF_RING_NUM +
			totalTxPkthdrRingCnt) * sizeof(struct rtl_mBuf));
#endif
#endif

#else
		pPkthdrList_start = (struct rtl_pktHdr *)
		    UNCACHED_MALLOC((totalRxPkthdrRingCnt +
				     totalTxPkthdrRingCnt) *
				    sizeof(struct rtl_pktHdr));
		ASSERT_CSP((uint32) pPkthdrList_start & 0x0fffffff);

		/* Allocate mbufs */
		pMbufList_start = (struct rtl_mBuf *)
		    UNCACHED_MALLOC((rxMbufRingCnt + RESERVERD_MBUF_RING_NUM +
				     totalTxPkthdrRingCnt) *
				    sizeof(struct rtl_mBuf));
		ASSERT_CSP((uint32) pMbufList_start & 0x0fffffff);
#endif
...
#ifdef CONFIG_RTL8196C_REVISION_B
			if (rtl_chip_version == RTL8196C_REVISION_A)
				txPkthdrRing_backup[i][txPkthdrRingCnt[i] -
						       1] |= DESC_WRAP;
#endif
		}
	...
	for (idx = 0; idx < RTL865X_SWNIC_RXRING_HW_PKTDESC; idx++) {
		...
		for (cnt = 0; cnt < rxPkthdrRingCnt[idx]; cnt++) {
			...
			seq_printf(s,
				   ...
				   pPkthdr->ph_mbuf->skb,
				   (rxPkthdrRing[idx][cnt] & DESC_OWNED_BIT) ==
				   DESC_RISC_OWNED ? " :CPU" : " :SWCORE",
				   (rxPkthdrRing[idx][cnt] & DESC_WRAP) !=
				   0 ? " :WRAP" : "",
				   cnt ==
				   currRxPkthdrDescIndex[idx] ? "  <===currIdx"
				   : "",
				   cnt ==
				   rxDescReadyForHwIndex[idx] ?
				   " <===readyForHw" : "");
#else
			seq_printf(s,
	...
	for (idx = 0; idx < RTL865X_SWNIC_TXRING_HW_PKTDESC; idx++) {
		...
		for (cnt = 0; cnt < txPkthdrRingCnt[idx]; cnt++) {
			...
			if (rtl_chip_version == RTL8196C_REVISION_A)
				pPkthdr =
				    (struct rtl_pktHdr
				     *)(txPkthdrRing_backup[idx][cnt] &
					~(DESC_OWNED_BIT | DESC_WRAP));
			else
#endif
				pPkthdr =
				    (struct rtl_pktHdr *)(txPkthdrRing[idx][cnt]
							  & ~(DESC_OWNED_BIT |
							      DESC_WRAP));
			...
			seq_printf(s,
				   ...
				   cnt, pPkthdr, pPkthdr->ph_mbuf,
				   pPkthdr->ph_mbuf->skb,
				   (txPkthdrRing[idx][cnt] & DESC_OWNED_BIT) ==
				   DESC_RISC_OWNED ? " :CPU" : " :SWCORE",
				   (txPkthdrRing[idx][cnt] & DESC_WRAP) !=
				   0 ? " :WRAP" : "",
				   cnt ==
				   currTxPkthdrDescIndex[idx] ? "  <===currIdx"
				   : "",
				   cnt ==
				   txPktDoneDescIndex[idx] ? "  <===txDoneIdx" :
				   "");
...
int32 rtl_dumpMbufRing(struct seq_file * s)
{
	...
	while (1) {
		mbuf =
		    (struct rtl_mBuf *)(rxMbufRing[idx] &
					~(DESC_OWNED_BIT | DESC_WRAP));
		seq_printf(s,
			   "mbuf[%03d]: 0x%p: ==> pkthdr[0x%p] ==> skb[0x%p]%s%s%s\n",
			   idx, mbuf, mbuf->m_pkthdr, mbuf->skb,
			   (rxMbufRing[idx] & DESC_OWNED_BIT) ==
			   DESC_RISC_OWNED ? " :CPU" : " :SWCORE",
			   (rxMbufRing[idx] & DESC_WRAP) ==
			   DESC_ENG_OWNED ? " :WRAP" : "",
...
int32 rtl_dumpIndexs(void)
{
	...
	for (i = 0; i < RTL865X_SWNIC_RXRING_HW_PKTDESC; i++) {
		rtlglue_printf
		    ("	TotalCnt: %d, currPkthdrIdx: %d currMbufIdx: %d readyForHwIdx: %d crossBoundFlag: %d.\n",
	...
	for (i = 0; i < RTL865X_SWNIC_RXRING_HW_PKTDESC; i++) {
		rtlglue_printf
		    ("	TotalCnt: %d, currPkthdrIdx: %d currMbufIdx: %d\n",
		     rxPkthdrRingCnt[i], currRxPkthdrDescIndex[i],
		     currRxMbufDescIndex);
	}
#endif
	...
	for (i = 0; i < RTL865X_SWNIC_TXRING_HW_PKTDESC; i++) {
		rtlglue_printf
		    ("	TotalCnt: %d, currPkthdrIdx: %d pktDoneIdx: %d.\n",
...
int32 rtl_check_tx_done_desc_swCore_own(int32 * tx_done_inx)
{
	...
	if ((*(volatile uint32 *)
	     &txPkthdrRing[tx_ring_idx][txPktDoneDescIndex[tx_ring_idx]] &
	     DESC_OWNED_BIT) == DESC_SWCORE_OWNED) {
