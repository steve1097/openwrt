...
static int32 write_proc_8021x_enable(struct file *filp, const char *buff,
				     unsigned long len, void *data);
static int32 write_proc_8021x_type(struct file *filp, const char *buff,
				   unsigned long len, void *data);
static int32 write_proc_8021x_auth_server_port(struct file *filp,
					       const char *buff,
					       unsigned long len, void *data);
static int32 write_proc_8021x_auth_port_mode(struct file *filp,
					     const char *buff,
					     unsigned long len, void *data);
...
static ssize_t dot1x_enable_single_write(struct file *file,
...
static ssize_t dot1x_type_single_write(struct file *file,
				       const char __user * userbuf,
				       size_t count, loff_t * off)
{
...
{
	return (single_open
		(file, read_proc_8021x_auth_port_mode, PDE_DATA(inode)));
}

static ssize_t dot1x_mode_single_write(struct file *file,
				       const char __user * userbuf,
				       size_t count, loff_t * off)
{
...
{
	return (single_open
		(file, read_proc_8021x_auth_server_port, PDE_DATA(inode)));
}

static ssize_t dot1x_server_port_single_write(struct file *file,
					      const char __user * userbuf,
					      size_t count, loff_t * off)
{
...
__IRAM_GEN inline static uint32 rtl_82dot1xHashEntry(uint8 * mac_addr)
{
...
int rtl_82dot1xAddEntry(uint8 * mac_addr,
			uint32 id, uint8 rx_port_num, uint8 auth_state)
{
	uint32 hash = 0;
...
static inline rtl802Dot1xCacheEntry *rtl_82dot1xFindEntryByMac(uint8 * mac_addr)
{
	...
	CTAILQ_FOREACH(entry, &table_dot1x->list[hash], cache_link) {
		if ((!memcmp(mac_addr, entry->mac_addr, ETH_ALEN))
		    && (entry->valid == DOT1X_TABLE_ENTRY_VALID)) {
	...
	//(dot1x_config.mode[pid].port_enable))
	{
		...
		CTAILQ_FOREACH(ep, &dot1x_list_inuse, cache_link) {
			if (ep->rx_port_num == pid) {
				...
				CTAILQ_REMOVE(&table_dot1x->list[hash], ep,
					      cache_link);
				CTAILQ_REMOVE(&dot1x_list_inuse, ep, tqe_link);
				CTAILQ_INSERT_TAIL(&dot1x_list_free, ep,
						   tqe_link);
				if (dot1x_config.type ==
				    DOT1X_AUTH_TYPE_MAC_BASED) {
					rtl865x_updateAuthState(0,
								ep->mac_addr);
				}
			}
		}
...
int rtl_82dot1xEnQueue(unsigned long task_priv, rtl802Dot1xQueue * q,
		       unsigned char *item, int item_size)
{
#if 0				//def __KERNEL__
	...
	memset(q->item_array[q->tail].item, 0,
	       sizeof(q->item_array[q->tail].item));
...
int rtl_82dot1xDeQueue(unsigned long task_priv, rtl802Dot1xQueue * q,
		       unsigned char *item, int *itemsize)
{
#if 0				//def __KERNEL__
	struct dev_priv *priv = (struct dev_priv *)task_priv;
#endif
	...
	//SAVE_INT_AND_CLI(flags);
	local_irq_save(flags);
	memcpy(item, &q->item_array[q->head].item,
	       q->item_array[q->head].item_size);
	q->item_num--;
...
int rtl_802dot1xIoctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	case RTL8651_IOCTL_DOT1X_GET_INFO:
		pdata = (uint32 *) args[3];
		rtl_82dot1xDeQueue((unsigned long)priv, &dot1x_queue,
				   queue_data, &queue_data_len);
		if (copy_to_user
		    ((void *)pdata, (void *)queue_data, queue_data_len)) {
			return -EFAULT;
		}
		break;
	...
	case RTL8651_IOCTL_DOT1X_SET_AUTH_RESULT:
		...
		if (copy_from_user
		    ((void *)&auth_result, (void *)pdata,
		     sizeof(rtl802Dot1xAuthResult))) {
			return -EFAULT;
		}
		//set to hw
#if defined(DOT1X_DEBUG)
		panic_printk
		    ("%s %d args=0x%x portnum=%d state=%d type=%d mac:0x%x:%x:%x:%x:%x:%x\n",
		     __FUNCTION__, __LINE__, args[1], auth_result.port_num,
		     auth_result.auth_state, auth_result.type,
		     auth_result.mac_addr[0], auth_result.mac_addr[1],
		     auth_result.mac_addr[2], auth_result.mac_addr[3]
		     , auth_result.mac_addr[4], auth_result.mac_addr[5]);
#endif
		if (auth_result.type == DOT1X_AUTH_TYPE_PORT_BASED)
			rtl_802dot1xSetPortAuthState(auth_result.port_num,
						     auth_result.auth_state);
		else if (auth_result.type == DOT1X_AUTH_TYPE_MAC_BASED)
			rtl865x_updateAuthState(auth_result.auth_state,
						auth_result.mac_addr);
		break;
#if 0
	case RTL8651_IOCTL_DOT1X_GET_PORT_STATE_INFO:
		pdata = (uint32 *) args[1];
		if (copy_to_user
		    (pdata, (void *)&port_state,
		     sizeof(rtl802Dot1xPortStateInfo))) {
			return -EFAULT;
		}
		break;
#endif
	...
	if (new_link_port_mask != old_link_port_mask) {
		...
		for (port_num = 0; port_num <= RTL8651_PHY_NUMBER; port_num++) {
			...
			if ((((1 << port_num) & new_link_port_mask) == 0) &&
				...
				if ((dot1x_config.mode[port_num].auth_mode ==
				     DOT1X_AUTH_MODE_SNOOPING)
				    && (dot1x_config.
					mode[port_num].port_enable)) {
					rtl_82dot1xdelEntryByPid(port_num);
				}
				port_down_state.port_mask |= (1 << port_num);
			} else if ((((1 << port_num) & new_link_port_mask) != 0)
				   && (((1 << port_num) & old_link_port_mask) ==
				       0)) {
				//down -->up
...
static int write_proc_8021x_mac_based_ctrl(struct file *file,
					   const char *buffer,
					   unsigned long count, void *data)
{
	return 0;
...
static int write_proc_8021x_port_based_ctrl(struct file *file,
					    const char *buffer,
					    unsigned long count, void *data)
{
...
static int32 read_proc_8021x_entry(struct seq_file *s, void *v)
{
	...
	CTAILQ_FOREACH(ep, &dot1x_list_inuse, cache_link) {
		if ((ep->valid == DOT1X_TABLE_ENTRY_VALID)) {
			seq_printf(s,
				   "portnum:%d mac:0x%x:%x:%x:%x:%x:%x authstate:%d id:%d\n",
				   ep->rx_port_num, ep->mac_addr[0],
				   ep->mac_addr[1]
				   , ep->mac_addr[2], ep->mac_addr[3],
				   ep->mac_addr[4], ep->mac_addr[5],
				   ep->auth_state, ep->id);
		}
	}
...
static int32 read_proc_8021x_auth_server_port(struct seq_file *s, void *v)
{
	seq_printf(s, "%s %d\n", "802dot1x server port:",
		   dot1x_config.server_port);

	return 0;
}

#else
static int32 read_proc_8021x_entry(char *page, char **start, off_t off,
				   int count, int *eof, void *data)
{
	int len = 0;
...
static int32 read_proc_8021x_auth_server_port(char *page, char **start,
					       ...
					       unsigned long len, void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if ((num != 1) || (port > RTL8651_PORT_NUMBER || port < 0)) {
			printk
			    ("rtl ethernet 8021x set server port parameter error!!\n");
			return len;
		}
...
static int32 read_proc_8021x_auth_port_mode(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < RTL8651_PORT_NUMBER; i++) {
		seq_printf(s, "%s %d %s %d %s %d %s %d\n",
			   "mode:", dot1x_config.mode[i].auth_mode, "portnum:",
			   i, "enable:", dot1x_config.mode[i].port_enable,
...
#else
static int32 read_proc_8021x_auth_port_mode(char *page, char **start, off_t off,
					    int count, int *eof, void *data)
{
	int len, i;
	...
	for (i = 0; i < RTL8651_PORT_NUMBER; i++) {
		len += sprintf(page + len, "%s %d %s %d %s %d %s %d\n",
			       "mode:", dot1x_config.mode[i].auth_mode,
			       "portnum:", i, "enable:",
			       dot1x_config.mode[i].port_enable, "dir",
			       dot1x_config.mode[i].auth_dir);
	}
					     ...
					     unsigned long len, void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		num =
		    sscanf(tmpbuf, "%d %d %d %d", &enable, &portnum, &mode,
			   &dir);
		...
		if ((mode == DOT1X_AUTH_MODE_SNOOPING)
		    || (mode == DOT1X_AUTH_MODE_PROXY)) {
			if (dot1x_config.type == DOT1X_AUTH_TYPE_PORT_BASED) {
				rtl_enablePortbase802dot1x(enable, portnum);
				rtl_setPortbase802dot1xDir(dir, portnum);
			} else if (dot1x_config.type ==
				   DOT1X_AUTH_TYPE_MAC_BASED) {
...
static int32 read_proc_8021x_enable(struct seq_file *s, void *v)
{
	seq_printf(s, "%s %d %s %d\n", "802dot1x enable:", dot1x_config.enable,
		   "unicast response enable", dot1x_config.enable_unicastresp);

	return 0;
}

#else
static int32 read_proc_8021x_type(char *page, char **start, off_t off,
				  int count, int *eof, void *data)
{
	int len;
	len =
...
static int32 read_proc_8021x_enable(char *page, char **start, off_t off,
				    int count, int *eof, void *data)
{
	int len;
	len =
	    sprintf(page, "%s %d %s %d\n", "802dot1x enable:",
		    dot1x_config.enable, "unicast response enable",
...
int32 rtl_enablePortbase802dot1x(int32 enable, int32 portnum)
{
	...
	if (enable)
		WRITE_MEM32(DOT1XPORTCR,
			    (READ_MEM32(DOT1XPORTCR) | (0x1 << (portnum * 3))));
	else
		WRITE_MEM32(DOT1XPORTCR,
			    (READ_MEM32(DOT1XPORTCR) &
			     (~(0x1 << (portnum * 3)))));
...
int32 rtl_enableMacbase802dot1x(int32 enable, int32 portnum)
{
	...
	if (enable)
		WRITE_MEM32(DOT1XMACCR,
			    (READ_MEM32(DOT1XMACCR) | (0x1 << portnum)));
	else
		WRITE_MEM32(DOT1XMACCR,
			    (READ_MEM32(DOT1XMACCR) & (~(0x1 << portnum))));
...
int32 enable_portbase_802dot1x(void)
{
	...
	rtl865x_layer2_reinit();
#endif
	memset(&dot1xPortBasedInfo, 0x00,
	       RTL8651_PORT_NUMBER * sizeof(rtl802Dot1xPortBasedInfo));

	/* enable each Port-based Access Control and in a state of  unauthorized */
	for (i = 0; i < RTL8651_PORT_NUMBER; i++) {
		WRITE_MEM32(DOT1XPORTCR,
			    (READ_MEM32(DOT1XPORTCR) | (0x1 << (i * 3))));
		dot1xPortBasedInfo[i].portAuthEnable = 1;
		init_timer(&dot1xPortBasedInfo[i].expire_timer);
		dot1xPortBasedInfo[i].expire_timer.data =
		    (unsigned long)&dot1xPortBasedInfo[i];
		dot1xPortBasedInfo[i].expire_timer.function = rtl_802dot1x_portbase_timeout;	/* expire function      */
		dot1xPortBasedInfo[i].expire_timer.expires =
		    jiffies + DOT1X_AUTH_TIME_OUT;
...
int rtl_initDot1xtable(int dot1x_tbl_list_max, int dot1x_tbl_entry_max)
{
	...
	table_dot1x =
	    (rtl802Dot1xCacheTable *) kmalloc(sizeof(rtl802Dot1xCacheTable),
					      GFP_ATOMIC);
	if (table_dot1x == NULL) {
	...
	table_dot1x->list =
	    (struct __rtl802Dot1xCacheListEntryHead *)kmalloc(dot1x_tbl_list_max
							      *
							      sizeof(struct
								     __rtl802Dot1xCacheListEntryHead),
							      GFP_ATOMIC);
	...
	for (i = 0; i < dot1x_tbl_entry_max; i++) {
		rtl802Dot1xCacheEntry *entry = (rtl802Dot1xCacheEntry *)
		    kmalloc(sizeof(rtl802Dot1xCacheEntry), GFP_ATOMIC);
		if (entry == NULL) {
...
int rtl_init802dot1x(void)
{
...
#if defined(CONFIG_RTL_PROC_NEW)
	proc_create_data("enable", 0644, root_8021x, &dot1x_enable_proc_fops,
			 NULL);
	...
	proc_create_data("server_port", 0644, root_8021x,
			 &dot1x_server_port_proc_fops, NULL);

	proc_create_data("entry", 0644, root_8021x, &dot1x_entry_proc_fops,
			 NULL);

#else
	if ((res_stats = create_proc_read_entry("enable", 0644, root_8021x,
						read_proc_8021x_enable,
						NULL)) == NULL) {
	...
	if ((res_stats = create_proc_read_entry("mode", 0644, root_8021x,
						read_proc_8021x_auth_port_mode,
						NULL)) == NULL) {
		printk("create_proc_read_entry failed!\n");
	}
	...
	if ((res_stats = create_proc_read_entry("server_port", 0644, root_8021x,
						read_proc_8021x_auth_server_port,
						NULL)) == NULL) {
		printk("create_proc_read_entry failed!\n");
	}
...
#if 0
	if ((res_stats =
	     create_proc_read_entry("rtl_8021x_port_based_ctrl", 0644,
				    root_8021x, read_proc_8021x_port_based_ctrl,
				    NULL)) == NULL) {
		printk("create_proc_read_entry failed!\n");
	}
	...
	if ((res_stats =
	     create_proc_read_entry("rtl_8021x_mac_based_ctrl", 0644,
				    root_8021x, read_proc_8021x_mac_based_ctrl,
				    NULL)) == NULL) {
		printk("create_proc_read_entry failed!\n");
	}
	...
	if ((res_stats =
	     create_proc_read_entry("rtl_8021x_l2_mac_entry", 0644, root_8021x,
				    read_proc_8021x_l2_mac_entry,
				    NULL)) == NULL) {
		printk("create_proc_read_entry failed!\n");
	}
...
static inline void rtl_802dot1xRelayTrapped(struct sk_buff *skb,
					    rtl_nicRx_info * info, uint32 id)
{
	rtl_nicTx_info nic_tx;
	int vid = info->vid;
	int pid = info->pid;
	uint32 forward_portmsk =
	...
	} else			//multicast/broadcast relay
	{
		if (pid != dot1x_config.server_port) {
			...
			rtl_direct_txInfo((1 << dot1x_config.server_port),
					  &nic_tx);
#if defined(DOT1X_DEBUG)
			panic_printk
			    ("%s %d forward_portmsk=0x%x (1<<dot1x_config.server_port)=0x%x pid=%d id=%u\n",
			     __FUNCTION__, __LINE__, forward_portmsk,
			     (1 << dot1x_config.server_port), pid, id);
#endif
		} else {
#if 0
			...
			panic_printk
			    ("%s %d forward_portmsk=0x%x (1<<dot1x_config.server_port)=0x%x pid=%d\n",
			     __FUNCTION__, __LINE__, forward_portmsk,
			     (1 << dot1x_config.server_port), pid);
#else
...
#if defined(DOT1X_DEBUG)
				panic_printk
				    ("%s %d entry->rx_port_num=%d pid=%d id=%u\n",
				     __FUNCTION__, __LINE__, entry->rx_port_num,
				     pid, id);
#endif
			}
			rtl_direct_txInfo(forward_portmsk, &nic_tx);
	...
	if (skb != NULL) {
		...
		if (RTL_swNic_send((void *)skb, skb->data, skb->len, &nic_tx) <
		    0) {
...
#if defined(DOT1X_DEBUG)
	panic_printk
	    ("%s %d pkttype=%d pid=%d id=%d srcmac=0x%x:%x:%x:%x:%x:%x\n",
	     __FUNCTION__, __LINE__, pkttype, pid, id, skb->data[6],
	     skb->data[7], skb->data[8], skb->data[9], skb->data[10],
	     skb->data[11]);
#endif
	if (pkttype == FAILED)
		return FAILED;

	if ((pkttype == DOT1X_EAPOL_TYPE_START)
	    || (pkttype == DOT1X_EAPOL_TYPE_EAP_RESPONSE)) {
		//add supplicant's mac to cache
		if (rtl_82dot1xAddEntry
		    (&skb->data[ETH_ALEN], id, pid,
		     DOT1X_AUTH_STATE_START) != SUCCESS) {
#if defined(DOT1X_DEBUG)
		...
		if (rtl_82dot1xModifyEntryById
		    (id, DOT1X_AUTH_STATE_SUCCESS, supplicant_mac,
		     &portnum) == SUCCESS) {
			if (dot1x_config.type == DOT1X_AUTH_TYPE_PORT_BASED) {
		...
		if (rtl_82dot1xModifyEntryByMac
		    (&skb->data[ETH_ALEN], id, pid,
		     DOT1X_AUTH_STATE_PROCESSING) != SUCCESS) {
			panic_printk
			    ("%s %d rtl_82dot1xModifyEntryByMac error!!!\n",
			     __FUNCTION__, __LINE__);
		}
	}
#endif
...
__MIPS16 int rtl_802dot1xRxHandle(struct sk_buff *skb, rtl_nicRx_info * info)
{
	int ret = 0;
	...
	if ((dot1x_config.mode[info->pid].port_enable) &&
		...
		ret = rtl_802dot1xProxyModeRxHandle(skb, info);
		   ...
		   (dot1x_config.mode[info->pid].auth_mode ==
		    DOT1X_AUTH_MODE_CLIENT)) {
	...
	default:
		printk("%s %d unknown dot1x auth mode!!", __FUNCTION__,
		       __LINE__);
		return SUCCESS;
...
void rtl_802dot1xFilltxInfo(struct sk_buff *skb, rtl_nicTx_info * info)
{
	...
	panic_printk("%s %d pad=0x%x portnum=%d portlist=0x%x skb->len=%d\n",
		     __FUNCTION__, __LINE__, pad, portnum, info->portlist,
		     skb->len);
#endif
