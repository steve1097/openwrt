...
extern int rtl_register_multi_wan_dev(char *ifname, int proto, int vid,
				      int napt);
extern int rtl_update_port_mapping_multi_wan_dev(char *ifname,
						 unsigned int member);
...
__IRAM int smux_dev_hard_start_xmit(struct sk_buff *skb,
...
static inline void smux_mirror_pkt(struct sk_buff *skb,
				   const struct smux_dev_info *dev_info,
				   const int flag);
...
static inline struct smux_dev_info *list_entry_smuxdev(const struct list_head
						       *le)
{
 ...
 ***************************************************************************/
static struct net_device *__find_smux_in_smux_group(struct smux_group *smux_grp,
						    const char *ifname)
{
	struct list_head *lh;
	struct smux_dev_info *sdev = NULL;
...
//      struct vlan_hdr *vhdr=NULL;
	...
	if ((skb->dev->priv_flags & IFF_VSMUX)
 ...
 */
static inline int smux_check_mac(struct smux_dev_info *dev_info,
				 struct sk_buff *skb)
{
 ...
 *	1: smux device accept this packet
 */
static inline int smux_check_vlan(struct smux_dev_info *dev_info,
				  struct sk_buff *skb)
{
	struct net_device *vdev;
	...
	if (dstAddr[0] & 1) {
		...
		list_for_each(lh, &grp->virtual_devs) {
			...
			if (((dev_info->proto == SMUX_PROTO_PPPOE)
			     && (protocol != htons(ETH_P_PPP_DISC))
			     && (protocol != htons(ETH_P_PPP_SES)))
			    || ((dev_info->proto == SMUX_PROTO_IPOE)
				&& ((protocol == htons(ETH_P_PPP_DISC))
				    || (protocol == htons(ETH_P_PPP_SES))))) {
				DPRINTK
				    ("TRACE %d: packet dropped on RX dev %s\n",
				     __LINE__, vdev->name);
				continue;
			}
#else
			if ((dev_info->proto == SMUX_PROTO_IPOE)) {
				...
				if ((dstAddr[0] == 0x01) && (dstAddr[1] == 0x00)
				    && (dstAddr[2] == 0x5e)) {
...
#endif
				{
					if ((protocol == htons(ETH_P_PPP_DISC))
					    || (protocol ==
						htons(ETH_P_PPP_SES))) {
						DPRINTK
						    ("TRACE %d: packet dropped on RX dev %s\n",
...
__IRAM int smux_dev_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (dev_info->vid == -1) {
		...
		if (dev_info->m_1p == 0)
			...
			skb->vlan_tci =
			    (dev_info->vid & VLAN_VID_MASK) | ((dev_info->m_1p -
								1) << 13);
	}
...
int smux_dev_ioctl(struct net_device *vdev, struct ifreq *ifr, int cmd)
{
	...
	extern int set_port_mapping(struct net_device *dev, struct ifreq *rq,
				    int cmd);
#endif
	...
	case SIOCSMIIREG:
		...
		if (real_dev->netdev_ops->ndo_do_ioctl
			...
			err =
			    real_dev->netdev_ops->ndo_do_ioctl(real_dev, &ifrr,
							       cmd);
		break;
	...
	case SIOCSITFGROUP:
		{
			...
			printk
			    ("%s %d set portmapping  (dev:%s  member:0x%x) \n",
...
#endif
#if defined(CONFIG_RTL_HARDWARE_NAT)
			rtl_update_port_mapping_multi_wan_dev(vdev->name,
							      dev_info->member);
#endif
		}
...
int smux_dev_change_mtu(struct net_device *vdev, int new_mtu)
{
	...
	SMUX_DEV_INFO(vdev)->smux_grp->real_dev->
	    netdev_ops->ndo_change_mtu(vdev, new_mtu);
...
static struct net_device *smux_register_device(const char *rifname,
					       const char *nifname,
					       int smux_proto, int vid,
					       int napt, int brpppoe,
					       int enable, int on_phy)
#else
static struct net_device *smux_register_device(const char *rifname,
					       const char *nifname,
					       int smux_proto, int vid,
...
#endif
{
	...
	new_dev =
	    alloc_netdev(sizeof(struct smux_dev_info), nifname, smux_setup);
	if (new_dev == NULL) {
	...
	if (grp && new_dev) {
		...
		if (smux_proto == SMUX_PROTO_BRIDGE) {
			memcpy(new_dev->dev_addr, "\xFE\xFF\xFF\xFF\xFF\xFF",
			       ETH_ALEN);
		} else {
#ifdef UNIQUE_MAC_PER_DEV
			if (list_empty(&grp->virtual_devs)) {
				memcpy(new_dev->dev_addr, real_dev->dev_addr,
				       ETH_ALEN);
			} else {
				list_for_each(lh, &grp->virtual_devs) {
		...
		if ((ret_dev = __find_smux_in_smux_group(grp, nifname)) == NULL) {
			...
			if ((vid == -1) && (smux_proto != SMUX_PROTO_BRIDGE)) {
				if ((vid = allocSmuxDevVid()) == -1)
					printk
					    ("fatal error, too many wan interface created.\n");
			}
#endif
			if (vid != -1)
			...
			if (new_dev) {
				if (smux_proto != SMUX_PROTO_BRIDGE && brpppoe) {
					...
					rtl_register_multi_wan_dev
					    (vdev_info->vdev->name, smux_proto,
					     vid, napt);
 ...
 * Returns      : 0 on Success
 ***************************************************************************/
static int smux_device_event(struct notifier_block *unused, unsigned long event,
			     void *ptr)
{
	...
	case NETDEV_DOWN:
		...
		list_for_each(lh, &grp->virtual_devs) {
			...
			if (dev_info) {
				...
				dev_change_flags(dev_info->vdev,
						 flgs & ~IFF_UP);
			}
		}
		read_unlock(&smux_lock);
...
static int smux_ioctl_handler(void __user * arg)
{
	...
	case ADD_SMUX_CMD:
		if (!capable(CAP_NET_ADMIN))
			return -EPERM;
#ifdef CONFIG_RTL_IGMP_PROXY_MULTIWAN
		if (smux_register_device
		    (args.rsmux_ifname, args.osmux_ifname, args.proto, args.vid,
		     args.napt, args.brpppoe, args.igmpproxy_enable,
		     args.on_phy))
#else
		if (smux_register_device
		    (args.rsmux_ifname, args.osmux_ifname, args.proto, args.vid,
		     args.napt, args.brpppoe))
#endif
		{
			err = 0;
		} else {
...
static ssize_t vlan_passthru_single_write(struct file *file,
...
{
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("vlan_passthru", 0, &proc_root,
			 &vlan_passthru_proc_fops, NULL);
#else
...
int rtl_smux_downstream_port_mapping_check(struct sk_buff *skb)
{
	...
	if (member & (1 << port)) {
		printk
		    ("%s %d from %s to %s member0x%x  member & (1<<port)=true \n",
