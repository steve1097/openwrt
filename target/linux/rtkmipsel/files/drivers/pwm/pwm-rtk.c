...
static inline void rtk_pwm_set(unsigned long val, struct rtk_pwm_chip *chip,
			  ...
			  int duty_ns, int period_ns)
{
	...
	if (duty_ns > period_ns) {
		duty_ns = period_ns;
		printk
		    ("User set duty > period. Set duty = period, we suggest to set again\n");
	}
	...
	if (duty_cycles < PWMDCR_MIN_DUTY) {
		...
		printk
		    ("RTK gtimer/PWM time resolution is 60000ns/60us. Set duty to 60us, we suggest to set again\n");
	}
	if (duty_cycles > PWMDCR_MAX_DUTY) {
		duty_cycles = PWMDCR_MAX_DUTY;
		printk
		    ("RTK MAX gtimer/PWM time setting is 120ms. Set duty to 120ms, we suggest to set again\n");
	}
	...
	if (period_cycles < PWMPCR_MIN_PERIOD) {
		...
		printk
		    ("RTK gtimer/PWM time resolution is 60000ns/60us. Set period to 60us, we suggest to set again\n");
	}
	if (period_cycles > PWMPCR_MAX_PERIOD) {
		period_cycles = PWMPCR_MAX_PERIOD;
		printk
		    ("RTK MAX gtimer/PWM time setting is 120ms. Set period to 120ms, we suggest to set again\n");
	}
