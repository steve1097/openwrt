...
#define PIO_RAW_CMD(cmd, data, w_io_len, r_io_len) \
    __spi_nand_pio_cmd((cmd
##_OP << 24), cmd##_FUNC, data, w_io_len, r_io_len, 0, 0)
			  
#define PIO_WRITE_READ_DATA(cmd, col_addr, addr_io, data_io, wr_bytes, wr_buf) \
			__spi_nand_pio_cmd((cmd
##_OP << 24), cmd##_FUNC, col_addr, addr_io, data_io, wr_bytes, wr_buf)
					    
#if 1				//JSW
					    typedef unsigned int
					    snffcn_ptr(unsigned int opcode,
						       unsigned int data,
					    ...
					    __spi_nand_pio_cmd(unsigned int
							       opcode,
							       snffcn_ptr
							       sffunc,
							       unsigned int
							       ...
							       wr_bytes,
							       void *wr_buf);
					    snffcn_ptr snffcn_pio_raw_cmd,
					    snffcn_pio_write_data,
					    snffcn_pio_read_data, snffcn_bbm,
					    snffcn_read_bbm_tbl;
...
/***********************************************
  ...
  ***********************************************/ 
					    ...
					    void program_execute(unsigned int
								  blk_pge_addr,
								  cmd_info_t *
								  snf_cmd_info);
					    void
					    page_data_read_to_cache_buf(unsigned
									int
									blk_pge_addr,
									cmd_info_t
									*
									snf_cmd_info);
					    void _pio_write(unsigned int
							     w_data,
							     unsigned int
							     w_io_len);
					    void _dma_write_read(unsigned int
								  dma_phy_addr,
								  unsigned int
								  dma_io_len,
								  unsigned int
								  wr_dir);
					    void dma_write_data(void *dma_addr,
								 unsigned int
								 dma_len,
								 unsigned int
								 blk_pge_addr,
								 cmd_info_t *
								 info);
					    void dma_read_data(void *dma_addr,
								unsigned int
								dma_len,
								unsigned int
								blk_pge_addr,
