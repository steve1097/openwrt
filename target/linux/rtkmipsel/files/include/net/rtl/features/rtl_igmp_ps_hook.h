...
void rtl_igmp_fdb_delete(struct net_bridge *br,
...
struct mfc_cache *rtl_ipmr_cache_find(struct mr_table *mrt, __be32 origin,
				       __be32 mcastgrp);
...
struct net_bridge_port *rtl_maybe_deliver(struct net_bridge_port *prev,
					  ...
					  void (*__packet_hook) (const struct
								 ...
								 * p,
								 struct sk_buff
								 * skb));
int rtl_deliver_clone(const struct net_bridge_port *prev, struct sk_buff *skb,
