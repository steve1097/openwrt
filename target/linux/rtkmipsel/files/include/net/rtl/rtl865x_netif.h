...
typedef struct _rtl865x_AclRule_s {
	union {
		...
		struct {
			...
			union {
				...
				struct {
					...
					uint32 _FOP:1, _FOM:1, _httpFilter:1,
					    _httpFilterM:1, _identSrcDstIp:1,
					    _identSrcDstIpM:1;
					union {
						uint8 _flag;
						struct {
							uint8 pend1:5, pend2:1, _DF:1,	//don't fragment flag
				...
				struct {
					...
					union {
						uint8 _flag;
						struct {
							uint8 _pend:2, _urg:1,	//urgent bit
							 _ack:1,	//ack bit
							 _psh:1,	//push bit
							 _rst:1,	//reset bit
							 _syn:1,	//sync bit
		...
		struct {
			...
			uint32 _INV:1, _ETY:1, _comb:1, _ip_tunnel:1,
			    _httpFilter:1, _httpFilterM:1, _identSrcDstIp:1,
			    _identSrcDstIpM:1;
	...
	uint32 ruleType_:5,
	    actionType_:4, pktOpApp_:3, priority_:3, direction_:2,
...
int32
rtl865x_config_callback_for_get_drv_netifName(int (*fun)
					      (const char *psName,
					       char *netifName));
