...
static inline void rtl_fb_update_entry(rtl_fb_entry * entry,
				       struct sk_buff *pskb,
...
__IRAM_FWD int32 rtl_fb_process_in_nic(struct sk_buff *pskb,
				       struct net_device *dev)
{
	rtl_fb_entry *entry;
	unsigned long flags;
...
static void rtl_fb_copy_to_user(rtl_fb_nl_entry * data, int cnt)
{
	...
	for (i = 0; i < RTL_FB_HASH_SIZE; i++) {
		hlist_for_each(pos, &fb_head.in_used_list[i]) {
			entry = (rtl_fb_entry *) pos;
			copy_to_user((void *)data->mac_addr,
				     (void *)entry->mac_addr, ETHER_ADDR_LEN);
			copy_to_user((void *)data->name,
				     (void
				      *)(((struct net_device
					   *)(entry->dev_matchKey))->name),
				     IFNAMSIZ);
	...
	case FB_CMD_SET_FWD:
		{
			...
			fb_paras.enable_fb_fwd =
			    fb_info.info.data.enable_fb_fwd;
			rtl_fb_flush();
			break;
		}
	case FB_CMD_SET_FILTER:
		{
			...
			fb_paras.enable_fb_filter =
			    fb_info.info.data.enable_fb_filter;
			break;
		}
	case FB_CMD_SET_ENTRY_NUM:
		{
			...
			if (fb_head.used_cnt > fb_info.info.data.entry_num) {
				for (i = fb_info.info.data.entry_num;
				     i < fb_head.used_cnt; i++) {
					entry =
					    (rtl_fb_entry *) fb_head.free_list.
					    first;
					hlist_del(&entry->hlist);
					rtl_fb_alloc_free(entry);
				}
				fb_info.info.data.entry_num = fb_head.used_cnt;
			} else if (fb_info.info.data.entry_num >
				   fb_paras.entry_num) {
				for (i = fb_paras.entry_num;
				     i < fb_info.info.data.entry_num; i++) {
			...
			tx_info.info.data.entry_num =
			    fb_info.info.data.entry_num;
			break;
		}
	case FB_CMD_GET_STATUS:
		{
			...
			tx_info.info.data.enable_fb_filter =
			    fb_paras.enable_fb_filter;
			tx_info.info.data.enable_fb_fwd =
			    fb_paras.enable_fb_fwd;
	...
	case FB_CMD_DUMP_ENTRYS:
		{
			...
			rtl_fb_copy_to_user((rtl_fb_nl_entry *) fb_info.info.
					    entry, fb_info.info.in_used);
...
#else
	nl_sk =
	    netlink_kernel_create(&init_net, NETLINK_RTK_FB, 0,
				  rtl_fb_netlink_receive, NULL, THIS_MODULE);
#endif

	if (!nl_sk) {
		printk(KERN_ERR
		       "RTK fast bridge can NOT create netlink socket.\n");
	}
	return 0;
}
