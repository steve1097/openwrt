...
//      int8 port_num;
	...
	if ((memcmp(skb->dev->name, RTL_PS_BR0_DEV_NAME, 3) == 0)
	    || (memcmp(skb->dev->name, RTL_PS_LAN_P0_DEV_NAME, 4) == 0)) {
		macAddr = (ether_addr_t *) (eth_hdr(skb)->h_source);
		found =
		    rtl_check_fdb_entry_check_exist(RTL_LAN_FID, macAddr,
						    FDB_DYNAMIC);
/*		printk("\nrecv packet from dev:%s\n", skb->dev->name);*/
		...
		if (found == SUCCESS) {
			...
			if (lan_restrict_tbl[port_num].enable == TRUE) {
				if ((lan_restrict_tbl[port_num].curr_num <
				     lan_restrict_tbl[port_num].max_num)) {
...
/*						printk("\nDROP:lan_restrict_tbl[%d] current number is %d\n", port_num, lan_restrict_tbl[port_num].curr_num);*/
						...
						memcpy(&(l2temp_entry.macAddr),
						       macAddr,
						       sizeof(ether_addr_t));
						rtl865x_addAuthFilterDatabaseEntryExtension
						    (fdbEntry.fid,
						     &l2temp_entry);
...
/*			printk(" \nnot found in hw table, src port is %d\n", skb->srcPort);*/
			if (lan_restrict_tbl[skb->srcPort].enable == TRUE) {
				...
				if (rtl_check_fdb_entry_check_srcBlock
				    (0, macAddr, &SrcBlk) == SUCCESS) {
...
/*					printk(" \nnot found ind hw and sw table\n");*/
					if ((lan_restrict_tbl
					     [skb->srcPort].curr_num <
					     lan_restrict_tbl[skb->srcPort].
					     max_num)) {
...
static int lan_restrict_set_singleport(uint8 portnum, int8 enable,
				       int32 max_num)
{
	int32 ret;
	if (enable == TRUE) {
...
static int lan_restrict_perport_setting(void)
{
	int i;
	for (i = 0; i < LAN_RESTRICT_PORT_NUMBER; i++) {
		lan_restrict_set_singleport(lan_restrict_tbl[i].port_num,
					    lan_restrict_tbl[i].enable,
					    lan_restrict_tbl[i].max_num);
	}
	return TRUE;
}
...
{
	if (lan_restrict_tbl[port].enable == TRUE) {
		if (lan_restrict_tbl[port].curr_num <
		    lan_restrict_tbl[port].max_num) {
	...
	if (buffer && !copy_from_user(tmpbuf, buffer, count)) {
		if (memcmp(strptr, "enable", strlen("enable")) == 0) {
			...
			while (entryPtr != NULL) {
				...
				if (portnumPtr == NULL) {
					printk
					    ("lan restrict setting format error1\n");
					break;
				}
				...
				if (enablePtr == NULL) {
					printk
					    ("lan restrict setting format error2\n");
					break;
				}
				if (_strncasecmp(enablePtr, "OFF", 3) == 0) {
					port_enable = FALSE;
				} else if (_strncasecmp(enablePtr, "ON", 2) ==
					   0) {
					port_enable = TRUE;
				} else {
					printk
					    ("lan restrict setting format error3\n");
					break;
				}
				...
				if (maxnumPtr == NULL) {
					printk
					    ("lan restrict setting format error4\n");
					break;
				}
				...
				/*
				   set Asic
				 */
				lan_restrict_set_singleport(port, port_enable,
							    maxnum);
			}
		}
	}
...
static int lan_restrict_read_proc(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < LAN_RESTRICT_PORT_NUMBER; i++) {
		...
		seq_printf(s, "%6s %6d %6d  ",
			   lan_restrict_tbl[i].enable ? "ON" : "OFF",
			   lan_restrict_tbl[i].max_num,
			   lan_restrict_tbl[i].curr_num);
		seq_printf(s, "\n");
	}
...
static ssize_t lan_restrict_single_write(struct file *file,
