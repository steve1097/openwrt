...
#include <linux/version.h>
enum LR_RESULT (*FastPath_hook11) (rtl_fp_napt_entry * fpNaptEntry,
...
enum LR_RESULT (*FastPath_hook2) (ipaddr_t ip, ipaddr_t mask, ipaddr_t gateway,
				  uint8 * ifname, enum RT_FLAGS flags) = NULL;
enum LR_RESULT (*FastPath_hook3) (ipaddr_t ip, ipaddr_t mask, ipaddr_t gateway,
				  uint8 * ifname, enum RT_FLAGS flags) = NULL;
...
int (*qosGetSkbMarkByNaptEntryHook) (rtl865x_napt_entry * naptEntry,
				     rtl865x_qos_mark * qosMark,
...
#if defined(CONFIG_RTL_HW_QOS_SUPPORT)
int rtl_qosGetSkbMarkByNaptEntryHook(rtl865x_napt_entry * naptEntry,
				     rtl865x_qos_mark * qosMark,
				     struct sk_buff *pskb)
{
	int ret = 0;
...
int32 rtl865x_handle_nat(struct nf_conn * ct, int act, struct sk_buff * skb)
{
	...
	proto =
	    (ct->tuplehash[0].tuple.dst.protonum ==
	     IPPROTO_TCP) ? RTL865X_PROTOCOL_TCP : RTL865X_PROTOCOL_UDP;
	...
	if (ct->status & IPS_SRC_NAT) {	/* outbound flow */
		...
		sp = (proto) ? ct->tuplehash[0].tuple.src.u.tcp.port : ct->
		    tuplehash[0].tuple.src.u.udp.port;
		dp = (proto) ? ct->tuplehash[0].tuple.dst.u.tcp.port : ct->
		    tuplehash[0].tuple.dst.u.udp.port;
		gp = (proto) ? ct->tuplehash[1].tuple.dst.u.tcp.port : ct->
		    tuplehash[1].tuple.dst.u.udp.port;
		...
		sp = (proto) ? ct->tuplehash[1].tuple.src.u.tcp.port : ct->
		    tuplehash[1].tuple.src.u.udp.port;
		dp = (proto) ? ct->tuplehash[1].tuple.dst.u.tcp.port : ct->
		    tuplehash[1].tuple.dst.u.udp.port;
		gp = (proto) ? ct->tuplehash[0].tuple.dst.u.tcp.port : ct->
		    tuplehash[0].tuple.dst.u.udp.port;
	} else
		return -1;
	...
	if (act == 2) {
		...
		if (NULL != skb) {
			...
			ret =
			    rtl_qosGetSkbMarkByNaptEntryHook(&rtl865xNaptEntry,
							     &rtl865xQosMark,
							     skb);
			...
			if (lanDev) {
				rtl865xPrio.downlinkPrio =
				    rtl_qosGetPriorityByMark(lanDev->name,
							     rtl865xQosMark.
							     downlinkMark);
			}
			if (wanDev) {
				rtl865xPrio.uplinkPrio =
				    rtl_qosGetPriorityByMark(wanDev->name,
							     rtl865xQosMark.
							     uplinkMark);
			}
	...
	DEBUGP
	    ("%s:%d:(%s): errno=%d\n %s (%u.%u.%u.%u:%u -> %u.%u.%u.%u:%u) g:(%u.%u.%u.%u:%u)\n",
	     __FUNCTION__, __LINE__, ((is_add) ? "add_nat" : "del_nat"), rc,
	     ((proto) ? "tcp" : "udp"), NIPQUAD(sip), sp, NIPQUAD(dip), dp,
	     NIPQUAD(gip), gp);
#endif
...
int rtl_hwnat_timer_update(struct nf_conn *ct)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
			expires =
			    udp_get_timeouts_by_state(UDP_CT_REPLIED,
						      (void *)ct, 1);
#else
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,0)
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
			expires =
			    udp_get_timeouts_by_state(UDP_CT_UNREPLIED,
						      (void *)ct, 1);
#else
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,0)
	...
	} else if (ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum ==
		   IPPROTO_TCP
		   && ct->proto.tcp.state < TCP_CONNTRACK_LAST_ACK) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
		expires =
		    tcp_get_timeouts_by_state(ct->proto.tcp.state, (void *)ct,
					      1);
#else
...
void rtl_delConnCache(struct nf_conn *ct)
{
	...
	if (ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip
		...
		rtlFpNaptEntry.intIp =
		    ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.u3.ip;
		rtlFpNaptEntry.intPort =
		    ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.u.all;
		rtlFpNaptEntry.extIp =
		    ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip;
		rtlFpNaptEntry.extPort =
		    ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all;
		rtlFpNaptEntry.remIp =
		    ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip;
		rtlFpNaptEntry.remPort =
	...
	if (((ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip
		...
		if (iph->protocol == IPPROTO_UDP) {
			if ((ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip
			    ...
			    if ((ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.
				 ip ==
				 ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u3.ip)
				&& ((sip & _br0_mask) != (_br0_ip & _br0_mask))
				&& ((dip & _br0_mask) == (_br0_ip & _br0_mask))
	...
	if ((ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip
		...
		if (iph->protocol == IPPROTO_UDP &&
		    ...
		    ((((sip & _br0_mask) == (_br0_ip & _br0_mask)) &&
		     ...
		     (((sip & _br0_mask) != (_br0_ip & _br0_mask)) &&
		      ((dip & _br0_mask) == (_br0_ip & _br0_mask)))) &&
		    (!IS_CLASSD_ADDR(ntohl(dip))) && ((routerTypeFlag == 1)
						      || (sip !=
							  (ct->tuplehash
			...
			if (!tcph->fin && !tcph->syn && !tcph->rst
				...
				if (time_after
				    (jiffies, smart_count_start_timer)) {
					smart_count_start_timer = jiffies + HZ;
					smart_count = 0;
				}
...
void rtl_fpAddConnCache(struct nf_conn *ct, struct sk_buff *skb)
{
	...
	if ((assured) || (TRUE == create_conn)) {
		if (ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum ==
		    IPPROTO_TCP) {
			protocol = NP_TCP;
...
static inline int32 rtl_addV6ConnCheck
    (struct nf_conn *ct, struct ipv6hdr *iph, struct tcphdr *tcph,
     struct sk_buff *skb) {
#ifdef CON_SESSION_SINGLE_DIRECTION
	int onlink = 0;
	int addr_chk = 0;
...
int rtl_AddV6ConnCache(struct nf_conn *ct, struct sk_buff *skb)
{
	...
	struct tcphdr *tcph =
	    (struct tcphdr *)(((unsigned char *)iph) + sizeof(struct ipv6hdr));
			     ...
			     unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(&gHwNatSetting, buffer, 8)) {
		if ((gHwNatSetting[0] == '-') && (gHwNatSetting[1] == '1')) {	/* hardware NAT disabled, operation mode = gateway */
			...
			rtl_get_extIp_router_timer_fn(0);
		} else if ((gHwNatSetting[0] == '8') && ((gHwNatSetting[1] == '5')	/* L2TP */
							 ||(gHwNatSetting[1] ==
							    '4') /* PPTP */ )) {
			ACCESS_ONCE(gHwNatEnabled) = 0;
		}
...
struct file_operations hw_nat_proc_fops = {
	.open = hw_nat_single_open,.write = hw_nat_single_write,.read =
...
struct file_operations sw_nat_proc_fops = {
	.open = sw_nat_single_open,.write = sw_nat_single_write,.read =
	    seq_read,.llseek = seq_lseek,.release = single_release,
};
#else
		...
		if (swconfig_vlan_enable)
			wanDev =
			    dev_get_by_name(&init_net,
					    RTL_SWCONFIG_DRV_PPP_NETIF_NAME);
		else
#endif
			wanDev =
			    dev_get_by_name(&init_net, RTL_DRV_PPP_NETIF_NAME);
#else
...
int rtl865x_localPublicRx(struct sk_buff *skb)
{
	...
	if (rxFromWan) {
		...
		if (pktInfo.fromLocalPublic == 1) {
			...
			{
				skb->pkt_type = PACKET_HOST;
				memcpy(eth_hdr(skb)->h_dest, skb->dev->dev_addr,
				       6);
#if 0
				dest = eth_hdr(skb)->h_dest;
				src = eth_hdr(skb)->h_source;
				printk
				    ("=========%s(%d),dst(%x:%x:%x:%x:%x:%x),src(%x:%x:%x:%x:%x:%x)\n",
				     __FUNCTION__, __LINE__, dest[0], dest[1],
				     dest[2], dest[3], dest[4], dest[5], src[0],
	...
	if (in_dev != NULL) {
		for (ifap = in_dev->ifa_list; ifap != NULL;
			...
			if (strcmp(dev->name, ifap->ifa_label) == 0) {
				if (strncmp(dev->name, "ppp", strlen("ppp")) ==
				    0)
...
struct file_operations qos_proc_fops = {
	.open = qos_single_open,.write = qos_single_write,.read =
	    seq_read,.llseek = seq_lseek,.release = single_release,
};
#else
...
#if defined(CONFIG_PROC_FS)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
	proc_qos = proc_create_data("qos", 0, &proc_root, &qos_proc_fops, NULL);
#else
...
extern bool tcp_in_window(const struct nf_conn *ct,
			  ...
			  const struct tcphdr *tcph, u_int8_t pf);

static inline struct nf_conn *rtl_resolve_normal_ct(struct net *net,
						    struct sk_buff *skb,
						    unsigned int dataoff,
						    u_int16_t l3num,
						    u_int8_t protonum,
						    struct nf_conntrack_l3proto
						    *l3proto, struct nf_conntrack_l4proto
						    *l4proto, int *set_reply, enum ip_conntrack_info
						    *ctinfo)
{
...
int syn_asic_arp(struct neighbour *n, int add)
{
...
#if defined(CONFIG_IPV6)&&defined(CONFIG_RTL_8198C)
	{
		...
		if (n->tbl && (n->tbl->family == AF_INET6)
			...
			rc_ipv6 =
			    add ? rtl8198c_addIpv6Arp(tmp_ip,
						      (void *)n->ha) :
			    rtl8198c_delIpv6Arp(tmp_ip);
	}
			    ...
			    const unsigned int *underflows)
{
	...
	xt_entry_foreach(iter, entry0, size) {
		ret =
		    rtl_check_for_masquerade_entry(iter, entry0, name, size,
						   valid_hooks, hook_entries,
						   newinfo);
	}
#else
		    ...
		    struct fib_info * fi)
{
...
#endif
	{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
		if (!ipDst
		    || (!ipv4_is_multicast(ipDst) && !ipv4_is_loopback(ipDst)
			&& (ipDst != 0xffffffff)))
#else
...
#endif
		{
			...
			/*2007-12-19 */
			printk
			    ("%s:%d:(%s): dst:%u.%u.%u.%u/%u, gw:%u.%u.%u.%u, dev: %s, errno=%d\n",
			     __FUNCTION__, __LINE__, "add_rt", NIPQUAD(ipDst),
			     cfg->fc_dst_len, NIPQUAD(ipGw),
			     dev_t ? dev_t : "null", rc);
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
		expires =
		    udp_get_timeouts_by_state(UDP_CT_UNREPLIED, (void *)cp, 0);
#else
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,0)
		expires = udp_get_timeouts_by_state(UDP_CT_UNREPLIED);
#else
...
*/
	if (cp->hw_acc) {
		rtl865xNaptEntry.protocol =
		    ((cp->protocol == IPPROTO_TCP) ? 1 : 0);
		...
		if (elapsed >= 0 && (cp->protocol == IPPROTO_UDP)) {
			...
			/*2007-12-19 */
			printk
			    ("%s:%d:(%s): expired time = %d\n %s (%u.%u.%u.%u:%u -> %u.%u.%u.%u:%u) g:(%u.%u.%u.%u:%u)\n",
			     __FUNCTION__, __LINE__, "poll_nat", timeval,
			     (cp->protocol == IPPROTO_TCP) ? "tcp" : "udp",
			     NIPQUAD(cp->daddr), cp->dport, NIPQUAD(cp->caddr),
			     cp->cport, NIPQUAD(cp->vaddr), cp->vport);
#endif
...
int32 rtl_ip_vs_conn_expire_check_delete(struct ip_vs_conn * cp)
{
	...
	if (cp->hw_acc) {
		...
		printk
		    ("%s:%d:(%s): errno=%d\n %s (%u.%u.%u.%u:%u -> %u.%u.%u.%u:%u) g:(%u.%u.%u.%u:%u)\n",
		     __FUNCTION__, __LINE__, "del_nat", rc,
		     (cp->protocol == IPPROTO_TCP) ? "tcp" : "udp",
		     NIPQUAD(cp->daddr.ip), cp->dport, NIPQUAD(cp->caddr.ip),
		     cp->cport, NIPQUAD(cp->vaddr.ip), cp->vport);
#endif
	}
...
#if	defined(CONFIG_RTL_HW_QOS_SUPPORT)
extern int32 rtl_qosGetSkbMarkByNaptEntry(rtl865x_napt_entry * naptEntry,
					  rtl865x_qos_mark * qosMark,
					  struct sk_buff *pskb);
#endif
				    ...
				    struct ip_vs_protocol *pp)
{
	 ...
	 */
#if 0
	if (!cp->hw_acc && !cp->app && cp->state == IP_VS_S_ESTABLISHED)
#else
...
#endif
	{
		...
		printk
		    ("%s:%d:(%s): errno=%d\n %s (%u.%u.%u.%u:%u -> %u.%u.%u.%u:%u) g:(%u.%u.%u.%u:%u)\n",
		     __FUNCTION__, __LINE__, "add_nat", rc, "tcp",
		     NIPQUAD(cp->daddr), cp->dport, NIPQUAD(cp->caddr),
		     cp->cport, NIPQUAD(cp->vaddr), cp->vport);
#endif
	}
				    ...
				    struct ip_vs_protocol *pp)
{
	...
	if (!cp->hw_acc && !cp->app) {
		...
		printk
		    ("%s:%d:(%s): errno=%d\n %s (%u.%u.%u.%u:%u -> %u.%u.%u.%u:%u) g:(%u.%u.%u.%u:%u)\n",
		     __FUNCTION__, __LINE__, "add_nat", rc, "udp",
		     NIPQUAD(cp->daddr), cp->dport, NIPQUAD(cp->caddr),
		     cp->cport, NIPQUAD(cp->vaddr), cp->vport);
#endif
	}
...
int32 rtl8198c_ipv6_router_add(struct rt6_info *rt)
{
	...
	if (tb6_id == RT6_TABLE_MAIN) {
		...
		if ((((ipDst.v6_addr32[0] == 0) && (ipDst.v6_addr32[1] == 0)
		      && (ipDst.v6_addr32[2] == 0) && (ipDst.v6_addr32[3] == 0))
...
#if 0
			printk
			    ("%s:%d:(%s): dst: 0x%8x%8x%8x%8x, fc_dst_len is %d, gw:  0x%8x%8x%8x%8x, dev: %s, errno=%d\n",
			     __FUNCTION__, __LINE__, "add_ipv6_rt",
			     ipDst.v6_addr32[0], ipDst.v6_addr32[1],
			     ipDst.v6_addr32[2], ipDst.v6_addr32[3], fc_dst_len,
			     ipGw.v6_addr32[0], ipGw.v6_addr32[1],
			     ipGw.v6_addr32[2], ipGw.v6_addr32[3],
			     dev_t ? dev_t : "null", rc);
#endif
		}
	}
					 ...
					 unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(tmpBuf, buffer, count)) {
		tmpBuf[count - 1] = 0;
		rtl_gc_overflow_timout =
		    simple_strtol((const char *)tmpBuf, NULL, 0);
...
struct file_operations gc_overflow_timout_proc_fops = {
	.open = gc_overflow_timout_single_open,.write =
	    gc_overflow_timout_single_write,.read = seq_read,.llseek =
					...
					int count, int *eof, void *data)
{
	...
	len =
	    sprintf(page, "rtl_gc_overflow_timout(%d), HZ(%d)\n",
		    rtl_gc_overflow_timout, HZ);
...
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
	proc_gc_overflow_timout =
	    proc_create_data("gc_overflow_timout", 0, &proc_root,
			     &gc_overflow_timout_proc_fops, NULL);
#else
	proc_gc_overflow_timout =
	    create_proc_entry("gc_overflow_timout", 0, NULL);
	if (proc_gc_overflow_timout) {
		proc_gc_overflow_timout->read_proc =
		    gc_overflow_timout_read_proc;
		proc_gc_overflow_timout->write_proc =
		    gc_overflow_timout_write_proc;
	}
#endif
}
#endif
...
static int print_log_read_proc(struct seq_file *s, void *v)
{
	...
	if (0 == strcmp(data, print_Mask_ID)) {
		...
		seq_printf(s,
			   "RTL_LogModuleMask(0x%x)\n\tbit means:\n\t NIC %d\n\t WIRELESS %d \n\t PROSTACK %d \n",
			   *(uint32 *) & RTL_LogModuleMask,
			   RTL_LogModuleMask.NIC, RTL_LogModuleMask.WIRELESS,
			   RTL_LogModuleMask.PROSTACK);
	}
...
struct file_operations print_log_proc_fops = {
	.open = print_log_single_open,.write = print_log_single_write,.read =
	    seq_read,.llseek = seq_lseek,.release = single_release,
};
#else
			       ...
			       int count, int *eof, void *data)
{
	...
	if (0 == strcmp(data, print_Mask_ID)) {
		len =
		    sprintf(page,
			    "RTL_LogTypeMask(0x%x)\n\tbit means:\n\t ERROR %d\n\t WARN %d \n\t INFO %d \n",
			    *(uint32 *) & RTL_LogTypeMask,
			    RTL_LogTypeMask.ERROR, RTL_LogTypeMask.WARN,
			    RTL_LogTypeMask.INFO);
	} else if (0 == strcmp(data, print_errMask_ID)) {
		len =
		    sprintf(page,
			    "RTL_LogErrorMask(0x%x)\n\tbit means:\n\t MEM %d\n\t SKB %d \n",
			    *(uint32 *) & RTL_LogErrorMask,
			    RTL_LogErrorMask.MEM, RTL_LogErrorMask.SKB);
	} else if (0 == strcmp(data, print_rateLimit_ID)) {
...
void log_print_proc_init(void)
{
	...
	if (proc_log_print_control) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
		proc_printMask =
		    proc_create_data(print_Mask_ID, 0, proc_log_print_control,
				     &print_log_proc_fops, print_Mask_ID);

		proc_printMask =
		    proc_create_data(print_errMask_ID, 0,
				     proc_log_print_control,
				     &print_log_proc_fops, print_errMask_ID);

		proc_printMask =
		    proc_create_data(print_rateLimit_ID, 0,
				     proc_log_print_control,
				     &print_log_proc_fops, print_rateLimit_ID);

		proc_printMask =
		    proc_create_data(print_module_ID, 0, proc_log_print_control,
				     &print_log_proc_fops, print_module_ID);
#else
		proc_printMask =
		    create_proc_entry(print_Mask_ID, 0, proc_log_print_control);
		if (proc_printMask) {
		...
		proc_errMask =
		    create_proc_entry(print_errMask_ID, 0,
				      proc_log_print_control);
		if (proc_errMask) {
		...
		proc_print_rateLimit =
		    create_proc_entry(print_rateLimit_ID, 0,
				      proc_log_print_control);
		...
		proc_printModule =
		    create_proc_entry(print_module_ID, 0,
				      proc_log_print_control);
		if (proc_printModule) {
...
int rtl_gc_threshold_check(struct net *net)
{
	...
	if (net == NULL) {
		if (atomic_read(&(init_net.ct.count)) >
...
void rtl_list_add_tail(struct nf_conn *ct, int proto, int flag)
{
	if (proto == PROT_UDP)
		...
		return list_add_tail(&ct->state_tuple,
				     Tcp_State_Hash_Head[ct->proto.tcp.state].
				     state_hash);
}
...
void rtl_list_move_tail(struct nf_conn *ct, int proto, int state)
{
	if (proto == PROT_UDP)
		...
		return list_move_tail(&ct->state_tuple,
				      Tcp_State_Hash_Head[(enum tcp_conntrack)
							  state].state_hash);
}
				   ...
				   int flag)
{
	...
	if (dir == IP_CT_DIR_ORIGINAL) {
		if (flag == 0)
			...
			return ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.
			    ip;
				     ...
				     int flag)
{
	...
	if (dir == IP_CT_DIR_ORIGINAL) {
		if (flag == 0)
			return ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.
			    all;
		else if (flag == 1)
			return ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.
			    all;
...
extern int32 rtl_fn_hash_insert_hooks(struct fib_table *tb,
...
static inline t_key rtl_tkey_extract_bits(t_key a, unsigned int offset,
					  unsigned int bits)
{
	if (offset < KEYLENGTH)
 ...
 * caller must hold RCU read lock or RTNL
 */
static inline struct rt_trie_node *rtl_tnode_get_child_rcu(const struct tnode
							   *tn, unsigned int i)
{
	BUG_ON(i >= 1U << tn->bits);
...
static struct leaf *rtl_leaf_walk_rcu(struct tnode *p, struct rt_trie_node *c)
{
	do {
		...
		if (c)
			idx =
			    rtl_tkey_extract_bits(c->key, p->pos, p->bits) + 1;
		else
			idx = 0;
	...
	hlist_for_each_entry_safe(li, tmp, lih, hlist) {
		list_for_each_entry_safe(fa, fa_node, &li->falh, fa_list) {
			...
			if (fi) {
				...
				cfg.fc_dst_len =
				    inet_mask_len(htonl(li->mask_plen));
...
int rtl_add_router_to_hw_l3_table(void)
{
	...
	if (tb) {
		...
		for (l = rtl_trie_firstleaf(t); l; l = rtl_trie_nextleaf(l)) {
			...
			hlist_for_each_entry_safe(li, tmp, lih, hlist) {
				list_for_each_entry_safe(fa, fa_node, &li->falh,
							 fa_list) {
...
static int rtl_add_wan_masq_info(void)
{
	...
	if ((wanDev = rtl865x_getWanDev()) && (wanDev->flags & IFF_UP)) {
		/*If wan dev changed, such as ppp0->eth1, we need to delete the old wan
		   dev eth1. */
		if ((wan_dev_name_bak[0] != 0)
		    && (strncmp(wan_dev_name_bak, wanDev->name, IFNAMSIZ))) {
		...
		if (rtl865x_getDevIpAndNetmask(wanDev, &ip, &mask) == SUCCESS) {
			...
			memcpy(wan_dev_name_bak, wanDev->name,
			       strlen(wanDev->name));
...
static int rtl_sync_ppp_info_to_hw_ppp_table(void)
{
	...
	char ppp_dev_name[MAX_IFNAMESIZE] = {
		0
	};
...
#if defined(CONFIG_SWCONFIG_SUPPORT_HW_NAT)
	if (swconfig_vlan_enable)
		strncpy(ppp_dev_name, RTL_SWCONFIG_DRV_PPP_NETIF_NAME,
			MAX_IFNAMESIZE);
	else
#endif
	...
	if (swconfig_vlan_enable) {
		if ((wan_dev =
		     __dev_get_by_name(&init_net,
				       hw_nat_wan_dev_name)) != NULL) {
			if ((wan_netif_addr =
			     rtl_get_netif_gmac(hw_nat_wan_dev_name))
			    && memcmp(wan_netif_addr, wan_dev->dev_addr,
				      ETHER_ADDR_LEN)) {
				memcpy(netif.macAddr.octet, wan_dev->dev_addr,
				       ETHER_ADDR_LEN);
				memcpy(netif.name, hw_nat_wan_dev_name,
				       MAX_IFNAMESIZE);
...
int rtl_check_pre_reserver_conn(struct nf_conntrack_tuple *tuple,
				...
				enum nf_nat_manip_type maniptype)
{
	...
	if ((gHwNatEnabled) && (maniptype == NF_NAT_MANIP_SRC) &&
		...
		rtl865xNaptEntry.intIp =
		    ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip;
		rtl865xNaptEntry.remIp =
		    ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip;
		rtl865xNaptEntry.extIp = tuple->src.u3.ip;
		rtl865xNaptEntry.intPort =
		    rtl865xNaptEntry.protocol ? ct->
		    tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.tcp.port : ct->
		    tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.udp.port;
		rtl865xNaptEntry.remPort =
		    rtl865xNaptEntry.protocol ? ct->
		    tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.tcp.port : ct->
		    tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port;
		rtl865xNaptEntry.extPort =
		    rtl865xNaptEntry.protocol ? tuple->src.u.tcp.port : tuple->
		    src.u.udp.port;

		rtl865x_getAsicNaptHashScore(&rtl865xNaptEntry,
					     &asicNaptHashScore);
		/*
...
int rtl_get_new_rover(__be16 * portptr, unsigned int min, u_int16_t off_old,
		      unsigned int range_size, u16 * rover,
		      struct nf_conntrack_tuple *tuple,
		      const struct nf_nat_range *range,
		      enum nf_nat_manip_type maniptype,
		      const struct nf_conn *ct)
{
	...
	if (ct->master || (ct->status & IPS_EXPECTED)) {
		...
		for (i = 0; i < range_size; i++) {
			if ((gHwNatEnabled) && (maniptype == NF_NAT_MANIP_SRC)) {
				rtl865x_optimizeExtPort(off++, range_size,
							&newOff);
			...
			if ((gHwNatEnabled) && (maniptype == NF_NAT_MANIP_SRC)) {
				...
				rtl865xNaptEntry.intPort =
				    ...
				    ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.
				    src.u.tcp.port : ct->
				    tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.
				    udp.port;
				rtl865xNaptEntry.remPort =
				    rtl865xNaptEntry.protocol ? ct->
				    tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.
				    tcp.port : ct->
				    tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.
				    udp.port;
				rtl865xNaptEntry.extPort =
				    rtl865xNaptEntry.protocol ? tuple->src.u.
				    tcp.port : tuple->src.u.udp.port;
...
static void rtl_get_extIp_router_timer_fn(unsigned long arg)
{
	...
	mod_timer(&hw_extIp_router_timer,
		  jiffies + RTL_GET_EXTIP_ROUTER_EXPIRE);
