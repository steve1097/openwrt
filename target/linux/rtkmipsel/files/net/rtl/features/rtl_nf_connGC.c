...
static void init_conntrack_state_hash(void)
{
	...
	for (i = 0;
	     i <
	     sizeof(Tcp_State_Hash_Head) / sizeof(struct tcp_state_hash_head);
	     i++) {
	...
	for (i = 0;
	     i <
	     sizeof(Udp_State_Hash_Head) / sizeof(struct udp_state_hash_head);
	     i++) {
	...
	drop_priority_max_idx =
	    sizeof(drop_priority) / sizeof(struct DROP_PRORITY);
	rtl_newGC_session_status_flags = RTL_FP_SESSION_LEVEL_IDLE;
	return;
}

int drop_one_conntrack(const struct nf_conntrack_tuple *orig,
		       const struct nf_conntrack_tuple *repl);
static inline void recalculate(void)
{
	int i;
	for (i = 0; i < PROT_MAX; i++) {
		atomic_set(&_prot_limit[i],
			   nf_conntrack_max * prot_limit[i] / 100);
	}
	...
	if (nf_conntrack_max
	    && unlikely(rtl_gc_threshold_check(NULL) == SUCCESS)) {
...
void clean_from_lists(void *ct, void *net)
{
	...
	if ((rtl_new_gc_get_ct_protonum(ct, IP_CT_DIR_ORIGINAL) == IPPROTO_TCP)
	    || (rtl_new_gc_get_ct_protonum(ct, IP_CT_DIR_ORIGINAL) ==
		IPPROTO_UDP)) {
		...
		if (nf_conntrack_max
		    && unlikely(rtl_gc_threshold_check(net) == SUCCESS)) {
			if (rtl_newGC_session_status_flags !=
			    RTL_FP_SESSION_LEVEL3) {
...
int32 rtl_connGC_addList(void *skb, void *ct)
{
	...
	case IPPROTO_TCP:
		if (Tcp_State_Hash_Head
		    [rtl_new_gc_get_ct_tcp_state(ct)].state_hash != NULL) {
...
void __nf_ct_refresh_acct_proto(void *ct,
				...
				int do_acct,
				unsigned char proto, void *extra1, void *extra2)
{
	...
	if (rtl_test_bit(ct, IPS_CONFIRMED_BIT) == FAILED) {
		...
		if (rtl_del_ct_timer(ct)) {
			rtl_new_gc_set_ct_timeout_expires(ct,
							  jiffies +
							  extra_jiffies);
			rtl_add_ct_timer(ct);
			event = IPCT_REFRESH;
			...
			case 6:
				...
				break;
			case 17:
				if (rtl_new_gc_get_ct_udp_status(ct) &
				    IPS_SEEN_REPLY) {
...
char __conntrack_drop_check(void *tmp)
{
	...
	if ((IS_CLASSD_ADDR(ntohl(dip))) ||
	    ...
	    ((sport < htons(1024)) || (dport < htons(1024))
	     || (sport2 < htons(1024)) || (dport2 < htons(1024)))
	    || ((sport == htons(8080)) || (dport == htons(8080))
		|| (sport2 == htons(8080)) || (dport2 == htons(8080)))) {
		return FAILED;
	} else {
		return SUCCESS;
	}
...
static inline int __drop_one_udp(void)
{
	...
	for (i = 0; i < drop_priority_max_idx; i++) {
		...
		head =
		    Udp_State_Hash_Head[drop_priority[i].state -
					UDP_UNREPLY].state_hash;
		if (!list_empty(head)) {
			struct list_head *nn = NULL;
			list_for_each_safe(ptr, nn, head) {
				tmp =
				    list_entry(ptr, struct nf_conn,
					       state_tuple);

				if (tmp->drop_flag == -1) {
					tmp->drop_flag =
					    __conntrack_drop_check(tmp);
				}
...
static inline int __drop_one_conntrack_process(struct list_head *head,
					       int dropPrioIdx, int factor,
					       int checkFlags, int tcpUdpState)
{
...
#if (HZ==100)
			if ((((rtl_get_ct_timer_expires(tmp) -
			       jiffies) >> (factor + 7)) <=
			     drop_priority[dropPrioIdx].threshold)
			    && rtl_del_ct_timer(tmp))
#elif (HZ==1000)
			if ((((rtl_get_ct_timer_expires(tmp) -
			       jiffies) >> (factor + 10)) <=
			     drop_priority[dropPrioIdx].threshold)
...
#error "Please Check the HZ defination."
			...
			{
				ret =
				    __drop_one_conntrack_process_hooks1(tmp,
									dropPrioIdx,
									factor,
									checkFlags,
									tcpUdpState);
				...
				__drop_one_conntrack_process_hooks2(tmp,
								    dropPrioIdx,
								    factor,
								    checkFlags,
								    tcpUdpState);
...
static inline int __drop_one_conntrack(int factor, int checkFlags)
{
	...
	for (i = 0; i < drop_priority_max_idx; i++) {
		...
		{
			head =
			    Tcp_State_Hash_Head[drop_priority[i].state].
			    state_hash;

			if (__drop_one_conntrack_process
			    (head, i, factor, checkFlags,
			     TCP_CONNTRACK_ESTABLISHED) == 1)
				return 1;
		...
		} else		//ramen--drop udp conntrack
		{
			...
			if (__drop_one_conntrack_process
			    (head, i, factor, checkFlags, UDP_ASSURED) == 1)
				return 1;
		}
	}
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
...
static inline int _isReservedIPAddr(unsigned int srcIP, unsigned int dstIP)
{
	if (IS_CLASSD_ADDR(ntohl(dstIP)) ||
	    IS_BROADCAST_ADDR(ntohl(dstIP)) || IS_ALLZERO_ADDR(ntohl(srcIP)))
		return 1;
		       ...
		       const struct nf_conntrack_tuple *repl)
{
	...
	if ((atomic_read(&_prot_limit[PROT_TCP]) <
	     atomic_read(&prot_counters[PROT_TCP])) && __drop_one_tcp()) {
	...
	if ((atomic_read(&_prot_limit[PROT_UDP]) <
	     atomic_read(&prot_counters[PROT_UDP])) && __drop_one_udp()) {
		if (rtl_newGC_session_status_flags == RTL_FP_SESSION_LEVEL_IDLE) {
			rtl_newGC_session_status_flags = RTL_FP_SESSION_LEVEL3;
			rtl_newGC_session_status_time =
			    jiffies + RTL_FP_SESSION_LEVEL3_INTERVAL;
	...
	if (__drop_one_conntrack(0) ||
	    __drop_one_conntrack(1) || __drop_one_conntrack(31))
		return 1;
	else
		return 0;
#endif
