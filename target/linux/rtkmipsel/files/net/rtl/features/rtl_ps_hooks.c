...
extern struct rtl865x_ReservedMCastRecord
    reservedMCastRecord[MAX_RESERVED_MULTICAST_NUM];
...
void (*update_dev_stats_hook) (struct nf_conn * ct, struct sk_buff * skb,
			       enum packet_dir packet_dir_t);
...
void rtl_update_dev_stats_hook(struct nf_conn *ct, struct sk_buff *skb,
			       enum packet_dir packet_dir_t)
{
...
enum LR_RESULT (*FastPath_hook5) (ipaddr_t ip, ether_addr_t * mac,
				  enum ARP_FLAGS flags) = NULL;
enum LR_RESULT (*FastPath_hook7) (ipaddr_t ip) = NULL;
enum LR_RESULT (*FastPath_hook8) (ipaddr_t ip, ether_addr_t * mac,
				  enum ARP_FLAGS flags) = NULL;
...
int32(*rtl_br_fdb_time_update_fphook) (void *br_dummy, void *fdb_dummy,
				       const unsigned char *addr) = NULL;
int32(*rtl_fp_dev_hard_start_xmit_check_fphook) (struct sk_buff * skb,
						 struct net_device * dev,
						 struct netdev_queue * txq) =
    NULL;
int32(*rtl_fp_dev_queue_xmit_check_fphook) (struct sk_buff * skb,
					    struct net_device * dev) = NULL;

void (*ppp_channel_pppoe_fphook) (struct ppp_channel * chan) = NULL;
int (*clear_pppoe_info_fphook) (char *ppp_dev, char *wan_dev,
				unsigned short sid, unsigned int our_ip,
				unsigned int peer_ip, unsigned char *our_mac,
				unsigned char *peer_mac) = NULL;
int (*set_pppoe_info_fphook) (char *ppp_dev, char *wan_dev, unsigned short sid,
    ...
    NULL;
unsigned long (*get_pppoe_last_rx_tx_fphook) (char *ppp_dev, char *wan_dev,
					      unsigned short sid,
					      unsigned int our_ip,
...
int32 rtl_fn_hash_insert_hooks(struct fib_table * tb, struct fib_config * cfg,
			       struct fib_info * fi)
{
#if defined( CONFIG_RTL_IPTABLES_FAST_PATH)
...
int32 rtl_fn_hash_replace_hooks(struct fib_table * tb, struct fib_config * cfg,
				struct fib_info * fi)
{
#if defined( CONFIG_RTL_IPTABLES_FAST_PATH)
#if defined(CONFIG_FAST_PATH_MODULE)
...
int32 rtl_dev_hard_start_xmit_hooks(struct sk_buff * skb,
				    struct net_device * dev,
				    struct netdev_queue * txq)
{
#if defined( CONFIG_RTL_IPTABLES_FAST_PATH)
...
static int filter_dos_wlan(struct sk_buff *skb)
{
	...
	if ((skb->dev) && (!strncmp(skb->dev->name, RTL_WLAN_NAME, 4)))	// wlan0, wlan1, wlan0-va0, ... and so on
	{
		if ((iph->protocol == IPPROTO_TCP) && ((*tflag & 0x3f) == TCP_SYN) && (iph->daddr == _br0_ip))	// xdos.exe 192.168.1.254 0-65535
		{
			...
			if ((wlan_block == 1)
			    &&
			    (memcmp(block_source_mac, &(skb->mac_header[6]), 6)
			     == 0)) {
...
#ifdef CONFIG_RTL8192CD
					issue_disassoc_from_kernel((void *)
								   skb->
								   dev->priv,
								   &(skb->
								     mac_header
...
int32 rtl_neigh_flush_dev_hooks(struct neigh_table * tbl,
...
int32 rtl_neigh_update_hooks(struct neighbour * neigh, const u8 * lladdr,
			     uint8 old)
{
#if defined(CONFIG_RTL_IPTABLES_FAST_PATH)
	if (old & NUD_VALID) {
...
int32 rtl_neigh_update_post_hooks(struct neighbour * neigh, const u8 * lladdr,
				  uint8 old)
{
#if defined(CONFIG_RTL_IPTABLES_FAST_PATH)
...
int32 rtl_neigh_periodic_timer_hooks(struct neighbour * n, unsigned int refresh)
{
	...
	if (n->tbl && (n->tbl->family == AF_INET6)
		...
		if (rtl8198c_ipv6ArpSync
		    (*((inv6_addr_t *) n->primary_key), refresh) > 0) {
			n->used = jiffies;
			n->dead = 0;
...
#endif
	{
...
#if defined(RTL_REFRESH_HW_L2_ENTRY_DECIDE_BY_HW_NAT)
		if (rtl865x_check_hw_nat_by_ip(htonl(*((u32 *) n->primary_key)))
		    == SUCCESS)
			tval =
			    rtl865x_arpSync(htonl(*((u32 *) n->primary_key)),
					    1);
		else
			tval =
			    rtl865x_arpSync(htonl(*((u32 *) n->primary_key)),
		...
		if (tval > 0)
#else
		if (rtl865x_arpSync(htonl(*((u32 *) n->primary_key)), refresh) >
		    0)
#endif
		{
			n->used = jiffies;
...
int32 rtl_neigh_timer_handler_pre_update_hooks(struct neighbour * neigh,
					       unsigned state)
{
...
#if defined(CONFIG_RTL_HARDWARE_NAT) && defined(CONFIG_RTL_LAYERED_DRIVER)  && defined(CONFIG_RTL_LAYERED_DRIVER_L3)
#if defined(RTL_REFRESH_HW_L2_ENTRY_DECIDE_BY_HW_NAT)
			if (rtl865x_check_hw_nat_by_ip
			    (htonl(*((u32 *) neigh->primary_key))) == SUCCESS)
				tval = rtl865x_arpSync(htonl(*((u32 *)
							       neigh->
							       primary_key)),
						       1);
			else
...
#if 0
			printk
			    ("%s:%d: ip:%u.%u.%u.%u, mac:%x:%x:%x:%x:%x:%x, tval is %d\n",
			     __FUNCTION__, __LINE__,
			     NIPQUAD(htonl(*((u32 *) neigh->primary_key))),
			     neigh->ha[0], neigh->ha[1], neigh->ha[2],
			     neigh->ha[3], neigh->ha[4], neigh->ha[5], tval);
#endif
#endif
		}
...
int32 rtl_neigh_timer_handler_during_update_hooks(struct neighbour * neigh,
						  unsigned state)
{
#if (defined(CONFIG_RTL_HARDWARE_NAT)&&defined(CONFIG_RTL_LAYERED_DRIVER) && defined(CONFIG_RTL_LAYERED_DRIVER_L3)) ||(defined(CONFIG_IPV6)&&defined(CONFIG_RTL_8198C))
...
int32 rtl_neigh_timer_handler_post_update_hooks(struct neighbour * neigh,
						unsigned state)
{
#if defined(CONFIG_RTL_IPTABLES_FAST_PATH)
...
int32 rtl___neigh_event_send_pre_hooks(struct neighbour * neigh,
				       struct sk_buff * skb)
{
#if defined(CONFIG_RTL_IPTABLES_FAST_PATH)
...
int32 rtl___neigh_event_send_post_hooks(struct neighbour * neigh,
					struct sk_buff * skb)
{
	return RTL_PS_HOOKS_CONTINUE;
}
...
extern int32 rtl_br_fdb_time_update_hook(void *br_dummy, void *fdb_dummy,
					 const unsigned char *addr);
int32 rtl___br_fdb_get_timeout_hooks(struct net_bridge *br,
				     ...
				     const unsigned char *addr)
{
	...
	if (rtl_br_fdb_time_update_hook((void *)br, (void *)fdb, addr) ==
	    FAILED) {
...
int32 rtl_masq_device_event_hooks(struct notifier_block * this,
...
int32 rtl_masq_inet_event_hooks(struct notifier_block * this,
...
int32 rtl_tcp_state_transition_hooks(struct ip_vs_conn * cp, int direction,
...
int32 rtl_udp_state_transition_hooks(struct ip_vs_conn * cp, int direction,
				     const struct sk_buff * skb,
				     struct ip_vs_protocol * pp)
{
#if defined(CONFIG_RTL_HARDWARE_NAT)
...
int32 rtl_dst_alloc_gc_post_check2_hooks(struct dst_ops * ops,
...
int32 __drop_one_conntrack_process_hooks1(struct nf_conn * ct, int dropPrioIdx,
					  int factor, int checkFlags,
					  int tcpUdpState)
{
#if defined(CONFIG_RTL_IPTABLES_FAST_PATH) || defined(CONFIG_RTL_HARDWARE_NAT)
	if (checkFlags == TRUE
...
int32 __drop_one_conntrack_process_hooks2(struct nf_conn * ct, int dropPrioIdx,
					  int factor, int checkFlags,
					  int tcpUdpState)
{
#if defined(CONFIG_RTL_IPTABLES_FAST_PATH) || defined(CONFIG_RTL_HARDWARE_NAT)
...
#if defined(CONFIG_BRIDGE)
int32 rtl_fdb_create_hooks(struct net_bridge_fdb_entry * fdb,
			   const unsigned char *addr)
{
...
int32 rtl_br_fdb_cleanup_hooks(struct net_bridge * br,
...
#if defined(CONFIG_RTL_LAYERED_DRIVER) && defined(CONFIG_RTL_LAYERED_DRIVER_L2) && defined(CONFIG_RTL865X_SYNC_L2)
		printk
		    ("time list:jiffies:%ld,hw_aging:%ld,f->ageing_timer:%ld\n",
...
int32 rtl_br_fdb_time_update_hook(void *br_dummy, void *fdb_dummy,
				  const unsigned char *addr)
{
			  ...
			  unsigned char *our_mac, unsigned char *peer_mac)
{
	if (clear_pppoe_info_fphook)
		return clear_pppoe_info_fphook(ppp_dev, wan_dev, sid, our_ip,
					       peer_ip, our_mac, peer_mac);
	else
		return 0;
}
			...
			unsigned char *our_mac, unsigned char *peer_mac)
{
	if (set_pppoe_info_fphook)
		return set_pppoe_info_fphook(ppp_dev, wan_dev, sid, our_ip,
					     peer_ip, our_mac, peer_mac);
	else
		return 0;
}
			      ...
			      unsigned long *last_rx, unsigned long *last_tx)
{
	if (get_pppoe_last_rx_tx_fphook)
		return get_pppoe_last_rx_tx_fphook(ppp_dev, wan_dev, sid,
						   our_ip, peer_ip, our_mac,
						   peer_mac, last_rx, last_tx);
	else
		return 0;
}
...
unsigned int udp_get_timeouts_by_state(enum udp_conntrack state, void *ct_or_cp,
				       int is_ct)
{
	struct net *net = NULL;
...
int rtl_getSchNum(char *s)
{
	...
	if (!strcmp(s, "drr"))
		type = RTL_DRR_TYPE;
	if (!strcmp(s, "dsmark"))
		type = RTL_DSMARK_TYPE;
#endif
	return type;
}

static int rtl_getClassPara(unsigned long cl1, struct Qdisc *sch,
			    struct rtl_class_para *rtl_class_para)
{
	...
	case RTL_HTB_TYPE:
		if (cl1 && rtl_class_para) {
			...
			memset(rtl_class_para, 0,
			       sizeof(struct rtl_class_para));
			memcpy(&(rtl_class_para->common), &(cl->common),
			       sizeof(struct Qdisc_class_common));
			...
			{
				memcpy(&(rtl_class_para->ceil), &(cl->ceil),
			...
			printk("rtl_class_para:%x,%x,[%s]:[%d].\n",
			       cl->common.classid, cl->level, __FUNCTION__,
			       __LINE__);
#endif
			memcpy(&(rtl_class_para->rate), &(cl->rate),
			       sizeof(struct psched_ratecfg));
	...
	case RTL_HFSC_TYPE:
		if (cl1 && rtl_class_para) {
			...
			memcpy(&(rtl_class_para->common), &(cl->cl_common),
			       sizeof(struct Qdisc_class_common));
			rtl_class_para->type = RTL_HFSC_TYPE;
			rtl_class_para->cl_flags = cl->cl_flags;
			rtl_class_para->cl = cl1;
			if ((cl->cl_flags & (HFSC_USC | HFSC_FSC | HFSC_RSC)) ==
			    0) {
			...
			if (cl->cl_parent) {
				rtl_class_para->parent =
				    (void *)(cl->cl_parent);
				parentcl = cl->cl_parent;

#if defined (CONFIG_RTL_HW_QOS_SUPPORT_DEBUG)
				printk
				    ("cl:%p,parent:%p,id:%x,level:%d,[%s]:[%d].\n",
				     cl, cl->cl_parent,
				     cl->cl_parent->cl_common.classid,
				     cl->cl_parent->level, __FUNCTION__,
				     __LINE__);
#endif
			} else {
#if defined (CONFIG_RTL_HW_QOS_SUPPORT_DEBUG)
	...
	default:
		...
		if (cl1 && rtl_class_para) {
			...
			printk("ret:%d,rtl_class_para:%x,[%s]:[%d].\n", ret,
			       cl->common.classid, __FUNCTION__, __LINE__);
#endif
		}
#endif
		break;
...
static int rtl_syncHwQueue(struct net_device *dev)
{
	...
	printk("dev:%s,defClassId:%x,dev->num_tx_queues:%d .[%s]:[%d].\n",
	       dev->name, defClassId, dev->num_tx_queues, __FUNCTION__,
	       __LINE__);
#endif
	for (i = 0; i < dev->num_tx_queues; i++) {
		...
		list_for_each_entry(q, &txq_root->list, list) {
			...
			if (q && q->parent) {
				sch = NULL;
				sch =
				    netdev_get_tx_queue(dev, 0)->qdisc_sleeping;
				if (sch == NULL) {
...
#if defined (CONFIG_RTL_HW_QOS_SUPPORT_DEBUG)
				printk
				    ("[%d][%d],parent:%x,handle:%x,flags:%x,[%s]:[%d].\n",
				     i, queueNum, q->parent, q->handle,
				     q->flags, __FUNCTION__, __LINE__);
#endif
				if (ret == -EINVAL) {
					spin_unlock_bh(qdisc_lock(txq_root));
					return -EINVAL;
				}
				{
					...
					queueInfo[queueNum].prio =
					    rtl_class_para.prio;
#endif

					//byte->bps
					queueInfo[queueNum].bandwidth =
					    rtl_class_para.min_bw << 3;
					queueInfo[queueNum].ceil =
					    rtl_class_para.max_bw << 3;
					queueInfo[queueNum].handle =
					    queueInfo[queueNum].queueId =
					    rtl_class_para.common.classid;
					memcpy(queueInfo[queueNum].ifname,
					       dev->name, sizeof(dev->name));
#if defined (CONFIG_RTL_HW_QOS_SUPPORT_DEBUG)
					printk
					    ("queueInfo[%d]:prio:%d,bw:%d,ceil:%d,handle:%x,dev:%s,[%s]:[%d].\n",
					     queueNum, queueInfo[queueNum].prio,
					     queueInfo[queueNum].bandwidth,
					     queueInfo[queueNum].ceil,
					     queueInfo[queueNum].handle,
					     queueInfo[queueNum].ifname,
					     __FUNCTION__, __LINE__);
#endif
					if (rtl_class_para.common.classid ==
					    defClassId)
						queueInfo[queueNum].flags |=
						    QOS_DEF_QUEUE;
					else
						queueInfo[queueNum].flags &=
						    (~QOS_DEF_QUEUE);

					if (queueInfo[queueNum].bandwidth ==
					    queueInfo[queueNum].ceil) {
					...
					while (cl
						...
						memset(&rtl_class_para, 0,
						       sizeof(struct
							      rtl_class_para));
						rtl_class_para.parent = NULL;
						rtl_getClassPara(cl, sch,
								 &rtl_class_para);
						if ((rtl_class_para.parent ==
						     NULL)
						    || (rtl_class_para.level ==
							maxlevel))
							break;
...
#if defined (CONFIG_RTL_HW_QOS_SUPPORT_DEBUG)
						printk
						    ("[%x],classinfo:id:%x,pri:%x,minbw:%d,maxbw:%d,level:%x,[%s]:[%d].\n",
						     i,
						     rtl_class_para.common.
						     classid,
						     rtl_class_para.prio,
						     rtl_class_para.min_bw,
						     rtl_class_para.max_bw,
						     rtl_class_para.level,
						     __FUNCTION__, __LINE__);
#endif
					}
...
#if defined (CONFIG_RTL_HW_QOS_SUPPORT_DEBUG)
						printk
						    ("[%x],topClassNum:%d,classinfo:id:%x,pri:%x,minbw:%d,maxbw:%d,level:%x,,[%s]:[%d].\n",
						     i, topClassNum,
						     rtl_class_para.common.
						     classid,
						     rtl_class_para.prio,
						     ...
						     __FUNCTION__, __LINE__);
#endif
						for (idx = 0; idx < topClassNum;
						     idx++) {
							if (rtl_class_paraHandle
							    [idx].common.
							    classid ==
							    rtl_class_para.
							    common.classid) {
...
#if defined (CONFIG_RTL_HW_QOS_SUPPORT_DEBUG)
							printk
							    ("----[%x],topClassNum:%d,rootclassinfo:id:%x,pri:%x,minbw:%d,maxbw:%d,level:%x,[%s]:[%d].\n",
							     i, topClassNum,
							     rtl_class_para.
							     common.classid,
							     rtl_class_para.
	...
	for (i = 0; i < queueNum; i++) {
		printk
		    ("[%d]:queueinfo:flag:%x,pri%d,bw:%d,ceil:%d,handle:%x,dev:%s\n",
		     i, queueInfo[i].flags, queueInfo[i].prio,
		     queueInfo[i].bandwidth, queueInfo[i].ceil,
		     queueInfo[i].handle, queueInfo[i].ifname);
	}
#endif
	if (((queueNum > RTL_MAX_HWQUEUE_NUM) || (queueNum == 0))
	    || (topClassNum == 0)) {
#if defined (CONFIG_RTL_HW_QOS_SUPPORT_DEBUG)
...
#if defined (CONFIG_RTL_HW_QOS_SUPPORT_DEBUG)
				printk
				    ("----[%x],topClassNum:%d,type:%x,clflags:%x,rootclassinfo:id:%x,[%s]:[%d].\n",
				     idx, topClassNum,
				     rtl_class_paraHandle[idx].type,
				     rtl_class_paraHandle[idx].cl_flags,
				     rtl_class_paraHandle[idx].common.classid,
				     __FUNCTION__, __LINE__);
#endif
				if ((rtl_class_paraHandle[idx].type ==
				     RTL_HFSC_TYPE)
				    &&
				    ((rtl_class_paraHandle[idx].cl_flags &
				      (HFSC_USC | HFSC_FSC | HFSC_RSC)) == 0)) {
					leafBandwidth =
					    rtl_get_hfsc_class_leafbw(sch,
								      rtl_class_paraHandle
								      [idx].cl);
		...
		printk("dev:%s, bw:%d,[%s]:[%d].\n", dev->name,
		       tmpPortBandwidth, __FUNCTION__, __LINE__);
#endif
		...
		for (i = 0; i < queueNum; i++) {
			if ((queueInfo[i].ceil == portBandwidth)
				...
				{
					queueInfo[i].bandwidth =
					    ((queueInfo[i].bandwidth >> 13) +
					     1) << 13;
					queueInfo[i].ceil =
					    ((queueInfo[i].ceil >> 13) +
					     1) << 13;
		...
		if (totalRbandwidth != 0
		    && ((totalGbandwidth + totalRbandwidth) > portBandwidth)) {
		...
		for (i = 0; i < queueNum; i++) {
			if (queueInfo[i].bandwidth != queueInfo[i].ceil) {
				...
				if (queueInfo[i].ceil > 0x200000)
					queueInfo[i].ceil =
					    ((queueInfo[i].ceil / 1000) << 10);
				else
					queueInfo[i].ceil =
					    ((queueInfo[i].ceil << 10) / 1000);
#endif
				...
				if (queueInfo[i].ceil > FULL_SPEED)
					queueInfo[i].ceil =
					    queueInfo[i].bandwidth = FULL_SPEED;
			}
		}
...
int rtl_hwQos_mark_classidMapHook(struct net_device *dev, struct tcf_proto *tp)
{
	...
	if (tp) {
		...
		printk("tpinfo:%s,%x,%x,[%s]:[%d].\n", tp->ops->kind,
		       tp->classid, tp->prio, __FUNCTION__, __LINE__);
#endif
...
#if defined (CONFIG_RTL_HW_QOS_SUPPORT_DEBUG)
					printk
					    ("mark:%x/%x,handle:%x,[%s]:[%d].\n",
					     mark, mask, handle, __FUNCTION__,
					     __LINE__);
#endif
...
#if defined (CONFIG_RTL_HW_QOS_SUPPORT_DEBUG)
				printk
				    ("dev:%s,[%d],tp->ops->kind:%s,parent:%x,inhandle:%x, [%s]:[%d].\n",
				     dev->name, t, tp->ops->kind, parent,
				     inhandle, __FUNCTION__, __LINE__);
#endif
...
int rtl_hwQosSyncTcHook(struct net_device *dev, struct Qdisc *q,
			unsigned int parent)
{
...
int rtl_ProcessHwQosIptablesIgnoreInfo(unsigned char *matchinfo, int flag)
{
	...
	if (flag == RTL_QOS_IPTABLES_IGNORE_FLUSH) {
		for (i = 0; i < RTL_QOS_MAX_IGNORE_MATCH_NUM; i++) {
			memset(&(ignore_info[i]), 0,
			       sizeof(struct rtl_ignore_mach));
		}
		ret = 1;
		...
		if (flag == RTL_QOS_IPTABLES_IGNORE_ADD) {
			for (i = 0; i < RTL_QOS_MAX_IGNORE_MATCH_NUM; i++) {
				if (ignore_info[i].vaild == 0) {
					strcpy(ignore_info[i].matchinfo,
					       matchinfo);
			...
			for (i = 0; i < RTL_QOS_MAX_IGNORE_MATCH_NUM; i++) {
				if ((ignore_info[i].vaild)
				    &&
				    (strcmp(ignore_info[i].matchinfo, matchinfo)
				     == 0)) {
...
int rtl_HwQosIptablesIgnoreInfoInit(void)
{
	...
	rtl_ProcessHwQosIptablesIgnoreInfo("multiport",
					   RTL_QOS_IPTABLES_IGNORE_ADD);
	rtl_ProcessHwQosIptablesIgnoreInfo("mark", RTL_QOS_IPTABLES_IGNORE_ADD);
	rtl_ProcessHwQosIptablesIgnoreInfo("comment",
					   RTL_QOS_IPTABLES_IGNORE_ADD);
	rtl_ProcessHwQosIptablesIgnoreInfo("length",
					   RTL_QOS_IPTABLES_IGNORE_ADD);
...
int rtl_process_iptablesMatchInfo(struct xt_match *match)
{
	...
	if (match) {
		for (i = 0; i < RTL_QOS_MAX_IGNORE_MATCH_NUM; i++) {
			if ((ignore_info[i].vaild)
			    && (strcmp(ignore_info[i].matchinfo, match->name) ==
				0)) {
				ret = 0;
				break;
			}
...
int rtl_hwQosIptables_HookProcess(struct xt_table *table)
{
	...
	if (!IS_ERR_OR_NULL(table)) {
...

		{
			...
			struct ipt_entry *e_jump[RTL_QOS_IPTABLES_CHAINNUM] =
			    { 0 };
			...
			xt_entry_foreach(e, loc_cpu_entry, total_size) {
				...
				const struct xt_entry_target *t = NULL, *t_qos =
				    NULL;
				int v = 0;
				rulenum++;
				...
				if (strcmp(t->u.user.name, XT_STANDARD_TARGET)
				    == 0) {
					v = ((struct xt_standard_target *)
					     t)->verdict;
					if (v == XT_RETURN) {
						qosFlag = 0;
					} else {
...
#if defined (CONFIG_RTL_HW_QOS_SUPPORT_DEBUG)
								printk
								    ("----------qosFlag is set![%d],e:%p, data:%s,[%s]:[%d].\n",
								     rulenum, e,
								     t->data,
								     __FUNCTION__,
								     __LINE__);
...
#if 0				//defined (CONFIG_RTL_HW_QOS_SUPPORT_DEBUG)
						printk
						    ("[%d][%d],e:%p, t:%s,data:%s ,flag:%x,%s,match: proto:%d,outif:%s[%s]:[%d].\n",
						     rulenum, qos_num, e_qos,
						     t_qos->u.user.name,
						     t_qos->data, flags,
						     t_qos->u.kernel.target->
						     name, e_qos->ip.proto,
						     e_qos->ip.outiface,
						     __FUNCTION__, __LINE__);
#endif
...
#if defined (CONFIG_RTL_HW_QOS_SUPPORT_DEBUG)
									printk
									    ("[%d][%d]swQosFlag:%d,match info:%s,[%s]:[%d]\n",
									     rulenum,
									     qos_num,
									     swQosFlag,
									     m_qos->
	...
	if (buffer && !copy_from_user(tmpBuf, buffer, len)) {
		...
		if (strncmp(cmd_addr, "qos", 3) == 0) {
			...
			if (strncmp(cmd_addr, "add", 3) == 0) {
				...
				rtl_ProcessHwQosIptablesIgnoreInfo(cmd_addr,
								   RTL_QOS_IPTABLES_IGNORE_ADD);
				...
				rtl_ProcessHwQosIptablesIgnoreInfo(cmd_addr,
								   RTL_QOS_IPTABLES_IGNORE_DEL);

			} else if (strncmp(cmd_addr, "flush", 3) == 0) {
				rtl_ProcessHwQosIptablesIgnoreInfo(NULL,
								   RTL_QOS_IPTABLES_IGNORE_FLUSH);
			} else if (strncmp(cmd_addr, "show", 3) == 0) {
				rtl_ProcessHwQosIptablesIgnoreInfo(NULL,
								   RTL_QOS_IPTABLES_IGNORE_SHOW);
			}
...
static ssize_t rtl_qos_single_write(struct file *file,
...
#if defined(CONFIG_PROC_FS)
	proc_rtl_qos =
	    proc_create_data("rtl_qos", 0, &proc_root, &rtl_qos_fops, NULL);
#endif
	rtl_qosTimerInit();
...
static void rtl_hwQosSyncCallback(void)
{
	...
	if (iptables_hook_info.last_hook_time
	    &&
	    (time_before_eq
	     ((iptables_hook_info.last_hook_time + RTL_DELAY_PROCESS_TIME * HZ),
	      current_time))) {
	...
	for (i = 0; i < RTL_MAX_QOSCONFIG_DEV_NUM; i++) {
		if (qosConf[i].valid && qosConf[i].enable) {
			if (qosConf[i].last_hook_time
			    &&
			    (time_before_eq
			     (qosConf[i].last_hook_time +
			      RTL_DELAY_PROCESS_TIME * HZ, current_time))) {
	...
	for (i = 0; i < RTL_MAX_QOSCONFIG_DEV_NUM; i++) {
		...
		if (qosConf[i].valid) {
			printk("[%d] dev:%s enable:%d\n", i,
			       qosConf[i].dev->name, qosConf[i].enable);
		}
	}
	return;
}

int rtl_QosSyncTcHook(struct net_device *dev, struct Qdisc *q,
		      unsigned int parent, int process)
{
...
#endif
	{
		if (dev->num_tx_queues) {
			...
			if (q && (q->enqueue)) {
				struct Qdisc *txq_root = txq->qdisc_sleeping;
				spin_lock_bh(qdisc_lock(txq_root));
				list_for_each_entry(sub_q, &txq_root->list,
						    list) {
					if (sub_q->parent)
						qosEnable = 1;
				}
...
extern int32 rtl865x_addPpp(uint8 * ifname, ether_addr_t * mac,
			    uint32 sessionId, int32 type);
extern int rtl_add_ps_drv_netif_mapping(struct net_device *dev,
					const char *name);
...
extern int rtl865x_add_pattern_acl_for_contentFilter(rtl865x_AclRule_t * rule,
						     char *netifName);
extern int rtl865x_del_pattern_acl_for_contentFilter(rtl865x_AclRule_t * rule,
						     char *netifName);
#endif
extern ps_drv_netif_mapping_t *rtl_get_ps_drv_netif_mapping_by_psdev(struct
								     net_device
								     *dev);
...
int br_updateQuerierInfo(unsigned int version, unsigned char *devName,
			 unsigned int *querierIp)
{
	int i;
	...
	for (i = 0; i < MAX_QUERIER_RECORD; i++) {
		if ((querierInfoList[i].version == version)) {
			...
			if ((version == 6)
			    &&
			    (memcmp(querierInfoList[i].querierIp, querierIp, 16)
			     == 0)) {
				strcpy(querierInfoList[i].devName, devName);
				querierInfoList[i].lastJiffies = jiffies;
				return 0;
	...
	for (i = 0; i < MAX_QUERIER_RECORD; i++) {
		if (querierInfoList[i].version == 0) {
			...
			if (version == 4) {
				...
				memcpy(querierInfoList[i].querierIp, querierIp,
				       16);
			}
			strcpy(querierInfoList[i].devName, devName);
			querierInfoList[i].lastJiffies = jiffies;
	...
	for (i = 0; i < MAX_QUERIER_RECORD; i++) {
		if (time_before
		    ((unsigned long)(querierInfoList[i].lastJiffies),
		     oldestJiffies)) {
...
int check_igmpQueryExist(struct iphdr *iph)
{
	...
	if (*(unsigned char *)((unsigned char *)iph + ((iph->ihl) << 2)) ==
	    0x11) {
		return 1;
	}
	...
	if (ipVer == 4) {
		...
		for (i = 0; i < MAX_QUERIER_RECORD; i++) {
			if ((querierInfoList[i].version == 4) &&
			    time_after((unsigned
					long)(querierInfoList[i].lastJiffies +
					      QUERIER_EXPIRED_TIME * HZ),
				       jiffies)
			...
			if ((querierInfoList[i].version == 6) &&
			    time_after((unsigned
					long)(querierInfoList[i].lastJiffies +
				       ...
				       jiffies)) {
				if (memcmp
				    (querierInfoList[i].querierIp, brIpv6Addr,
				     16) < 0) {
					ret = 0;
				}
			}
	...
	for (i = 0; i < MAX_RESERVED_MULTICAST_NUM; i++) {
		if (reservedMCastRecord[i].valid == 1) {
			...
			seq_printf(s, "    [%d] Group address:%d.%d.%d.%d\n",
				   cnt, reservedMCastRecord[i].groupAddr >> 24,
				   (reservedMCastRecord[i].groupAddr &
				    0x00ff0000) >> 16,
				   (reservedMCastRecord[i].groupAddr &
				    0x0000ff00) >> 8,
				   (reservedMCastRecord[i].groupAddr & 0xff));
		}
	}
	if (cnt == 0)
		seq_printf(s, "NULL\n");
	...
	for (i = 0; i < BR_HASH_SIZE; i++) {
		...
		hlist_for_each_entry_safe(f, n, &bridge0->hash[i], hlist) {
			if (MULTICAST_MAC(f->addr.addr)) {
				seq_printf(s,
					   "[%d][%d]mCastMac:0x%x:%x:%x:%x:%x:%x,use_count is %lu,ageing_timer is %lu\n",
					   i, j, f->addr.addr[0],
					   f->addr.addr[1], f->addr.addr[2],
					   f->addr.addr[3], f->addr.addr[4],
					   f->addr.addr[5], f->used,
					   (jiffies <
					    f->updated) ? 0 : (jiffies -
							       f->updated));
				for (k = 0; k < FDB_IGMP_EXT_NUM; k++) {
					if (f->igmp_fdb_arr[k].valid) {
						seq_printf(s,
							   "\t<%d>clientMac:0x%x:%x:%x:%x:%x:%x,ageing_time:%lu\n",
							   k,
							   f->igmp_fdb_arr
							   [k].SrcMac[0],
							   f->igmp_fdb_arr
			    ...
			    int count, int *eof, void *data)
{
	...
	len +=
	    sprintf(page + len, "Block Info :%d,%d\n\n", chkUnknownMcastEnable,
		    maxUnknownMcastPPS);
	cnt = 0;
	...
	for (i = 0; i < MAX_RESERVED_MULTICAST_NUM; i++) {
		if (reservedMCastRecord[i].valid == 1) {
			...
			len +=
			    sprintf(page + len,
				    "    [%d] Group address:%d.%d.%d.%d\n", cnt,
				    reservedMCastRecord[i].groupAddr >> 24,
				    (reservedMCastRecord[i].groupAddr &
				     0x00ff0000) >> 16,
				    (reservedMCastRecord[i].groupAddr &
				     0x0000ff00) >> 8,
	...
	if (buffer && !copy_from_user(&tmpbuf, buffer, count)) {
		...
		if (tokptr != NULL) {
			if (!memcmp(tokptr, "enable", 6)) {
				...
				rtl_configMulticastSnoopingFastLeave(fastLeave,
								     ageTime);
				...
				cnt =
				    sscanf(tokptr, "%d.%d.%d.%d", &ipAddr[0],
					   &ipAddr[1], &ipAddr[2], &ipAddr[3]);

				groupAddr =
				    (ipAddr[0] << 24) | (ipAddr[1] << 16) |
				    (ipAddr[2] << 8) | (ipAddr[3]);
...
static ssize_t rlx_br_igmpsnoop_single_write(struct file *file,
...
int igmp_db_write(struct file *file, const char __user * buffer, size_t count,
		  loff_t * data)
{
...
static ssize_t rlx_br_mcast_fast_fwd_single_write(struct file *file,
						  const char __user * userbuf,
						  size_t count, loff_t * off)
{
...
static int br_igmpQueryRead(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < MAX_QUERIER_RECORD; i++) {
		if ((querierInfoList[i].version == 4)
		    &&
		    time_after((unsigned long)(querierInfoList[i].lastJiffies +
					       QUERIER_EXPIRED_TIME * HZ),
			       jiffies)) {
			if (jiffies > querierInfoList[i].lastJiffies) {
				elapseJiffies =
				    jiffies - querierInfoList[i].lastJiffies;
			} else {
				elapseJiffies =
				    jiffies + ((unsigned long)0xFFFFFFFF -
					       querierInfoList[i].lastJiffies) +
				    1;
			}
			cnt++;
			seq_printf(s, "%s %d.%d.%d.%d %u\n",
				   querierInfoList[i].devName,
				   ((querierInfoList[i].querierIp[0] >> 24) &
				    0xFF),
				   ((querierInfoList[i].querierIp[0] >> 16) &
				    0xFF),
				   ((querierInfoList[i].querierIp[0] >> 8) &
				    0xFF),
				   (querierInfoList[i].querierIp[0] & 0xFF),
				   elapseJiffies / HZ);
		}
	}
	if (cnt == 0)
...
static ssize_t rlx_br_igmp_query_single_write(struct file *file,
					      const char __user * userbuf,
					      size_t count, loff_t * off)
{
...
static ssize_t rlx_br_mld_version_single_write(struct file *file,
					       const char __user * userbuf,
					       size_t count, loff_t * off)
{
...
static ssize_t rlx_br_igmp_querier_info_single_write(struct file *file,
						     const char __user *
...
static ssize_t rlx_br_mld_querier_info_single_write(struct file *file,
						    const char __user * userbuf,
						    size_t count, loff_t * off)
{
			    ...
			    int count, int *eof, void *data)
{
	...
	len +=
	    sprintf(page, "igmpProxyNeedFastLeave: %c\n",
		    IGMPPRoxyFastLeave + '0');
	if (len <= off + count)
		*eof = 1;
	*start = page + off;
...
static ssize_t rlx_br_igmp_proxy_single_write(struct file *file,
					      const char __user * userbuf,
					      size_t count, loff_t * off)
{
...
#endif
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("br_igmpsnoop", 0, &proc_root,
			 &rlx_br_igmpsnoop_proc_fops, NULL);
	proc_create_data("br_igmpDb", 0, &proc_root,
			 &igmp_db_seq_file_operations, NULL);
	proc_create_data("br_igmpQuery", 0, &proc_root,
			 &rlx_br_igmp_query_proc_fops, NULL);
	proc_create_data("br_igmpProxy", 0, &proc_root,
			 &rlx_br_igmp_proxy_proc_fops, NULL);
	proc_create_data("br_mCastFastFwd", 0, &proc_root,
			 &rlx_br_mcast_fast_fwd_proc_fops, NULL);
#if defined CONFIG_RTL_MLD_SNOOPING
	proc_create_data("br_mldVersion", 0, &proc_root,
			 &rlx_br_mld_version_proc_fops, NULL);
#endif
#else
...
static unsigned short br_ipv4Checksum(unsigned char *pktBuf,
				      unsigned int pktLen)
{
...
static unsigned char *br_generateIgmpQuery(struct net_bridge *br)
{
	...
	if ((landev =
	     __dev_get_by_name(&init_net, RTL_PS_BR0_DEV_NAME)) != NULL) {
		in_dev = (struct in_device *)(landev->ip_ptr);
		if (in_dev != NULL) {
			for (ifap = in_dev->ifa_list; ifap != NULL;
			     ifap = ifap->ifa_next) {
				if (strcmp(RTL_PS_BR0_DEV_NAME, ifap->ifa_label)
				    == 0) {
					if (igmpVersion == 3) {
						memcpy(&igmpV3QueryBuf[26],
						       &ifap->ifa_address, 4);
					} else {
						memcpy(&igmpV2QueryBuf[26],
						       &ifap->ifa_address, 4);
					}
				...
				for (ifap = in_dev->ifa_list; ifap != NULL;
					...
					if (strcmp
					    (br->dev->name,
					     ifap->ifa_label) == 0) {
						if (igmpVersion == 3) {
							memcpy(&igmpV3QueryBuf
							       [26],
							       &ifap->
							       ifa_address, 4);
						} else {
							memcpy(&igmpV2QueryBuf
							       [26],
							       &ifap->
							       ifa_address, 4);
						}
...
//if igmpproxy opened, the igmp query send is handled by igmpproxy daemon
	...
	list_for_each_entry_safe(p, n, &br->port_list, list) {
		if ((p->state == BR_STATE_FORWARDING)
		    && (strncmp(p->dev->name, "peth", 4) != 0)
		    && (strncmp(p->dev->name, "pwlan", 5) != 0)) {
			if (prev != NULL) {
...
#else
				prev->dev->netdev_ops->ndo_start_xmit(skb2,
								      prev->
								      dev);
#endif
				fwdCnt++;
...
#if defined(CONFIG_RTL_MLD_SNOOPING)
static unsigned short br_ipv6Checksum(unsigned char *pktBuf,
				      unsigned int pktLen,
				      unsigned char *ipv6PseudoHdrBuf)
{
	unsigned int checksum = 0;
	unsigned int count = pktLen;
	unsigned short *ptr;
...
static unsigned char *br_generateMldQuery(struct net_bridge *br)
{
	...
	if (mldVersion == 2) {
		...
		checkSum =
		    br_ipv6Checksum(&mldQueryBuf[62], 28, ipv6PseudoHdrBuf);
	} else {
		ipv6PseudoHdrBuf[35] = 24;
		checkSum =
		    br_ipv6Checksum(&mldQueryBuf[62], 24, ipv6PseudoHdrBuf);
	}
...
void rtl_IgmpSnooping_BrInit_Hook(const char *name, struct net_device *dev)
{
	...
	if (strcmp(name, RTL_PS_BR0_DEV_NAME) == 0) {
...
#if defined (CONFIG_RTL_HARDWARE_MULTICAST)
		if (res == 0) {
			rtl_setIgmpSnoopingModuleDevInfo(brIgmpModuleIndex,
							 &devInfo);
		}
#endif
		...
		if (bridge0 != NULL) {
			...
			bridge0->mCastQuerytimer.expires =
			    jiffies + MCAST_QUERY_INTERVAL * HZ;
			bridge0->mCastQuerytimer.function =
			    (void *)br_mCastQueryTimerExpired;
	...
	if (strcmp(name, RTL_PS_BR1_DEV_NAME) == 0) {
...
#if defined (CONFIG_RTL_HARDWARE_MULTICAST)
		if (res == 0) {
			rtl_setIgmpSnoopingModuleDevInfo(brIgmpModuleIndex_2,
							 &devInfo2);
		}
#endif
		...
		if (bridge1 != NULL) {
			...
			bridge1->mCastQuerytimer.expires =
			    jiffies + MCAST_QUERY_INTERVAL * HZ;
			bridge1->mCastQuerytimer.function =
			    (void *)br_mCastQueryTimerExpired;
...
#if defined(CONFIG_RTL_HARDWARE_MULTICAST)
int rtl865x_generateBridgeDeviceInfo(struct net_bridge *br,
				     rtl_multicastDeviceInfo_t * devInfo)
{
	struct net_bridge_port *p, *n;
	...
	if (strcmp(br->dev->name, RTL_PS_BR0_DEV_NAME) != 0
	    && strcmp(br->dev->name, RTL_PS_BR1_DEV_NAME) != 0)
#else
	if (strcmp(br->dev->name, RTL_PS_BR0_DEV_NAME) != 0)
#endif
	{
...
static char igmp_type_check(struct sk_buff *skb, unsigned char *gmac,
			    unsigned int *gIndex, unsigned int *moreFlag)
{
	struct iphdr *iph;
	__u8 hdrlen;
	...
	*moreFlag = 0;
	...
	if ((igmph->type == IGMP_HOST_MEMBERSHIP_REPORT) ||
		...
		struct igmpv3_report *igmpv3report =
		...
		for (i = 0; i < ntohs(igmpv3report->ngrec); i++) {
			...
			if (i == 0) {
				...
				igmpv3grec =
				    (struct igmpv3_grec *)((unsigned char *)
							   igmpv3grec + 8 +
							   ntohs
							   (igmpv3grec->
							    grec_nsrcs)
			...
			*gIndex = *gIndex + 1;
			...
			ConvertMulticatIPtoMacAddr(groupAddr, gmac);
			if (((igmpv3grec->grec_type == IGMPV3_CHANGE_TO_INCLUDE)
			     || (igmpv3grec->grec_type ==
				 IGMPV3_MODE_IS_INCLUDE))
			    && (ntohs(igmpv3grec->grec_nsrcs) == 0)) {
				return 2;	/* leave and delete it */
			} else
			    if ((igmpv3grec->grec_type ==
...
int chk_igmp_ext_entry(struct net_bridge_fdb_entry *fdb, unsigned char *srcMac)
{
			...
			unsigned char *srcMac, unsigned char portComeIn)
{
	...
	DEBUG_PRINT
	    ("add_igmp,DA=%02x:%02x:%02x:%02x:%02x:%02x ; SA=%02x:%02x:%02x:%02x:%02x:%02x\n",
	     add[0], add[1], add[2], add[3], add[4], add[5], srcMac[0],
	...
	for (i2 = 0; i2 < FDB_IGMP_EXT_NUM; i2++) {
		if (fdb->igmp_fdb_arr[i2].valid == 0) {
			...
			DEBUG_PRINT("portUsedNum[%d]=%d\n\n",
				    bitmask_to_id(portComeIn),
				    fdb->portUsedNum[bitmask_to_id
						     (portComeIn)]);
			return;
		}
	}
...
void del_igmp_ext_entry(struct net_bridge_fdb_entry *fdb, unsigned char *srcMac,
			unsigned char portComeIn, unsigned char expireFlag)
{
	int i2;
	unsigned char *add;
	add = fdb->addr.addr;
	...
	for (i2 = 0; i2 < FDB_IGMP_EXT_NUM; i2++) {
		if (fdb->igmp_fdb_arr[i2].valid == 1) {
			if (!memcmp(fdb->igmp_fdb_arr[i2].SrcMac, srcMac, 6)) {
				if (expireFlag) {
					fdb->igmp_fdb_arr[i2].ageing_time -=
					    300 * HZ;
					...
					DEBUG_PRINT
					    ("\nexpireFlag:%d,del_igmp_ext_entry,DA=%02x:%02x:%02x:%02x:%02x:%02x ; SA=%02x:%02x:%02x:%02x:%02x:%02x success!!!\n",
					     expireFlag, add[0], add[1], add[2],
					     add[3], add[4], add[5], srcMac[0],
					     srcMac[1], srcMac[2], srcMac[3],
					     srcMac[4], srcMac[5]);
				}
				DEBUG_PRINT
				    ("\ndel_igmp_ext_entry,DA=%02x:%02x:%02x:%02x:%02x:%02x ; SA=%02x:%02x:%02x:%02x:%02x:%02x success!!!\n",
				     add[0], add[1], add[2], add[3], add[4],
				     add[5], srcMac[0], srcMac[1], srcMac[2],
				...
				if (portComeIn != 0) {
					...
					if (fdb->portUsedNum[index] <= 0) {
						DEBUG_PRINT
						    ("portUsedNum[%d] == 0 ,update portlist from (%x)  ",
						     index, fdb->portlist);
						fdb->portlist &= ~portComeIn;
						DEBUG_PRINT("to (%x) \n",
							    fdb->portlist);
						...
						if (fdb->portUsedNum[index] < 0) {
							DEBUG_PRINT
							    ("!! portUsedNum[%d]=%d < 0 at (del_igmp_ext_entry)  \n",
							     index,
							     fdb->portUsedNum
							     [index]);
							fdb->portUsedNum[index]
			   ...
			   unsigned char *srcMac, unsigned char portComeIn)
{
	...
	DEBUG_PRINT
	    ("update_igmp,DA=%02x:%02x:%02x:%02x:%02x:%02x ; SA=%02x:%02x:%02x:%02x:%02x:%02x\n",
	     add[0], add[1], add[2], add[3], add[4], add[5], srcMac[0],
	     srcMac[1], srcMac[2], srcMac[3], srcMac[4], srcMac[5]);
		...
		if (fdb->igmp_fdb_arr[i2].valid == 1) {
			if (!memcmp(fdb->igmp_fdb_arr[i2].SrcMac, srcMac, 6)) {
				...
				if (fdb->igmp_fdb_arr[i2].port != portComeIn) {
					...
					unsigned char port_orig =
					    fdb->igmp_fdb_arr[i2].port;
					...
					if (fdb->portUsedNum[index] <= 0) {
						...
						if (fdb->portUsedNum[index] < 0) {
							DEBUG_PRINT
							    ("!! portNum[%d] < 0 at (update_igmp_ext_entry)\n",
							     index);
							fdb->portUsedNum[index]
							    = 0;
						}
...
static void br_update_igmp_snoop_fdb(unsigned char op, struct net_bridge *br,
				     struct net_bridge_port *p,
				     unsigned char *dest, struct sk_buff *skb)
{
...
#else
	if (p && p->dev && p->dev->name
	    && !memcmp(p->dev->name, RTL_PS_LAN_P0_DEV_NAME, 4)) {
		port_comein = 0x01;
	}

	if (p && p->dev && p->dev->name
	    && !memcmp(p->dev->name, RTL_PS_WLAN_NAME, 4)) {
		port_comein = 0x80;
	}
#endif
//      src=(unsigned char*)(skb->mac.raw+ETH_ALEN);
	...
	if (op == 1) {		/* add */
				...
				if (dev) {
					...
					if (dev->do_ioctl != NULL) {
						dev->do_ioctl(dev,
							      ...
							      0x8B80);
						DEBUG_PRINT
						    ("... add to wlan mcast table:  DA:%02x:%02x:%02x:%02x:%02x:%02x ; SA:%02x:%02x:%02x:%02x:%02x:%02x\n",
						     StaMacAndGroup[0],
						     StaMacAndGroup[1],
						     StaMacAndGroup[2],
						     StaMacAndGroup[3],
...
#if defined(CONFIG_COMPAT_NET_DEV_OPS)
				if (dev->do_ioctl != NULL) {
					dev->do_ioctl(dev, (struct ifreq *)
						      StaMacAndGroup, 0x8B81);
					DEBUG_PRINT
					    ("(del) wlan0 ioctl (del) M2U entry da:%02x:%02x:%02x-%02x:%02x:%02x; sa:%02x:%02x:%02x-%02x:%02x:%02x\n",
					     StaMacAndGroup[0],
					     StaMacAndGroup[1],
					     StaMacAndGroup[2],
					     StaMacAndGroup[3],
		...
		if (dst->portlist) {
			...
			{
				DEBUG_PRINT
				    ("----all joined sta are gone,make it expired after 10 seconds----\n");
...
static void br_flood(struct net_bridge *br, struct sk_buff *skb,
		     ...
		     void (*__packet_hook) (const struct net_bridge_port * p,
					    struct sk_buff * skb), bool forward)
{
	struct net_bridge_port *p;
	struct net_bridge_port *prev;
...
static void br_multicast(struct net_bridge *br, unsigned int fwdPortMask,
			 struct sk_buff *skb, int clone,
			 void (*__packet_hook) (const struct net_bridge_port *
...
void br_multicast_deliver(struct net_bridge *br, unsigned int fwdPortMask,
			  struct sk_buff *skb, int clone)
{
	br_multicast(br, fwdPortMask, skb, clone, __rtl_br_deliver);
}

void br_multicast_forward(struct net_bridge *br, unsigned int fwdPortMask,
			  struct sk_buff *skb, int clone)
{
...
#if defined(CONFIG_RTL_HARDWARE_MULTICAST)
unsigned int rtl865x_getPhyFwdPortMask(struct net_bridge *br,
					  ...
					  unsigned int destIpAddr)
{
	...
	//if(net_ratelimit())
	printk
	    ("%s:%d,br:%s,srcPort is %d,srcVlanId is %d,srcIpAddr is 0x%x,destIpAddr is 0x%x\n",
	     __FUNCTION__, __LINE__, br->dev->name, srcPort, srcVlanId,
	...
	if (strcmp(br->dev->name, RTL_PS_BR0_DEV_NAME) == 0
	    && (brFwdPortMask & br0SwFwdPortMask)) {
		return -1;
	}

	if (strcmp(br->dev->name, RTL_PS_BR1_DEV_NAME) == 0
	    && (brFwdPortMask & br1SwFwdPortMask)) {
		return -1;
	}
#else
	...
	if (net_ratelimit())
		printk
		    ("%s:%d,destIpAddr is 0x%x, srcIpAddr is 0x%x, srcVlanId is %d, srcPort is %d\n",
		     __FUNCTION__, __LINE__, destIpAddr, srcIpAddr, srcVlanId,
		     srcPort);
#endif
	...
	if (strcmp(br->dev->name, RTL_PS_BR0_DEV_NAME) == 0) {
		...
		fwdDescriptor.fwdPortMask = 0xFFFFFFFF;
		ret =
		    rtl_getMulticastDataFwdInfo(nicIgmpModuleIndex_2,
						&multicastDataInfo,
						&multicastFwdInfo);
	}
#else
	...
	if (net_ratelimit())
		printk
		    ("%s:%d,ret:%d,srcPort is %d,srcVlanId is %d,srcIpAddr is 0x%x,destIpAddr is 0x%x\n",
		     __FUNCTION__, __LINE__, ret, srcPort, srcVlanId, srcIpAddr,
		     destIpAddr);
#endif
#endif
	if (ret != 0) {
	...
	if (net_ratelimit())
		printk("[%s:%d]fwdDescriptor.fwdPortMask=%x,dip:%x,sip:%x.\n",
		       __FUNCTION__, __LINE__, destIpAddr, srcIpAddr,
		       fwdDescriptor.fwdPortMask);
#endif
	...
	fwdDescriptor.fwdPortMask =
	    rtl865x_getPhyFwdPortMask(br, brFwdPortMask) & (~(1 << srcPort));
#if 0
	if (net_ratelimit())
		printk("srcPort = %x, fwdDescriptor.fwdPortMask=%x, [%s:%d]\n",
		       srcPort, fwdDescriptor.fwdPortMask, __FUNCTION__,
		       __LINE__);
#endif
	...
	if (net_ratelimit())
		printk
		    ("%s:%d,srcPort is %d,srcVlanId is %d,srcIpAddr is 0x%x,destIpAddr is 0x%x,pmsk:%x\n",
		     __FUNCTION__, __LINE__, srcPort, srcVlanId, srcIpAddr,
		     destIpAddr, fwdDescriptor.fwdPortMask);
#endif
			 ...
			 unsigned int *gIndex, unsigned int *moreFlag)
{
	...
	*moreFlag = 0;
	...
	if (ipv6h->nexthdr == 0) {
		protoType =
		    (unsigned char *)((unsigned char *)ipv6h +
				      sizeof(struct ipv6hdr));
	} else {
	...
	if (protoType[0] == 0x3a) {
		...
		if (icmpv6h->icmp6_type == ICMPV6_MGM_REPORT) {
			...
			icmpv6_McastAddr =
			    (unsigned char *)((unsigned char *)icmpv6h + 8);
#ifdef	DBG_ICMPv6
...
#ifdef	DBG_ICMPv6
			printk
			    ("MCAST_IPV6Addr:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x \n",
			     icmpv6_McastAddr[0], icmpv6_McastAddr[1],
			     icmpv6_McastAddr[2], icmpv6_McastAddr[3],
			     icmpv6_McastAddr[4], icmpv6_McastAddr[5],
			     icmpv6_McastAddr[6], icmpv6_McastAddr[7],
			     icmpv6_McastAddr[8], icmpv6_McastAddr[9],
			     icmpv6_McastAddr[10], icmpv6_McastAddr[11],
			     icmpv6_McastAddr[12], icmpv6_McastAddr[13],
			     icmpv6_McastAddr[14], icmpv6_McastAddr[15]);
			...
			icmpv6_McastAddr =
			    (unsigned char *)((unsigned char *)icmpv6h + 8 + 4);
#ifdef	DBG_ICMPv6
			printk("Type: 0x%x (Multicast listener report v2) \n",
			       icmpv6h->icmp6_type);
#endif

			struct mld2_report *mldv2report =
			    (struct mld2_report *)icmpv6h;
			struct mld2_grec *mldv2grec = NULL;
#ifdef	DBG_ICMPv6
			printk("%s:%d,*gIndex is %d,mldv2report->ngrec is %d\n",
			       __FUNCTION__, __LINE__, *gIndex,
			       ntohs(mldv2report->mld2r_ngrec));
#endif
			...
			for (i = 0; i < ntohs(mldv2report->mld2r_ngrec); i++) {
				...
				if (i == 0) {
					...
					mldv2grec =
					    (struct mld2_grec
					     *)((unsigned char *)mldv2grec + 4 +
						16 +
						ntohs(mldv2grec->grec_nsrcs) *
						16 +
						(mldv2grec->grec_auxwords) * 4);
				}
				...
				*gIndex = *gIndex + 1;
				...
				icmpv6_McastAddr =
				    (unsigned char *)&mldv2grec->grec_mca;
				//printk("%s:%d,groupAddr is %d.%d.%d.%d\n",__FUNCTION__,__LINE__,NIPQUAD(groupAddr));
				if (icmpv6_McastAddr[0] != 0xFF)
					return -1;
				CIPV6toMac(icmpv6_McastAddr, gmac);
				...
				printk
				    ("group_mac [%02x:%02x:%02x:%02x:%02x:%02x] \n",
				     gmac[0], gmac[1], gmac[2], gmac[3],
				     gmac[4], gmac[5]);

				printk("grec_type = %d, src_num = %d\n",
				       mldv2grec->grec_type,
				       mldv2grec->grec_nsrcs);
#endif

				if (((mldv2grec->grec_type ==
				      MLD2_CHANGE_TO_INCLUDE)
				     || (mldv2grec->grec_type ==
					 MLD2_MODE_IS_INCLUDE))
				    && (ntohs(mldv2grec->grec_nsrcs) == 0)) {
					return 2;	/* leave and delete it */
				} else
				    if ((mldv2grec->grec_type ==
					    ...
					    struct in6_addr destIpAddr)
{
	...
	memcpy(multicastDataInfo.groupAddr, &destIpAddr,
...
#endif
	existMulticastEntry =
	    rtl8198C_findMCastv6Entry(dip, sip, (unsigned short)srcVlanId,
				      (unsigned short)srcPort);
	if (existMulticastEntry != NULL) {
		/*it's already in cache */
#if 0
		if (net_ratelimit())
			printk("[%s:%d]already in cache\n", __FUNCTION__,
			       __LINE__);
#endif
		return 0;
	...
	if ((fwdDescriptor.fwdPortMask & tagged_portmask) == 0) {
		...
		ret =
		    rtl8198C_addMulticastv6Entry(dip, sip,
						 (unsigned short)srcVlanId,
						 (unsigned short)srcPort,
						 &fwdDescriptor, 1, 0, 0, 0);
	}
...
int rtl_IgmpSnooping_Input_Hook(struct net_bridge *br, struct sk_buff *skb,
				struct sk_buff *skb2)
{
	int retValue = 0;
	const unsigned char *dest = eth_hdr(skb)->h_dest;
		...
		if (is_multicast_ether_addr(dest)) {
			if (MULTICAST_MAC(dest)
				...
				if (iph->daddr == htonl(0xEFFFFFFA)
				    || iph->daddr == htonl(0xE1010101))
#else
				if (iph->daddr == htonl(0xEFFFFFFA)
				    || iph->daddr == htonl(0xE00000FB))
#endif
				{
					/*for microsoft upnp */
					reserved = 1;
				...
				if (proto == IPPROTO_IGMP) {
					...
					if (IgmpRxFilter_Hook != NULL) {
						...
						if (IgmpRxFilter_Hook
						    (skb, NF_INET_LOCAL_IN,
						     skb->dev, NULL,
					...
					if (!strcmp
						...
						rtl_igmpMldProcess
						    (brIgmpModuleIndex,
						     skb_mac_header(skb),
						     p->port_no, &fwdPortMask);
					br_multicast_forward(br, fwdPortMask,
							     skb,
							     (skb2 ? 1 : 0));
...
#endif
					{
						...
						if ((srcVlanId != 0)
							...
							if (rtk_vlan_support_enable == 0) {
								rtl865x_ipMulticastHardwareAccelerate
								    (br,
								     multicastFwdInfo.
								     fwdPortMask,
								     srcPort,
				...
				if (proto == IPPROTO_ICMPV6) {
					while (moreFlag) {
						...
						if (tmpOp > 0) {
							...
							if (operation == 1)
								printk
								    ("icmpv6 add from frame finish\n");
							else if (operation == 2)
								printk
								    ("icmpv6 del from frame finish\n");
#endif
							br_update_igmp_snoop_fdb
							    (operation, br, p,
							     macAddr, skb);
...
#endif
					rtl_igmpMldProcess(brIgmpModuleIndex,
							   skb_mac_header(skb),
							   p->port_no,
							   &fwdPortMask);
...
#if defined(CONFIG_RTK_VLAN_SUPPORT)
							if (rtk_vlan_support_enable == 0) {
								rtl865x_ipv6MulticastHardwareAccelerate
								    (br,
								     multicastFwdInfo.
								     fwdPortMask,
								     srcPort,
...
int rtl_IgmpSnooping_BrXmit_Hook(struct net_bridge *br,
				 const unsigned char *dest, struct sk_buff *skb)
{
	...
	if (dest[0] & 1)	// multicast
	{
		if (igmpsnoopenabled && MULTICAST_MAC(dest)) {
			...
			if (((proto == IPPROTO_UDP) || (proto == IPPROTO_TCP))
				...
				ret =
				    rtl_getMulticastDataFwdInfo
				    (brIgmpModuleIndex, &multicastDataInfo,
				     &multicastFwdInfo);
#if defined (CONFIG_RTL_HW_MCAST_WIFI)
				if (hwwifiEnable) {
					existMulticastEntry =
				...
				if (IGMPPRoxyFastLeave) {
					...
					if ((1 << srcPort & RTL_WANPORT_MASK)
					    && multicastFwdInfo.unknownMCast ==
					    TRUE) {
						kfree_skb(skb);
						return 1;
					}
...
#endif
				br_multicast_deliver(br,
						     multicastFwdInfo.
						     fwdPortMask, skb, 0);
#if defined (CONFIG_RTL_HW_MCAST_WIFI)
				if ((hwwifiEnable && ret == SUCCESS) ||
				    (hwwifiEnable == 0 && ret == SUCCESS
				     && multicastFwdInfo.cpuFlag == 0))
#else
				if ((ret == SUCCESS)
				    && (multicastFwdInfo.cpuFlag == 0))
			...
			if ((proto == IPPROTO_UDP) || (proto == IPPROTO_TCP)) {
				multicastDataInfo.ipVersion = 6;
				memcpy(&multicastDataInfo.sourceIp,
				       &ipv6h->saddr, sizeof(struct in6_addr));
				memcpy(&multicastDataInfo.groupAddr,
				       &ipv6h->daddr, sizeof(struct in6_addr));
				...
				if (TEST_PACKETS_ADDRESS(dest))
					printk
					    ("ret = %d, fwdPortMask = %x, [%s:%d]\n",
					     ret, multicastFwdInfo.fwdPortMask,
					     __FUNCTION__, __LINE__);
#endif
				br_multicast_deliver(br,
						     multicastFwdInfo.
				...
				if ((ret == SUCCESS)
					...
					struct in6_addr sip;
					struct in6_addr dip;
					memcpy(&sip, multicastDataInfo.sourceIp,
					       sizeof(struct in6_addr));
					memcpy(&dip,
					       multicastDataInfo.groupAddr,
					       sizeof(struct in6_addr));
...
void br_igmp_fdb_expired(struct net_bridge_fdb_entry *ent)
{
	...
	for (i2 = 0; i2 < FDB_IGMP_EXT_NUM; i2++) {
		if (ent->igmp_fdb_arr[i2].valid == 1) {
			...
			// when timeout expire
			if (time_before_eq
			    (ent->igmp_fdb_arr[i2].ageing_time,
			     igmp_walktimeout)) {
				DEBUG_PRINT("%s:%d\n", __FUNCTION__, __LINE__);
				SA = ent->igmp_fdb_arr[i2].SrcMac;
				DEBUG_PRINT
				    ("expired src mac:%02x,%02x,%02x,%02x,%02x,%02x\n",
				     SA[0], SA[1], SA[2], SA[3], SA[4], SA[5]);

				DA = ent->addr.addr;
				DEBUG_PRINT
				    ("fdb:%02x:%02x:%02x-%02x:%02x:%02x\n",
				...
				if (rtl_M2UDeletecheck(DA, SA)) {
					...
					dev =
					    __dev_get_by_name(&init_net,
							      RTL_PS_WLAN0_DEV_NAME);

					if (dev) {
						unsigned char
						    StaMacAndGroup[20];
						memcpy(StaMacAndGroup, DA, 6);
						memcpy(StaMacAndGroup + 6, SA,
						       6);
#if defined(CONFIG_COMPAT_NET_DEV_OPS)
						if (dev->do_ioctl != NULL) {
							dev->do_ioctl(dev,
...
#endif
							DEBUG_PRINT
							    ("(fdb expire) wlan0 ioctl to DEL! M2U entry da:%02x:%02x:%02x-%02x:%02x:%02x; sa:%02x:%02x:%02x-%02x:%02x:%02x\n",
							     StaMacAndGroup[0],
							     StaMacAndGroup[1],
							     StaMacAndGroup[2],
							     StaMacAndGroup[3],
...
#endif
							DEBUG_PRINT
							    ("(fdb expire) wlan1 ioctl to DEL! M2U entry da:%02x:%02x:%02x-%02x:%02x:%02x; sa:%02x:%02x:%02x-%02x:%02x:%02x\n",
							     StaMacAndGroup[0],
							     StaMacAndGroup[1],
							     StaMacAndGroup[2],
							     StaMacAndGroup[3],
				...
				del_igmp_ext_entry(ent, SA,
						   ent->igmp_fdb_arr[i2].port,
						   1);
...
void rtl_igmp_fdb_delete_hook(struct net_bridge *br,
			      struct net_bridge_fdb_entry *f)
{
	if (f->is_static && f->igmpFlag && (MULTICAST_MAC(f->addr.addr)
#if defined (CONFIG_RTL_MLD_SNOOPING)
		...
		if (time_before_eq(f->updated + 300 * HZ, jiffies)) {
			DEBUG_PRINT
			    ("fdb_delete:f->addr.addr is 0x%02x:%02x:%02x-%02x:%02x:%02x\n",
			     f->addr.addr[0], f->addr.addr[1], f->addr.addr[2],
			     f->addr.addr[3], f->addr.addr[4], f->addr.addr[5]);
...
/*mCastFwd*/
#if defined(CONFIG_RTL_IGMP_PROXY)
int rtl865x_checkMfcCache(struct net *net, struct net_device *dev,
			  uint32 origin, uint32 mcastgrp)
{
...
int rtl865x_checkUnknownMCastLoading(struct rtl_multicastDataInfo *mCastInfo)
{
	...
	for (i = 0; i < MAX_UNKNOWN_MULTICAST_NUM; i++) {
		if ((unKnownMCastRecord[i].valid == 1)
		    && (unKnownMCastRecord[i].groupAddr ==
			mCastInfo->groupAddr[0])) {
			break;
		}
	}
	...
	if (i == MAX_UNKNOWN_MULTICAST_NUM) {
		for (i = 0; i < MAX_UNKNOWN_MULTICAST_NUM; i++) {
			if (time_before
			    (unKnownMCastRecord[i].lastJiffies + HZ, jiffies)
			    || time_after(unKnownMCastRecord[i].lastJiffies,
					  jiffies + HZ)) {
...
int rtl865x_ipMulticastFastFwd_hook(struct sk_buff *skb)
{
	...
	if ((skb->dev == NULL)
	    || (strncmp(skb->dev->name, RTL_PS_BR0_DEV_NAME, 3) == 0)) {
		return -1;
	}
#ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
	...
	if ((dest[0] == 0x01) && (dest[1] == 0x00) && (dest[2] == 0x5e)) {
		...
		if (((proto == IPPROTO_UDP) || (proto == IPPROTO_TCP))
			...
			if ((strncmp(skb->dev->name, RTL_PS_WAN0_DEV_NAME, 4) ==
			     0) && needCheckMfc) {
#if	defined (CONFIG_RTL_IGMP_PROXY)
				existMfc =
				    rtl865x_checkMfcCache(&init_net, dev,
							  iph->saddr,
							  iph->daddr);
...
#endif
				{
					if (rtl865x_checkUnknownMCastLoading
					    (&multicastDataInfo) ==
						...
						if ((skb->srcVlanId != 0)
							...
							existMulticastEntry =
							    ...
							    (multicastDataInfo.
							     ...
							     srcVlanId,
							     srcPort);
							if (existMulticastEntry
							    != NULL
							    &&
							    existMulticastEntry->
							    inAsic) {
								rtl865x_blockMulticastFlow
								    (srcVlanId,
								     srcPort,
								     multicastDataInfo.
								     sourceIp
			...
			if (!strcmp(skb->dev->name, RTL_PS_ETH_NAME_ETH2)) {
				ret =
				    rtl_getMulticastDataFwdInfo
				    (brIgmpModuleIndex_2, &multicastDataInfo,
				     &multicastFwdInfo);
				bridge = bridge1;
			...
			if ((ret != 0) || multicastFwdInfo.reservedMCast
			    || multicastFwdInfo.unknownMCast) {
				if (multicastFwdInfo.unknownMCast
				    &&
				    (strncmp
				     (skb->dev->name, RTL_PS_WAN0_DEV_NAME,
				      4) == 0)) {
...
#if defined (CONFIG_RTL_HARDWARE_MULTICAST)
			existMulticastEntry =
			    rtl865x_findMCastEntry(multicastDataInfo.groupAddr
						   [0],
						   multicastDataInfo.sourceIp
						   [0], srcVlanId, srcPort);

			if ((existMulticastEntry == NULL) ||
			    ((existMulticastEntry != NULL)
			     && (existMulticastEntry->inAsic))) {
...
#else
			list_for_each_entry_safe(p, n, &bridge0->port_list,
						 list)
#endif
			{
				port_bitmask = (1 << p->port_no);
			...
			if (prev != NULL) {
				...
				prev->dev->hard_start_xmit(skb, prev->dev);
#else
				prev->dev->netdev_ops->ndo_start_xmit(skb,
								      prev->
								      dev);
#endif
				fwdCnt++;
	...
	if ((dest[0] == 0x33) && (dest[1] == 0x33) && (dest[2] != 0xff)) {
		...
		if ((proto == IPPROTO_UDP) || (proto == IPPROTO_TCP)) {
			multicastDataInfo.ipVersion = 6;
			memcpy(&multicastDataInfo.sourceIp, &ipv6h->saddr,
			       sizeof(struct in6_addr));
			memcpy(&multicastDataInfo.groupAddr, &ipv6h->daddr,
			       sizeof(struct in6_addr));
			/*
			 ...
			 */
			ret =
			    rtl_getMulticastDataFwdInfo(brIgmpModuleIndex,
							&multicastDataInfo,
							&multicastFwdInfo);
